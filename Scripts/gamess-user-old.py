...
class GMSUser():
    ...
    def SetUpGAMESSFiles(self,prgfile):
        self.gmspath,self.gmscmd,self.gmsarg,self.gmsscrdir,self.gmsexamdir=self.ReadProgramPathFile(prgfile)
    ...
    def SaveInputFile(self,filename,check=True):
        ...
        text=self.gmsinpobj.MakeInputDataText(self.allinputnames,self.inputvaldic,self.textvaldic)
    ...
    def SetValueFromFU(self,namvar):
        ...
        if namvar == '$STATPT:IACTAT':
             atmlst=self.GetSelectedAtomsFromFU()
             if len(atmlst) <= 0: return
             text=self.IACTATTextFromFU(atmlst)
             self.SetInputValue(namvar,text,True)
    ...
    def SetAllFMOValueFromFU(self):
        if not self.model.mol:
            ...
            return
        # required FMO data
        namvarlst=['$FMOXYZ:$FMOXYZ','$FMOHYB:$FMOHYB','$FMOBND:$FMOBND','$DATA:$DATA',
                   '$FMO:FRGNAM','$FMO:INDAT','$FMO:ICHARG','$FMO:LAYER']
        ...
        bdalst=self.model.frag.ListBDA()
    ...
    def SetFMOValueFromFU(self,namvar):
        ...
        *varval INDAT  [INPUT,FILE,FROMFU]
        ...
        if namvar == '$FMOHYB:$FMOHYB': # or namandvar == 'all':           
            #baslst=[self.basis]
    ...
    def IACTATTextFromFU(self):
        actlst=[]
        for i in range(len(self.model.mol.atm)):
            if atom.active: actlst.append(i+1)
    ...
    def OpenTextEditor(self,title,text,mode):
        ...
        self.texteditor=True
        self.editor=subwin.TextEditor_Frm(parent,-1,winpos,winsize,title,text,retmethod,mode=mode)
    ...
    def OpenSettingGAMESSPanel(self,parent):
        ...
        self.gmssetwin=SetPathAndCmdForGAMESS_Frm(parent,-1,winpos,self.prgfile,prgnam,item,remark)
...
class GMSExpert_Frm(wx.Frame):
    ...
    def __init__(self,mdlwin,id,gmsuser,mode,inputnamelst,
                 winpos=[-1,-1],winsize=[300,400]): #winsize=[300,400]):
        """ 
...
class GMSBeginner_Frm(wx.Frame):
    def __init__(self,mdlwin,id,gmsuser,winpos=[-1,-1]):
        ...
        winsize=lib.WinSize([280,460]) #430]) # 455
        #if fu:
        #if const.SYSTEM == const.MACOSX: winsize=(275,445)
        if not mdlwin:
            wx.Frame.__init__(self,None,id,title,pos=winpos,size=winsize,
    ...
    def GetWidgetValues(self):
        ...
        # method
        value=self.comboboxdic['Method'].GetValue()
    ...
    def ComboBoxChoice(self,curmeth,item,value):    
        ...
        if item == 'Method':
            ...
            if len(submenulst) <= 0:
                ...
                ilayer=int(ilayer)
                self.gmsuser.wavelayer[ilayer]=value
            ...
            ilayer=self.scllay.GetValue()
            ilayer=int(ilayer)
            self.gmsuser.basislayer[ilayer]=value
            #varval='Basis:'+value
            if varval in self.uservarvaldic:
                vals=self.uservarvaldic[varval]
                self.SetUserInputValues(vals,True)
    ...
    def MakeBasisData(self,basis):
        ...
        ndfunc=''
        if 'NDFUNC' in valdic: ndfunc=valdic['NDFUNC']
    ...
    def CoordinateFromFU(self,method,check=True):     
        ...
        #nlayer,dumlst=self.model.ListAtomLayer()
        ...
        if parmeth == 'FMO':
            retcode=True
            if check:
                mess='Have you done fragment attribute setting in FU?'
                retcode=lib.MessageBoxYesNo(mess,'GMSBeginner:OnCoordinateFromFU')
    
            if not retcode: # dlg.ShowModal() == wx.ID_NO:
                #self.model.winctrl.OpenAssignLayerPanel()
                #dlg.Destroy()
                return
            else: 
                ...
                if not self.model.mol.IsAttribute('ICHARG'):
                    frgchglst=self.model.frag.ListFragmentCharge()
    ...
    def ChangeCoordButtonColor(self):
        ...
        if self.gmsuser.coordinternal: self.btnintr.SetForegroundColour(oncol)
    ...
    def OnMenu(self,event):
        ...
        # ExecBatch
         ...
         # Help menu items
            ...
            title='GAMESS Assist For Beginner in FU ver.'
            lib.About(title,const.FUMODELLOGO)
        elif item == 'Help':
            self.gmsuser.HelpMessage()
        # Setting    
...
class GMSInput():
    ...
    def MakeInputDataText(self,inputnamelst,inputvaldic,textvaldic):
        ...
        if namvar in textvaldic:
            ...
            if '$BASIS:GBASIS' not in inputvaldic: txt=txt+'\n'            
...
class SetPathAndCmdForGAMESS_Frm(wx.MiniFrame):
    def __init__(self,parent,id,winpos,prgfile,program,item,remark):
        ...
        winsize=lib.WinSize((400,165))  #; winpos=(-1,-1)
    ...
    def OnClose(self,event):
        if not self.setpath:
            ...
            if retcode: # == wx.YES:
                #if wx.YES:
                self.WrieGAMESSPathFile(self.prgfile,self.path,self.cmd,self.arg,self.scr,self.tests)
                self.setpath=True
            else: pass
            #dlg.Destroy()
...
class FMOInputText():
    ...
    def FMOHYBTextFromFU(self,basnamlst):
        ...
        for basnam in basnamlst:
            ...
            if len(temp) <= 0:
                text=text+'Warnning: Hybrid orbitals for "'+basnam+'" is not found.\nPlease create yourself.'
            else: text=text+temp
        text=text.rstrip()
        #text=text+'\n'
        return text    
    ...
    def FMOBNDTextFromFU(self,basnamlst):
        basnamlst=basnamlst+['MINI']
        frgbdalst=self.model.frag.ListBDA()
        eol='\n'; blk=' '
        text=''
    ...
    def FMODATATextFromFU(self,jobtitle,basdatlst):
        ...
        nlayer,dummy=self.model.frag.ListAtomLayer()
        text=3*' '+jobtitle+'\n'
        text=text+blk+'C1\n'
    ...
    def XXFMOAttribTextFromFU(self,attrib):
        ndat=0; text=''
    ...
    def MakeFrgnamText(frgnamlst):
        ...
        text=lib.StringListToText(frgnamlst,header,sep,width,colu,nw) 
        nfrg=len(frgnamlst)
        return nfrg,text
