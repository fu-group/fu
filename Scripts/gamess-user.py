...
class GMSUser(object):
    def __init__(self,parent,mode='beginner'):
        ...
        #print('userinputvaldic in GMSUser __init__=',self.userinputvaldic)
        self.usertextvaldic={}
    ...
    def OpenBeginnerWin(self,mdlwin):
        ...
        #
        self.mode='beginner'
        self.beginner=GMSBeginner_Frm(mdlwin,self)
    ...
    def OpenExpertWin(self,mdlwin,inputnamelst,winpos=[100,100]):
        ...
        self.mode='expert'
        #
        self.expert=GMSExpert_Frm(mdlwin,self,self.mode,inputnamelst,winpos=winpos)
        self.expertwin=True
        self.expert.Show()
    ...
    def SaveInput(self):
        ...
        if self.nlayer > 1:
            ...
            #const.CONSOLEMESSAGE('nlayer in gnsuser='+str(self.nlayer))
            #const.CONSOLEMESSAGE('nlayer in fragobj='+str(nlayer))
    ...
    def SaveInputFile(self,filename,check=True):
        ...
        if '$FMOXYZ:$FMOXYZ' in self.textvaldic:
    ...
    def FindGroupNameOfName(self,name):
        ...
        """
        grpnam=''
        for nam,group in list(self.namegroupdic.items()):
            if nam == name:
                grpnam=group; break
        return grpnam
    ...
    def SetFMOValueFromFU(self,namvar):
        ...
        *varval INDAT  [INPUT,FILE,FROMFU]
        ...
        #namvar='$DATA:$DATA'
            ...
            #const.CONSOLEMESSAGE('basislayer='+str(self.basislayer))
            for bas in self.basislayer:
                ...
                """
                basdatlst.append(bas)
                #print('bas=',bas)
            #const.CONSOLEMESSAGE('basdatlst='+str(basdatlst))
        ...
        #namvar='$FMO:FRGNAM'; nfrgvar='$FMO:NFRAG'
            ...
            if len(text) > 0:
                ...
                #const.CONSOLEMESSAGE('text='+text)
                #const.CONSOLEMESSAGE('nlayer='+str(nlayer))
                
                if nlayer > 1 and len(text) > 0:
                    #text='     layer(1)='+text
                    header='  layer(1)='; sep=','; width=-1; colu=5; nw=10
                    frgchglst=text.split(',')
                    frgchglst=[int(x) for x in frgchglst]
    ...
    def RemoveUnsetFromInputNamelst(self,name):
        ...
        """
        varlst=[]
        if name in self.varlstdic: varlst=self.varlstdic[name]
        else: return
    ...
    def UnsetConflictVars(self,namvar,value):
        ...
        for symbol,namvarvallst in list(self.conflictdic.items()):
            #try: 
    ...
    def GetDefaultNamVarValue(self,namvar):
        ...
        for var in varlst:
            namv=name+':'+var
            if namv != namvar: continue
            if namvar in self.varvaldic: value=self.varvaldic[namvar]
    ...
    def GetRequiredNames(self,namvar,value):
        ...
        """
        reqnamlst=[]
        for namvarval,namelst in list(self.requireddic.items()):
            items=namvarval.split('=')
            namv=items[0].strip()
            ns=namv.find('!')
            if ns >= 0: 
    ...
    def IsInputNamVar(self,namvar):
        if namvar in self.inputvaldic: return True
        else: return False
    ...
    def AddInputNameAndVars(self,name,varlst):
        ...
        if name not in self.inpnamvardic: self.inpnamvardic[name]=[]
        if len(varlst) <= 0: varlst=self.varlstdic[name]
    ...
    def RemoveInputNameVars(self,name,varlst):
        ...
        if namvar not in self.inputvaldic:
    ...
    def PresetInputnames(self):
        namelst=[]
        for name, value in list(self.needdic.items()):
    ...
    def ReadInputFileAndSetInputValues(self,filename):
        ...
        if '$FMOXYZ:$FMOXYZ' in self.inputvaldic:
    ...
    def ReadGMSInputText(self,inputtextlst):
        ...
        for name,varval in list(inputvaldic.items()):
    ...
    def MakeInputValDic(self,inpdatlst,inputvaldic):
        ...
        newtextvaldic={}
        self.inputfileinfo=''
        for name,varvallst in list(inputvaldic.items()):
    ...
    def MakeCurrentText(self):
        ...
        """
        if '$FMO:NFRAG' in self.inputvaldic:
            if self.inputvaldic['$FMO:NFRAG'][1] == self.setcolor:
                self.SetAllFMOValueFromFU() 
    ...
    def ReturnFromTextEditor(self,title,text,cancel):
        ...
        if title[:23] == 'View current input data':
            ...
            if len(text) > 0:
                ...
                if namvar in self.textvaldic: del self.textvaldic[namvar]
                if namvar in self.inputvaldic: self.inputvaldic[namvar]=['INPUT',self.unsetcolor] 
    ...
    def RunGAMESS(self,parent,check=True):
        ...
        if check:
            if os.path.exists(self.gmsoutputfile):
                retcode=lib.MessageBoxYesNo(self.gmsoutputfile+" exsists. Can I delete it?","")
                if not retcode: return
            #if dlg == wx.NO: 
            #    dlg.Destroy(); return
        ...
        if self.SetExecInputFile(self.gmsinputfile):
            ...
            rungmscmd=rungmscmd.replace('\n','//')
            # check scratch file
            head,tail=os.path.split(gmsinputfile)
            basename,ext=os.path.splitext(tail)
            datfile = os.path.join(self.gmsscrdir, basename + '.dat')
            datfile=os.path.abspath(datfile)
            if os.path.exists(datfile):
                mess='There is "'+datfile +'" file.\n'
                mess+='Can I delete it?'

                dlg = wx.MessageDialog(None, mess, 'RunGamess',
                                       style=wx.YES_NO | wx.ICON_QUESTION | wx.STAY_ON_TOP)
                if dlg.ShowModal() == wx.ID_NO: return
                self.DeleteGAMESSScratch(basename)
                mess='"'+datfile+ '" was deleted.'
    ...
    def OpenExePrgWin(self,parent):
        ...
        winpos=[0,0]; winsiz=[600,300]
        if lib.GetPlatform() == 'MACOSX': 
            winpos=[0,20]; winsiz=[600,150]
...
class GMSExpert_Frm(wx.Frame):
    ...
    def __init__(self,mdlwin,gmsuser,mode,inputnamelst,
        ...
        if not mdlwin:
            wx.Frame.__init__(self,None,-1,title,pos=winpos,size=winsize,
                              style=wx.SYSTEM_MENU|wx.CAPTION|wx.CLOSE_BOX|wx.RESIZE_BORDER) #|wx.FRAME_FLOAT_ON_PARENT)        #
        else:
            wx.Frame.__init__(self,mdlwin,-1,title,pos=winpos,size=winsize,
                              style=wx.SYSTEM_MENU|wx.CAPTION|wx.CLOSE_BOX|wx.RESIZE_BORDER|wx.FRAME_FLOAT_ON_PARENT)        #
        #
        ...
        if self.mode == 'expert':
            ...
            lib.InsertTitleMenu(self.menubar,'[GamessUser]')
            # statusbar
    ...
    def CreatePanel(self):
        ...
        if self.mode == 'expert':
            ...
            #sttit.SetToolTip('Input job title and hit "ENETR"')  
            lib.SetTipString(sttit,'Input job title and hit "ENETR"')
        ...
        if self.mode == 'expert':
            ...
            #btnrun.SetToolTip('Run GAMESS after saving input data')  
            lib.SetTipString(btnrun,'Run GAMESS after saving input data')
            wx.StaticLine(self.panel,pos=[75,yloc-10],size=[2,yloc+30],style=wx.LI_VERTICAL)            
            btnview =wx.ToggleButton(self.panel,-1,"View",pos=[90,yloc],size=(50,22))
            btnview.Bind(wx.EVT_TOGGLEBUTTON,self.OnView)     
            #btnview.SetToolTip('View current input data on memory')  
            lib.SetTipString(btnview,'View current input data on memory')
            btnsav =wx.Button(self.panel,-1,"Save",pos=[155,yloc],size=(50,22))
            btnsav.Bind(wx.EVT_BUTTON,self.OnSave)
            #btnsav.SetToolTip('Save input data on file')  
            lib.SetTipString(btnsav,'Save input data on file')
            btncls =wx.Button(self.panel,-1,"Close",pos=[220,yloc],size=(50,22))
            btncls.Bind(wx.EVT_BUTTON,self.OnClose)
            #btncls.SetToolTip('Close the panel')  
            lib.SetTipString(btncls,'Close the panel')
    ...
    def CreateCtrlPanel(self,panpos,pansize):
        ...
        lbheight=80 #len(self.gmsuser.inputnamelst)*self.fontheight+10
        ...
        if self.curname in self.donenamedic:
        ...
        yloc += 8
        # buttons
        self.btnrmv=wx.Button(self.ctrlpanel,-1,"Remove",pos=[15,yloc],size=(70,22))
        #self.btnrmv.SetToolTip('Remove selected name group')        
        lib.SetTipString(self.btnrmv,'Remove selected name group')
        ...
        yloc +=20
        self.tcladd=wx.TextCtrl(self.ctrlpanel,-1,'',pos=(15,yloc),size=(80,22),
                                style=wx.TE_PROCESS_ENTER)
        self.tcladd.Bind(wx.EVT_TEXT_ENTER,self.OnCtrlAdd)
        self.tcladd.Bind(wx.EVT_TEXT,self.OnTextEntered)
        #self.tcladd.SetToolTip('Input name group and hit "ENTER"')  
        lib.SetTipString(self.tcladd,'Input name group and hit "ENTER"')
        yloc +=25
        wx.StaticLine(self.ctrlpanel,pos=[8,yloc],size=[width-2,2],style=wx.LI_HORIZONTAL)
        yloc += 8
        self.btnundo =wx.Button(self.ctrlpanel,-1,"Undo",pos=[20,yloc],size=(50,22))
        self.btnundo.Bind(wx.EVT_BUTTON,self.OnCtrlUndo)
        #self.btnundo.SetToolTip('Undo Remove/Add once')  
        lib.SetTipString(self.btnundo,'Undo Remove/Add once')
    ...
    def CreateInputerPanel(self,panpos,pansize):
        ...
        # buttons
        yloc=btnyloc
        btnrsetall=wx.Button(self.inputpanel,-1,"ResetAll",pos=(20,yloc),size=(70,22))
        btnrsetall.Bind(wx.EVT_BUTTON,self.OnInputResetAll)
        #btnrsetall.SetToolTip('Reset all values to defaults')  
        lib.SetTipString(btnrsetall,'Reset all values to defaults')
        btnrset=wx.Button(self.inputpanel,-1,"Reset",pos=(100,yloc),size=(50,22))
        btnrset.Bind(wx.EVT_BUTTON,self.OnInputReset)
        #btnrset.SetToolTip('Reset selected value to default')  
        #btndeft=wx.Button(self.inputpanel,-1,"Default",pos=(100,yloc),size=(50,20))
        lib.SetTipString(btnrset,'Reset selected value to default')
    ...
    def CreateGridPanel(self,panel,panpos,pansize,nrow):
        ...
        try: self.grid.ShowScrollbars(wx.SHOW_SB_NEVER,wx.SHOW_SB_DEFAULT)
        except: pass
    ...
    def OnSelectRightClick(self,event):
        ...
        try:
            ...
            name=item+':'+item
            mess=''
            if name in self.gmsuser.varvaltipsdic:
...
#        if self.curval != '':    
        ...
        if self.selectedcol == 2: # and self.selectedrow != 0:
            ...
            if curvalnam == 'TEXT' and namvar in self.gmsuser.textvaldic:
            ...
            for val in curvallst:
                ...
                if varval in self.curvaltipdic: 
        ...
        if self.curvar != '':
            mess=''
            if self.curvar in self.curvartipdic: 
    ...
    def SetDataToGridTable(self,name):
        ...
        if name in self.gmsuser.inpnamvardic:
        ...
        self.curnamtip=''
        if name+':'+name in self.gmsuser.varvaltipsdic: 
        ...
        for i in range(nrow):
            ...
            if len(varsubs) >= 2:
                if varsubs[1].isdigit() and var != '':
                    varpar=varsubs[0]
                    if var not in self.varsubdic: self.varsubdic[var]=[]
                    namvar=name+':'+var
                    val=self.gmsuser.varvaldic[namvar]
                    vallst=self.gmsinpobj.SplitValues(val)
                    self.varsubdic[var]=vallst
                    if varpar not in self.curvaldic: self.curvaldic[varpar]=[]
                    self.curvaldic[varpar].append(var)
                    for val in vallst:
                        # namvarval=$BASIS:GBASIS-1:STO
                        namvarval=self.curname+':'+var+':'+val
                        if var not in self.varsubtipdic: self.varsubtipdic[var]=[]
                        text=''
                        if namvarval in self.gmsuser.varvaltipsdic:
    ...
    def MakeNameVarTipDic(self,name):
        vartipdic={}
        if name not in self.gmsuser.varvaldic: return vartipdic
    ...
    def OnCtrlRemove(self,event):
        ...
        #
        if self.curname in self.donenamedic: del self.donenamedic[self.curname]
    ...
    def OnCtrlAdd(self,event):
        ...
        if name in self.gmsuser.inputnamelst:
            ...
            return
        if name not in self.gmsuser.varlstdic:
...
class GMSBeginner_Frm(wx.Frame):
    def __init__(self,mdlwin,gmsuser,winpos=[-1,-1]):
        """
        ...
        winsize=lib.WinSize([320,490]) #[280,440] #430]) # 455
        #if fu:
        #if const.SYSTEM == const.MACOSX: winsize=(275,445)
        if not mdlwin:
            wx.Frame.__init__(self,None,-1,title,pos=winpos,size=winsize,
        ...
        if lib.GetPlatform() == 'MACOSX' and wx.version()[0:2] != '2.':
            self.buttonstyle=wx.BORDER_NONE
        else: self.buttonstyle=0
        #self.inputer=None
        ...
        """
        const.CONSOLEMESSAGE('usernamelst='+str(self.usernamelst))
        const.CONSOLEMESSAGE('uservarlstdic='+str(self.uservarlstdic))
        const.CONSOLEMESSAGE('usevarvaldic='+str(self.uservarvaldic))
        const.CONSOLEMESSAGE('userinputvaldic='+str(self.userinputvaldic))
        const.CONSOLEMESSAGE('usertextvaldic='+str(self.usertextvaldic))
        """
        ...
        for name in combobox:
            ...
            for varval in self.uservarlstdic[name]:
                ...
                if len(items) >= 2:
                    ...
                    if namvarval not in self.submenudic:
    ...
    def SetInitialValues(self):
        ...
        if varval in self.uservarvaldic:
    ...
    def CreatePanel(self):
        ...
        hcmb=25; wcmb=130
        yloc=5
        st11=wx.StaticText(self.panel,-1,"Method:",pos=(x0+5,yloc+2),
                           size=(50,20)) 
        #st11.SetToolTip('Select method')
        lib.SetTipString(st11,'Select method')
        methodlst=self.choicelstdic['Method']
        cmbmeth=wx.ComboBox(self.panel,-1,'',choices=methodlst, \
                               pos=(x0+70,yloc),
                               size=(wcmb,hcmb),style=wx.CB_READONLY)                      
        ...
        wx.StaticLine(self.panel,pos=(x0+210,yloc-5),size=(2,35),
                      style=wx.LI_VERTICAL)
        btnopt=wx.Button(self.panel,-1,"OPTIONS",pos=(x0+230,yloc+2),
                         size=(70,22))
        btnopt.Bind(wx.EVT_BUTTON,self.OnButton) #ContrlOption)
        #btnopt.SetToolTip('Global options')
        lib.SetTipString(btnopt,'Global options')
        self.buttondic['OPTIONS']=btnopt
        ##yloc2=33 # 30
        xloc=90; hcmb=20; wcmb=110
        yloc += 30
        sl12=wx.StaticLine(self.panel,pos=(-1,yloc),size=(w,2),
                           style=wx.LI_HORIZONTAL)
        yloc += 8
        st22=wx.StaticText(self.panel,-1,"Job title:",pos=(x0+5,yloc),
                           size=(60,18)) 
        self.tcltit=wx.TextCtrl(self.panel,-1,self.gmsuser.jobtitle,
                                pos=(x0+70,yloc),size=(w-90,22),style=wx.TE_PROCESS_ENTER)
        ...
        yloc += 30
        self.txtchg=wx.StaticText(self.panel,-1,"Charge:",pos=(x0+5,yloc),
                                  size=(45,18)) 
        self.sclchg=wx.SpinCtrl(self.panel,-1,value="0.0",pos=(x0+60,yloc),
                                size=(52,hcmb),style=wx.SP_ARROW_KEYS,min=-999)                                    
        if wx.version()[0:2] == '2.': self.sclchg.SetValueString(self.gmsuser.charge)
        else:
            try: self.sclchg.SetValue(int(self.gmsuser.charge))
            except: self.sclchg.SetValue(self.gmsuser.charge)
        #self.fmo=self.gmsrbt2.GetValue()
        ##if self.method != "MO": self.sclchg.Disable()
        self.txtspn=wx.StaticText(self.panel,-1,"Spin multiplicity:",
                                  pos=(x0+120,yloc),size=(110,18))
        
        self.sclspn=wx.SpinCtrl(self.panel,-1,value="1",pos=(x0+245,yloc),
                                size=(45,hcmb),style=wx.SP_ARROW_KEYS,min=1)                             
        if wx.version()[0:2] == '2.': self.sclspn.SetValueString(self.gmsuser.spin)
        else:
            try: self.sclspn.SetValue(int(self.gmsuser.spin))
            except: self.sclspn.SetValue(self.gmsuser.spin)
        # coordinate
        yloc += 25
        wx.StaticText(self.panel,-1,"Coordinate from:",pos=(x0+5,yloc+2),size=(120,18)) 
        ...
        self.btnintr=wx.Button(self.panel,-1,"FU",pos=(x0+130,yloc),
                               size=(40,hbt),style=self.buttonstyle)
        
        self.btnintr.SetBackgroundColour(wx.WHITE)
        self.btnfile=wx.Button(self.panel,-1,"File",pos=(x0+180,yloc),
                               size=(40,hbt),style=self.buttonstyle)
        self.btnfile.SetBackgroundColour(wx.WHITE)
        ...
        btncrd=wx.Button(self.panel,-1,"Option",pos=(x0+240,yloc+2),size=(60,hbt))
        #self.btnrun.Disable()
        ...
        sl50=wx.StaticLine(self.panel,pos=(-1,yloc),size=(w,2),
                           style=wx.LI_HORIZONTAL)
        yloc += 10
        st50=wx.StaticText(self.panel,-1,"Computer nodes:",pos=(x0+5,yloc+2),
                           size=(110,18))
        ...
        self.tclnod=wx.TextCtrl(self.panel,-1,self.gmsuser.nodes,
                                pos=(x0+120,yloc),size=(40,20))        
        st52=wx.StaticText(self.panel,-1,"cores/node:",pos=(x0+165,yloc+2),
                           size=(70,18))
        self.tclcor=wx.TextCtrl(self.panel,-1,self.gmsuser.cores,
                                pos=(x0+250,yloc),size=(30,20))                
        self.tclnod.Bind(wx.EVT_TEXT,self.OnCompuNodes)
        self.tclcor.Bind(wx.EVT_TEXT,self.OnCompuCores)
        yloc += 25
        st53=wx.StaticText(self.panel,-1,"memory:",pos=(x0+15,yloc+2),
                           size=(50,18))
        self.tclmem=wx.TextCtrl(self.panel,-1,self.gmsuser.memory,
                                pos=(x0+80,yloc),size=(30,20))                
        #
        st54=wx.StaticText(self.panel,-1,"disk:",pos=(x0+120,yloc+2),
                           size=(30,18))
        self.tcldsk=wx.TextCtrl(self.panel,-1,self.gmsuser.disk,
                                pos=(x0+160,yloc),size=(50,20))                
        st531=wx.StaticText(self.panel,-1,"GB/node",pos=(x0+215,yloc+2),
                            size=(55,18))
        ...
        yloc += 25 #25
        linf=wx.StaticLine(self.panel,pos=(-1,yloc),size=(xsize,2),
                           style=wx.LI_HORIZONTAL)    
        #yloc=h-28 #38 # += 8
        yloc += 8
        self.runbt=wx.Button(self.panel,-1,"RunGMS",pos=(x0+10,yloc),
                             size=(70,hbt))
        self.runbt.Bind(wx.EVT_BUTTON,self.OnRunGAMESS)
        if not self.gmsuser.madeinput: self.runbt.Disable()
        hlv=h-yloc+5 #-2 #8
        linv=wx.StaticLine(self.panel,pos=(x0+100,yloc-5),size=(2,hlv),
                           style=wx.LI_VERTICAL)
        vwbt=wx.Button(self.panel,wx.ID_ANY,"View",pos=(x0+120,yloc),
                       size=(50,hbt))
        vwbt.Bind(wx.EVT_BUTTON,self.OnViewInput)
        apbt=wx.Button(self.panel,wx.ID_ANY,"Save",pos=(x0+180,yloc),
                       size=(50,hbt))
        apbt.Bind(wx.EVT_BUTTON,self.OnSave)
        clbt=wx.Button(self.panel,wx.ID_ANY,"Reset",pos=(x0+240,yloc),
                       size=(50,hbt))
    ...
    def CreateRunTypePanel(self,yloc):            
        ...
        wcmb=130
        yloc += 3
        st31=wx.StaticText(self.panel,-1,"Run type:",pos=(x0+5,yloc),
                           size=(xloc,18)) 
        ...
        cmbrun=wx.ComboBox(self.panel,-1,'',choices=runlst, \
                               pos=(x0+xloc+5,yloc-2), size=(wcmb,hcmb),
        ...
        btnrun=wx.Button(self.panel,-1,"Option",pos=(x0+xloc+wcmb+20,yloc+2),
                         size=(60,22))
        #self.btnrun.Disable()
        ...
        """
        yloc += 8
        st32=wx.StaticText(self.panel,-1,"Wave function:",pos=(x0+5,yloc),
                           size=(xloc,18)) 
        st32.SetForegroundColour("black")
        #self.wfnlst=self.abiwfnlst
        wavelst=self.choicelstdic['Wave']
        cmbwav=wx.ComboBox(self.panel,-1,'',choices=wavelst, \
                               pos=(x0+xloc+15,yloc-2), size=(80,hcmb), #wcmb-35,hcmb),
                               style=wx.CB_READONLY)                      
        ...
        #self.cmbwfn.Bind(wx.EVT_COMBOBOX,self.OnInputWave)
        self.btndisp=wx.Button(self.panel,-1,"-D",pos=(x0+xloc+wcmb-34,yloc),
                               size=(35,22),style=self.buttonstyle)
        self.btndisp.SetBackgroundColour(wx.WHITE)
        self.btndisp.Bind(wx.EVT_BUTTON,self.OnButton) #Dispersion)
        #self.btndisp.SetToolTip("Use Gromm's empirical dispersion 2010")
        lib.SetTipString(self.btndisp,"Use Gromm's empirical dispersion 2010")
        ...
        btnwav=wx.Button(self.panel,-1,"Option",pos=(x0+xloc+wcmb+20,yloc+2),
                         size=(60,22))
        #self.gmsbt2.Disable()
        btnwav.Bind(wx.EVT_BUTTON,self.OnButton) #OnWaveOthers)
        self.buttondic['Wave']=btnwav
        yloc += 25
        self.txtbas=wx.StaticText(self.panel,-1,"Basis set:",pos=(x0+5,yloc),
                                  size=(xloc,18)) 
        ...
        cmbbas=wx.ComboBox(self.panel,-1,'',choices=basislst, \
                               pos=(x0+xloc+5,yloc), size=(wcmb,hcmb),
                               style=wx.CB_READONLY)                      
        ...
        btnbas=wx.Button(self.panel,-1,"Others",pos=(x0+xloc+wcmb+20,yloc+2),
                         size=(60,22))
        #self.gmsbt3.Disable()
        ...
        yloc += 30
        #layer apply
        self.txtlay=wx.StaticText(self.panel,-1,"Set to layer:",
                                  pos=(x0+20,yloc),size=(70,18)) 
        self.scllay=wx.SpinCtrl(self.panel,-1,value="1",pos=(x0+105,yloc),
                              size=(40,20),style=wx.SP_ARROW_KEYS,min=1,max=5)                            
        self.scllay.Bind(wx.EVT_SPINCTRL,self.OnLayer)
        self.txtind=wx.StaticText(self.panel,-1,"",pos=(x0+145,yloc+2),
                                  size=(10,18)) 
        #self.stlay.SetForegroundColour('gray')
        #self.scllay.Disable()
        self.btnlay=wx.Button(self.panel,wx.ID_ANY,"Apply",pos=(x0+165,yloc+2),
                              size=(50,22))
        self.btnlay.Bind(wx.EVT_BUTTON,self.OnLayerApply)
        self.btndel=wx.Button(self.panel,wx.ID_ANY,"Del",pos=(x0+230,yloc+2),
                              size=(35,22))
        ...
        btnslv=wx.Button(self.panel,-1,"Option",pos=(x0+xloc+wcmb+20,yloc+2),
                         size=(60,22))
        #self.gmsbt4.Disable()
    ...
    def CreatePropertyPanel(self,yloc):
        w=self.size[0]; tcheight=self.size[1]-415 #405 #375
        xloc=70 #90 
        hcmb=25 # 25; 
        wcmb=125
        x0 = 10
        ...
        stprp=wx.StaticText(self.panel,-1,"Properties:",pos=(x0+5,yloc4),
                            size=(60,18)) 
        mess = 'Setect and/or input propeties directly (separated by a ",")'
        #stprp.SetToolTip(mess)
        lib.SetTipString(stprp,mess)
        btnprp=wx.Button(self.panel,-1,"Option",pos=(x0+xloc+wcmb+45,yloc4+2),
                         size=(60,22))
        ...
        cmbprp=wx.ComboBox(self.panel,-1,'',choices=propertieslst, \
                               pos=(x0+xloc+10,yloc-2), size=(wcmb,hcmb),
                               style=wx.CB_READONLY)                      
        ...
        yloc += 25
        self.tclprp=wx.TextCtrl(self.panel,-1,'',pos=(x0+10,yloc),
                                size=(w-30,tcheight), # 25
                                style=wx.TE_MULTILINE|wx.TE_PROCESS_ENTER)
    ...
    def EnableAllWidgets(self):
        ...
        for item,obj in list(self.comboboxdic.items()): 
            obj.Enable(); self.widgetstatusdic['combo:'+item]=True    
        for item,obj in list(self.buttondic.items()): 
    ...
    def OnReset(self,event):
        #print('Entered in OnReset')
        self.ResetCoordButtonColor()
        self.ResetDispButtonColor()
        self.gmsuser.ResetAllInputNames()
        self.SetInitialValues()
        self.SetWidgets()
        # reset basis set data
        bas = self.comboboxdic['Basis'].GetValue()
        method = self.comboboxdic['Method'].GetValue()
        self.ComboBoxChoice(method, 'Basis', bas)
    ...
    def OnComboBox(self,event):
        ...
        item=''
        value=obj.GetValue()
        for label,object in list(self.comboboxdic.items()):
            if obj == object:
                item=label; break
    ...
    def ComboBoxChoice(self,curmeth,item,value):    
        ...
        if item == 'Method':
            ...
            varval='Basis:'+value
            #print('varval in OnButton=',varval)

            self.ResetUserInputValue('$BASIS:NGAUSS')
    ...
    def SetSelectedValueToComboBox(self,name,var,val):
        ...
        group=''
        for grpnam,varlst in list(self.uservarlstdic.items()):
            #try: 
            if var in varlst:
    ...
    def ResetUserGroupValues(self,group):
        ...
        if group not in self.uservarlstdic:
    ...
    def ReturnMethod(self,selected,winlabel):
        ...
        if item == 'Method':
            ...
            if value == 'FMO':
                ...
                if selected in nbodydic: nbody=nbodydic[selected]
                else: nbody='2'
                namvar='$FMO:NBODY'
                ...
                if unamvar in self.uservarvaldic: 
                ...
                if len(namvarvaltxt) > 0:
                    ...
                    for namvar,val in list(namvarvaldic.items()):
    ...
    def MakeUserVarSubItemList(self,name,value):
        ...
        if namvar == 'Method:FMO':
            ...
            for namvarval,val in list(self.submenudic.items()):
                items=namvarval.split(':')
                if len(items) <= 2: continue
                if len(items) >= 3:
                    if items[0] != name: continue
    ...
    def MakeVarSubItemListForWave(self,usergrpnam,varnam):
        ...
        if grpnamvar in self.uservarvaldic:
    ...
    def OnButton(self,event):
        ...
        #lab=obj.GetLabel()
        item=''
        for label,object in list(self.buttondic.items()):
            if obj == object:
                item=label; break
    ...
    def SetOptionsForUserGroup0(self,grpnam,item):
        ...
        for name,optvarlst in list(optndic.items()):
            if name not in namvardic: namvardic[name]=[]
        ...
        for name in reqnamlst: 
            ...
            namvardic[name]=namvardic[name]+optvarlst
        #
        for name,varlst in list(namvardic.items()):
            self.gmsuser.AddInputNameAndVars(name,varlst)
    ...
    def SetOptionsForUserGroup1(self,grpnam,item):
        ...
        if grpvar in self.uservarvaldic:          
    ...
    def GetUserOptions(self,usergrpnam,namvar):
        ...
        dat=''
        if varnam in self.uservarvaldic: dat=self.uservarvaldic[varnam]
        ...
        for txtdat in datitems: # txtdat:'$CONTRL:MPLEVL=[$CONTRL:SCFTYP]'
            ...
            if len(items) >= 2: 
                ...
                for namv in namlst:
                    ...
                    if name not in optndic: optndic[name]=[]
        ...
        # other method, QMMM,DC,EL
            ...
            if vartxt != '[]':
                ...
                for namvar in varlst:
                    ...
                    if name not in namdic: namdic[name]=[]
                    namdic[name].append(var)
                    #self.gmsuser.inputnamelst.append(name)
                for name,varlst in list(namdic.items()):
                    self.gmsuser.AddInputNameAndVars(name,varlst)
            
        else:
            #print 'methnam',methnam
            if method != self.prvmethod: self.RemoveNameVarsOfPrvMethod()
            if methnam in self.uservarvaldic:
    ...
    def MakeBasisData(self,basis):
        ...
        vals=''
        if uservarval in self.uservarvaldic:
        ...
        basnam=''; ngauss=''
        if 'GBASIS' in valdic and 'NGAUSS' in valdic:
            basnam=valdic['GBASIS']; ngauss=valdic['NGAUSS']
        ndfunc=''
        if 'NDFUNC' in valdic:
            ndfunc=valdic['NDFUNC']
        else: ndfunc=0
        if 'NPFUNC' in valdic:
            npfunc = valdic['NPFUNC']
        else: npfunc=0
        basdatlst=[basnam,ngauss,ndfunc,npfunc]

        print(('basdatlst=',basdatlst))

        return basdatlst
    
    def GetBasisNDFUNCExponent(self,basnam,elm):
        print(('GetBasisNDFUNCExponent. basnam,elm=',basnam,elm))
        expval=''; expdic={}
        ...
        for elmexp in explst:
            ...
            expdic[elm]=items[1]
        if elm in expdic: expval=expdic[elm]
        else: expval='Not defined in FU. Please manual edit this.' 
        return expval

    def GetBasisNPFUNCExponent(self, basnam, elm):
        if elm != ' H':
            return ''
        print(('GetBasisNPFUNCExponent. basnam,elm=', basnam, elm))
        expval = '';
        expdic = {}
        uservarval = 'Basis:' + basnam + ':NPFUNC'
        #print('uservarval=', uservarval)
        exptxt = ''
        if uservarval in self.uservarvaldic:
            exptxt = self.uservarvaldic[uservarval]
        ...
        #print('uservarvaldic=', self.uservarvaldic)
        explst = self.gmsinpobj.SplitValues(exptxt)
        for elmexp in explst:
            # if elmexp[0] == "'": elmexp=elmexp[1:]
            # if elmexp[-1] == "'": elmexp=elmexp[:-1]
        ...
        if elm in expdic:
            ...
            expval = 'Not defined in FU. Please manual edit this.'
        return expval

    def ChangeDispButtonColor(self):
        btncolor=wx.WHITE #BLACK #'black'
        if self.gmsuser.grimmdisp: btncolor=[255,255,0] #RED #'red'  
        self.btndisp.SetBackgroundColour(btncolor)
        self.btndisp.Refresh()
    ...
    def CoordinateFromFU(self,method,check=True):     
        if self.gmsuser.coordinternal: self.gmsuser.coordinternal=False          
        else: 
            self.gmsuser.coordinternal=True
            self.gmsuser.coordfile=False
        self.ChangeCoordButtonColor()
        if not self.gmsuser.coordinternal: return
        ...
        if parmeth == 'FMO':
            const.CONSOLEMESSAGE('CoordinateFromFU. Method=FMO')
            
            nochgfrg=self.model.frag.ListNoneChargeFragment(atomattr=False)
            oddelefrg=self.model.frag.ListOddNumberOfElectronFragments()

            if len(nochgfrg) > 0:
                mess='There are '+str(len(nochgfrg))+' unassigned charges in '
                mess=mess+'fragments.\n'
                mess=mess+'Are they all zeros?'
                ans=lib.MessageBoxYesNo(mess,'CoordinateFromFU')
                if ans: 
                    self.model.frag.AssignFragmenCharge(0,nochgfrg)
                    wx.MessageBox('CoordinateFromFU. nochgfrg='+str(nochgfrg))  
                else: return                   
            if len(oddelefrg) > 0:
                mess='There are '+str(len(oddelefrg))+' odd number of electrons '
                mess=mess+'fragments.\n'
                mess=mess+'Is it OK?'
                ans=lib.MessageBoxYesNo(mess,'CoordinateFromFU')
                if not ans:
    ...
    def OnCoordinateFile(self,event):
        ...
        if self.gmsuser.coordfile: self.gmsuser.coordfile=False           
        else: 
            self.gmsuser.coordfile=True
            self.gmsuser.coordinternal=False 
        self.ChangeCoordButtonColor()
        if not self.gmsuser.coordfile: return
    ...
    def OpenInputerWin(self,wintitle,inputnamelst): #,namvarlst):     
        ...
        mode='beginner' #0
        self.inputer=GMSExpert_Frm(self.mdlwin,self.gmsuser,mode,inputnamelst,
    ...
    def ResetCoordButtonColor(self):
        self.gmsuser.coordinternal=False
        self.gmsuser.coordfile=False
        self.btnintr.SetForegroundColour(wx.BLACK) #'black')
        self.btnfile.SetForegroundColour(wx.BLACK) #'black')

    def ResetDispButtonColor(self):
        # reset -D button color
        self.gmsuser.grimmdisp=False
        self.ChangeDispButtonColor()

    def ChangeCoordButtonColor(self):
        offcol=wx.WHITE; oncol=[255,255,0] #RED
        self.btnintr.SetBackgroundColour(offcol)
        self.btnfile.SetBackgroundColour(offcol)
        #if self.gmsuser.coordinternal: intrcol=offcol #RED #'red' #'brown'
        #else: filecol=wx.YELLOW #RED #'red' #'brown'
        if self.gmsuser.coordinternal: self.btnintr.SetBackgroundColour(oncol)
        if self.gmsuser.coordfile: self.btnfile.SetBackgroundColour(oncol)
    ...
    def GetAndSetProperties(self):
        ...
        for varval in alllst:
            ...
            set=True
            if var in unsetvardic: set=False
            if uvarval in self.uservarvaldic:
    ...
    def OnProperty(self,event):       
        ...
        for varval in items:
            var=varval.split('=')[0].strip()
            uvarval='Properties:'+var
            if uvarval not in self.uservarvaldic: mess=mess+var+','
        ...
        for grpnam in self.usernamelst:
            if grpnam == 'Method':
                ...
                for wave in wavelst:
                    ...
                    unamvar=grpnam+':'+wave
                    if unamvar in self.gmsuser.uservarvaldic:
    ...
    def SetWidgets(self):
        ...
        try:       
            ...
            for item,status in list(self.widgetstatusdic.items()): 
    ...
    def SetValueToComboBox(self):
        ...
        """
        for name,combo in list(self.comboboxdic.items()):
            #combo=self.comboboxdic[name]
...
class GMSInput(object):
    ...
    def DefaultValues(self,namelst):
        defaultvaldic={}
        for name in namelst:
            if name not in self.varlstdic:
                pname=name[:-1]
                if pname not in self.varlstdic:
    ...
    def MakeInputDataText(self,inputnamelst,inputvaldic,textvaldic):
        ...
        for name in inputnamelst:
            ...
            text=text+' '+name+'\n'; temp=3*' '
            nvar=0
            for namvar,datlst in list(inputvaldic.items()): # lst:[val,color] 
            ...
            text=text.rstrip()
            # process TEXT data
            for namvar,datlst in list(inputvaldic.items()):
                if datlst[1] == self.unsetcolor: continue
                if namvar.split(':')[0] != name: continue
                if datlst[0] != 'TEXT': continue
                temp=''
                if namvar in textvaldic:
        ...
        if namvar in textvaldic:
            ...
            if '$BASIS:GBASIS' not in inputvaldic:
                txt=txt+'\n'
    ...
    def ReadInputDocText(self,docfile):
        ...
        for s in f.readlines():
            ...
            # @valneed: required names
                ...
                if nam not in valrequireddic: valrequireddic[nam]=[]
            ...
            # @valoptn: option names
                ...
                if nam not in valconflictdic: valconflictdic[nam]=[]
                items=self.GetRequiredOptionItems(val)
...
class SetPathAndCmdForGAMESS_Frm(wx.MiniFrame):
    ...
    def CreatePanel(self):
        ...
        self.tclpath=wx.TextCtrl(self.panel,-1,self.gmspath,pos=(75,yloc-2),size=(245,htcl))
        btnbrwp=wx.Button(self.panel,wx.ID_ANY,"browse",pos=(330,yloc-2),size=(50,22))
        btnbrwp.Bind(wx.EVT_BUTTON,self.OnBrowsePath)
        yloc += 25
        wx.StaticText(self.panel,-1,"Command:",pos=(10,yloc),size=(60,18)) 
        self.tclcmd=wx.TextCtrl(self.panel,-1,self.gmscmd,pos=(75,yloc-2),size=(245,htcl))
        btnbrwc=wx.Button(self.panel,wx.ID_ANY,"browse",pos=(330,yloc-2),size=(50,22))
        btnbrwc.Bind(wx.EVT_BUTTON,self.OnBrowseCMD)
        
        yloc += 25
        wx.StaticText(self.panel,-1,"Arguments:",pos=(10,yloc),size=(65,18)) 
        self.tclarg=wx.TextCtrl(self.panel,-1,self.gmsarg,pos=(80,yloc-2),size=(295,htcl))
        yloc += 25
        wx.StaticText(self.panel,-1,"Scratch:",pos=(10,yloc),size=(60,18)) 
        self.tclscr=wx.TextCtrl(self.panel,-1,self.gmsscr,pos=(75,yloc-2),size=(245,htcl))
        btnscr=wx.Button(self.panel,wx.ID_ANY,"browse",pos=(330,yloc-2),size=(50,22))
        btnscr.Bind(wx.EVT_BUTTON,self.OnBrowseScratch)
        yloc += 25
        wx.StaticText(self.panel,-1,"tests:",pos=(10,yloc),size=(60,18)) 
        self.tcltests=wx.TextCtrl(self.panel,-1,self.gmstests,pos=(75,yloc-2),size=(245,htcl))
        btntests=wx.Button(self.panel,wx.ID_ANY,"browse",pos=(330,yloc-2),size=(50,22))
        btntests.Bind(wx.EVT_BUTTON,self.OnBrowseTests)
        yloc += 30
        btnapl=wx.Button(self.panel,wx.ID_ANY,"Apply",pos=(100,yloc-2),size=(50,22))
        btnapl.Bind(wx.EVT_BUTTON,self.OnApply)
        btnrmv=wx.Button(self.panel,wx.ID_ANY,"Clear",pos=(170,yloc-2),size=(45,22))
        btnrmv.Bind(wx.EVT_BUTTON,self.OnClear)
        btncan=wx.Button(self.panel,wx.ID_ANY,"Close",pos=(250,yloc-2),size=(50,22))
    ...
    def OnClose(self,event):
        if not self.setpath:
            mess ="Do you want to keep the GAMESS path and command on \n"
            mess+="FUDATASET/Programs/gamess/gamesspath file."
            retcode=lib.MessageBoxYesNo(mess,"")
            if retcode: # == wx.YES:
                #if wx.YES:
...
class FMOInputText():
    ...
    def FMODATATextFromFU(self,jobtitle,basdatlst):
        ...
        #const.CONSOLEMESSAGE('gmsuser.nlayer='+str(self.gmsuser.nlayer))
        text=3*' '+jobtitle+'\n'
        text=text+blk+'C1\n'
        for i in range(nlayer):
            for j in range(len(elmlst)):
                ...
                basnam = basdatlst[i]
                bastxt=FMOInputText.BasisTextData(basnam,elmlst[j])
                #print('bastxt after BasisTextData =',bastxt)
                elm=elmlst[j]; ian=const.ElmNmb[elm]
                line=' '+elm

                line=line+'-'+str(i+1)+5*blk+str(ian)+eol
                line+=6*blk+bastxt+eol
                """
                line=line+'-'+str(i+1)+5*blk+str(ian)+eol
                basnam=basdatlst[i][0]
                ...
                if basdatlst[i][3] != '':
                    hydexp = self.gmsuser.beginner.GetBasisNPFUNCExponent(basnam, elm)
                    if len(hydexp) > 0:
                        txt='p 1 ; 1 '+hydexp+' 1.0'
                        line=line+8*blk+txt+eol
                    else:
                        line = line + 8 * blk + 'supply pol function data.' + eol
                """
    ...
    def FMOAttribTextFromFU(self,attrib):
        ...
        if not attrib in attriblst:
            #const.CONSOLEMESSAGE('FMOAttribTextFromFU-Wong attrib='+attrib)
            return 0,''
                
        attriblst=self.model.frag.GetFragmentAttribute(attrib)
        
        #const.CONSOLEMESSAGE('FMOAttribTextFromFU-attrib='+attrib)
        #const.CONSOLEMESSAGE('FMOAttribTextFromFU-attriblst='+str(attriblst))
    ...
    def FMOFrgchgTextFromFU(self):
        ...
        for frgnam in frgnamlst:
            frgchglst.append(self.model.mol.frgattribdic[frgnam][0])
    ...
    @staticmethod
    def BasisTextData(basnam,elm):
        #print('Entered in BasisTextData. basnam,elm=',basnam,elm)
        poples=['STO-3G','3-21G',
                '6-31G','6-31G*','6-31G**',
                '6-311G','6-311G*','6-311G**']
        basccs= ['cc-pVDZ','aug-cc-pVDZ',
                 'cc-pVTZ','aug-cc-pVTZ']
        bastxt=''
        if basnam in poples:
            bastxt=FMOInputText.BasisPople(basnam,elm)
            #print('Pople basis. bastxt=',bastxt)
        elif basnam in basccs:
            bastxt=FMOInputText.BasisCC(basnam,elm)
            #print('CC basis. bastxt=',bastxt)
        #print('basnam in BasisTextData=', basnam)
        #print('bastxt in BasisTextData=',bastxt)
        return bastxt

    @staticmethod
    def BasisPople(basnam,element):
        """
        STO:  Available H-Xe(54)
        N21:  Available H-Xe(54) for N=3
        N31:  Available H-Kr(36), for NGAUSS=6
        N311: Available H-Ne(10), for NGAUSS=6.
        """
        #print('Entered in BasisPople')

        dfunc631 ={'C':'d 1 ; 1 0.800 1.0',
                   'N':'d 1 ; 1 0.800 1.0',
                   'O':'d 1 ; 1 0.800 1.0',
                   'S':'d 1 ; 1 0.650 1.0'}
        dfunc6311 = {'C':'d 1 ; 1 0.626 1.0',
                     'N':'d 1 ; 1 0.913 1.0',
                     'O':'d 1 ; 1 1.292 1.0',
                     'S':'d 1 ; 1 0.650 1.0' }
        pfunc={'6-31':'p 1 ; 1 1.10 1.0',
               '6-311':'p 1 ; 1 0.750 1.0'}

        #print ('basnam, elm in BasisPople=',basnam,element)
        poltxt = 'Please put polarization function here'
        putbas = 'Please put basis function data here'
        bastxt=''
        elm1=element.strip().upper()
        elm=elm1
        if len(elm1) == 1: elm=' '+elm1
        atmnum=const.ElmNmb[elm]
        #print('elm,elm1,atmnum  in BasisPople=',elm,elm1,atmnum)

        basnam+=' '
        items=basnam.split('G')
        bas=items[0]
        pol=items[1].strip()
        #print('basnam, bas, pol in BasisPople',basnam,bas,pol)
        #
        if bas == 'STO-3':
            if atmnum > 54:
                bastxt=putbas
    ...
    @staticmethod
    def BasisCC(basnam,element):
        """ H-He(1-2), Li-Ne(3-10), Na-Ar(11-18), Ga-Kr(20-36)
        ...
        """
        ccnamdic = {'cc-pVDZ': 'CCD', 'cc-pVTZ': 'CCT',
                    'aug-cc-pVDZ':'ACCD','aug-cc-pVTZ':'ACCT'}
        elm=element.strip().upper()
        if len(elm) == 1: elm=' '+elm
        atmnum=const.ElmNmb[elm]

        putmess='Please put '+basnam+' basis set data here'

        if atmnum <= 36:
            if basnam in ccnamdic:
                bastxt=ccnamdic[basnam]
        else:
            bastxt=putmess
