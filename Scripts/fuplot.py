...
class fuPlot(wx.Frame):
    # main program for draw FMO property graphs
    def __init__(self, parent):  # ,winpos,fuplot):
        winsize = lib.WinSize((520, 250), True)  # 300))
        # print 'SYSTEM',const.SYSTEM
        # if const.SYSTEM != const.WINDOWS: winsize=(400,300)
        winpos = [0, 20]
        # if lib.GetPlatform() == "WINDOWS": winsize=lib.WinSize((420,300),True)
        self.title = 'fuPlot'
        ...
        # set icon
        try:
            lib.AttachIcon(self, const.FUPLOTICON)
        except:
            pass
        #
        """
        ...
        lib.InsertTitleMenu(self.menubar, '[fuPlot]')
        self.Bind(wx.EVT_MENU, self.OnMenu)
        # accept drop files
        droptarget = lib.DropFileTarget(self)
        self.SetDropTarget(droptarget)
        #
        self.coorpmdlwin=True
        self.prpobjdic = {}  # {prpnam:FMOOutputFileReader instance,...}
    ...
    def SetFontToPlot(self):
        if 'SetFontToPlot' in self.pltargs:
    ...
    def CreatePropertyPanel(self):
        ...
        wx.StaticText(self.prppan, wx.ID_ANY, 'Plot data:', pos=(xloc, yloc), size=(70, 20))
        self.btngist = wx.Button(self.prppan, -1, "Info", pos=(xloc + 80, yloc - 2), size=(45, 22))
        self.btngist.Bind(wx.EVT_BUTTON, self.OnViewInfo)
        lib.SetTipString(self.btngist, 'View file information')
        self.btnvprp = wx.Button(self.prppan, -1, "Filter items", pos=(xloc + 140, yloc - 2), size=(90, 22))
        self.btnvprp.Bind(wx.EVT_BUTTON, self.OnViewPropDic)
        lib.SetTipString(self.btnvprp, 'View filter items for check')
        yloc += 20
        self.tcsel = wx.TextCtrl(self.prppan, -1, "", pos=(xloc, yloc), size=(xsize, 22),
        ...
        lib.SetTipString(stprp, 'Select items and push "View" or "Plot"')
        # yloc += tcheight+5
        self.btnview = wx.Button(self.prppan, wx.ID_ANY, "View", pos=(xloc + 80, yloc), size=(45, 22))
        self.btnview.Bind(wx.EVT_BUTTON, self.OnViewProperty)
        lib.SetTipString(self.btnview, 'View selected property')
        self.btnplt = wx.Button(self.prppan, wx.ID_ANY, "Plot", pos=(xloc + 140, yloc), size=(45, 22))
        self.btnplt.Bind(wx.EVT_BUTTON, self.OnPlotProperty)
        lib.SetTipString(self.btnplt, 'Plot selected property')
        yloc += 25
    ...
    def CreateSelectDataPanel(self):
        ...
        lib.SetTipString(self.lbdat, 'List of data obtained by filer')
        # command button
        # xloc=wclb/2;
        yloc1 = yloc + hclb + 10
        xloc = 10
        btnclr = wx.Button(self.panel, wx.ID_ANY, "Clear", pos=(xloc, yloc1), size=(50, 22))
        btnclr.Bind(wx.EVT_BUTTON, self.OnClearData)
        lib.SetTipString(btnclr, 'Clear all data')
        btnrmv = wx.Button(self.panel, wx.ID_ANY, "Remove", pos=(xloc + 65, yloc1), size=(65, 22))
        btnrmv.Bind(wx.EVT_BUTTON, self.OnRemoveData)
        lib.SetTipString(btnrmv, 'Remove data from the list')
        btnsel = wx.Button(self.panel, -1, label='Set for plot',
                           pos=(xloc + 145, yloc1), size=(90, 22))
        lib.SetTipString(btnsel, 'Set selected data for plot')
        btnsel.Bind(wx.EVT_BUTTON, self.OnSelectForPlot)
        yloc1 += 25
        wx.StaticLine(self.panel, pos=(-1, yloc1), size=(w / 2-2, 2), style=wx.LI_HORIZONTAL)
        # button to pop-up derived data creation panel
        yloc1 += 10
        self.btndrvdat = wx.ToggleButton(self.panel, -1, label='Make derived data',
                                         pos=(60, yloc1), size=(130, 22))
        lib.SetTipString(self.btndrvdat, 'Open derived data panel (toggle)')
    ...
    def ViewInfo(self):
        ...
        try:
            ...
            return
        if self.selected in self.datadic:
            head = 'Information of "' + self.selected + '"\n\n'
            if self.datadic[self.selected][1] == 'cube':
                info, natoms, atmcc = rwfile.ReadCubeFile(filename)
                text = head+cube.DrawCubeData_Frm.MakeCubeDataInfoText(filename, 'Cube', natoms, info)
            else:
                text = self.infotext

        elif self.selected in self.drvdatadic:
        ...
        if len(text) > 0:
            ...
            filtext='Output file="'+filename+'"\n'
            filtext+='Filter file="' + self.filterfile + '"\n\n'
            text = head + filtext + self.infotext
            self.OpenTextViewer(title, text)

    def OnViewPropDic(self,event):
        self.ViewFilterItemDic()
    ...
    def OnViewProperty(self, event):
        ...
        if prp == 'Opt.Coordinates':
            ...
            text = self.TextOrbitalEnergy()
        elif prp == 'Orbitals':
            text = self.TextOrbital()
        elif prp == 'PIE' or prp == 'PIEDA':
            text = self.TextPIEDA()
        elif prp == 'CTCharge':
            text = self.TextCTCharge()
        elif prp == 'Mulliken.Q(2)-Q(1)':
            text = self.TextMullikenQ(prp)
        elif prp == 'Mulliken.Q(2)':
            text = self.TextMullikenQ(prp)
        elif prp == 'Mulliken.Q(1)':
            text = self.TextMullikenQ(prp)
        elif prp == 'Mono.Energies':
            text=self.TextMonoEnergies()
        elif prp == 'Mono.Iteration':
            text=self.TextMonoIteration()

        elif prp == 'Cube':
            text = 'file=' + self.selected + '\n'
            text = text + 'prp=' + prp + '\n\n'
        else:
            text = self.TextProperty(prp)
        #
        title = 'View ' + prp
        self.OpenTextViewer(title, text)

    def ListMolElements(self):
        elmlst = []
        if self.coorpmdlwin:
            elmlst=self.model.ListElement()
            elmlst=self.model.ListElement(False)
        return elmlst

    def TextMullikenQ(self,prp):
        text = 'file=' + self.selected + '\n'
        text = text + prp + '\n\n'
        if prp == 'Mulliken.Q(2)-Q(1)':
            datlst = self.prpdic['Mulliken.Q(2)-Q(1)']
        elif prp == 'Mulliken.Q(2)':
            datlst = self.prpdic['Mulliken.Q(2)']
        elif prp == 'Mulliken.Q(1)':
            datlst = self.prpdic['Mulliken.Q(1)']
        ...
        for i in range(len(datlst)):
            ...
            if value == 'end':
                ...
                continue
            if len(elm) == 0:
                text = text + (('%5d' % (i + 1)) + ':  ' + ('%8.5f' % value) + '\n')
            else:
                text = text + (('%5d' % (i+1))+'('+elm[i]+'):  '+('%8.5f' % value) + '\n')
    ...
    def TextCTCharge(self):
        if 'Q(I->J)' not in self.prpdic['PIEDA']:
            return 'No CT charge data'
        prp = 'PIEDA';
        item = 'Q(I->J)'
        text = 'FMO output file=' + self.selected + '\n'
        text = text + 'CTCharge Q(I->J) in Mulliken population' + '\n\n'
        ifrglst = self.prpdic['PIEDA']['I']
        jfrglst = self.prpdic['PIEDA']['J']
        npair = len(ifrglst)

        frgnam = self.prpdic['Fragment.Name']
        nfrg = len(frgnam)
        text+='    I        J    Population\n'
        for k in range(npair):
            if self.prpdic['PIEDA']['Q(I->J)'][k] == 'end':
                break
            text += ('%5d' % jfrglst[k])+' -> '+('%5d' % ifrglst[k])+'   '
            text+= '%9.4f' % self.prpdic['PIEDA']['Q(I->J)'][k]
            text+='\n'

        # Note on covalently bonded fragments
        bonded=self.TextBondedPairs('charge transfer')
        if len(bonded) > 0:
            text += bonded
        return text

    def TextPIEDA(self):
        if not 'PIEDA' in self.prpdic:
            return 'No PIEDA data.'
        prp = 'PIEDA'
        keylst = list(self.prpdic[prp].keys())
        text = 'FMO output file=' + self.selected + '\n'
        text = text + 'PIEDA between fragments I and J(kcal/mol)\n\n'
        #
        piedaitems=['total','Ees','Eex','Ect+mix','Edisp','Gsol']
        prppiedaitems=list(self.prpdic['PIEDA'].keys())
        ifrglst= self.prpdic['PIEDA']['I']
        jfrglst = self.prpdic['PIEDA']['J']
        npair=len(ifrglst)
        text+='    I    J      '
        for item in piedaitems:
            if not item in prppiedaitems: return
            text+=item + (10-len(item))*' '
        text+='\n'
        for k in range(npair):
            if ifrglst[k] == 'end': break
            ifrg = int(ifrglst[k])
            jfrg = int(jfrglst[k])
            text += '%5d' % ifrg + '%5d' % jfrg + 2 * ' '
            temp=''
            for item in piedaitems:
                if not item in prppiedaitems: continue
                temp+='%10.4f' % self.prpdic['PIEDA'][item][k]
            text+=temp+'\n'
        # Note on covalently bonded fragments
        bonded=self.TextBondedPairs('interactions')
        if len(bonded) > 0:
            text += bonded

        return text

    def TextBondedPairs(self,item='interactions'):
        """

        :param str item: item name, 'interactions' or 'charge transfer'
        :return: text(str) - note
        """
        if not 'PIEDA' in self.prpdic:
            return 'No PIEDA data.'
        text=''
        ...
        npair=len(ifrglst)
        for k in range(npair):
            if self.prpdic['PIEDA']['R'][k] == 'end': continue
            if self.prpdic['PIEDA']['R'][k] == 0:
                bonded+='['+str(ifrglst[k])+', '+str(jfrglst[k])+'], '
        if len(bonded) > 0:
            text = '\n'
            text += 'Note: The fragments in parentheses listed below are joined'
            text += ' by a covalent bond.\n'
            text += ' Therefore, the '+item+ ' of these pairs has'
            text += ' no physical meaning in FMO. \n'
            text += '   '+bonded[:-2]
        return text

    def TextMonoEnergies(self):
        prp='Mono.Energies'
        if not prp in self.prpdic:
            return 'No '+prp+' data.'
        text = 'FMO output file=' + self.selected + '\n'
        text = text + 'Monomer energy(a.u.)\n\n'
        #
        datlst = self.prpdic[prp]
        nfrg=len(datlst)
        if datlst[-1] == 'end':
            nfrg-=1
        if 'Fragment.Name' in self.prpdic:
            frgnam = self.prpdic['Fragment.Name']
        else:
            frgnam=nfrg*['']
        text+='    I (name)        Energy\n'
        for i in range(nfrg):
            text+=('%5d' % (i+1)) + ' ('+frgnam[i]+')'+(8-len(frgnam[i]))*' '
            text+= '%12.6f' % self.prpdic[prp][i] +'\n'
        return text

    def TextMonoIteration(self):
        prp='Mono.Iteration'
        if not prp in self.prpdic:
            return 'No '+prp+' data.'
        text = 'FMO output file=' + self.selected + '\n'
        text = text + 'Max. monomer energy change at each iterations(a.u.)\n\n'
        datlst = self.prpdic[prp]
        ndat=len(datlst)
        if datlst[-1] == 'end':
            ndat-=1
        text += '  Iter    Max.deltaE\n'
        for i in range(ndat):
            text+=('%5d' % (i+1)) + '%15.6f' % self.prpdic[prp][i] +'\n'
        return text
    ...
    def AreFragments(self):
        nfrg = 0
        if self.model.mol is None:
            lib.MessageBoxOK('No molecule object in fu-MdlWin.', 'AreFragments')
            return None
        natm = len(self.model.mol.atm)
        if natm != self.prpdic['NATOMS'][0]:
            mess = 'Number of atoms in fu-MdlWin and  molecule and that in '
            mess += self.selected + '. Unable to continue'
            lib.MessageBoxOK(mess, 'AreFragments')
            return None
        nfrg = 0
        for atom in self.model.mol.atm:
            if atom.frgnam != '': nfrg += 1
        if nfrg == 0:
            return False
        else:
            return True

    def SetFragmentsToMol(self):
        try:
            natm = len(self.model.mol.atm)
        except:
            return False
        ...
        for i in range(len(self.model.mol.atm)):
            self.model.mol.atm[i].frgnmb = self.prpdic['IFG'][i]
        # fragment name
        for i in range(len(self.model.mol.atm)):
            ifg = self.model.mol.atm[i].frgnmb - 1
            self.model.mol.atm[i].frgnam = self.prpdic['Fragment.Name'][ifg]
    ...
    def OpenTextViewer(self, title, text):
        winpos = wx.GetMousePosition()
        winsize = [480, 300]
        parent = self
        #self.viewer = subwin.TextViewer_Frm(parent, winpos, winsize, title, text,menu=True)
        self.editor = subwin.TextEditor_Frm(parent, -1, winpos, winsize, title, text=text,
                                            mode='Viewer')
        self.editor.menubar.Remove(0)

    def ViewOutputFile(self):
        self.selected = self.lbdat.GetStringSelection()
        if self.selected == '':
            wx.MessageBox('No file is selected','ViewOutputFile')
            return
        filename = self.datadic[self.selected][0]
        winpos = wx.GetMousePosition()
        winsize = [600, 500]
        parent = self
        title='Output file'
        self.viewout = subwin.TextEditor_Frm(parent, -1, winpos, winsize, title,
                                             textfile=filename, mode='Viewer')

    def ViewFilterFile(self):
        self.selected = self.lbdat.GetStringSelection()
        filename=self.datadic[self.selected][1]+'.filter'
        if not os.path.exists(filename):
            mess='The filter file "'+filename+'" does not exists.'
        ...
        parent = self
        title='Filter file'
        self.viewfilt = subwin.TextEditor_Frm(parent, -1, winpos, winsize, title,
                                              textfile=filename, mode='Viewer')
    ...
    def MakeOptCoordList(self, opt):
        ...
        elm = [];
        seq = []
        if 'NATOMS' in self.prpdic:
            natm = int(self.prpdic['NATOMS'][0])
        else:
            natm = len(self.model.mol.atm)
        if natm <= 0:
            ...
            return coordlst
        if 'NUC.CHARGE' in prpdic:
            chg = prpdic['NUC.CHARGE']
        elif 'ELEMENT' in prpdic:
            elm = prpdic['ELEMENT']
        else:
            seq = list(range(natm));
            seq = numpy.array(seq);
    ...
    def SetPropertyItems(self):
        ...
        self.lcprp.InsertColumn(0, '#', width=20, format=wx.LIST_FORMAT_LEFT)
        self.lcprp.InsertColumn(1, 'Item', width=150, format=wx.LIST_FORMAT_LEFT)
        ...
        for i in range(len(self.propertyitems)):
            ...
            if txt in self.prptypdic:
                nmb = self.prptypdic[txt][1]
            else:
                nmb = 1
    ...
    def GetCubeFile(self):
        ...
        if ext == '.mep' or ext == '.den' or ext == '.cub':
            return filename
        else:
            return ''
    ...
    def OnPlotProperty(self, event):
        prplst = self.GetSelectedProperty()
        if len(prplst) <= 0:
            wx.MessageBox("Please select a property.", "OnPlotProperty")
            return
        prp = prplst[0]
    ...
    def PlotProperty(self, prp):  # ,plttyp):
        ...
        if prp == 'Orbital':
            ...
            if prptyp == 'bar' or prptyp == 'line':
                if not const.MATPLOTLIB:
                    mess = 'Cannot plot. MatPlotLib is not installed.'
                    lib.MessageBoxOK(mess, 'fuplot(PlotProperty)')
                    return
                # if prporg == "":
                ...
                if atom == 'atom':
                    self.PlotOnMatPlotLib(prp, datlst[0], 'atom')
                else:
                    winlabel = 'Plot property'
                    winsize = lib.WinSize((600, 300))
                    self.mpl = subwin.MatPlotLib_Frm(self, -1, self.model, [0, 20], winsize, winlabel)
                    self.mpl.Show()
                    # self.mpl.Clear()
    ...
    def PlotOnMatPlotLib(self, prp, datlst, selectitem):
        ...
        if not const.MATPLOTLIB:
            ...
            lib.MessageBoxOK(mess, 'fuplot(PlotProperty)')
            return
        molnam=self.MolNamFromFilename()
        ctrlflag = self.model.ctrlflag
        if selectitem == 'atom':
            natm = len(self.model.mol.atm)
            if natm != len(datlst):
                mess = 'Cannot plot, since numbers of data and atoms are different.'
            ...
            nfrg = len(frgnamlst)
            if nfrg != len(datlst):
                mess = 'Cannot plot, since numbers of data and fragments are different.'
                lib.MessageBoxOK(mess, 'fuplot(PlotInMatPlotLib2)')
                return
        else:
            return
        #
        self.mpl = subwin.PlotAndSelectAtoms(self.mdlwin, -1, self.model, prp,
                                             selectitem)
        maxvalue = max(datlst)
        selval = maxvalue * 0.5
    ...
    def MolNamFromFilename(self,datnum=-1):
        if datnum < 0:
            datnum=self.selected
        filename = self.datadic[datnum][0]
    ...
    def OnSelectForPlot(self, event):
        ...
        if self.selected == '':
            ...
            molnam=self.MolNamFromFilename()
            #print('molnam=inpfile=',molnam)
            curmol,curmolobj=self.model.molctrl.GetCurrentMol()
            curnam=curmolobj.name
            self.coorpmdlwin = True
            if curnam != molnam:
                self.coorpmdlwin = False
                molidx=self.model.molctrl.GetMolIndex(molnam)
                if molidx < 0:
                    # open the mol object
                    title='fuplot.OnSelectForPlot'
                    mess='The molecule obeject is not present in MdlWin.\n'
                    mess+='It can be created using the Gamess-FMO input file.\n'
                    mess+='Would you like to open the "'+molnam+'" file?'
                    dlg = wx.MessageDialog(None, mess, title,
                            style=wx.YES_NO | wx.ICON_QUESTION | wx.STAY_ON_TOP)
                    if dlg.ShowModal() == wx.ID_YES:
                        wcard='inp(*.inp)|*.inp'
                        inpfile=lib.GetFileName(parent=None, wcard=wcard, rw='r', check=False,
                                                defaultname=molnam,message=title)
                        head, tail = os.path.split(inpfile)
                        if tail == molnam:
                            if os.path.exists(inpfile):
                                self.model.BuildMol('inp',inpfile,addbond=True)
                                self.coorpmdlwin = True
                            else:
                                mess='The file is not found.\n'
                                mess+='The coorpolation with MdlWin is turned off.'
                                wx.MessageBox(mess,title)
                        else:
                            mess='Wrong input file name. Ignored.\n'
                            mess += 'The coorpolation with MdlWin is turned off.'
                            wx.MessageBox(mess, title)
                else:
                    # switch mol object
                    self.model.SwitchMol(molidx, True, True)
                    self.coorpmdlwin = False

            #print('coorpmdlwin=',self.coorpmdlwin)
            title = 'Progress to create property data'
            mess = 'Creating property data ...'
            self.textbox = subwin.ProgressBar_Frm(self, mess=mess, title=title)
            self.textbox.StartProgressBar()

            self.SelectForPlot()

            if self.textbox: self.textbox.OnClose(1)
    ...
    def SelectForPlot(self):
        ...
        #
        filternam = ''
        if self.selected in self.datadic: filternam = self.datadic[self.selected][1]
        self.propertyitems = []
        if filternam == 'cube':
            ...
            title = ''
            for keynam, lst in list(self.prptypdic.items()):
                # info=False
                prptyp = lst[0]
                if prptyp == 'info':
                    ...
                    self.propertyitems.append(keynam)

            if not self.IsDerivedData(self.selected):
                # add Mulliken.Q(2)-Q(1)
                if 'Mulliken.Q(1)' in self.prpdic and \
                        'Mulliken.Q(2)' in self.prpdic:
    ...
    def AddMullikenQ2M1ToPrpDic(self):
        q21 = []
        if 'Mulliken.Q(2)' not in self.prpdic: return
        if 'Mulliken.Q(1)' not in self.prpdic: return
    ...
    def EnableFMOButtons(self, on):
        if on:
            ...
            self.btnresult.Enable()
            self.btnviewprop.Enable();
            self.cbprop.Enable()
            self.btnplt.Enable();
            self.btncls.Enable()
            ...
            self.btnresult.Disable()
            self.btnviewprop.Disable();
            self.cbprop.Disable()
            self.btnplt.Disable();
            self.btncls.Disable()
    ...
    def SavePropChoice(self, on):
        ...
        if on:  # save
            if self.ckbpie.GetValue():
                self.pltpie = 1
            else:
                self.pltpie = 0
            if self.ckbctc.GetValue():
                self.pltctchg = 1
            else:
                self.pltctchg = 0
            if self.ckbmul.GetValue():
                self.pltmulchg = 1
            else:
                self.pltmulchg = 0
            if self.ckbesp.GetValue():
                self.pltespot = 1
            else:
                self.pltespot = 0
            if self.ckbden.GetValue():
                self.pltden = 1
            else:
                self.pltden = 0
            if self.ckborb.GetValue():
                self.pltorb = 1
            else:
                self.pltorb = 0
        else:  # recover
            ...
            if self.pltespot == 1:
                self.ckbesp.SetValue(True)
            else:
                self.ckbesp.SetValue(False)
            if self.pltden == 1:
                self.ckbden.SetValue(True)
            else:
                self.ckbden.SetValue(False)
            if self.pltorb == 1:
                self.ckborb.SetValue(True)
            else:
                self.ckborb.SetValue(False)

    def PlotFragmentProperty(self, fmoprp):
        # fmoprp: 'PIE,PIEDA','CTCharge', or 'MullikenCharge'
        methnam = 'fuplot(PlotFragmentProperty)'
        natm = self.prpdic['NATOMS'][0]
        frgnam = self.prpdic['Fragment.Name']
        nfrg = len(frgnam)
        try:
            ifg = self.prpdic['IFG']
        ...
        if nfrg != len(frgnamout):
            ...
            if len(ifg) != natm:
                ...
                if nter > 0:
                    mess = 'Molecule includes ' + str(nter)
                    mess = mess + '" TERs". Is it OK to delte them?'
                    ans = lib.MessageBoxYesNo(mess, methnam)
                    if not ans:
                        return  # dlg.ShowModal() == wx.ID_NO: return
                    else:
                        self.model.DeleteAllTers()
            # natm=len(self.model.mol.atm)
            if len(ifg) != natm:
                mess = 'The number of atoms in output file is different from taht '
                mess = mess + 'of current molecule.\n'
                mess = mess + 'Unable to plot.'
                lib.MessageBoxOK(mess, methnam)
                return

        if fmoprp == 'PIEDA' or fmoprp == 'PIE' or fmoprp == 'CTCharge':
            if 'PIEDA' not in self.prpdic:
        ...
        # check number of data in dat1d
        if len(dat1d) != int(nfrg*(nfrg-1)/2) +1: # +1: 'end' in dat1d
            selected = self.lbdat.GetStringSelection()
            mess='Number of 1d data does not match to 2d; '
            mess+='unable to plot 2d properties.\n'
            mess+='Please check the output/filter file.\n'
            mess+='output file ="'+self.datadic[selected][0]+'"\n'
            mess += 'filter file ="' + self.datadic[selected][1]+'.filter"\n'
            wx.MessageBox(mess,'PlotFragmentProperty')
            return
        ...
        if fmoprp == 'CTCharge':
            if 'Q(I->J)' not in self.prpdic['PIEDA']:
                ...
                pltprp = 1
                if valrange == 0:
                    mess = 'All CT charges are zeros. Unable to plot.\n'
                    mess = mess + 'Please close the CT charge plot panel'
                    lib.MessageBoxOK(mess, methnam)
                    # return
        elif fmoprp == 'PIEDA':
        ...
        self.graphobjdic[fmoprp] = graphobj
        self.graphobjdic[fmoprp].Show()
        if pltprp == 0:
            valrange = 50.0
            if self.IsDerivedData(self.selected): valrange = 5.0
            self.graphobjdic[fmoprp].SetYRange(valrange)
        elif pltprp == 1:
            self.graphobjdic[fmoprp].SetYRange(valrange)
        else:
            self.graphobjdic[fmoprp].DrawGraph(True)
    ...
    def MakePIEDAPlotData(self, nfrg, molint):
        ...
        for i in range(len(pieda)):
            ...
            for j in range(len(pieda[i])):
                if i == j:
                    pieda[i][j].append(obe)
                else:
                    pieda[i][j].append(0.0)
        return False, cmplst, piedadat
    ...
    def ReadProperties(self):
        ...
        if self.selected in self.datadic:
            ...
            prpdic, prptypdic, vardic = \
                self.prpobjdic[filternam].ReadOutputFile(file)
        elif self.selected in self.drvdatadic:
            self.drvprpinfo = {}
            ...
            for i in range(1, len(cmpdat)):
                ...
                tmpprpdic, tmpprptypdic, tmpvardic = \
                    self.prpobjdic[filternam].ReadOutputFile(file)
                try:
                    matm = len(tmpprpdic['IFG'])
                    if 'end' in tmpprpdic['IFG']: matm -= 1
                    if 'natm' in self.drvprpinfo:
                        self.drvprpinfo['natm'].append(matm)
                except:
                    matm = 0
                try:
                    ...
                    mfrg = 1
                if 'nfrg' in self.drvprpinfo:
                    self.drvprpinfo['nfrg'].append(mfrg)
                # two-body properties
                prpdic = self.DerivedPIEDAData(nfrg, ifrg, prpdic, mfrg, tmpprpdic,
                                               arith)
                # one-body energy
                prpnam = 'Mono.Energies'
                if prpnam in prpdic:
                    prpdic = self.DerivedOneBodyProperty(ifrg, prpdic, prpnam, mfrg,
                                                         tmpprpdic, arith)
                ...
                prpnam = 'Mulliken.Q(1)'
                if prpnam in prpdic:
                    prpdic = self.DerivedMullikenCharge(mfrg, iatm, prpdic, prpnam,
                                                        matm, tmpprpdic, arith)
                prpnam = 'Mulliken.Q(2)'
                if prpnam in prpdic:
                    prpdic = self.DerivedMullikenCharge(mfrg, iatm, prpdic, prpnam,
                                                        matm, tmpprpdic, arith)
    ...
    def DerivedPIEDAData(self, nfrg, ifrg, prpdic, mfrg, tmpprpdic, arith):
        ...
        # CTCharge
        if 'PIEDA' in tmpprpdic:
            if 'Q(I->J)' in tmpprpdic['PIEDA'] and mfrg > 1:
                ...
                for i in range(1, mfrg - 1):
                    for j in range(i):
                        ...
                        if tmpprpdic['PIEDA']['Q(I->J)'][k] == 'end':
                            continue
                        if arith == 'sub':
                            ctcharge[k0] -= tmpprpdic['PIEDA']['Q(I->J)'][k]
                        else:
                            ctcharge[k0] += tmpprpdic['PIEDA']['Q(I->J)'][k]
        # pieda data
        cmplst = ['total', 'tot', 'Ees', 'Eex', 'Ect+mix', 'Edisp', 'Gsol']
        if 'PIEDA' in tmpprpdic and mfrg > 1:
            for cmp in cmplst:
                if cmp not in tmpprpdic['PIEDA']: continue
    ...
    def DerivedOneBodyProperty(self, ifrg, prpdic, prpnam, mfrg, tmpprpdic, arith):
        if prpnam in tmpprpdic:
            ...
            for i in range(mfrg):
                ...
                if arith == 'sub':
                    monop[i + ifrg] -= tmpprpdic[prpnam][i]
                else:
                    monop[i + ifrg] += tmpprpdic[prpnam][i]
            prpdic[prpnam] = monop
        return prpdic
    ...
    def PIEDAArrayData(self, nfrg):
        ...
        cmpdic = {};
        nmb = 0
        for item, data in list(self.prpdic['PIEDA'].items()):
        ...
        for cmp, data in list(cmpdic.items()):
        ...
        for ii in range(ndat):
            ...
            if pieda:
                try:
                    ...
                    #print(('ii,tot,es,ex,ct=',ii,tot,es,ex,ct))
                except:
                    mess='Failed to obtain PIEDA component(s)\n'
                    mess+='i,j='+str(i)+', '+str(j)+'\n'
                    mess+='es='+str(es)+'\n'
                    mess+='ex='+str(es)+'\n'
                    mess += 'ct=' + str(es) + '\n'
                    wx.MessageBox(mess,'fuplot.PIEDAArrayData')
                    tot=999.0
                    es=9990
                    ex=999.0
                    ct=999.0
                    ###continue
                di = 0.0
                if 'Edisp' in cmpdic: di = cmpdic['Edisp'][ii]
                tmp.append(tot);
                tmp.append(es)
                tmp.append(ex);
                tmp.append(ct);
        ...
        for i in range(nfrg):
            if pieda:
                piedadat[i][i] = piedadat[i][i] + ncmp * [0.0]
            else:
                piedadat[i][i] = piedadat[i][i] + [0.0]
    ...
    def CountNumberOfPIEDAData(self):
        nd = 0
        if 'PIEDA' not in self.prpdic: return nd
        tot = 'total'
        if tot not in self.prpdic['PIEDA']: tot = 'tot'
    ...
    def GetPltArgs(self, methnam):
        data = None
        if methnam in self.pltargs: data = self.pltargs[methnam]
    ...
    def LinearToSquareArray(self, ndim, dat1d=[], symmetric=True):
        if 'LinearToSquareArray' in self.pltargs:
    ...
    def AddIndexToSquareArray(self, ndim, dat2d=[]):
        # add numbering
        if 'AddIndexToSquareArray' in self.pltargs:
    ...
    def OnRemoveData(self, event):
        selected = self.lbdat.GetStringSelection()
        if selected in self.datadic: del self.datadic[selected]
        if selected in self.drvdatadic: del self.drvdatadic[selected]
    ...
    def GetFMOProp(self, dataname):
        fmodat = None
        if dataname in self.fmodatadic: fmodat = self.fmodatadic[dataname]
        return fmodat

    def GetFMOPropName(self, fmodatadic, id):
        dataname = ''
        lst = list(fmodatadic.keys())
        for name in lst:
            ns = name.find(':')
            iddat = name[:ns]
            if iddat == id:
    ...
    def OnDerivedPorpPanel(self, event):
        ...
        if onoff:
            ...
            drvpansize = [] # [240,120] 3[size[0] / 2+20,120] # 95]
            #
    ...
    def AddDerivedDataToDic(self, drvnam, drvcmp):
        ...
        if drvnam not in self.drvdatadic: self.drvdatadic[drvnam] = []
    ...
    def IsDerivedData(self, dataname):
        if dataname in self.drvdatadic:
            return True
        else:
            return False
    ...
    def GetDataNameById(self, dataid):
        """ dataid(positive int) """
        name = ''
        for name, datlst in list(self.datadic.items()):
            items = name.split(':', 1)
    ...
    def MakeDataList(self):
        datalist = list(self.datadic.keys())
        if len(self.drvdatadic) > 0:
            drvlist = list(self.drvdatadic.keys())
    ...
    def MenuItems(self):
        ...
        # view output file
        submenu.Append(-1, 'View output file', 'View output file')
        submenu.AppendSeparator()
        # submenu.Append(-1,'Open cube file','Open cube file')
        # submenu.AppendSeparator()
        submenu.Append(-1, 'Open filter file', 'Open filter file')
        submenu.Append(-1, 'View filter file', 'Check filter file')
        #submenu.AppendSeparator()
        #submenu.Append(-1, 'View property dic', 'Check filter')
        # Quit
        submenu.AppendSeparator()
    ...
    def OpenFilterFile(self):
        ...
        self.ConsoleMessage('filter file='+filterfile)
        return filterfile

    def ChangeFilterFile(self,filterfile,prt=False):
        if not os.path.exists(filterfile):
            mess='the filter file "'+filterfile+'" does not exist.'
            wx.MessageBox(mess,'ChangeFilterFile')
            return

        title='ChangeFilterFile'
        mess='Do you want to apply the filter file to all output files (yes)?\n'
        mess+='If no, only the selected output file.'
        dlg = wx.MessageDialog(None, mess, title ,
                               style=wx.YES_NO | wx.ICON_QUESTION | wx.STAY_ON_TOP)
        if dlg.ShowModal() == wx.ID_YES:
            selectedlst=list(self.datadic.keys())
        else:
            selected = self.lbdat.GetStringSelection()
            if selected == '':
                wx.MessageBox('No file is selected', title)
                return
            selectedlst=[selected]

        filternam,ext = os.path.splitext(filterfile)

        for name in selectedlst:
            fil=self.datadic[name][0]
            self.datadic[name][1]=filternam
            self.prpobjdic[filternam] = FMOOutputFileReader(self.model,
                                                    filternam, filterfile, self)
        mess='Please click the "Set for plot" button to recompute properties.'
        wx.MessageBox(mess,title)

        if prt:
            self.PrintFileAndFilterName()

    def PrintFileAndFilterName(self,namelst=None):
        if namelst is None:
            namelst=list(self.datadic.keys())
        text='Data name and its outputfile and filtername\n'

        for name in namelst:
            text+=name+', '+self.datadic[name][0]+', '+self.datadic[name][1]+'\n'
        print(text)
    ...
    def MakeDataDicOfOutputFiles(self, files, filterfile):
        ...
        nmblst = [];
        namedic = {}
        for name, file in list(self.datadic.items()):
            items = name.split(':')
        ...
        for fil in files:
            ...
            self.idmax += 1;
            name = str(self.idmax) + ':' + tail
            base, ext = os.path.splitext(fil)
            # if case == "Open output file":
            if ext == '.out':
                # self.filterfile=self.GetFilterFile()
                self.filternam, ext = os.path.splitext(self.filterfile)
                self.prpobjdic[self.filternam] = FMOOutputFileReader(self.model,
                                                            self.filternam, self.filterfile, self)
    ...
    def SetDataList(self):
        lst = []
        for name, fil in list(self.datadic.items()): lst.append(name)
        """ not completed for derived property """
        self.drvprplst = list(self.drvdatadic.keys())
    ...
    def ViewFilterItemDic(self):
        filterfile = self.datadic[self.selected][1] + '.filter'
        text ='Items and their values obtained from the output file'
        text+=' according to the filter,\n'
        text+='"'+filterfile+'".\n'
        text+='You can get the dictionary of the items in the PyShell console,\n'
        text+='>>> fup.prpdic\n'
        text+='where "fup" is an instance of "fuplot".\n\n'
        text+=self.TextFilterItemDic()
        self.OpenTextViewer('ViewFilterItemDic', text)

    def TextFilterItemDic(self):
        prplst = list(self.prpdic.keys())
        prplst.sort()
        text='Dictionary of filter items:\n'
        for prp in prplst:
            text+='"'+prp+'" :\n'
            text+='   '+str(self.prpdic[prp])+'\n'
        return text

    def PrintPropertyDic(self):
        self.selected = self.lbdat.GetStringSelection()
    ...
    def HelpMessage(self):
        ...
        helpctrl = ctrl.HelpCtrl(self.model, self)
        helpctrl.Help('fuplot')

        try:
            helpctrl.helpwin.SetFocus()
        except:
            pass
    ...
    def OnMenu(self, event):
        ...
        # File menu items
        if item == "Open output file":
            self.OpenFiles(item)
        elif item == 'View output file':
            self.ViewOutputFile()
        elif item == "Open filter file":
            filterfile=self.OpenFilterFile()
            self.ChangeFilterFile(filterfile)
        elif item == 'View filter file':
            self.ViewFilterFile()

        elif item ==  'View property dic':
            self.ViewFilterItemDic()
        #elif item == "Open cube file":
        #    self.OpenFiles(item)
    ...
    def OpenMatPlotLibFrame(self):
        ...
        parentsize = self.GetClientSize()
        winpos = [parentpos[0], parentpos[1] + parentsize[1] + 50]
        winsize = lib.WinSize((600, 550))
        winlabel = 'matplotlibwin'
        self.mpl = subwin.MatPlotLib_Frm(self, -1, self.model, winpos, winsize, winlabel)
        self.mpl.Show()
    ...
    def OnClose(self, event):
        try:
            self.mdlwin.SetTitle(self.mdlwintitle)
        except:
            pass
        try:
            self.cubewin.Close(1)
        except:
            pass
        try:  # close remarkwin in mdlwin
            for fmoprp, graphobj in list(self.graphobjdic.items()):
...
class FMOOutputFileReader(object):
    """ GAMESS-FMO output file reader

    fmofil=FMOOutputFileReader( parent, name, filterfile, fuplot)

    :param obj parent: parent object(fumodel object in fu)
    :param str name: filter name
    :param str filterfile: filename of fileter file
    :param obj fuplot: fuplot object
    """
    def __init__(self, parent, name=None, filterfile=None, fuplot=None):
        self.model = parent
        self.fuplot = fuplot
    ...
    def ReadFilterFile(self, filterfile=None, filtertext=None, prt=False):
        """
        ...
        if filtertext is None:
            with open (filterfile, 'r') as f:
                filtertext=f.read()
        filterlst=filtertext.split('\n')
        #
        for s in filterlst: # f.readlines():
            if s[:1] == '#': continue
            nc = s.find('#', 1)
            if nc >= 0: s = s[:nc]
            s = s.strip()
            ...
            if s.find('varstring') >= 0:
                ...
                vardic[varnam][0] = colmn
                vardic[varnam][1] = item
                # const.CONSOLEMESSAGE('vardic='+str(vardic))

                continue
            if s.find('vartype') >= 0:
                vartype = s.split('=')
                vardic[varnam][2] = vartype[1].strip()
                continue
            ...
            if s.find('priorstring') >= 0:
                ...
                prpdefdic[defno][keywrd] = [colmn, value]
                continue
            if s.find('readlines') >= 0:
                keywrd, value = lib.GetKeyAndValue(s)
                prpdefdic[defno][keywrd] = value
                continue
        ...
        if prt:
            self.PrintItemsOfFilterFile()

        return constdic, prpdefdic, vardic, drvprpdic

    def PrintItemsInFilterFile(self):
        print('filterfile=',self.filterfile)
        print('constdic=',self.constdic)
        print('prpdefdic=',self.prpdefdic)
        print('vardic=',self.vardic)
        print('drvprpdic=',self.drvprpdic)
    ...
    def ReadVarValue(self, filename, colmn, varstring):
        value = None
        n = colmn + len(varstring) + 1
        #f = open(filename, 'r')
        f = lib.OpenFileAsRead(filename)
    ...
    def ReadOutputFile(self, filename,filterfile=None,prt=None):  # ,constdic,prpdefdic):
        if filterfile is None:
            constdic = self.constdic;
            prpdefdic = self.prpdefdic;
            vardic = self.vardic
        else:
            constdic,prpdefdic,vardic,devprpdic=self.ReadFilterFile(filterfile, prt=False)
            self.filterfile=filterfile
        ...
        orbs = {}
        #
        for varnam, items in list(vardic.items()):
            colmn = items[0]
            varstring = items[1]
            vartype = items[2]
            if vartype == 'varname':
                ...
                varvalue = self.ReadVarValue(filename, colmn, varstring)
                varnamdic[varnam] = varvalue
        #
        loop = True
        idat = -1
        while loop:
            ...
            if nam == 'Orbitals':
                ...
                if len(orbsetdic) > 0:
                    ...
                    nmb = 1;
                    typ = prpdefdic[idat]['plottype'][0]
                    prptypdic[prplabel[0]] = [typ, nmb]
            else:
                prpdic = self.ReadPropertyInOutput(filename, prpdefdic[idat],
                                                   constdic, varnamdic)
                #
                if len(prpdic) > 0:
                    prplabel = prpdefdic[idat]['property']
                    ...
                    if len(prplabel) == 1:
                        ...
                        if typ == 'coord' or typ == 'pie' or typ == 'multi':
                            ...
                            prplabeldic[prplabel[0]] = prpdic[item]
                        ...
                        for i in range(len(prplabel)):
                            ...
                            typ = plttyp[i]
                            nmb = self.CountNumberOfData(prplabel[i], typ,
                                                         prplabeldic)
                            prptypdic[prplabel[i]] = [plttyp[i], nmb]
        if prt:
            print('filename=',filename)
            print('filterfile', filterfile)
            print('prplabeldic=',prplabeldic)
            print('prptypdic=',prptypdic)
            print('varnamdic=',varnamdic)

        return prplabeldic, prptypdic, varnamdic

    def GetDerivedPropertyName(self):
        drvprplst = []
        for prpnam, vars in list(self.drvprpdic.items()):
    ...
    def MakeDerivedProperty(self, prpdic, prptypdic):
        ...
        for prpnam, vars in list(self.drvprpdic.items()):
            if vars[0] in prpdic and vars[1] in prpdic:
                value = []
    ...
    def ReadOrbitals(self, filename, beginstring, skiplines,prt=None):
    ...
    def CountNumberOfData(self, prplabel, plttyp, prpdic):
        nmb = 1
        if plttyp == 'coord':
            if 'NUC.CHARGE' in prpdic[prplabel]:
                nmb = prpdic[prplabel]['NUC.CHARGE'].count('end')
            elif 'ELEMENT' in prpdic[prplabel]:
    ...
    def ReadPropertyInOutput(self, filename, prpdefdic, constdic,
        ...
        searchoncol = 0;
        searchoffcol = 0
        searchontext = nulltext;
        searchofftext = nulltext
        skiplines = 0;
        nlines = 0
        if 'searchonstring' in prpdefdic:
            search = True
            searchon = False
            searchoff = False
            seachoncol = prpdefdic['searchonstring'][0] - 1
            searchontext = prpdefdic['searchonstring'][1]
            if 'searchoffstring' in prpdefdic:
            ...
            searchontext = nulltext
            searchoffcol = 0;
            searchofext = nulltext
        if 'priorstring' in prpdefdic:
            prior = True
        ...
        if 'beginstring' in prpdefdic:
            ...
            if 'skiplines' in prpdefdic:
                skiplines = prpdefdic['skiplines']
        if 'readlines' in prpdefdic:
            rdlines = True
            rlines = prpdefdic['readlines']
            value = varnamdic[rlines]
            #print(('ReadPropertyInOutput. value=',value))
            try: value = int(value)
            except:
                print(('error in readlines. should be an integer. value=',value))
        #
        line = 0
        # read output file
        #f = open(filename, 'r')
                ...
                if searchon:
                    ...
                    if foundbegin:
                        ...
                        if nc >= 0 and line > skiplines:
                            ...
                            if rdlines:
                                count += 1
                                if count == 1: maxlines = varnamdic[rlines]
                                ss = s
                                prpdic = self.GetItemsInLine(prpdefdic, constdic,
                                                             prpdic, ss)
                                if count >= maxlines:
                                    foundbegin = False
                                    rdlines = False
    ...
    def GetItemsInLine(self, prpdefdic, constdic, prpdic, ss):
        ...
        namlst = getitems[1]
        for i in range(len(namlst)):
            if namlst[i] not in prpdic: prpdic[namlst[i]] = []
            try:
            ...
            val = val.replace(',', '')
            if dattyp[0] == 'auto':
                type = lib.StringType(val)
            else:
                type = dattyp[i].split('*', 1)[0].strip()
            if type == 'float':
                sc = 1.0
                if dattyp[0] != 'auto':
                    for name, value in list(constdic.items()):
...
class FMOProperty(object):
    ...
    def GetPropertyItems(self):
        ...
        for name in self.graphnamtxt:
            if name in self.propdic: itemlst.append(name)
        return itemlst
    ...
    def SetAttributes(self):
        ...
        try:
            ...
            pass
        if self.nbody > 1:
            if 'pieda' in prpdic: self.pieda = prpdic['pieda']  # =0:pie, =1:pieda
            if 'pieda' in prpdic: self.ctchg = prpdic['pieda']  # =0:no data, 1:yes
            if 'mulchg' in prpdic: self.mulchg = prpdic['mulchg']  # 0:not avilable, 1*yes
        ...
        self.etfmo2 = 0.0
        if self.nbody > 1:
            if 'escf(2)' in prpdic: self.etfmo2 = prpdic['escf(2)']  # 0 # fmo2,fmo3
        else:
            self.etfmo2 = prpdic['etotal']
        self.etfmo3 = 0
        self.ecfmo2 = 0  # fmo2,3
        self.ecfmo3 = 0  #

        self.dft = False
        if "dfttyp" in prpdic: self.dft = True
        # molecule data
        self.natmfrg = []  # each fragment
        if 'natm' in prpdic: self.natm = prpdic['natm']
        if 'nbas' in prpdic: self.nbas = prpdic['nbas']
        if 'nfrg' in prpdic: self.nfrg = prpdic['nfrg']
        if 'tchg' in prpdic: self.tchg = prpdic['tchg']
        self.nbasfrg = []  # each fragment
        self.jobtitle = ''  # job comment in output
        try:
    ...
    def ConvertPIEDAForPlot(fmopieda):
        ...
        for i in range(len(fmopieda)):
            ...
            if not pie:
                ...
                if len(fmopieda[i]) == 10:
                    tot = fmopieda[i][9]
                elif len(fmopieda[i]) == 11:
                    tot = fmopieda[i][10]
                else:
...
class PlotOrbitalEnergy(object):
    def __init__(self, parent, orbitallst):
        ...
        self.drawenergy = OrbitalEnergyGraph_Frm(self.parent, -1, self, winsize=self.winsize,
                                                 orbitallst=self.orbitallst)
...
class OrbitalEnergyGraph_Frm(wx.MiniFrame):
    def __init__(self, parent, id, parobj, winpos=[-1, -1], winsize=[-1, -1],
        ...
        if self.mode == 0:
            ...
            lib.InsertTitleMenu(menubar, '[OrbitalEnergy]')
            self.Bind(wx.EVT_MENU, self.OnMenu)
        #
    ...
    def CreatePanel(self):
        ...
        self.orbgraph = graph.EnergyGraph(panel, -1, [xloc, yloc], [self.wplt, self.hplt], 'white',
                                          retobj=self)  # yloc += 25
...
class PIEDAGraph_Frm(wx.Frame):
    def __init__(self, parent, id, winpos, winsize, onedmode, prpmode,
                 childmode):  # ,pltprp,datnam,fmodat,molint): #,molnam,prop):
        if len(winsize) <= 0: winsize = lib.WinSize((700, 400), True)  # (680,350) # (520,350)
        ...
        lib.InsertTitleMenu(self.menubarplot, '[PlotPIE/CTC]')
        ...
        # if lib.GetPlatform() == 'MACOSX' and wx.version()[0:1] != '2':
        #    wx.MessageBox('Please right_click drawing area when no graph is plotted')
    ...
    def OnKeyDown(self, event):
        ...
        if keycode == saveimgkey:  # 73: # 'i' in ASCII code
            imgdir = self.model.setctrl.GetDir('Images')

            wx.MesasgeBox('imgdir=' + str(imgdir))
            base = 'fup-pieda1d-'
            ...
            if keycode == wx.WXK_LEFT:
                xmove = -10;
                arrowkey = True
                if shiftdown: xmove = -1000000
            elif keycode == wx.WXK_RIGHT:
                xmove = 10;
                arrowkey = True
                if shiftdown: xmove = 1000000
            if arrowkey: self.graph.ShiftGraphByKeyDown(xmove=xmove)
    ...
    def CreatePIEDACmdPanel(self):
        ...
        self.piedacmd1pan = wx.Panel(self.pltpan, wx.ID_ANY, pos=(0, yloc),
                                     size=(w, self.hcmdpanel))  # ,style=wx.DOUBLE_BORDER)
        ...
        if lib.GetPlatform() == 'MACOSX' and wx.version()[0:1] != '2':
            self.sclfrg.Bind(wx.EVT_LEFT_DOWN,self.OnChangeFragmentLeftDown)
        """
        frglst = list(range(self.nfrg))
        frglst = [str(x + 1) for x in frglst]
        self.cmbfrg = wx.ComboBox(self.piedacmd1pan, -1, choices=frglst, pos=(80, 4),
                                  size=(60, 25), style=wx.TE_PROCESS_ENTER)
        self.cmbfrg.Bind(wx.EVT_TEXT_ENTER, self.OnChangeFragmentTextEnter)
        self.cmbfrg.Bind(wx.EVT_COMBOBOX, self.OnChangeFragment)
        # plot energy range
        xloc = 5;
        yloc = 35  # 30
        wx.StaticText(self.piedacmd1pan, -1, "y.range(+/-):", pos=(xloc, yloc + 2), size=(75, 18))
        self.tclyrng = wx.TextCtrl(self.piedacmd1pan, -1, "50", pos=(xloc + 85, yloc), size=(55, 20),
        ...
        self.ckbcmp[4] = wx.CheckBox(self.piedacmd1pan, wx.ID_ANY, "1b", pos=(xloc + 154, yloc))  # ,size=(50,18))
        ...
        self.btnall = wx.ToggleButton(self.piedacmd1pan, label='all(on/off)', pos=(xloc + 205, yloc), size=(80, 20))
        self.btnall.Bind(wx.EVT_TOGGLEBUTTON, self.OnAllCompOnOff)
        self.btnall.SetValue(True)  # default is on (see self.piedacmp)
        # separate line
        wbtn = 68
        wx.StaticLine(self.piedacmd1pan, pos=(0, self.hcmdpanel / 2 + 1), size=(w - 90, 2), style=wx.LI_HORIZONTAL)
        # sort
        xloc = 155;
        yloc = 35  # 30
        ...
        yloc = 8
        # separator line
        wx.StaticLine(self.piedacmd1pan, pos=(w - 90, 0), size=(2, self.hcmdpanel), style=wx.LI_VERTICAL)
        # 2d button
        self.tglcld = wx.ToggleButton(self.piedacmd1pan, label='2DGraph', pos=(w - 80, yloc), size=(70, 18))
        self.tglcld.Bind(wx.EVT_TOGGLEBUTTON, self.OnOpenChildGraph)
        # molview button
        yloc = 35  # 30
        self.tglvew = wx.ToggleButton(self.piedacmd1pan, label='MolView', pos=(w - 80, yloc), size=(70, 18))
        self.tglvew.Bind(wx.EVT_TOGGLEBUTTON, self.OnMolView)
        self.tglvew.SetValue(self.openremarkwin)
        if not self.parent.coorpmdlwin:
            self.tglvew.Disable()
    ...
    def CreateCTChargeCmdPanel(self):
        ...
        yloc = h - self.hcmdpanel
        #
        self.ctcmd1pan = wx.Panel(self.pltpan, wx.ID_ANY, pos=(0, yloc),
                                  size=(w, self.hcmdpanel))  # ,style=wx.DOUBLE_BORDER)
        ...
        frglst = list(range(self.nfrg))
        frglst = [str(x + 1) for x in frglst]
        self.cmbfrg = wx.ComboBox(self.ctcmd1pan, -1, choices=frglst, pos=(80, 4),
                                  size=(60, 25), style=wx.TE_PROCESS_ENTER)
        self.cmbfrg.Bind(wx.EVT_TEXT_ENTER, self.OnChangeFragmentTextEnter)
        self.cmbfrg.Bind(wx.EVT_COMBOBOX, self.OnChangeFragment)
        # plot energy range
        ...
        self.tclyrng = wx.TextCtrl(self.ctcmd1pan, -1, "0.5", pos=(xloc + 80, yloc), size=(55, 20),
                                   style=wx.TE_PROCESS_ENTER)
        self.tclyrng.Bind(wx.EVT_TEXT_ENTER, self.OnChangeYRange)
        # separate line
        xsep = 146
        ...
        # separate line
        wbtn = 68  # 55
        wx.StaticLine(self.ctcmd1pan, pos=(0, self.hcmdpanel / 2 + 1), size=(w - 90, 2), style=wx.LI_HORIZONTAL)
        # sort
        xloc = 155;
        yloc = 35  # 30
        ...
        # yloc=8
        # separator line
        # wx.StaticLine(self.ctcmd1pan,pos=(w-80,0),size=(2,self.hcmdpanel),style=wx.LI_VERTICAL)
        yloc = 8
        # separator line
        wx.StaticLine(self.ctcmd1pan, pos=(w - 90, 0), size=(2, self.hcmdpanel), style=wx.LI_VERTICAL)
        # 2d button
        self.tglcld = wx.ToggleButton(self.ctcmd1pan, label='2DGraph', pos=(w - 80, yloc), size=(70, 18))
        self.tglcld.Bind(wx.EVT_TOGGLEBUTTON, self.OnOpenChildGraph)
        # molview button
        yloc = 35  # 30
        self.tglvew = wx.ToggleButton(self.ctcmd1pan, label='MolView', pos=(w - 80, yloc), size=(70, 18))
        self.tglvew.Bind(wx.EVT_TOGGLEBUTTON, self.OnMolView)
        self.tglvew.SetValue(self.openremarkwin)
        if not self.parent.coorpmdlwin:
            self.tglvew.Disable()
    ...
    def CreateMullikenCmdPanel(self):
        ...
        yloc = h - self.hcmdpanel
        #
        self.mulchgcmdpan = wx.Panel(self.pltpan, wx.ID_ANY, pos=(0, yloc),
                                     size=(w, self.hcmdpanel))  # ,style=wx.DOUBLE_BORDER)
        ...
        frglst = list(range(self.nfrg))
        frglst = [str(x + 1) for x in frglst]
        self.cmbfrg = wx.ComboBox(self.mulchgcmdpan, -1, choices=frglst, pos=(80, 4),
                                  size=(60, 25), style=wx.TE_PROCESS_ENTER)
        self.cmbfrg.Bind(wx.EVT_TEXT_ENTER, self.OnChangeFragmentTextEnter)
        self.cmbfrg.Bind(wx.EVT_COMBOBOX, self.OnChangeFragment)
        # plot energy range
        ...
        self.tclyrng = wx.TextCtrl(self.mulchgcmdpan, -1, "50", pos=(xloc + 80, yloc), size=(55, 20),
                                   style=wx.TE_PROCESS_ENTER)
        self.tclyrng.Bind(wx.EVT_TEXT_ENTER, self.OnChangeYRange)
        xsep = 146
        ...
        yloc = 30
        # separator line
        wx.StaticLine(self.mulchgcmdpan, pos=(w - 90, yloc - 2), size=(2, 30), style=wx.LI_VERTICAL)
        yloc = 35  # 30
        # molview button
        self.tglvew = wx.ToggleButton(self.mulchgcmdpan, label='MolView', pos=(w - 80, yloc), size=(70, 18))
    ...
    def CreateChildGraphPanel(self):
        ...
        self.childmenu = self.MenuGraph()
        self.childgraphwin.SetMenuBar(self.childmenu)

        # icon
        try:
            lib.AttachIcon(self.childgraphwin, const.FUPLOTICON)
        except:
            pass
    ...
    def OnEach(self, event):
        ...
        if self.parent.mdlwin and self.parent.coorpmdlwin:
    ...
    def SetMulliknBodyWidget(self, mullbody):
        ...
        for i in range(len(self.mullbody)):
            if not self.activemul[i]:
                self.rbtmul[i].Disable()
            else:
                self.rbtmul[i].Enable()
        for i in range(len(self.mullbody)):
    ...
    def SetWidgetStates(self):
        ...
        self.cmbfrg.SetValue(str(self.ifrg))
    ...
    def MolView(self, on):
        def TryAgainMess():
            mess = 'The molecule object is not in fu-MdlWin\n'
            mess += "Please open the molecule's file(pdb or other file)\n"
            mess += "in fu-MdlWin and try again."
            dlg = lib.MessageBoxOK(mess, "MolView")

        self.openremarkwin = on
        self.mdlwin = self.parent.mdlwin
        self.model = self.parent.model
        if not self.mdlwin:
            dlg = lib.MessageBoxOK('fu-MdlWin is not opened.', "MolView")
            return
        if not self.model.mol:
            TryAgainMess()
            return
        nat = len(self.model.mol.atm)
        if nat != self.parent.prpdic['NATOMS'][0]:
            mess = 'Number of atoms in output and that in MdlWin is different.\n'
        ...
        if self.parent.AreFragments() is None:
            TryAgainMess()
        elif not self.parent.AreFragments():
            retcode = self.parent.SetFragmentsToMol()
        ...
        if on:
            ...
            try:
                self.parent.mdlwin.CloseRemarkWin()
            except:
                pass
    ...
    def SetFragmentNamesToMol(self):
        pass
    ...
    def SortDataNmb(self, pltdat=[]):
        if 'SortDataNmb' in self.pltargs:
    ...
    def SetPIEDAComponentWidget(self, piedacmp):
        ...
        # if self.pltprp == 0:
        for i in range(len(self.piedacmp)):
            if not self.activecmp[i]:
                self.ckbcmp[i].Disable()
            else:
                self.ckbcmp[i].Enable()
        for i in range(len(self.piedacmp)):
    ...
    def OnMenuGraph(self, event):
        ...
        if item == "Save bitmap":
            ...
            wx.MessageBox('Sorry, not implemented')
            return
            ...
            self.OnClose(0)
        elif item == "Copy bitmap":
            lib.CopyBitmapToClipboard(self.graph.buffer)

        elif item == "Clear clipboard":
        ...
        # data
            ...
            for i in range(len(items)):
                items[i].strip()
                if items[i][:1] == '>':
                    self.larger = float(items[i][1:].strip());
                    lg = True
                elif items[i][:1] == '<':
                    self.smaller = float(items[i][1:].strip());
                    sm = True
    ...
    def HelpMessage(self):
        text = '----------------\n'
        text += 'Key and function\n'
        text += '----------------\n'
        text += 'i                 save image on FUDATASET/image\n\n'
        text += 'right_arrow       shift graph to the right\n'
        text += '                  (equivalent to drag graph to the right)\n'
        text += 'shft+right_arrow  shift graph to the right most\n'
        text += 'left_arrow        shift graph to the left\n'
        text += '                  (equivalent to drag graph to the left)\n'
        text += 'shft+left_arrow    shift graph to the left most\n\n'

        text += '----------------\n'
        text += 'Mouse click/drag\n'
        text += '----------------\n'
        text += 'click a bar        display the values\n'
        text += 'drag to right      see "right_key" press\n'
        text += 'drag to left       see "left_key" press\n'

        title = 'PIEDA graph'
        [x, y] = self.GetPosition()
        winpos = [x + 20, y + 20]
        winsize = [500, 400]
        subwin.HelpMessage(text, title=title, winpos=winpos, winsize=winsize,
                           parent=self,
                           datatype='Text', viewer='Text')
                ...
                for j in range(nfrg):
                    ...
                    for k in range(len(self.fmoprp[i][j])):
                        if self.fmoprp[i][j][k][1] > self.larger:
                            ndat += 1;
                            tmp.append([ndat, self.fmoprp[i][j][k][1]])
                            if i == 0 and j == 0:
    ...
    def SavePIEDAData(self, on):
        if on:
            ...
            if len(self.savfmoprp):
                self.fmoprp = self.savfmoprp;
                self.savfmoprp = []
                self.order = self.savorder;
                self.savorder = []
                self.frgdist = self.savfrgdist;
                self.savfrgdist = []
                self.nfrg = self.savnfrg;
                self.savnfrg = -1
                self.natm = self.savnatm;
                self.savnatm = -1
            else:
    ...
    def OnMenuChildGraph(self, event):
        ...
        if item == "Copy bitmap":
            lib.CopyBitmapToClipboard(self.childgraph.buffer)
            ...
            for i in range(len(self.pltdat)):
                ...
                if self.eachfrg:
                    if self.ifrg == self.pltdat[i][0]:
                        pltcol.append(curcol);
                        continue
                    elif self.frgdist[i][self.ifrg - 1][1] < 0.001:
                        pltcol.append(concol);
                        continue
    ...
    def GetRankColor(self, value):
        ...
        if value >= 0:
            ...
            if rank > self.rankposi - 1:
                ...
                color = self.rankcolorposi[rank]
        else:
            rank = int(abs(value) / vunit)
            if rank < 0 or rank > self.ranknega - 1:
                color = self.extcolornega
            else:
                color = self.rankcolornega[rank]

        return color

    def XXOnChangeFragmentLeftDown(self, event):
        obj = event.GetEventObject()
        try:
            value = obj.GetValue()
        except:
            wx.MessageBox('Please input integer value. ')
            return
    ...
    def OnChangeFragmentTextEnter(self, event):
        obj = event.GetEventObject()
        try:
            value = obj.GetValue()
        except:
    ...
    def OnChangeFragment(self, event):
        ...
        self.ifrg = int(self.cmbfrg.GetValue())
        if self.ifrg > self.nfrg: self.ifrg = self.nfrg
        if self.ifrg <= 0: self.ifrg = 1
        self.ChangeFragment(self.ifrg)
    ...
    def SetPIEDAData(self, natm, nfrg, frgnam=[], fmoprp=[], frgdist=[]):
        if 'SetPIEDAData' in self.pltargs:
    ...
    def DrawGraph(self, redrawmol):
        ...
        xlab = 'fragment number';
        xlab1 = 'atom number'
        if self.pltprp == 0:
            ftitle = 'Plot PIEDA: ';
            gtitle = 'PIE/PIEDA: ';
            ylab = 'Energy (kcal/mol)'
            childtitle = '2D PIE/PIEDA';
            rem = 'kcal/mol'
        if self.pltprp == 1:
            ftitle = 'Plot CT Charge';
            gtitle = 'CT Charge: ';
            ylab = 'Charge (a.u.)'
            ctitle = '2D CT Charge';
            rem = '     a.u.'
        if self.pltprp == 2:
            ftitle = 'Plot Mullike Charge';
            gtitle = 'Mulliken Charge: ';
            ylab = 'Charge (a.u.)'
            ctitle = '2D Mulliken Charge';
            rem = '     a.u.';
            xlab = xlab1
        ...
        if self.pltprp == 0:
            if self.eachfrg:
                pltdat = self.MakeFragmentPIEDAData(self.ifrg)  # self.MakeFragmentData(self.ifrg)
            else:
                pltdat = self.MakePIEDABondingEnergyData()  # MakeAllPIEDAData()
        if self.pltprp == 1:
            if self.eachfrg:
                pltdat = self.MakeFragmentCTChargeData(self.ifrg)
            else:
                pltdat = self.MakeAllCTChergeData()
        if self.pltprp == 2:
            if self.eachfrg:
                pltdat = self.MakeFragmentMullikenData(self.ifrg)
            else:
                pltdat = self.MakeAllMullikenData()
        ...
        if self.eachfrg and self.pltprp != 2:
            self.graph.SetAdditionalInfo(distinfo)
        else:
            self.graph.SetAdditionalInfo([])
    ...
    def MakePIEDARemarkList(self, pltdat):
        if 'MakePIEDARemarkList' in self.pltargs:
    ...
    def PrintMinData(self):
        ...
        for i in range(len(self.pltdat)):
            if self.pltdat[i][1] < minval:
                minval = self.pltdat[i][1];
                idat = i
        dat = str(idat)
        self.parent.RunMethod("print fuplt.graph['" + name + "'].pltdat[" + dat + "]")

    def OnFocus(self, event):
        # self.pltpan.SetFocus()
        # self.Refresh()
        ###self.DrawGraph(True)
        self.Refresh()
        # self.Resize()
    ...
    def OnPaint(self, event):
        ####wx.MessageBox('PIEDAGrap OnPaint is called')

        if lib.GetPlatform() != 'MACOSX':
            event.Skip()
            return
        if self.parent.mdlwin.remarkwin:
            self.parent.mdlwin.remarkwin.DrawColorRemark()
        return
        # self.graph.Plot(True)
        # self.Refresh(False)
