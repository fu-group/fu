...
def GetFFDefaultFuncForm(ffname):
    ...
    if ffname in ffnamedic: name=ffnamedic[ffname]
    else:
...
class ForceField():
        ...
        for s in f.readlines():
            ...
            for kw,nw in potopt:
                if ss.startswith(kw,0,nw):
                    item=ss.split()
                    if kw in optdic: continue #optdic[kw].append(item[1])
    ...
    def ReadFFPotTerms(self):
        ...
        for s in f.readlines():
            ...
            for kw, nw in potterms:            
                if ss.startswith(kw,0,nw):
                    ...
                    if keywd in potdic: potdic[keywd].append(lst)
        ...
        self.pottermlst=list(potdic.keys())
    ...
    def GetFFPotTermList(self):
        termlst=list(self.pottermdic.keys())
        termlst.sort()
        return termlst
    ...
    def AddFFPotTerm(self,termname,termdata):
        if termname in self.pottermdic:
...
class TINKEROptimization_Frm(wx.Frame):
    ...
    def SetOptPrgList(self):
        ...
        for p in pxyz:
            if p in self.tinprgdic: xyzprglst.append(p)
        for p in pint:
            if p in self.tinprgdic: intprglst.append(p)
        for p in prgd:
            if p in self.tinprgdic: rgdprglst.append(p)
    ...
    def OnOpenPrmFile(self,event):
        ...
        #print 'prmnam',prmnam
        if prmnam not in self.fffiledic: self.ffnamelst.append(prmnam)
    ...
    def SetFFOptions(self):
        ...
        if "chg-14-scale" in optdic: self.chg14scale=optdic["chg-14-scale"]
        else: self.chg14scale=1.0 
        self.tclchg14.SetValue(str(self.chg14scale))
        if "vdw-14-scale" in optdic: self.vdw14scale=optdic["vdw-14-scale"]
        else: self.vdw14scale=1.0
        self.tclvdw14.SetValue(str(self.vdw14scale))
        if "dielectric" in optdic: self.dielec=optdic["dielectric"]
    ...
    def WriteTinkerPrmFile(self,prmfile,potdic,ap):
        ...
        # write options
        if 'atomorg' in potdic: atom=potdic['atomorg']
...
class AssignFFParam_Frm(wx.Frame):
    def __init__(self,parent,id,winpos,winsize):
        ...
        self.uterms=list(parent.tgtff.pottermdic.keys())
    ...
    def ListItemsInRefFFLC(self,makelst):
        ...
        if self.curterm == 'atom':
            ...
            for datnmb,type,cls,atmnam,atomic,mass,vale,com in termdic['atom']:
                ...
                index=self.lstprm.InsertStringItem(sys.maxsize,datnmb)
        ...
        if self.curterm == 'bond':
            ...
            for i in range(len(termdic['torsion'])):
                ...
                index=self.lstprm.InsertStringItem(sys.maxsize,datnmb) #str(nq))                
                self.lstprm.SetStringItem(index,1,text)                
                ii=1
                if n >= 6:
                    for j in range(5,n,3):   
    ...
    def ListElmValeClsForAtom(self,makelst):
        ...
        for seqnmb,type,cls,atmnam,atomic,mass,vale,com in term:
            ...
            index=self.lstctrl.InsertStringItem(sys.maxsize,str(seqnmb+1))
    ...
    def ListElmValeClsForBond(self,makelst):
        ...
        for datnmb,atm1,nam1,atm2,nam2,cls1,cls2,bondk,bondr,remark in term:
            ...
            index=self.lstctrl.InsertStringItem(sys.maxsize,datnmb)
    ...
    def SetSelectItem(self):
        ...
        selectitemlst=list(tmpdic.keys())
    ...
    def SetFFSelectItem(self):
        ...
        selectitemlst=list(seldic.keys())
        if len(selectitemlst) <= 0: return
        selectitemlst.sort()
        selectitemlst=['all']+selectitemlst
