...
class Customize_Frm(wx.Frame):
    ...
    """
    def __init__(self,parent,model,winpos=[0,0],winsize=[440,410],
                 winlabel='Customize'):
        ...
        #x#self.pagenamlst=['Setting','General','Model','Add-on','Shortcut','Project'] #,'Mouse','Misc']
        self.pagenamlst = ['General', 'Model','Others']
        #x#self.parampagedic={'Setting':0,'General':1,'Model':2,'Add-on':3,'Shortcut':4,'Project':5}
        self.parampagedic = { 'General': 0, 'Model': 1,'Others':2}
        #x#self.pages=[self.Page0,self.Page1,self.Page2,self.Page3,self.Page4,self.Page5]
        self.pages = [self.Page0, self.Page1, self.Page2]
        self.pageinslst=len(self.pagenamlst)*[None]
        #x#self.fileext=['.py','.general','.model','.add-on','.shortcut'] #,'.project']
        self.fileext = ['.py', '.general', '.model','.others']
        #
        ...
        self.OnPageChanged(self.curpage)

        self.StatusMessage('Make parameter set files and set them in the "FUDATASET//Custom directory')
    ...
    def CreatePagePanels(self):
        npage=len(self.pagenamlst)
        pagecolor='light gray'
        # set panels
        for i in range(npage):
            self.pagepanels.append(wx.Panel(self.notebook,wx.ID_ANY))
            self.pagepanels[i].SetBackgroundColour(pagecolor)
            self.notebook.InsertPage(i,self.pagepanels[i],self.pagenamlst[i])
        # page method
        for i in range(npage):
            self.pages[i]()

    def Page0(self): # General
        self.pageinslst[0]=CustomGeneral(self,self.pagepanels[0],self.model)

    def Page1(self): # Model
        self.pageinslst[1]=CustomModel(self,self.pagepanels[1],self.model)

    def Page2(self): # Others
        self.pageinslst[2]=CustomOthers(self,self.pagepanels[2],self.model)
    ...
    def AttachHelpButton(self,winobj,winpos=[0,0],name=None):
        # help button
        helpdir=self.setctrl.GetDir('FUdocs')
        helpfile='Customizer//html//Customizer'
        if name is not None:
            helpfile=helpfile+'-'+name+'.html'
        else:
            helpfile+='.html'
        helpfile=os.path.join(helpdir,helpfile)
        if os.path.exists(helpfile):
            subwin.Help_Button(winobj,winpos=winpos,helpfile=helpfile)
        else:
            mess='The help file "'+helpfile+'" does not exist.'
            wx.MessageBox(mess,'Customize_Frm.AttachHelpButton')

    def OnPageChanged(self,event):
        messlst=['Make/modify/delete general parameters',
                 'Make/modify/delete model parameters',
                 'Make/modify/delete othrs parameters']
        if not self.gonewpage:
            newpage=self.notebook.ChangeSelection(self.prvpage)
        else:
    ...
    def MakeCurParamSetDic(self):
        ...
        #x#curprj=os.path.split(self.setctrl.GetCurPrj())[1]
        curparamsetdic["Setting"]=curset.split('.')[0]
        #x#curparamsetdic["Project"]=curprj.split('.')[0]
        cursetfile=os.path.expanduser(cursetfile)
        curfilelst=Customize_Frm.ReadSetFile(cursetfile)

        setparamlst=self.setctrl.GetParamSetNameList()
        for i in range(len(curfilelst)):
            try:
                curparamsetdic[self.pagenamlst[i]] = setparamlst[i]
            except:
                pass
    ...
    def MakeParamSetFileDic(self):
        """ modified to self.pagenamlst=['General','Model','Others'] """
        paramsetfiledic={}
        #x#for i in range(len(self.pagenamlst)-1):
        for i in range(len(self.pagenamlst)):
            filelst=lib.GetFilesInDirectory(self.customdir)

        baselst,extlst=self.MakeParamSetName(filelst)
        if '.files' in extlst:
            idx=extlst.index('.files')
            self.GetInitialParamFiles(filelst[idx])
        else:
            self.initialparamfiles=['','']

        pagenums={'.general':'General','.model':'Model','.others':'Others'}
        for ext,grp in pagenums.items():
            paramsetfiledic[grp]=[]
        for i in range(len(extlst)):
            if extlst[i] in pagenums:
                pg=pagenums[extlst[i].lower()]
                paramsetfiledic[pg].append(baselst[i])
        return paramsetfiledic

    def AppendParamSetFile(self,panelname,filename,curset=False):
        """ panelname: 'Model','General','Others' """
        head,tail=os.path.split(filename)
        base,ext=os.path.splitext(tail)
        if base in self.paramsetfiledic[panelname]:
            mess='There is the same set file name.\n'
            mess+='Do you want to replace(yes) or cancel(no)?'
            dlg = wx.MessageDialog(None, mess, 'AppendParamSetFile',
                         style=wx.YES_NO | wx.ICON_QUESTION | wx.STAY_ON_TOP)
            if dlg.ShowModal() == wx.ID_NO:
                return None
            else:
                if curset:
                    self.curparamsetdic[panelname] = base
                return base
        else:
            self.paramsetfiledic[panelname].append(base)
            if curset:
                self.curparamsetdic[panelname]=base
            return base

    def GetInitialParamFiles(self,filename):
        #
        generalfile=''
        modelfile=''
        ...
        for s in textlst:
            ...
            if var.lower() == 'general':
                generalfile=value
            elif var.lower() == 'model':
                modelfile=value
        self.initialparamfiles=[generalfile,modelfile]

    def ResetSettingFile(self,setnam,curparamsetdic):
        self.curparamsetdic=curparamsetdic
        # setting panel instance
        #x#self.pageinslst[0].curparamsetdic=self.curparamsetdic
        # reset current param set
        if setnam == 'all' or setnam == 'General':
            #x#self.pageinslst[1].ResetCurrentSet(self.curparamsetdic['General'])
            self.pageinslst[0].ResetCurrentSet(self.curparamsetdic['General'])
        if setnam == 'all' or setnam == 'Model':
            #x#self.pageinslst[2].ResetCurrentSet(self.curparamsetdic['Model'])
            self.pageinslst[1].ResetCurrentSet(self.curparamsetdic['Model'])
        if setnam == 'all' or setnam == 'Others':
            #x#self.pageinslst[2].ResetCurrentSet(self.curparamsetdic['Model'])
            self.pageinslst[2].ResetCurrentSet(self.curparamsetdic['Others'])
        #x#if setnam == 'all' or setnam == 'Add-on':
        #x#    self.pageinslst[3].ResetCurrentSet(self.curparamsetdic['Add-on'])
        #x#if setnam == 'all' or setnam == 'Shortcut':
        #x#    self.pageinslst[4].ResetCurrentSet(self.curparamsetdic['Shortcut'])
    ...
    def MakeParamSetName(self,filelst):
        ...
        for i in range(len(filelst)):
            ...
            baselst.append(base)
            extlst.append(ext)
        return baselst,extlst

    def ConvertParamsForCustomize(self,paramsdic,paramtypedic):
        modeldic={0:'line',1:'stick',2:'ball-and-stick',3:'CPK'}
        for prmnam in list(paramsdic.keys()):
            value=paramsdic[prmnam]
            if not prmnam in paramtypedic:
                continue
            ...
            if prmnam == 'mol-model' and paramsdic[prmnam] in modeldic:
                paramsdic[prmnam]=modeldic[paramsdic[prmnam]]
        return paramsdic

    def ConvertColor4To3InDic(self,colordic):
        for item,color in list(colordic.items()):
            #color=numpy.array(color);
            #color=255*color;
            color=color[:3]
            #if not rgb255:
            #    color=[int(float(x) * 255.0) for x in color]
            colordic[item]=color
        return colordic

    def GetCurrentParamSet(self,setnam):
        if setnam in self.curparamsetdic: return self.curparamsetdic[setnam]
        else: return None

    def SetCurrentParamSet(self,setnam,curset):
        self.curparamsetdic[setnam]=curset

        #test#self.pageinslst[0].ResetCurrentParamFile(setnam,curset)
    ...
    def MakeParamTypeDic(self,paramdata):
        paramtypedic={}
        for prmnam,data in list(paramdata.items()):
            paramtypedic[prmnam]=data[3]
        return paramtypedic

    def MakeParamWidgetDic(self,paramobjdic):
        paramwidgetdic={}
        for prmnam,lst in list(paramobjdic.items()):
    ...
    def GetButtonColors(self,buttonobjdic):
        ...
        """
        colordic={}
        for varnam,obj in list(buttonobjdic.items()):
            try:
    ...
    def SetColorOnButtons(self,colorobjdic,colordic): #,objlabel,varnamlst,colordic):
        ...
        for item, color in list(colordic.items()):
            ...
            tmpcolor=[int(x*255.0) for x in color]
            colorobjdic[item][0].SetBackgroundColour(tmpcolor)

    def GetColorOnButtons(self,colorobjdic,colordic):
        """
        """
        btncolordic={}
        for item, color in list(colordic.items()):
            # convert color to 0-1
            ###item=lib.StripQuotations(item)
            wxcolor=colorobjdic[item][0].GetBackgroundColour()
            btncolordic[item]=[(float(x)/255.0) for x in wxcolor]
        return btncolordic
    ...
    def ViewSelectedFile(self,cmbfil,filext):
        ...
        self.OpenTextViewer('View'+filename,filename)
    ...
    def OpenTextViewer(self, title, textfile):
        winpos = wx.GetMousePosition()
        winsize = [480, 300]
        parent = self
        #self.viewer = subwin.TextViewer_Frm(parent, winpos, winsize, title, text,menu=True)
        self.editor = subwin.TextEditor_Frm(parent, -1, winpos, winsize, title, textfile=textfile,
                                            mode='Viewer')
        self.editor.menubar.Remove(0)
    ...
    def WriteSetDefaultFile(self,filename):
        #  ext: .general, .model or .othes
        base,ext=os.path.splitext(filename)
        defaultfile=os.path.join(self.customdir,'default.files')
        ...
        with open(defaultfile,'w') as fw:
            fw.write(text)

        mess='The "'+filename+'" file has been set as a default file.'
        self.Message(mess)

    @staticmethod
    def ReadSetFile(filename):
        general=''
        model='' # addon=''; shortcut=''
        others=''
        if not os.path.exists(filename):
            return [general,model,others] # ,addon,shortcut]
        ...
        for s in lines:
            ...
            if items[0] == 'fum.setctrl.LoadModelParamSet':
                model=items[1][:-1]
                model=model[1:-1]
            #x#if items[0] == 'fum.setctrl.LoadAddonMenuItems':
            #x#    addon=items[1][:-1]; addon=addon[1:-1]
            if items[0] == 'fum.setctrl.LoadOthers':
                others=items[1][:-1]; others=others[1:-1]
        #x#return [general,model,addon,shortcut]
        return [general, model, others]

    @staticmethod
    def ReadParamSetFile(filename,paramtypedic):
        # :return: paramdic[subgroup][param]=value
        modeldic={'0':'line','1':'stick','2':'ball-and-stick','3':'CPK'}
...
class CustomOthers():
    ...
    def __init__(self,parent,pagepanel,model):
        ...
        self.classnam='CustomOthers'
        self.panelnam='Others'
        self.parent=parent
        ...
        #
        self.grouplst, self.paramsdic, self.tipsdic, self.paramtypedic= \
                self.GetOthersParams()
        self.curgroup=self.grouplst[0]
        if self.curset != '':
            pass
            # read params in file
            # update self.paramsdic.update(xxx)
        # make self.menuoptlst and self.menuitem <--- subgroups name list
        self.MakeParamValueList(self.curgroup)
        # create panel
        self.CreatePanel()
        # load default value
        self.Initialize()

    def MakeParamValueList(self,curgrp):
        if not curgrp in self.paramsdic:
            return

        self.paramvaluelst=[]
        subdic=self.paramsdic[curgrp]
        sublst=list(subdic.keys())
        sublst.sort()
        self.paramnamelst = sublst[:]
        for item in self.paramnamelst:
            self.paramvaluelst.append(subdic[item])
        if self.curparam == '':
            self.curparam = self.paramnamelst[0]

    def GetOthersParams(self):
        """

        :return: grouplst,paramsdic,tipsdic,typedic

        group number in setctrl.TableOfParamters()
           0:internal,1:general, 2:model, 3:font.image.message, 4:draw,
           5:shortcut key, 6: openbabel, rdkit, 7: misc
        """
        grouplst=['draw'] #,'message']
        groupkeys= {'draw':4, 'messages':3}
        groupnum={4:'draw'}  #,3:'messages'}
        paramtable=self.setctrl.GetParamTable()
        paramsdic={}
        tipsdic={}
        typedic={}
        for grp in grouplst:
        ...
        paramlst=list(paramtable.keys())
        for param in paramlst:
            data=paramtable[param]
            try:
                num=data[3]
            except:
                print('missing datagroup param=',param)
                continue
            if num == 4: # or num == 3:
                paramsdic[groupnum[num]][param]=paramtable[param][0]
                typedic[param] = paramtable[param][1]
                tipsdic[groupnum[num]][param] = paramtable[param][2]
    ...
    def CreatePanel(self):
        ...
        yloc=10
        # create file button
        title='Others param value file:'
        sttip=wx.StaticText(self.panel,-1,title,pos=(10,yloc),size=(160,18))
        lib.SetTipString(sttip,'Select parameter value setting file')
        self.cmbfil=wx.ComboBox(self.panel,wx.ID_ANY,"",pos=(170,yloc-2),size=(180,hcb))
        self.cmbfil.Bind(wx.EVT_COMBOBOX,self.OnParamFile)
        self.cmbfil.SetItems(self.filelst)
        self.cmbfil.SetStringSelection(self.curset) #self.curparamsetdic['Shortcut'])
        btnview=wx.Button(self.panel,-1,"View",pos=(360,yloc-2),size=(40,20))
        lib.SetTipString(btnview, 'View the file')
        btnview.Bind(wx.EVT_BUTTON,self.OnViewFile)

        self.parent.AttachHelpButton(self.panel, winpos=[415, yloc-10],name='Others')
        #
        yloc += 25
        wx.StaticLine(self.panel,pos=(-1,yloc),size=(w,4),style=wx.LI_HORIZONTAL)
        yloc += 10
        wx.StaticText(self.panel,-1,"Subgroup:",pos=(10,yloc),size=(110,18))
        self.cmbmenu=wx.ComboBox(self.panel,wx.ID_ANY,"",pos=(130,yloc-2),
                                 size=(200,hcb))
        self.cmbmenu.Bind(wx.EVT_COMBOBOX,self.OnParamItem)
        self.cmbmenu.SetItems(self.grouplst)
        self.cmbmenu.SetValue(self.curgroup)
        ...
        ybtn=150
        xpanlst=xsize-20
        hpanlst=h-yloc-ybtn+10
        pansize=(xpanlst,hpanlst)
        panpos=[10,yloc]
        lstpan=self.CreateListPanel(panpos,pansize)
        self.SetStringData()
        # set string item
        yloc=300
        pos=[-1,yloc]
    ...
    def CreateListPanel(self,panpos,pansize):
        [w,h]=pansize
        self.lstctrl=EditableListCtrl(self.panel,-1,pos=panpos,size=pansize, #250),
                         style=wx.LC_REPORT|wx.LC_NO_HEADER|wx.LC_EDIT_LABELS|wx.LC_SINGLE_SEL)
                         #style = wx.LC_REPORT | wx.LC_EDIT_LABELS | wx.LC_SINGLE_SEL)
        self.lstctrl.Bind(wx.EVT_LIST_BEGIN_LABEL_EDIT, self.OnItemEdit)
        self.lstctrl.Bind(wx.EVT_LIST_ITEM_RIGHT_CLICK,self.OnItemRightClick)
        self.lstctrl.SetBackgroundColour([200,200,200])
        width1 = int((w-40-30)/2)-30
        width3 = int((w - 40 - 30) / 2) - 22
        width4=22
        self.lstctrl.InsertColumn(0,'#',width=40,format=wx.LIST_FORMAT_RIGHT)
        self.lstctrl.InsertColumn(1,'parameter',width=width1)
        self.lstctrl.InsertColumn(2,'=',width=30)
        self.lstctrl.InsertColumn(3,'value',width=width3)
        self.lstctrl.InsertColumn(4, 'image', width=width4) # not used

    def OnItemRightClick(self,event):
        itemIdx=self.lstctrl.GetFirstSelected()
        itemobj = self.lstctrl.GetItem(itemIdx, col=1)
        paramname=itemobj.GetText()
        if paramname.find('-color') >= 0:
            color = lib.ChooseColorOnPalette(self.parent, False, -1)
            if len(color) <= 0:
                return
        if len(color) == 4:
            color[3]=1.0
        value=str(color)
        self.lstctrl.SetItem(itemIdx, 3, value)
        color=[int(float(x)*255.0) for x in color]
        self.lstctrl.SetItemTextColour(itemIdx,color) # self.changedcolor)
    ...
    def Initialize(self):
        ...
        else: self.OnParamFile(1)
        #
        self.saveas=False
        self.saved=True

    def SetStringData(self):
        ###if menuitem == '': return
        # delete all items
        self.lstctrl.DeleteAllItems()
        #
        #self.selectcolumn=-1; i=0; nkey=0
        #indx=self.lstctrl.InsertItem(200,'')
        for i in range(len(self.paramnamelst)):
            param=str(self.paramnamelst[i])
            value=str(self.paramvaluelst[i])
            indx=self.lstctrl.InsertItem(40,str(i+1))
            self.lstctrl.SetItem(indx,1,param)
            self.lstctrl.SetItem(indx,2,'=')
            self.lstctrl.SetItem(indx,3,value)
            self.lstctrl.SetItem(indx, 4, '')
            if param.find('-color') > 0:
                color=self.paramvaluelst[i]
                color=[int(float(x) * 255.0) for x in color]
                try:
                    color[3]=255
                except: pass
                self.lstctrl.SetItemTextColour(indx, color)
    ...
    def OnItemEdit(self,event):
        ...
        #itemIdx=selectedcol
        self.cmbfil.SetValue('')
        colm=event.GetColumn()
        if colm == trgcolumn:
            itemobj = self.lstctrl.GetItem(itemIdx, col=trgcolumn)
            param=self.lstctrl.GetItem(itemIdx, col=1).GetText()
            value=itemobj.GetText()
            try:
                self.lstctrl.SetItemTextColour(itemIdx,self.changedcolor)
            except: pass
            if param.find('-color') > 0:
                color=lib.StringToList(value,conv=True)
                color=[int(x*255.0) for x in color]
                try:
                    color[3]=255
                except: pass
                self.lstctrl.SetItemTextColour(itemIdx, color)
        else:
            event.Veto()

    def GetParamsFromWidgets(self,changedonly=False): #,tosetting=True):
        itemnum=self.lstctrl.GetItemCount()
        newparamsdic={}
        for itemIdx in range(itemnum):
            param = self.lstctrl.GetItem(itemIdx, col=1).GetText()
            if param == '': continue
            value = self.lstctrl.GetItem(itemIdx, col=3).GetText()
            value=lib.StringToVariableType(value.strip())
            if changedonly:
                color = self.lstctrl.GetItemTextColour(itemIdx)
                if color == self.changedcolor:
                    newparamsdic[param]=value
            else:
                newparamsdic[param] = value

        #if tosetting:
            #pass
            #newparamsdic=self.CustomAndSettingParams(newparamsdic,tocustom=False)

        return newparamsdic

    def xxCustomAndSettingParams(self,tocustom=True):
        """ Converts the parameter names in the Settings module to those in
            the Custom module and vice versa.

        :param bool tocustom: True to convert form setting to custom names,
                              False vice versa
        """
        convdic={'win-size': ['win-size-w','win-size-h'],
                 'shell-win-size':['shell-win-size-w','shell-win-size-h']}
        paramsdic=self.paramsdic
        paramtypedic=self.paramtypedic
        if tocustom:
            for nam, lst in convdic:
                if nam in paramsdic:
                    winsize = paramsdic[nam]
                    paramsdic[lst[0]] = winsize[0]
                    paramsdic[lst[1]] = winsize[1]
                    del paramsdic[nam]
                    paramtypedic[lst[0]]='int'
                    paramtypedic[lst[1]] = 'int'
        else:
            for nam, lst in convdic:
                if lst[0] in paramsdic and lst[1] in paramsdic:
                    dat=[paramsdic[lst[0]],paramsdic[lst[1]]]
                    paramsdic[nam] = dat
                    for wandh in lst:
                        del paramsdic[wandh]
                    paramtypedic[nam]='list'

    def OnParamItem(self,event):
        self.saveas=False
        self.curparam=self.cmbmenu.GetValue()
        self.SetStringData()
        self.cmbfil.SetValue('')
    ...
    def OnParamFile(self,event):
        ...
        filename=curset
        if len(filename) <= 0: return
        filename=self.parent.MakeFullPathName(filename,'.others')
        ans=self.parent.IsFileExists(filename)
        if not ans: return

        self.paramdic=rwfile.ReadParameterFile(filename)
        self.SetStringData()
        # update parent curset
        self.curset=curset
        self.parent.SetCurrentParamSet(self.panelnam,self.curset)

        mess='Parameters file "'+filename+'".'
        self.parent.Message(mess)
    ...
    def OnViewFile(self,event):
        self.saveas=False
        self.parent.ViewSelectedFile(self.cmbfil,'.others')

    def IsSaved(self):
        return self.saved

    def LoadDefault(self):
        if not self.paramsdic: return
        self.saveas=False

        self.grouplst,paramsdic,self.tipsdic, self.typedic=self.GetOthersParams()
    ...
    def SaveFileAs(self):
        cstdir=self.setctrl.GetDir('Customize')
        filename=lib.GetFileName(None,'paramfile (*.others)|*.others',rw='w',
                                 check=True,changedir=cstdir)
        if len(filename) <= 0:
            return

        self.saveas=False
        self.parent.Message('')
        self.newparamsdic=self.GetParamsFromWidgets()
        #
        comm='others parameters\n'
        comm+='the elements of color list [r,g,b,a] are in the range of 0-1'
        group='others'
        rwfile.WriteParameterFile(filename,self.paramtypedic,self.newparamsdic,
                                        groupname=group,comment=comm)
        #
        setname=self.parent.AppendParamSetFile('Others',filename,curset=True)
        if setname is None:
            return
        ...
        self.curset=setname
        self.cmbfil.SetItems(self.filelst)
        self.cmbfil.SetValue(setname)

        self.parent.Message('Saved "'+filename+'".')
        self.saved=True

    def Cancel(self):
        mess='Others param values assignment was canceled.'
...
class CustomSetting():
    ...
    """
    def __init__(self,parent,pagepanel,model):
        self.classnam='Setting'
        #x#self.panelnam='Setting'
    ...
    def CreatePanel(self):
        ...
        lib.SetTipString(sttip,'Select setting name(file,*setting)')
        ...
        lib.SetTipString(sttip,'Select project for settings')
        ...
        lib.SetTipString(btnviewprj,'View or edit the file with editor')
    ...
    def OnSetFile(self,event):
        ...
        #
        for label,cmb in list(self.comboboxlabeldic.items()):
            cmb.SetStringSelection(self.curparamsetdic[label])
        # set labels
        text="Param set in the loaded setting file: "+'"'+self.curparamsetdic[self.panelnam]+'"'
        self.stseting.SetLabel(text)
        for label, obj in list(self.statictextdic.items()):
            text=label+' params set file '+'"'+self.curparamsetdic[label]+'"'
            obj.SetLabel(text)
    ...
    def OnParamFile(self,event):
        ...
        for prmnam, cmb in list(self.comboboxlabeldic.items()):
    ...
    def ResetCurrentParamFile(self,setnam,curset):
        ...
        if setnam not in self.comboboxlabeldic: return
        self.comboboxlabeldic[setnam].SetStringSelection(curset)

    def xxOnProject(self,event):
    ...
    def OnViewParamSetFile(self,event):
       ...
       if label in self.comboboxlabeldic: cmb=self.comboboxlabeldic[label]
       else: cmb=None
       filext='.'+label.lower()
    ...
    def ClearAllTextInTCL(self):
        for key, value in list(self.tcliddic.items()):
            print(('label',value[0]))
            value[1].SetValue('')

    def SetCurFileInComboBox(self,prmnam):
        for key,lst in list(self.comboboxlabeldic.items()):
    ...
    def SaveFile(self):
        ...
        if os.path.exists(filename):
            ...
            for prmnam,prmfile in list(self.paramsdic.items()):
                if prmnam == 'General':
    ...
    def UpdateParamSetInSetting(self,textlst):
        ...
        for text in textlst:
            ...
            if ns >= 0 and 'General' in self.paramsdic:
    ...
    def LoadDefault(self):
        for prmnam, cmb in list(self.comboboxlabeldic.items()): cmb.SetValue('None')
...
class CustomModel():
    ...
    def DefineModelWidgetItems(self):
        ...
        self.elmnamlst=list(self.elmcolordic.keys())
        self.rescolordic=self.defaultparamsdic['aa-residue-color']
        self.resnamlst=list(self.rescolordic.keys())
        self.chaincolordic=self.defaultparamsdic['aa-chain-color']
        self.chainnamlst=list(self.chaincolordic.keys())
    ...
    def CreatePanel(self):
        ...
        btnview=wx.Button(self.panel,-1,"View",pos=(360,yloc-2),size=(40,20))
        btnview.Bind(wx.EVT_BUTTON,self.OnViewFile)
        self.parent.AttachHelpButton(self.panel, winpos=[415, yloc-10],name='Model')
        #
        yloc += 25
        wx.StaticLine(self.panel,pos=(-1,yloc),size=(w,4),style=wx.LI_HORIZONTAL)
        yloc += 10
        wx.StaticText(self.panel,-1,"Default model:",pos=(10,yloc),size=(100,18))
        self.cmbmdl=wx.ComboBox(self.panel,-1,"",pos=(120,yloc-2),size=(120,hcb))
        self.cmbmdl.SetItems(self.modellst)
        yloc += 25
        wx.StaticText(self.panel,-1,"Atom sphere radius:",pos=(10,yloc),size=(130,18))
        self.tclatmrad=wx.TextCtrl(self.panel,-1,'',pos=(140,yloc-2),size=(40,htx))
        self.tclwidglst.append(self.tclatmrad)
        wx.StaticText(self.panel,-1,"bond thickness:",pos=(195,yloc),size=(95,18))
        self.tclbndthck=wx.TextCtrl(self.panel,-1,'',pos=(300,yloc-2),size=(40,htx))
        self.tclwidglst.append(self.tclbndthck)
        yloc += 25
        wx.StaticText(self.panel,-1,"Hydrogen bond color:",pos=(10,yloc),size=(120,18))
        self.btnhbcol=wx.Button(self.panel,-1,"",pos=(140,yloc-2),size=(20,hbt))
        self.btnhbcol.Bind(wx.EVT_BUTTON,self.OnButtonColor)
        wx.StaticText(self.panel,-1,"bond thickness:",pos=(175,yloc),size=(100,18))
        self.tclhbthick=wx.TextCtrl(self.panel,-1,str(self.hbthick),pos=(300,yloc-2),size=(40,htx))
        self.tclwidglst.append(self.tclhbthick)
        yloc += 25
        wx.StaticText(self.panel,-1,"Chain tube color:",pos=(10,yloc),size=(120,18))
        self.btntubecol=wx.Button(self.panel,-1,"",pos=(140,yloc-2),size=(20,hbt))
        self.btntubecol.Bind(wx.EVT_BUTTON,self.OnButtonColor)
        wx.StaticText(self.panel,-1,"tube thickness:",pos=(175,yloc),size=(100,18))
        self.tcltuberad=wx.TextCtrl(self.panel,-1,'',pos=(300,yloc-2),size=(40,htx))
        self.tclwidglst.append(self.tcltuberad)
        yloc += 20
        wx.StaticLine(self.panel,pos=(-1,yloc),size=(w,4),style=wx.LI_HORIZONTAL)
        ...
        # elm color panel
        xloc=25; xloc0=xloc
        yloc=yloc0; width1=110
        wx.StaticText(self.panel,-1,"(Element color)",pos=(xloc+10,yloc),size=(100,18))
        yloc += 20
        self.colorpanpos['element']=[[xloc,yloc],[width1,height]]
        ...
        # res color panel
        xloc += width1+25; width2=110; yloc=yloc0; xloc1=xloc
        wx.StaticText(self.panel,-1,"(Residue color)",pos=(xloc+10,yloc),size=(90,18))
        yloc += 20
        # chain color panel
        self.colorpanpos['residue']=[[xloc,yloc],[width2,height]]
        ...
        yloc=yloc0
        xloc2=xloc
        wx.StaticText(self.panel,-1,"(Chain color)",pos=(xloc+20,yloc),size=(80,18))
        yloc += 20
        self.colorpanpos['chain'] = [[xloc, yloc], [width3, height]]
        chapan,colorobjdic=self.CreateColorButtonPanel([xloc,yloc],[width3,height],
                         'chain',self.chainnamlst,self.chaincolordic)
        self.colorpanelobjdic['chain']=[chapan,colorobjdic,[xloc,yloc],[width2,height]]
        # elm add/del button
        self.inputtcldic={}; self.adddelbtnlst=[]
        yloc=yloc0+height+30; #xloc0 -= 5
        item='element'
        self.CreateAddDelButton(0,item,(xloc0,yloc-2))
        # res add/del button
        yloc=yloc0+height+30; #xloc1 -= 5
        item='residue'
        self.CreateAddDelButton(2,item,(xloc1,yloc-2))
        # chain add/del button
        yloc=yloc0+height+30; #xloc2 -= 5
        item = 'chain'
        self.CreateAddDelButton(4,item,(xloc2,yloc-2))
        yloc=yloc+20
        # put common buttons
        pos=[-1,yloc]
        ...
        for tclobj in self.tclwidglst:
            tclobj.Bind(wx.EVT_SET_CURSOR,self.OnTclFocus)

    def OnTclFocus(self,event):
        self.cmbfil.SetValue('')

    def CreateAddDelButton(self,addbtnnmb,item,pos):
        htx=20
        xloc=pos[0]; yloc=pos[1]
        #ID=wx.NewId()
        tclinp=wx.TextCtrl(self.panel,-1,'',pos=(xloc,yloc),size=(40,htx),style=wx.TE_PROCESS_ENTER)
        tclinp.Bind(wx.EVT_TEXT_ENTER,self.OnInputItem)
        lib.SetTipString(tclinp,'Press the "Add" or "Del" button after typing')
        #self.inputtcldic[ID]=[addbtnnmb,tclinp] # 0: add button number
        #self.buttondic[addbtnnmb]=tclinp
        btnadd=wx.Button(self.panel,-1,"Add",pos=(xloc+45,yloc),size=(30,20))
        btnadd.Bind(wx.EVT_BUTTON,self.OnAddButton)
        lib.SetTipString(btnadd,'Add a parameter in the left input window')
        #btnadd.SetValue(True)
        ###self.adddelbtnlst.append(btnadd)
        btndel=wx.Button(self.panel,-1,"Del",pos=(xloc+80,yloc),size=(30,20))
        btndel.Bind(wx.EVT_BUTTON,self.OnDelButton)
        lib.SetTipString(btndel,'Delete a parameter.')

        self.tcldic[tclinp]=item
        self.addbuttondic[btnadd] = item
        self.delbuttondic[btndel]=  item

    def CreateColorButtonPanel(self,pos,size,item,itemnamlst,colordic):
        self.buttondic={}
        ...
        colpan.SetScrollRate(5,self.hscroll)
        colorobjdic=self.CreateColorButtonsOnPanel(
                         colpan, itemnamlst, colordic)
        return colpan,colorobjdic

    def CreateColorButtonsOnPanel(self,panel,varnamlst,colordic):
        """

        panel: self.atmpan,self.respan, or self.chapan
        varnamlst itemcolordic.keys(), item='element','resdiue', or 'chain'
        coloddic  itemcolordic
        """
        yloc=8
        htx=20
        panel.Scroll(0,0)
        ...
        for varnam in varnamlst:
            svarnam=varnam
            if not isinstance(varnam,str):
                svarnam=str(varnam)
            text=wx.StaticText(panel,-1,str(varnam),pos=(15,yloc-4),size=(30,18))

            if varnam in self.itemcolordic:
    ...
    def DestroyColorButtonsOnPanel(self,colorobjdic):
        for item,objs in list(colorobjdic.items()):
    ...
    def GetParamsFromWidgets(self):
        ...
        self.paramwidgetdic = self.parent.MakeParamWidgetDic(paramobj)
        newparamsdic={}
        for prmnam, widget in list(self.paramwidgetdic.items()):
            if   widget[0] == 'TextCtrl': newparamsdic[prmnam]=widget[1].GetValue()
            elif widget[0] == 'ComboBox': newparamsdic[prmnam]=widget[1].GetValue()
            elif widget[0] == 'Button':
                wxcolor=widget[1].GetBackgroundColour() # wx.Colour object
                color = [(float(x)/255.0) for x in wxcolor]
                try:
                    color[3]=1.0
                except: pass
                newparamsdic[prmnam]=color
            elif widget[0] == 'Method':
                itemlst=['element-color','aa-residue-color','aa-chain-color']
                itemname=['element','residue','chain']
                for i in range(len(itemlst)):
                    item=itemlst[i]
                    colordic=self.paramsdic[item] #['element-color']
                    colorobjdic=self.colorpanelobjdic[itemname[i]][1] # ['element'][1]
                    newcolordic=self.parent.GetColorOnButtons(colorobjdic,colordic)
                    newparamsdic[item]=newcolordic
    ...
    def SetParamsToWidgets(self,paramsdic):
        ...
        for prmnam, value in list(paramsdic.items()):
            if prmnam not in self.paramwidgetdic: continue
            ...
            if   widget[0] == 'TextCtrl':  widget[1].SetValue(str(value))
                ...
                color=[int(x*255.0) for x in value]
                widget[1].SetBackgroundColour(color[:3]) # wx.Colour object
            elif widget[0] == 'Method':
                itemlst = ['element-color', 'aa-residue-color', 'aa-chain-color']
                itemname = ['element', 'residue', 'chain']
                for i in range(len(itemlst)):
                    item=itemlst[i]
                    colordic = paramsdic[item]
                    #if item == 'element-color':
                    #    colordic=self.StripQuortOfElementName(colordic)
                    colorobjdic = self.colorpanelobjdic[itemname[i]][1]
                    self.parent.SetColorOnButtons(colorobjdic, colordic)

    def xxStripQuortOfElementName(self,colordic):
        """  """
        changedic = {}
        for elm0 in colordic.keys():
            elm = lib.StripQuotations(elm0)
            if elm0 != elm:
                changedic[elm0] = elm

        for elm0 in changedic.keys():
            elm = changedic[elm0]
            colordic[elm] = colordic[elm0]
            del colordic[elm0]
    ...
    def CountModifiedParams(self):
        nmody=0
        for prmnam, value in list(self.currentparams.items()):
            if prmnam in self.newparamdic:
                if self.currentparams[prmnam] != self.newparamdic[prmnam]: nmody += 1
        return nmody

    def LoadDefaultElmColor(self,rgb255=255):
        """  """
        colordic=copy.deepcopy(ctrl.SettingCtrl.DefaultElementColor())
        for item,color in list(colordic.items()):
            colordic[item]=color[:3]
        return colordic

    def LoadDefaultResColor(self):
        """  """
        colordic=copy.deepcopy(ctrl.SettingCtrl.DefaultAAResidueColor())
        for item,color in list(colordic.items()):
            colordic[item]=color[:3]
        return colordic
    ...
    def LoadDefaultChainColor(self):
        """  """
        colordic=copy.deepcopy(ctrl.SettingCtrl.DefaultAAChainColor())
        for item,color in list(colordic.items()):
            colordic[item]=color[:3]
        return colordic

    def LoadDefault(self,case=0):
        self.parent.Message('')
        self.paramsdic=copy.deepcopy(self.defaultparamsdic)
        self.cmbfil.SetValue('')
        if case == -1: # initial setting
            self.SetParamsToWidgets(self.paramsdic)
            self.DefineModelWidgetItems()
            return
        else:
            elmcolordic=self.LoadDefaultElmColor()
            #elmcolordic=self.AttachQuortToName(elmcolordic)
            rescolordic=self.LoadDefaultResColor()
            chaincolordic=self.LoadDefaultChainColor()
            itemlst=['element','residue','chain']
            methlst=['element-color','aa-residue-color','aa-chain-color']
            colordiclst=[elmcolordic,rescolordic,chaincolordic]
            for i in range(len(itemlst)):
                item=itemlst[i]
                self.paramsdic[methlst[i]]=colordiclst[i]
                itemcolordic=colordiclst[i]
                itemnamlst=list(itemcolordic.keys())
                # destroy color panel
                self.colorpanelobjdic[item][0].Destroy()
                # recreate color panel
                panpos=self.colorpanpos[item][0]
                pansize=self.colorpanpos[item][1]
                panobj,colorobjdic=self.CreateColorButtonPanel(panpos,pansize,
                                                  item,itemnamlst,itemcolordic)
                self.colorpanelobjdic[item]=[panobj,colorobjdic,panpos,pansize]
            ...
            self.DefineModelWidgetItems()
        #
        mess = 'All parameters have been reset to default values'
        self.parent.Message(mess)

        self.cmbfil.SetValue('')

    def SetDefault(self):
        pname=self.cmbfil.GetStringSelection()
        if pname == '':
            mess='Please select a "Model paramvalue file".'
            wx.MessageBox(mess,'SetDefault')
            return

        filename=pname+'.model'
        self.parent.WriteSetDefaultFile(filename)

    def GetDefaultModelParams(self):
        defaultdic={}
        for prmnam,type in list(self.paramtypedic.items()):
            defaultdic[prmnam]=copy.deepcopy(self.setctrl.GetDefaultParam(prmnam))
            if defaultdic[prmnam] == None:
                if prmnam != 'logfile-dir':
                    print(('Not found prmnam=',prmnam))
    ...
    def OnAddButton(self,event):
        ...
        item = self.addbuttondic[obj]
        value=self.GetInputItemValue(item)

        idx,lenlstnam=self.GetItemIndex(item,value)
        if idx >= 0:
            mess='There is "'+str(value)+'" in "('+item+'color)", can not add.'
            self.parent.Message(mess)
            return

        itemcolordic=self.GetItemColorDict(item)
        if itemcolordic is None:
            wx.MessageBox('Program error!', 'CustomModel.OnDelButton')
            return
        itemcolordic[value]=numpy.array([1.0,1.0,1.0])
        itemnamlst=list(itemcolordic.keys())
        if item == 'chain':
            itemnamlst.sort()
        # destroy color panel
        ...
        self.colorpanelobjdic[item]=[panobj,colorobjdic,panpos,pansize]
        #
        mess = '"' + str(value) + '" in "' + item + 'color" was added.'
        self.parent.Message(mess)

        tclobj = self.GetInputTclObj(item)
        if tclobj is not None:
            tclobj.SetValue('')

        self.ScrollPanelToItem(item, value)
    ...
    def OnDelButton(self, event):
        self.parent.Message('')
        obj = event.GetEventObject()
        item = self.delbuttondic[obj]
        value=self.GetInputItemValue(item)
        idx,lenlstnam=self.GetItemIndex(item,value)
        if idx < 0:
            mess='No "'+str(value)+'" in "('+item+'color)", can not delete.'
            self.parent.Message(mess)
            return
        ...
        if itemcolordic is None:
            wx.MessageBox('Program error!', 'CustomModel.OnDelButton')
            return
        delval = itemcolordic.pop(value, None)
        itemnamlst=list(itemcolordic.keys())
        if item == 'chain':
            itemnamlst.sort()
        # destroy color panel
        ...
        self.colorpanelobjdic[item]=[panobj,colorobjdic,panpos,pansize]
        #
        mess = '"' + str(value) + '" in "' + item + 'color" was deleted.'
        self.parent.Message(mess)
        ...
        if idx-1 < 0:
            num=0
        value=itemnamlst[num]

        self.ScrollPanelToItem(item, value)

        self.cmbfil.SetValue('')

    def GetInputTclObj(self,item):
        tclobj=None
        for obj in self.tcldic.keys():
            if self.tcldic[obj] == item:
                tclobj=obj
                break
        return tclobj

    def GetInputItemValue(self,item):
        tclobj=self.GetInputTclObj(item)

        if tclobj is None:
            return None

        value = tclobj.GetValue()
        # to upper case
        if item == 'element':
            value=value.upper();
            value=value.rjust(2,' ')
            ...
            if not value.isdigit():
                mess = 'Wrong input value. "' + value + '". Integers only'''
                wx.MessageBox(mess,'OnInputItem')
                return None
            else:
                value=int(value)
                maxval=max(self.chainnamlst)
                if value > maxval:
                    value= maxval+1
                if tclobj is not None:
                    tclobj.SetValue(str(value))

        self.cmbfil.SetValue('')
        return value

    def GetItemIndex(self,item,value):
        if   item == 'element':
            namlst=list(self.elmcolordic.keys())
        elif item == 'residue':
            namlst = list(self.rescolordic.keys())
        elif item == 'chain':
            namlst = list(self.chaincolordic.keys())
            namlst.sort()

        if value in namlst:
            idx=namlst.index(value)
        else:
            idx=-1
        return idx,len(namlst)

    def GetItemColorDict(self,item):
        itemcolordic=None
        if   item == 'element':
            itemcolordic=self.elmcolordic
        elif item == 'residue':
            itemcolordic=self.rescolordic
        elif item == 'chain':
            itemcolordic=self.chaincolordic
        else:
            wx.MessageBox('Unknown item=',item)
        return itemcolordic

    def OnInputItem(self,event):
        self.parent.Message('')
        obj=event.GetEventObject()
        item=self.tcldic[obj]
        # get input value
        value=self.GetInputItemValue(item)
        if value is None:
            mess = 'No "' + str(value) + '" in "' + item + 'color".'
            self.parent.Message(mess)
            return
    ...
    def ScrollPanelToItem(self,item,value):
        hline= 1   #  != self.hscroll, self.hscroll: y scroll rate
        idx, lennamlst=self.GetItemIndex(item,value)
        if idx == 0:
            idx=1
        x=0
        if idx > 0:
    ...
    def OnParamFile(self,event):
        ...
        if not ans: return
        # load params
        paramsdic=rwfile.ReadParameterFile(filename)
        ...
        colorobjdic=self.CreateColorButtonsOnPanel(pan,self.elmnamlst,self.elmcolordic)
        self.colorpanelobjdic['element']=[pan,colorobjdic]
        pan=self.colorpanelobjdic['residue'][0]
        colorobjdic=self.CreateColorButtonsOnPanel(pan,self.resnamlst,self.rescolordic)
        self.colorpanelobjdic['residue']=[pan,colorobjdic]
        pan=self.colorpanelobjdic['chain'][0]
        colorobjdic=self.CreateColorButtonsOnPanel(pan,self.chainnamlst,self.chaincolordic)
    ...
    def SaveFileAs(self):
        cstdir = self.setctrl.GetDir('Customize')
        filename=lib.GetFileName(None,'paramfile (*.model)|*.model',rw='w',
                                 check=True,changedir=cstdir)
        if len(filename) <= 0:
            return
        ...
        self.parent.Message('')
        self.newparamsdic=self.GetParamsFromWidgets()
        #self.newparamsdic=self.AttachQuortToElementName(newparamsdic)
        #
        comm='model parameters\n'
        comm+='the elements of color list [r,g,b,a] are in the range of 0-1'
        group='model'
        ...
        rwfile.WriteParameterFile(filename,self.paramtypedic,self.newparamsdic,
                                        groupname=group,comment=comm)
        #
        setname=self.parent.AppendParamSetFile('Model',filename,curset=True)
        if setname is None:
            return
    ...
    def xxAttachQuortToName(self,paramdic):
        changedic={}
        for elm0 in paramdic.keys():
            elm = elm0.strip()
            if len(elm) == 1:
                elmnam = '" ' + elm + '"'
                changedic[elm0] = elmnam

        for elm0 in changedic.keys():
            elmnam = changedic[elm0]
            paramdic[elmnam] = paramdic[elm0]
            del paramdic[elm0]
        return paramdic

    def xxAttachQuortToElementName(self,newparamsdic):
        changedic={}
        if 'element-color' in newparamsdic:
            newparamsdic['element-color']=\
                self.AttachQuortToName(newparamsdic['element-color'])
            #for elm0 in newparamsdic['element-color'].keys():
            #    elm=elm0.strip()
            #    if len(elm) == 1:
            #        elmnam='" '+elm+'"'
            #        changedic[elm0]=elmnam
            ...
            #for elm0 in changedic.keys():
            #    elmnam=changedic[elm0]
            #    newparamsdic['element-color'][elmnam]=newparamsdic['element-color'][elm0]
            #    del newparamsdic['element-color'][elm0]
        return newparamsdic
...
class CustomAddOns():
    ...
    def CreatePanel(self):
        ...
        lib.SetTipString(sttip,'Select add-on menu definition name(file,*.add-on)')
        ...
        lib.SetTipString(stscr,'All script files(*.py) in "Customize" directory')
        ...
        lib.SetTipString(stmenu,'Selected scripts for the menu')
        ...
        lib.SetTipString(self.btnchk,'Check/Uncheck all items')
        btnadd=wx.Button(self.panel,wx.ID_ANY,"Add to menu",pos=(90,yloc-2),size=(80,20))
        btnadd.Bind(wx.EVT_BUTTON,self.OnAddToMenu)
        #btnadd.SetToolTipString('Add checked scripts to menu items')
        lib.SetTipString(btnadd,'Add checked scripts to menu items')
        # menu panel
        #yloc -= 30
        xloc=w/2+20
        ...
        lib.SetTipString(self.btnclear,'Delete all items')
        ...
        lib.SetTipString(btndel,'Remove checked items')
        ...
        lib.SetTipString(btnup,'Move upward')
        ...
        lib.SetTipString(btndwn,'Move downward')
        yloc += 25
    ...
    def ClearScriptPanel(self):
        for scrnam,widget in list(self.scriptchkboxdic.items()): widget.Destroy()
        self.scriptchkboxdic={}

    def ClearMenuPanel(self):
        for scrnam,widget in list(self.menuchkboxdic.items()): widget.Destroy()
        self.menuchkboxdic={}
    ...
    def OnCheckAll(self,event):
        ...
        for scrnam,widget in list(self.scriptchkboxdic.items()): widget.SetValue(value)

    def OnAddToMenu(self,event):
        self.saveas=False
        self.menuitemlst=[]; self.menuchecklst=[]
        for scrnam, chkbox in list(self.scriptchkboxdic.items()): #[name]=[chkbox]
            if chkbox.GetValue():
    ...
    def OnClearMenu(self,event):
        ...
        for scrnam,widget in list(self.menuchkboxdic.items()): widget.SetValue(value)
    ...
    def OnDelMenu(self,event):
        ...
        self.saveas=False
        dellst=[]
        for scrnam,widget in list(self.menuchkboxdic.items()):
    ...
    def CheckSelected(self):
        for scrnam,widget in list(self.scriptchkboxdic.items()):
            try:
...
class CustomGeneral():
    ...
    def __init__(self,parent,pagepanel,model):
        ...
        self.winctrl=model.winctrl
        self.setctrl=model.setctrl
        #x#self.menuctrl=model.menuctrl
        self.pagepanel.SetBackgroundColour('light gray')
        #
        self.saved=True
        self.saveas=False
    ...
    def CreatePanel(self):
        ...
        self.tclwidglst=[]
        # create file button
        title='General param value file:'
        sttip=wx.StaticText(self.panel,-1,title,pos=(10,yloc),size=(160,18))
        lib.SetTipString(sttip,'Select parameter value setting file')
        self.cmbfil=wx.ComboBox(self.panel,wx.ID_ANY,"",pos=(170,yloc-2),size=(180,hcb))
        self.cmbfil.Bind(wx.EVT_COMBOBOX,self.OnParamFile)
        ...
        btnview=wx.Button(self.panel,-1,"View",pos=(360,yloc-2),size=(40,20))
        self.parent.AttachHelpButton(self.panel, winpos=[415, yloc-10],name='General')
        ...
        yloc += 20 # win-size
        wx.StaticText(self.panel,-1,"Main window color: ",pos=(20,yloc),size=(120,18))
        self.btnwincol=wx.Button(self.panel,wx.ID_ANY,"",pos=(150,yloc),size=(20,hbt))
        self.btnwincol.Bind(wx.EVT_BUTTON,self.OnButtonColor) #self.OnWindowColor(obj=self.btnwincol))
        wx.StaticText(self.panel,-1,"In geometry edit mode: ",pos=(190,yloc),size=(150,18))
        self.btngeocol=wx.Button(self.panel,wx.ID_ANY,"",pos=(350,yloc),size=(20,hbt))
        self.btngeocol.Bind(wx.EVT_BUTTON,self.OnButtonColor) #self.OnWindowColorGeomMode)
        yloc += 25 # shell-win-shape (style)
        wx.StaticText(self.panel,-1,"MouseMode/MolChoice window shape(0:horiz,1:vert): ",pos=(20,yloc),size=(330,18))
        self.tclmuswin=wx.TextCtrl(self.panel,-1,'',pos=(360,yloc-2),size=(25,htx))
        self.tclwidglst.append(self.tclmuswin)
        self.tclmolwin=wx.TextCtrl(self.panel,-1,'',pos=(395,yloc-2),size=(25,htx))
        self.tclwidglst.append(self.tclmolwin)
        yloc += 25 #
        ...
        self.btnselcol=wx.Button(self.panel,wx.ID_ANY,"",pos=(80,yloc),size=(20,hbt))
        self.btnselcol.Bind(wx.EVT_BUTTON,self.OnButtonColor) #self.OnSelectColor)
        #yloc += 25 # sel-sphere color
        wx.StaticText(self.panel,-1,"sphere/box selected region: ",pos=(110,yloc),size=(175,18))
        self.btnsphcol=wx.Button(self.panel,wx.ID_ANY,"",pos=(285,yloc),size=(20,hbt))
        ...
        lib.SetTipString(stlabel,'Element/atom/residure name/number color')
        self.btnlblcol=wx.Button(self.panel,wx.ID_ANY,"",pos=(365,yloc),size=(20,hbt))
        self.btnlblcol.Bind(wx.EVT_BUTTON,self.OnButtonColor)
        yloc += 25 # draw-message-color
        stmess=wx.StaticText(self.panel,-1,"mode message: ",pos=(20,yloc),size=(95,18))
        lib.SetTipString(stmess,'Message color for mouse mode')
        self.btnmescol=wx.Button(self.panel,wx.ID_ANY,"",pos=(120,yloc),size=(20,hbt))
        self.btnmescol.Bind(wx.EVT_BUTTON,self.OnButtonColor)
        #yloc += 25 # rot-axis-arrow-color
        wx.StaticText(self.panel,-1,"rot-axis arrow: ",pos=(150,yloc),size=(90,18))
        self.btnaxicol=wx.Button(self.panel,wx.ID_ANY,"",pos=(245,yloc),size=(20,hbt))
        ...
        self.btnvibcol=wx.Button(self.panel,wx.ID_ANY,"",pos=(380,yloc),size=(20,hbt))
        self.btnvibcol.Bind(wx.EVT_BUTTON,self.OnButtonColor)
        yloc += 25 #line thickness
        wx.StaticText(self.panel,-1,"(Line thickness)",pos=(10,yloc),size=(100,18))
        yloc += 20
        wx.StaticText(self.panel,-1,"rot-axis arrow: ",pos=(20,yloc),size=(90,18))
        self.tclrotaxi=wx.TextCtrl(self.panel,-1,'',pos=(120,yloc-2),size=(25,htx))
        self.tclwidglst.append(self.tclrotaxi)
        wx.StaticText(self.panel,-1,"xyz-axis: ",pos=(160,yloc),size=(55,18))
        self.tclxyzaxi=wx.TextCtrl(self.panel,-1,'',pos=(220,yloc-2),size=(25,htx))
        self.tclwidglst.append(self.tclxyzaxi)
        wx.StaticText(self.panel,-1,"cell-box-thick: ",pos=(260,yloc),size=(90,18))
        self.tclcellbox=wx.TextCtrl(self.panel,-1,'',pos=(360,yloc-2),size=(25,htx))
        self.tclwidglst.append(self.tclcellbox)
        yloc += 25 # FMO menu
        wx.StaticText(self.panel,-1,"(Misc)",pos=(10,yloc),size=(40,18))
        yloc += 20
        wx.StaticText(self.panel,-1,"FMO menu enable:",pos=(20,yloc),size=(120,18))
        self.chkfmo=wx.CheckBox(self.panel,-1,"",pos=(150,yloc+2),size=(20,20))
        # stereo eye
        wx.StaticText(self.panel,-1,"Stereo-eye(1:cross,2:parallel):",pos=(190,yloc),
                      size=(280,18))
        self.tcleye=wx.TextCtrl(self.panel,-1,'',pos=(380,yloc),size=(25,htx))
        self.tclwidglst.append(self.tcleye)
        yloc += 25 # 'max-undo'
        stundo=wx.StaticText(self.panel,-1,"Max undo:",pos=(20,yloc),size=(70,18))
        lib.SetTipString(stundo,"Max. number of undo for molecular structure change")
        self.tclundo=wx.TextCtrl(self.panel,-1,"3",pos=(90,yloc),size=(20,htx))
        self.tclwidglst.append(self.tclundo)
        # 'auto-add-hydrogens'
        stauto=wx.StaticText(self.panel,-1,"Auto add hydrogens:",pos=(120,yloc),size=(130,18))
        lib.SetTipString(stauto,"Add hydrogens to polypeptide when PDB data is read")
        self.chkauto=wx.CheckBox(self.panel,-1,"",pos=(260,yloc+2),size=(20,18))
        #'dump-draw-items'
        stdump=wx.StaticText(self.panel,-1,"Dump draw items:",pos=(290,yloc),size=(120,18))
        lib.SetTipString(stdump,"Dump draw items at close")
        self.chkdump=wx.CheckBox(self.panel,-1,"",pos=(410,yloc+2),size=(20,18))
        yloc += 25
        # put buttons
        pos=[-1,yloc]
        self.butnpan=CommonActionButtons(self.parent,self.pagepanel,self,pos)
        # bind a method to textctrl widgets
        for tclobj in self.tclwidglst:
            tclobj.Bind(wx.EVT_SET_FOCUS, self.OnTclFocus)
    ...
    def OnCheckBox(self,event):
        obj=event.GetEventObject()
        label=obj.GetLabel()
        mess='Sorry, this is under construction.'
        if label == "horizontal":
            message=label+': ' +mess
        elif label == "vertical":
            message=label+': '+ mess
            ...
            message=''
        elif label == "fmo menu enable":
            message=''

        if len(message) > 0:
            wx.MessageBox(message, 'CustomizeGenetral.OnRadioButton')
    ...
    def ParamObjDefinition(self):
        paramobj =  { 'win-color':['Button',self.btnwincol,None,'color'],
                      'win-color-geom-mode':['Button',self.btngeocol,None,'color'],
                      'mousemode-win-shape':['TextCtrl',self.tclmuswin,None,'int'],
                      'molchoice-win-shape': ['TextCtrl', self.tclmolwin, None, 'int'],
                      ...
                      'cell-box-thick': ['TextCtrl', self.tclcellbox, None, 'int'],
                      'fmo-menu':['CheckBox',self.chkfmo,None,'bool'],
                      'stereo-eye':['TextCtrl',self.tcleye,None,'int'],
    ...
    def OnParamFile(self,event):
        ...
        if not ans: return
        # load params
        self.paramsdic=rwfile.ReadParameterFile(filename) #,self.paramtypedic)
        # convert color 4 to 3 components
    ...
    def SetParamSetNameToComboBox(self,setname):
        self.cmbfil.SetValue(setname)
    ...
    def GetParamsFromWidgets(self):
        ...
        for prmnam, widget in list(self.paramwidgetdic.items()):
            if   widget[0] == 'TextCtrl': newparamsdic[prmnam]=widget[1].GetValue()
            elif widget[0] == 'RadioButton': newparamsdic[prmnam]=widget[1].GetValue()
            elif widget[0] == 'CheckBox': newparamsdic[prmnam]=widget[1].GetValue()
            elif widget[0] == 'Button':
                wxcolor=widget[1].GetBackgroundColour() # wx.Colour object
                # convert color data to 0-1
                color=[(float(x)/255.0) for x in wxcolor]
                try:
                    color[3]=1.0
                except: pass
                newparamsdic[prmnam]=color # wxcolor.Get() # convert RGB to 0-255
    ...
    def SetParamsToWidgets(self,paramsdic):
        ...
        for prmnam, value in list(paramsdic.items()):
            if not prmnam in self.paramwidgetdic:
                continue
            widget=self.paramwidgetdic[prmnam]
            if widget[0] == 'TextCtrl':
                widget[1].SetValue(str(value))
            elif widget[0] == 'ComboBox': widget[1].SetValue(str(value))
            elif widget[0] == 'RadioButton':
                value=CalcValue(value)
                widget[1].SetValue(value)
            elif widget[0] == 'CheckBox':
                value=CalcValue(value)
                widget[1].SetValue(value)
            elif widget[0] == 'Button':
                # convert color data to 0-255
                value=[int(x * 255.0) for x in value]
                widget[1].SetBackgroundColour(value) # wx.Colour object
            else: print(('Error: widget in SetParamsToWidget. param name=',parnam))

    def CountModifiedParams(self):
        nmody=0
        for prmnam, value in list(self.paramsdic.items()):
            if prmnam in self.newparamdic:
    ...
    def LoadInternDefault(self):
        print('LoadInternDefault')
        #self.saveas=False
        #self.paramsdic=self.DefaultParams()
        #self.cmbfil.SetValue('')
        #self.SetParamsToWidgets(self.paramsdic)

    def xxSetDefault(self):
        pname=self.cmbfil.GetStringSelection()
        if pname == '':
            mess='Please select a "Model paramvalue file".'
            wx.MessageBox(mess,'SetDefault')
    ...
    def DefaultParams(self):
        ...
        for prmnam,type in list(self.paramtypedic.items()):
            defaultdic[prmnam]=copy.deepcopy(self.setctrl.GetDefaultParam(prmnam))
            if type == 'bool':
                 if defaultdic[prmnam]: defaultdic[prmnam]='True'
    ...
    def SaveFileAs(self):
        cstdir=self.setctrl.GetDir('Customize')
        filename=lib.GetFileName(None,wcard='param (*.general)|*.general',rw='w',
                                 check=True,changedir=cstdir)
        if len(filename) <= 0:
            return

        self.newparamsdic=self.GetParamsFromWidgets()
        comm='general parameters\n'
        comm+='the elements of color list [r,g,b,a] are in the range of 0-1'
        group='general'
        rwfile.WriteParameterFile(filename,self.paramtypedic,self.newparamsdic,
                                        groupname=group,comment=comm)
        #
        setname=self.parent.AppendParamSetFile('General',filename,curset=True)
        if setname is None:
            return
...
class CustomProject():
    ...
    def CreatePanel(self):
        ...
        # project file
        #x#wx.StaticText(self.panel,-1,"Project name (Folder):",pos=(10,yloc),size=(140,18))
        #x#self.cmbfil=wx.ComboBox(self.panel,wx.ID_ANY,"",pos=(160,yloc-2),size=(200,hcb))
        #x#self.cmbfil.Bind(wx.EVT_COMBOBOX,self.OnParamFile)
        #x#self.cmbfil.Bind(wx.EVT_TEXT_ENTER,self.OnNewFile)
        yloc += 25
        # directopry name
        #if self.mode == 0: self.dirname=''
        #x#wx.StaticText(self.panel,-1,"Root dir.:",pos=(20,yloc),size=(80,18))
        #x#self.tclpath=wx.TextCtrl(self.panel,-1,self.prjdir,pos=(100,yloc-2),size=(235,18))
        #x#self.widgetobjdic[1]=self.tclpath
        #x#self.btnbrws=wx.Button(self.panel,wx.ID_ANY,"Browse",pos=(340,yloc-2),size=(55,20))
        #x#self.btnbrws.Bind(wx.EVT_BUTTON,self.OnBrowseDir)
        #x#if self.mode == 2 or self.mode >= 4:
        #x#    self.tclpath.Disable(); self.btnbrws.Disable()
        yloc += 25
        # setfile
        title='Setting file:'
        ...
        lib.SetTipString(self.btnview,'View or edit the file with editor')
        ...
        if self.prjmode == 'Setting':
            ...
            #btnok.SetToolTipString('Accept params in the panel. It is needed to push
            #                        "Save" to keep them in a file')
            lib.SetTipString(btnok,'Accept params in the panel. It is needed to push\
                             "Save" to keep them in a file')
            ...
            lib.SetTipString(btncan,'Reset params in the panel to initial values')
    ...
    def OnMode(self,event):
        ...
        #x#self.cmbfil.SetItems(self.prjlst)
        self.cmbset.SetItems(self.setfilelst)
        #x#self.cmbfil.SetValue(self.curprj)
        #x#self.SetAllItemsInWidgets(self.curprj)
        self.OnParamFile(1)
    ...
    def MakeParamSetFileDic(self):
        ...
        #x#self.pagenamlst=['Setting','General','Model','Add-on','Shortcut','Project']
        self.pagenamlst = ['General', 'Model']
        #x#self.fileext=['.py','.general','.model','.add-on','.shortcut','.project']
        self.fileext = ['.py', '.general', '.model']
        self.customdir=self.setctrl.GetDir('Customize')
        #x#self.prjdir=self.setctrl.GetDir('Projects')
        paramsetfiledic={}
        # param set files
        for i in range(len(self.pagenamlst)):
        ...
        # project files
        #x#filelst=lib.GetFilesInDirectory(self.prjdir,['.project'])
    ...
    def GetAllItemsInWidgets(self):
        ...
        prjitemlst=10*['']
        # i=1,3,4,5,6
        for i, obj in list(self.widgetobjdic.items()): prjitemlst[i]=obj.GetValue()
        prjitemlst[0]=self.curprj
        if self.curprj in self.prjdatdic: prjitemlst[2]=self.prjdatdic[self.curprj][2]
        else: prjitemlst[2]=os.path.join(prjitemlst[1],self.curprj)
        if self.curprj in self.prjdatdic: prjitemlst[3]=self.prjdatdic[self.curprj][3]
        else:
    ...
    def SetAllItemsInWidgets(self,prjnam):
        for i, obj in list(self.widgetobjdic.items()):
    ...
    def CheckDuplicate(self,newnam):
        ...
        if newnam in self.prjdatdic: dup=True
        else: dup=False
        if dup:
...
class CommonActionButtons():
    ...
    def CreatePanel(self):
        ...
        ###self.btndefi=wx.Button(panel,wx.ID_ANY,"Load internal",pos=(xloc,yloc-2),size=(100,20))
        #self.btndefi.SetToolTipString('Load default parameters')
        ###lib.SetTipString(self.btndefi,'Load intenal default parameters')
        ###self.btndefi.Bind(wx.EVT_BUTTON,self.OnInternDefault)
        ###xloc+=110
        xloc=100
        # load default button
        self.btndefa=wx.Button(panel,wx.ID_ANY,"Load default",pos=(xloc,yloc-2),size=(100,20))
        #self.btndefa.SetToolTipString('Load default parameters')
        lib.SetTipString(self.btndefa,'Load default parameters of the program')
        self.btndefa.Bind(wx.EVT_BUTTON,self.OnDefault)
        # save button
        xloc+=120
        self.btnsav=wx.Button(panel,wx.ID_ANY,"Save",pos=(xloc,yloc-2),size=(40,20))
        self.btnsav.Bind(wx.EVT_BUTTON,self.OnSaveFile)
        #self.btnsav.SetToolTipString('Write params to file')
        lib.SetTipString(self.btnsav,'Write params to file')
        # Set this to default
        ###xloc+=60
        ###self.btnset=wx.Button(panel,wx.ID_ANY,"Set default",pos=(xloc,yloc-2),size=(80,20))
        ###self.btnset.Bind(wx.EVT_BUTTON,self.OnSetDefault)
        #self.btnsav.SetToolTipString('Write params to file')
        ###lib.SetTipString(self.btnset,'Set this file to default')

        # saveas button
        #x# self.btnsavas=wx.Button(panel,wx.ID_ANY,"Save as",pos=(270,yloc-2),size=(60,20))
        #x# self.btnsavas.SetToolTipString('Save as')
        #x# self.btnsavas.Bind(wx.EVT_BUTTON,self.OnSaveFileAs)
        # cancel button
        xloc += 60
        btncan=wx.Button(panel,wx.ID_ANY,"Close",pos=(xloc,yloc-2),size=(50,20))
        #x# btncan.SetToolTipString('Making a fresh start. You can not cancel "Del","Save","Save as" operations')
        btncan.Bind(wx.EVT_BUTTON,self.OnClose)

    def OnInternDefault(self,event):
        self.pageobj.LoadInternDefault()
    ...
    def OnSetDefault(self,event):
        self.pageobj.SetDefault()

    def xxOnSaveFileAs(self,event):
        self.pageobj.SaveFileAs()

    def xxOnDelFile(self,event):
