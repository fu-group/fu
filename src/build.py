...
class RotateBond_Frm(wx.MiniFrame):
    def __init__(self,parent,id,winpos=[],winsize=[],
        ...
        lib.InsertTitleMenu(self.menubar,'[RotateBond]')
        #
    ...
    def CreatePanel(self):
        ...
        lib.SetTipString(self.tclpnts,mess)
        ...
        lib.SetTipString(btninv,'Invert movable atom group')
        yloc += 25
        ...
        lib.SetTipString(btnget,'Set selected two atoms to the rotation axis')
        # input two points
        yloc += 25
        ...
        lib.SetTipString(st2,mess)
        self.btnnext=wx.Button(self.panel,-1,">",pos=(xloc+180,yloc),
                               size=(25,hbtn))
        ...
        lib.SetTipString(self.btnnext,mess)
        self.btnprev=wx.Button(self.panel,-1,"<",pos=(xloc+145,yloc),
                               size=(25,hbtn))
        ...
        lib.SetTipString(self.btnprev,mess)
        ...
        lib.SetTipString(btnrmv,'Remove checked data from the list')
        yloc += 25
        ...
        lib.SetTipString(btnclr,'Clear the list')
        #yloc=h-hpan
        #yloc += 10; xloc=5
        ...
        lib.SetTipString(self.ckbsel,'Select movable atoms automatically')
        ...
        btnundo=wx.Button(self.panel,wx.ID_ANY,"Undo",
                          pos=(xloc2,yloc),size=(50,hbtn))
        btnundo.Bind(wx.EVT_BUTTON,self.OnUndo)
        #btnundo.SetToolTipString('Undo')
        lib.SetTipString(btnundo,'Undo')
        wx.StaticText(self.panel,-1,"Bond Rotation:",
                      pos=(xloc2+70,yloc+2),size=(95,18))
        label='Off'
        if self.buildmode: label='On'
        btnbld=wx.ToggleButton(self.panel,-1,label,pos=(xloc2+170,yloc),
                               size=(45,hbtn))
        ...
        lib.SetTipString(btnbld,'Toggle between build(On) and view(Off) mode')
    ...
    def ClearCheckBoxPanel(self):
        for name,obj in list(self.axischkboxdic.items()): obj.Destroy()
        self.axischkboxdic={}
        self.curname=''
        self.model.menuctrl.OnSelect("2-objects",True)
        self.model.menuctrl.OnSelect("Atom",True)
    ...
    def GetCheckedList(self):
        checkedlst=[]
        for name,obj in list(self.axischkboxdic.items()):
    ...
    def OnCheckAll(self,event):
        ...
        for name,obj in list(self.axischkboxdic.items()): obj.SetValue(value)

    def CheckName(self,name,on):
        if name == '': return
        if on: value=True
        else: value=False
        for pntnam,obj in list(self.axischkboxdic.items()):
...
class ModelBuilder_Frm(wx.MiniFrame):
    ...
    def __init__(self,parent,id,winpos=[],winsize=[],winlabel='ModelBuilder',
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[ModelBuilder]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
        #
        self.CreatePanel()
        #
    ...
    def CreatePanel(self):
        ...
        self.column=int(float((w-22))/40.0)
        if self.column < 1:
            self.column=1
        if self.column == 1:
            hp=70
        ...
        self.btncone=wx.Button(self,-1,"Link",pos=(xloc2,yloc2),
                               size=(45,25))
        self.btncone.Bind(wx.EVT_BUTTON,self.OnLink)
        # Reset button
        btnrset=subwin.Reset_Button(self.panel,-1,self.OnReset)
        yloc=5
            ...
            for j in range(self.column):
                ...
                if os.path.exists(bmpfile):
                    ...
                    btnobj=wx.Button(self.scrollpanel,-1,label,pos=(xloc,yloc),
                                     size=bmpsize)
                    btnobj.SetBackgroundColour('white')
...
class MutateAAResudes_Frm(wx.MiniFrame):
    def __init__(self,parent,id,winpos=[],winlabel='MutateResidue'):
        ...
        winsize=lib.WinSize((260,240),menu=True) #(235,220)
        if len(winpos) <= 0:
            pos=parent.GetPosition()
            size=parent.GetSize()
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[MutateResidue]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
        #
...
class Polypeptide_Frm(wx.Frame):
    def __init__(self,parent,id):
        ...
        winsize=lib.WinSize((270,370)) #320)
        #pos=parent.GetPosition()
        #size=parent.GetSize()
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[Polypeptide]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
        #
        self.SetParams()
        # make new mol
    ...
    def SetParams(self):
        self.chidic={"ALA":1,"ARG":1,"ASN":1,"ASP":1,"CYS":1,
                     ...
                     "ACE":0,"NME":0}
        self.resnam=list(self.chidic.keys())
        self.resnam.sort()
    ...
    def CreatePanel(self):
        ...
        #
        yloc=5
        self.rbtalp=wx.RadioButton(self.panel,-1,"alpha-helix",pos=(10,yloc),
                                   style=wx.RB_GROUP) #size=(60,18))
        self.rbtalp.Bind(wx.EVT_RADIOBUTTON,self.OnAlpha)
        self.rbtbet=wx.RadioButton(self.panel,-1,"beta-strand",pos=(105,yloc)) #,size=(60,18))
        self.rbtbet.Bind(wx.EVT_RADIOBUTTON,self.OnBeta)
        self.rbtoth=wx.RadioButton(self.panel,-1,"other",pos=(205,yloc)) #,size=(60,18))
        self.rbtoth.Bind(wx.EVT_RADIOBUTTON,self.OnOther)
        self.SetConformation()
        yloc += 22; ws=45 #40
        wx.StaticText(self.panel,-1,"phi",pos=(10,yloc),size=(25,htx))
        self.tclphi=wx.TextCtrl(self.panel,-1,self.phi,pos=(45,yloc),
                                size=(ws,htx))
        self.tclphi.Bind(wx.EVT_TEXT,self.OnAngle)
        wx.StaticText(self.panel,-1,"psi",pos=(95,yloc),size=(25,htx))
        self.tclpsi=wx.TextCtrl(self.panel,-1,self.psi,pos=(125,yloc),
                                size=(ws,htx))
        self.tclpsi.Bind(wx.EVT_TEXT,self.OnAngle)
        wx.StaticText(self.panel,-1,"omg",pos=(180,yloc),size=(30,htx))
        self.tclomg=wx.TextCtrl(self.panel,-1,self.omg,pos=(215,yloc),
                                size=(ws,htx))
        self.tclomg.Bind(wx.EVT_TEXT,self.OnAngle)
        yloc += 25
        wx.StaticText(self.panel,-1,"chi1",pos=(10,yloc),size=(25,htx))
        self.tclchi1=wx.TextCtrl(self.panel,-1,self.chi1,pos=(45,yloc),
                                 size=(ws,htx))
        self.tclchi1.Bind(wx.EVT_TEXT,self.OnAngle)
        self.tclchi1.Disable() # not supported yet
        wx.StaticText(self.panel,-1,"chi2",pos=(95,yloc),size=(25,htx))
        self.tclchi2=wx.TextCtrl(self.panel,-1,self.chi2,pos=(125,yloc),
                                 size=(ws,htx))
        self.tclchi2.Bind(wx.EVT_TEXT,self.OnAngle)
        self.tclchi2.Disable() # not supported yet
        wx.StaticText(self.panel,-1,"chi3",pos=(180,yloc),size=(30,htx))
        self.tclchi3=wx.TextCtrl(self.panel,-1,self.chi3,pos=(215,yloc),
                                 size=(ws,htx))
        self.tclchi3.Bind(wx.EVT_TEXT,self.OnAngle)
        self.tclchi3.Disable() # not supported yet
        yloc += 25
        wx.StaticText(self.panel,-1,"chi4",pos=(10,yloc),size=(25,htx))
        self.tclchi4=wx.TextCtrl(self.panel,-1,self.chi4,pos=(45,yloc),
                                 size=(ws,htx))
        self.tclchi4.Bind(wx.EVT_TEXT,self.OnAngle)
        self.tclchi4.Disable() # not supported yet
        self.SetAngle()
        #
        btncls=wx.Button(self.panel,wx.ID_ANY,"Reset chi",pos=(170,yloc-2),
                         size=(70,22))
        ...
        yloc += 10; hsiz=ysize-yloc-40
        self.tcldsp=wx.TextCtrl(self.panel,pos=(10,yloc),size=(w-20,hsiz),
                                style=wx.TE_MULTILINE)
        self.DispSequence()
        #yloc += 35
        yloc=ysize-30
        btnreset=wx.Button(self.panel,wx.ID_ANY,"New",pos=(45,yloc),
                           size=(45,22))
        btnreset.Bind(wx.EVT_BUTTON,self.OnNew)
        btnund=wx.Button(self.panel,wx.ID_ANY,"Del last",pos=(105,yloc),
                         size=(65,22))
        btnund.Bind(wx.EVT_BUTTON,self.OnDelLast)
        btncls=wx.Button(self.panel,wx.ID_ANY,"Close",pos=(185,yloc),
                         size=(50,22))
        btncls.Bind(wx.EVT_BUTTON,self.OnClose)
    ...
    def CalcResiduePhiPsiOmega(self,resnam,resnmb):
        ...
        for atom in self.model.mol.atm:
            ...
            if restmp not in resseqdic: resseqdic[restmp]=[]
    ...
    def OnDelLast(self,event):
        ...
        if len(lst) > 0:
            ...
            self.resnmb -= 1
            self.model.DrawMol(True)
            self.Message(res+":"+str(nmb)+" residue is deleted.")
            self.DispSequence()
    ...
    def SaveFile(self,savefile):
        if not os.path.exists(savefile):
            wx.MessageBox("File not found. file= "+savefile,"")
            return
    ...
    def OnMenu(self,event):
        ...
        if item == "Open sequence file" or item == "Merge sequence file":
            ...
            if len(filename) > 0:
                ...
                errmess,self.amiresseqlst=rwfile.ReadSequenceFile(filename)
                if len(errmess) > 0:
                    wx.MessageBox(errmess,'ReadSequenceFile')
                    return
        ...
        comment ='# Created by fu. DateTime='+lib.DateTimeText()+'\n'
        comment +='# res num phi,psi,omg,chi1,chi2,chi3,chi4,chiral\n\n'

        if item == "Save sequence file":
            filename=self.seqfile
            if len(filename) > 0:
                errmess=rwfile.WriteSequenceFile(filename,self.amiresseqlst,comment=comment)
                if len(errmess) > 0:
                    self.Message('Error in "WriteSequenceFile": '+errmess)
                else:
                    mess='Created sequence file='+filename
                    self.Message(mess)
            else:
        ...
        if item == "Peptide dihedral angles":
            ...
            winsize=[300,250]
            subwin.TextBox_Frm(self, winsize=winsize,title = 'phi and psi of second structures',
                               text = helptext, ok = False, cancel = False, yes = False, no = False, retmethod = None)
            return
        elif item == "Document": self.HelpDocument()
        elif item == "Tutorial": self.Tutorial()
        """
...
class TorsionEditor_Frm(wx.Frame):
    def __init__(self,parent,id,model,winpos,winlabel='Open Torsion Editor'):
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[TorsionEditor]')
        #
        self.Bind(wx.EVT_PAINT,self.OnPaint)
        self.Bind(wx.EVT_CLOSE,self.OnClose)
        self.Bind(wx.EVT_MENU,self.OnMenu)#
    ...
    def CreateZmatrixPanel(self):
        ...
        # Menu
        yloc=5
        wx.StaticText(self.panel,-1,"Mode:",pos=(10,yloc),size=(35,18))
        self.rbtmus=wx.RadioButton(self.panel,wx.ID_ANY,'mouse',pos=(60,yloc), \
                                    style=wx.RB_GROUP)
        ...
        yloc=yloc+30 #25
        # zmt points textctrl
        wx.StaticText(self.panel,-1,"p1:",pos=(10,yloc+2),size=(22,18))
        self.tclpnt1=wx.TextCtrl(self.panel,-1,'-1',pos=(40,yloc),size=(40,20))
        self.tclpnt1.Bind(wx.EVT_TEXT,self.OnP1Input)
        wx.StaticText(self.panel,-1,"p2:",pos=(90,yloc+2),size=(22,18))
        self.tclpnt2=wx.TextCtrl(self.panel,-1,'-1',pos=(120,yloc),size=(40,20))
        self.tclpnt2.Bind(wx.EVT_TEXT,self.OnP2Input)
        wx.StaticText(self.panel,-1,"p3:",pos=(170,yloc+2),size=(22,18))
        self.tclpnt3=wx.TextCtrl(self.panel,-1,'-1',pos=(200,yloc),
                                 size=(40,20))
        self.tclpnt3.Bind(wx.EVT_TEXT,self.OnP3Input)
        wx.StaticText(self.panel,-1,"p4:",pos=(250,yloc+2),size=(22,18))
        self.tclpnt4=wx.TextCtrl(self.panel,-1,'-1',pos=(280,yloc),
                                 size=(35,20))
        self.tclpnt4.Bind(wx.EVT_TEXT,self.OnP4Input)
        yloc=yloc+25
        # reset selected atoms for move
        btnsel=wx.Button(self.panel,wx.ID_ANY,"Select",pos=(20,yloc),
                         size=(60,20))
        btnsel.Bind(wx.EVT_BUTTON,self.OnResetSel)
        wx.StaticText(self.panel,-1,"selecte atoms for move",pos=(90,yloc),
                      size=(150,18))
        ...
        self.rbtlen=wx.RadioButton(self.panel,wx.ID_ANY,'length(p1-p2)',
                                   pos=(10,yloc),style=wx.RB_GROUP)
        self.rbtlen.Bind(wx.EVT_RADIOBUTTON,self.OnLength)
        self.tcllen=wx.TextCtrl(self.panel,-1,'0.0',pos=(180,yloc),
                                size=(55,20))
        #self.tcllen.Bind(wx.EVT_TEXT,self.OnLengthValue)
        self.tclleni=wx.TextCtrl(self.panel,-1,'0.01',pos=(240,yloc),
                                 size=(40,20))
        ...
        self.tclang=wx.TextCtrl(self.panel,-1,'0.0',pos=(180,yloc),
                                size=(55,20))
        #self.tclang.Bind(wx.EVT_TEXT,self.OnAngleValue)
        self.tclangi=wx.TextCtrl(self.panel,-1,'2.00',pos=(240,yloc),
                                 size=(40,20))
        ...
        self.tcltor=wx.TextCtrl(self.panel,-1,'0.0',pos=(180,yloc),
                                size=(55,20))
        #self.tcltor.Bind(wx.EVT_TEXT,self.OnTorsionValue)
        self.tcltori=wx.TextCtrl(self.panel,-1,'2.00',pos=(240,yloc),
                                 size=(40,20))
        ...
        yloc=yloc+40
        # OK, Cancel button
        btnundo=wx.Button(self.panel,wx.ID_ANY,"Undo",pos=(60,yloc),
                          size=(50,20))
        btnundo.Bind(wx.EVT_BUTTON,self.OnUndo)
        btnapl=wx.Button(self.panel,wx.ID_ANY,"Apply",pos=(130,yloc),
                         size=(50,20))
        btnapl.Bind(wx.EVT_BUTTON,self.OnApply)
        btnok=wx.Button(self.panel,wx.ID_ANY,"OK",pos=(210,yloc),size=(45,20))
    ...
    def FindMoveAtomGroup(self):
        ...
        na=1
        while na > 0:
            grplst=list(grpdic.keys())
            na=0
            for i in grplst:
                for j in condat[i]:
                    if j == self.p1: continue
                    if j == self.p2: continue
                    if j in grpdic: continue
                    else:
                        grpdic[j]=1; na += 1
        if self.p2 in grpdic: move=False
        if self.p3 in grpdic: move=False
        if self.p4 in grpdic: move=False
        grpdic[self.p1]=1
        grplst=list(grpdic.keys())
        grplst.sort()
        #
...
class ZMatrixEditor_Frm(wx.Frame):
    def __init__(self,parent,id,winpos=[],winsize=[],winlabel='ZmatrixEditor'): #winsize=[510,350]): #winsize=[300,400]):
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[ZMTEditor]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
        ...
        #
        self.MakeZMData()
        self.displst=list(range(len(self.zelm)))
        #
        self.CreatePanel()
        self.CreateGrid()
    ...
    def CreatePanel(self):
        ...
        self.panel.SetBackgroundColour('light gray')
        hbtn=25; wbtn=40; hcb=25; yup=2; up=2
        if not lib.IsWxVersion2(pltfrm='MACOSX'):
            hbtn=40; wbtn=50; hcb=25; yup=8; up=15
        # Reset button
        btnrset=subwin.Reset_Button(self.panel,-1,self.OnReset) 
        ...
        self.cmbsel=wx.ComboBox(self.panel,-1,'',choices=self.extractmenu, \
                           pos=(60,yloc-2), size=(120,hcb),style=wx.CB_READONLY)
        ...
        lib.SetTipString(self.cmbsel,mess)
        ...
        lib.SetTipString(self.tclsel,mess)
        ...
        else: self.grid=wxgrid.Grid(self.panel,-1,pos=[10,yloc],size=pansize)
        ...
        else: self.grid.Bind(wx.grid.EVT_GRID_CELL_CHANGING,self.OnGridCellChange)
        ...
        lib.SetTipString(btncmd2,'Commands for mdlwin')
        btnget=wx.BitmapButton(self.panel,-1,bitmap=self.get1bmp,
                               pos=(180,yloc-yup),size=(hbtn,hbtn))
        ...
        lib.SetTipString(self.btntry,mess)
    ...
    def GetVarNamValue(self,varnam):
        value=None
        for keynam,var in list(self.zvardic.items()):
            if var == varnam:
    ...
    def Try(self,on):
        if on:
            ...
            atmlst=list(range(len(self.trymol.atm)))
    ...
    def UpdateZmtParams(self):
        ...
        for keynam,value in list(self.changeprmdic.items()):
    ...
    def SetActiveDisp(self,idsp,prm,on,refresh=False):
        ...
        if on:
            ...
            if idsp >= 0:
                ...
                self.grid.SetCellTextColour(idsp,col,color)
            if keynam in self.activedic: del self.activedic[keynam]
    ...
    def SetActiveReverseAll(self,prm):
        ...
        for i in range(len(self.displst)):
            iatm=self.displst[i]
            keynam=str(iatm)+':'+str(prm)
            if keynam in self.activedic: on=False
            else: on=True
            self.SetActiveDisp(i,prm,on)
        self.grid.Refresh()
    ...
    def ResetAllParams(self):
        ...
        self.displst=list(range(len(self.zelm)))
        rowlst=list(range(len(self.zelm))); prmlst=[0,1,2]
    ...
    def Command1(self,cmd,label):
        ...
        if cmd == '---': pass
            ...
            self.activedic={}; rowlst=list(range(len.self.zelm))
            collst=[5,7,9]
    ...
    def Extract(self,item,text):
        ...
        text=text.strip()
        if item == 'all':
            displst=list(range(len(self.zelm)))
        elif item == 'seqnmb':
            if len(text) <= 0:
            ...
            for obj in objlst:
                ...
                for keynam,value in list(obj.items()):
                    iatm,iprm=self.UnPackKeyNam(keynam)
                    if not iatm in displst: displst.append(iatm)
        elif item == 'active':
            for keynam,value in list(self.activedic.items()):
                iatm,iprm=self.UnPackKeyNam(keynam)
                if not iatm in displst: displst.append(iatm)
    ...
    def OnReset(self,event):
        ...
        self.curmol,self.mol=self.model.molctrl.GetCurrentMol()
        self.MakeZMData()
        self.displst=list(range(len(self.model.mol.atm)))
        self.CreatePanel()
        self.CreateGrid()
        #self.MakeZMData()
        self.SetDataToGridTable()
    ...
    def CommandCol(self,cmd,label):
        ...
        if label in prmcmd:
            ...
            if cmd == 'set active all': self.SetActiveAll(prm,True)
                ...
                rowlst=list(range(len(self.zelm)))
                self.SetParamValue(rowlst,[prm],reset=True,refresh=True)
    ...
    def SetStandardLength(self,atmlst,selected=True):
        if len(atmlst) <= 0:
            ...
            else: atmlst=list(range(len(self.zelm)))
        for i in atmlst:
            j=self.zpnt[i][0]
    ...
    def ResetItem(self,item,cells='current'):
        ...
        if cells == 'current': atmlst=[self.selectedrow]
            ...
            sellst=self.ListSelectedRow()
            for i in sellst: atmlst.append(self.displst[i])
        else: atmlst=list(range(len(self.zelm))) # all
        for i in atmlst:
            if item == 'atmnam':
    ...
    def RenumberVarNam(self,insert):
        ...
        if len(self.zvardic) > 0:
            zvardic=copy.deepcopy(self.zvardic); self.zvardic={}
            zvarkeylst=list(zvardic.keys()); self.parvarlst=[]
    ...
    def RenumberChange(self,insert):
        ...
        for i in range(len(oldobjlst)):
            oldobj=oldobjlst[i]; objkeylst=list(oldobjlst[i].keys())
            newobj=newobjlst[i]
    ...
    def SetChangeFlag(self,atmlst,collst,on,select=False):
        def SetValue(obj,keynam,on):
            if on: obj[keynam]=True
                ...
                if keynam in obj: del obj[keynam]

        if len(collst) <= 0: collst=list(range(10))
        if len(atmlst) <= 0: atmlst=list(range(len(self.zelm)))
            ...
            for j in collst:
                if j in colpnt:
                    ...
                    if not self.IsIndependentVar(keynam,self.zvardic):
                        for key,varnam in list(self.zvardic.items()):
    ...
    def ResetVariableName(self,atmlst,prmlst,resetdata=True):
        ...
        """
        if len(atmlst) <= 0: atmlst=list(range(len(self.zelm)))
        if len(prmlst) <= 0: prmlst=list(range(3))
        #
        for iatm in atmlst:
            for iprm in prmlst:
                keynam=str(iatm)+':'+str(iprm)
                if keynam in self.zvardic: del self.zvardic[keynam]
    ...
    def Reorder(self,iatm,dicobj,add=True):
        ...
        for keynam,val in list(dicobj.items()):
    ...
    def SetActiveColor(self):
        ...
        for i in range(len(self.displst)):
            ...
            for j in range(3):
                ...
                col=prmcol[j]
                if keynam in self.activedic and self.activedic[keynam]:
                    color=self.activecolor
    ...
    def SetParamColor(self,refresh=False):
        ...
        for idsp in range(len(self.displst)):
            ...
            for prm in range(3):
                keynam=str(iatm)+':'+str(prm)
                col=prmcoldic[prm]
                if keynam in self.changeprmdic:
    ...
    def SetCellColor(self,rowlst,collst):
        if len(rowlst) <= 0: rowlst=list(range(len(self.displst)))
        if len(collst) <= 0: collst=list(range(10))
        colprmdic={5:0,7:1,9:2}; prmcollst=[5,7,9]
        colpntdic={4:0,6:1,8:2}; pntcollst=[4,6,8]
        for idsp in rowlst:
            ...
            for col in collst:
                ...
                if col in prmcollst: # param
                    ...
                    if self.IsIndependentVar(keynam,self.zvardic):
                        ...
                        if keynam in self.changeprmdic: chgprm=True
                        if keynam in self.activedic:
                    ...
                    ipnt=colpntdic[col]
                    keynam=str(iatm)+':'+str(ipnt)
                    if keynam in self.changepntdic:
                        color=self.changecolor
                    if self.zpnt[iatm][ipnt] < 0: color=self.inputcolor
                    self.grid.SetCellTextColour(idsp,col,color)
                else:
                    if col == 0: # atmnam
                        if iatm in self.changeatmnam:
                            color=self.changecolor
                    elif col == 1: # resdat
                        if iatm in self.changeresdat:
                            color=self.changecolor
                    elif col == 2: continue
                    elif col == 3: # elm
                        if iatm in self.changeelm: color=self.changecolor
                    self.grid.SetCellTextColour(idsp,col,color)
    ...
    def ListDependentVar(self,keynam,zvardic):
        ...
        iatm,iprm=self.UnPackKeyNam(keynam)
        if keynam not in zvardic:
            keynamlst.append(keynam)
        else:
            varnam=zvardic[keynam]
            for key,var in list(zvardic.items()):
    ...
    def IsIndependentVar(self,keynam,zvardic):
        ans=True
        if keynam not in zvardic: ans=True
        else:
            varnam=zvardic[keynam]
            iatm,iprm=self.UnPackKeyNam(keynam)
    ...
    def SetParamValue(self,rowlst,prmlst,reset=False,refresh=False):
        ...
        if len(rowlst) <= 0: rowlst=list(range(len(self.zelm)))
        if len(prmlst) <= 0: prmlst=list(range(3))
        #
        ...
        for row in rowlst:
            ...
            for iprm in prmlst:
                ...
                if reset:
                    ...
                    if keynam in self.zvardic:
                        if keynam in self.zvardicsav:
    ...
    def OnTipString(self,event):
        ...
        if label in tipdic: mess=tipdic[label]
...
class AddHydrogen_Frm(wx.Frame):
    def __init__(self,parent,id,model,winpos,scrdir='',winlabel=''):
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[AddHydrogen]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
        #
        self.Initialize()
        #
    ...
    def CreatePanel(self):
        ...
        wx.StaticText(self.panel,-1,"Keep existing hydrogens?",pos=(10,yloc),
                      size=(180,18))
        self.rbtkep=wx.RadioButton(self.panel,-1,"Yes",pos=(180,yloc),
                                   size=(50,18),
                                   style=wx.RB_GROUP)
        self.rbtrmv=wx.RadioButton(self.panel,-1,"No",pos=(240,yloc),
                                   size=(50,18))
        ...
        lib.SetTipString(self.ckbkep,'Keep existing hydrogens')
        ...
        lib.SetTipString(self.ckbmis,'List missing atoms (PDB data only)')
        ...
        self.rbtntr=wx.RadioButton(self.panel,-1,"leave as is",
                             pos=(xloc+85,yloc),size=(90,18),style=wx.RB_GROUP)
        self.rbtnh3=wx.RadioButton(self.panel,-1,"NH3+",pos=(xloc+180,yloc),
                                   size=(60,18))
        self.rbtnh2=wx.RadioButton(self.panel,-1,"NH2",pos=(xloc+250,yloc),
                                   size=(60,18))
        ...
        self.rbtctr=wx.RadioButton(self.panel,-1,"leave as is",
                             pos=(xloc+85,yloc),size=(90,18),style=wx.RB_GROUP)
        self.rbtcoo=wx.RadioButton(self.panel,-1,"COO-",pos=(xloc+180,yloc),
                                   size=(60,18))
        #self.rbtcoh=wx.RadioButton(self.panel,-1,"COH",pos=(235,yloc),size=(50,18))
        self.rbtcooh=wx.RadioButton(self.panel,-1,"COOH",pos=(xloc+250,yloc),
                                    size=(65,18))
        ...
        lib.SetTipString(self.rbthe1,'Attach at epsilon N')
        self.rbthd1=wx.RadioButton(self.panel,-1,"HID",pos=(xloc+140,yloc),
                                   size=(50,18))
        #self.rbthd1.SetToolTipString('Attach at delta N')
        lib.SetTipString(self.rbthd1,'Attach at delta N')
        self.rbthpr=wx.RadioButton(self.panel,-1,"HIP",pos=(xloc+200,yloc),
                                   size=(50,18))
        mess='Attach at delta N and epsilon N (protonated form)'
        #self.rbthpr.SetToolTipString(mess)
        lib.SetTipString(self.rbthpr,mess)
        ...
        self.rbtrstfrm=wx.CheckBox(self.panel,-1,"frame data",pos=(120,yloc),
                                   size=(95,18))
        #self.rbtrstfrm.SetToolTipString('Use frame data')
        lib.SetTipString(self.rbtrstfrm,'Use frame data')
        self.rbtrstlen=wx.CheckBox(self.panel,-1,"bond length",pos=(220,yloc),
                                   size=(100,18))
        #self.rbtrstlen.SetValue(False)
        mess='Attach hydrogens based-on bond length'
        #self.rbtrstlen.SetToolTipString(mess)
        lib.SetTipString(self.rbtrstlen,mess)
        self.rbtrstlen.SetValue(True)
        self.rbtrstfrm.SetValue(False)
    ...
    def Apply(self):
        ...
        #
        atmlst=list(range(len(self.model.mol.atm))) #self.model.ListTargetAtoms()
        ...
        if polypep:
            ...
            atmlst=list(range(len(self.model.mol.atm)))
...
class WaterBox_Frm(wx.Frame):
    def __init__(self,parent,id,winpos=[],winlabel='BoxWter'):
        ...
        winsize=lib.WinSize([310,360]) #340])
        if len(winpos) <= 0:
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[WaterBox]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
        #
        self.watnam='WAT'
    ...
    def CreatePanel(self):
        ...
        self.panel.SetBackgroundColour("light gray")
        w=self.size[0]; h=self.size[1]
        self.dispxloc = 20; self.dispyloc = 5
        self.DispSoluteSize()
        ...
        self.rbtbox1=wx.RadioButton(self.panel,-1,"cubic box length (A),",
                                    pos=(x0+25,yloc),style=wx.RB_GROUP)
        self.rbtbox1.Bind(wx.EVT_RADIOBUTTON,self.OnRadioButton)
        yloc += 20
        wx.StaticText(self.panel,-1,"x:",pos=(x0+50,yloc+2),size=(10,18))
        self.tcldx1=wx.TextCtrl(self.panel,-1,str(self.xyzbox),pos=(x0+65,yloc),
                                size=(50,htxt))
        wx.StaticText(self.panel,-1,"y:",pos=(x0+130,yloc+2),size=(10,18))
        self.tcldy1=wx.TextCtrl(self.panel,-1,str(self.xyzbox),pos=(x0+145,yloc),
                                size=(50,htxt))
        wx.StaticText(self.panel,-1,"z:",pos=(x0+210,yloc+2),size=(10,18))
        self.tcldz1=wx.TextCtrl(self.panel,-1,str(self.xyzbox),pos=(x0+225,yloc),
                                size=(50,htxt))
        yloc += 22
        self.rbtbox2=wx.RadioButton(self.panel,-1,
                    "distance between solute and box wall (A),",pos=(x0+25,yloc))
        self.rbtbox2.Bind(wx.EVT_RADIOBUTTON,self.OnRadioButton)
        yloc += 20
        wx.StaticText(self.panel,-1,"x:",pos=(x0+50,yloc+2),size=(10,18))
        self.tcldx2=wx.TextCtrl(self.panel,-1,str(self.xdis),pos=(x0+65,yloc),
                                size=(50,htxt))
        #self.tclpnt1.Bind(wx.EVT_TEXT,self.OnP1Input)
        wx.StaticText(self.panel,-1,"y:",pos=(x0+130,yloc+2),size=(10,18))
        self.tcldy2=wx.TextCtrl(self.panel,-1,str(self.ydis),pos=(x0+145,yloc),
                                size=(50,htxt))
        #self.tclpnt2.Bind(wx.EVT_TEXT,self.OnP2Input)
        wx.StaticText(self.panel,-1,"z:",pos=(x0+210,yloc+2),size=(10,18))
        self.tcldz2=wx.TextCtrl(self.panel,-1,str(self.zdis),pos=(x0+225,yloc),
                                size=(50,htxt))
        # shell thickness
        yloc += 30
        self.tclshl=wx.TextCtrl(self.panel,-1,str(self.thick),pos=(x0+210,yloc),
                                size=(50,htxt))
        yloc += 30
        wx.StaticLine(self.panel,pos=(-1,yloc),size=(w,2),style=wx.LI_HORIZONTAL)                      
        yloc += 5
        wx.StaticText(self.panel,-1,"Minimum heavy atom distance between",
                      pos=(x0+5,yloc),size=(280,18))
        yloc+=22
        wx.StaticText(self.panel,-1,"solute and water molecule (A):",
                      pos=(x0+30,yloc),size=(210,18))
        self.tcldc=wx.TextCtrl(self.panel,-1,str(self.cdis),pos=(x0+230,yloc-2),
                               size=(40,htxt))
        yloc += 25
        wx.StaticLine(self.panel,pos=(-1,yloc),size=(w,2),
                      style=wx.LI_HORIZONTAL)
        yloc += 8
        wx.StaticText(self.panel,-1,"Add ions:    Na+",pos=(x0+5,yloc),
                      size=(110,18))
        self.tclpos=wx.TextCtrl(self.panel,-1,str(self.nna),pos=(x0+120,yloc-2),
                                size=(40,htxt))
        wx.StaticText(self.panel,-1,"Cl-",pos=(x0+170,yloc),size=(20,18))
        self.tclneg=wx.TextCtrl(self.panel,-1,str(self.ncl),pos=(x0+200,yloc-2),
                                size=(40,htxt))
        yloc += 25; xloc=50
        # Apply, OK, Cancel buttons
        wx.StaticLine(self.panel,pos=(-1,yloc),size=(w,2),
                      style=wx.LI_HORIZONTAL)
        yloc += 8
        btnundo=wx.Button(self.panel,wx.ID_ANY,"Undo",pos=(x0+xloc,yloc),
                          size=(50,25))
        btnundo.Bind(wx.EVT_BUTTON,self.OnUndo)
        btnapl=wx.Button(self.panel,wx.ID_ANY,"Apply",pos=(x0+xloc+65,yloc),
                         size=(50,25))
        btnapl.Bind(wx.EVT_BUTTON,self.OnApply)
        ###btnok=wx.Button(self.panel,wx.ID_ANY,"OK",pos=(160,yloc),size=(45,20))
        ###btnok.Bind(wx.EVT_BUTTON,self.OnOK)
        btncl=wx.Button(self.panel,wx.ID_ANY,"Close",pos=(x0+xloc+130,yloc),
                        size=(50,25))
    ...
    def Apply(self):
        ...
        #
        self.MakeBoxWater()
        # add boxpnts data to mol object
        self.SetBoxPntsToMolObject()

        BusyInd(False)

    def SetBoxPntsToMolObject(self):
        """ not completed 23Aug2017"""
        if self.model.mol.boxpnts is not None:
            mess = 'The current mol object has boxpnts data.\n'
            mess += ' Would you like to overwrite?'
            dlg = wx.MessageBox(mess,'WaterBos.SetBoxPntsToMolObjetc',
                                wx.YES|wx.NO)
            if dlg == wx.NO: return
        #
        x = self.xbox
        ...
        #pntlst = [[0,0,0],[x,0,0],[0,y,0],[0,0,z],
        #          [x,y,0],[0,y,z],[x,0,z],[x,y,z]]

        pntlst = [[x0,y0,z0],[x0+x,y0,z0],[x0,y0+y,z0],[x0,y0,z0+z],
                  [x0+x,y0+y,z0],[x0,y0+y,z0+z],[x0+x,y0,z0+z],
                  [x0+x,y0+y,z0+z]]
        #
        invrot=numpy.linalg.inv(self.rot)
        ctr=[0.0,0.0,0.0]
        pnts = []
        coord = lib.RotMol(invrot,ctr,pntlst)
        for i in range(8):
            atom=self.wat.atm[i]
            px = coord[i][0]+self.com[0]
            py = coord[i][1]+self.com[1]
            pz = coord[i][2]+self.com[2]
            pnts.append([px,py,pz])

        self.model.mol.boxpnts = pnts
    ...
    def MakeBoxWater(self):
        ...
        mess=mess+" and the total number of atoms in the system = " + str(natm)
        self.model.Message(mess,0,'black')
        self.model.ConsoleMessage(mess)
        # check short contact
        # for debuggin
    ...
    def RenumberWater(self):
        ...
        nwat=maxwat
        for i in range(0,len(self.wat.atm),3):
            nwat += 1;
    ...
    def RotateWaterBox(self):
        ...
        for i in range(len(self.wat.atm)):
            atom=self.wat.atm[i]
    ...
    def MakeCavityInWater(self):
        ...
        try: # fortran code
            ...
            jatm=list(atmdic.keys()); jatm.sort(); iatm=[]
            for i in jatm:
                if i in excldic: continue
                iatm.append(i)
            nwat=len(iatm)
    ...
    def MakeWaterMolData(self):
        ...
        for i in range(0,len(self.watelmcc),3):
            k += 1
            # O
    ...
    def ExpandWaterBox(self):
        ...
        for i in range(len(self.watelmcc)):
...
class AAResiduePicker(wx.Panel):
    ...
    def CreateResButtons(self):
        ...
        x1=15; wbtn=48; x2=x1+wbtn; x3=x2+wbtn; x4=x3+wbtn; x5=x4+wbtn
        xloc=[x1,x2,x3,x4,x5]
        #
        for i in range(4):
            ii=5*i; ie=ii+5; jj=-1
            for j in range(ii,ie):
                ...
                btnres=wx.Button(self,-1,res,pos=(xloc[jj],yloc),size=(42,22)) #(38,22))                               
                btnres.Bind(wx.EVT_BUTTON,self.OnButton)
            yloc += 25
        #
        ace=self.resnamlst[20]
        btnace=wx.Button(self,-1,ace,pos=(x1,yloc),size=(42,22))
        btnace.Bind(wx.EVT_BUTTON,self.OnButton)
        #self.buttondic[btnace.Id]=ace
        nme=self.resnamlst[21]
        btnnme=wx.Button(self,wx.ID_ANY,nme,pos=(x2,yloc),size=(42,22))
        btnnme.Bind(wx.EVT_BUTTON,self.OnButton)
        #self.buttondic[btnnme.Id]=nme
        wx.StaticText(self,-1,"chirality:",pos=(120,yloc+6),size=(50,20))
        self.rbtlll=wx.RadioButton(self,-1,"L",pos=(180,yloc+6),
                                   style=wx.RB_GROUP) #size=(60,18))
        self.rbtlll.Bind(wx.EVT_RADIOBUTTON,self.OnLChirality)
        self.rbtddd=wx.RadioButton(self,-1,"D",pos=(210,yloc+6)) #,size=(60,18))
...
class RigidBodyOptimizer_Frm(wx.Frame):
    def __init__(self,parent,id,model,winpos=[]):
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[RigidBodyOpt]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
        #
    ...
    def CreatePanel(self):
        ...
        lib.SetTipString(self.rbtvarrot,mess)
        ...
        lib.SetTipString(strms,'Convergence criteria in kcal per Angstrom')
    ...
    def OnSetBond(self,event):
        if self.setbond:
            ...
            grpnam='AllAtoms'
            self.grplst.append(grpnam)
            self.grpatmlstdic[grpnam]=list(range(len(self.model.mol.atm)))
    ...
    def OnvdWParam(self,event):
        ...
        if value == 'Internal': self.vdwprmdic=const.LJParams
            ...
            filename=lib.GetFileName(wcard=wcard,rw='r',message=mess)
            if len(filename) <= 0: return
    ...
    def OnAtomCharge(self,event):
        ...
        if value == 'None': pass
            ...
            self.atmchglst=self.ComputeQEqCharge()
        elif value == 'File':
    ...
    def ComputeQEqCharge(self):
        ...
        if value == 'None':
            ...
            for i,lst in list(self.discongrpdic.items()):
...
class VariableDefinition_Frm(wx.Frame):
    def __init__(self,parent,id,model,winpos=[]):
        ...
        lib.InsertTitleMenu(self.menubar,'[VarDefWin]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
    ...
    def CreateRotAxisPanel(self,yloc,height):
        ...
        lib.SetTipString(self.rbtallrotopt,mess)
    ...
    def CreateGroupPanel(self,yloc,height):
        ...
        lib.SetTipString(self.rbtallgrpopt,mess)
...
class SetModelingGroup_Frm(wx.Frame):
    def __init__(self,parent,id,model,winpos=[]):
        ...
        menud=self.MenuItems()
        self.SetMenuBar(menud) # self.menubar.menuitem)
        lib.InsertTitleMenu(self.menubar,'[SetModelingGroup]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
    ...
    def CreatePanel(self):
        ...
        lib.SetTipString(btnedit,'Set rotatable bonds to the list. Takes time for large molecule!')

        btnaply=wx.Button(self.panel,-1,"Apply",pos=(xloc+70,yloc),
                          size=(60,hbtn))
        btnaply.Bind(wx.EVT_BUTTON,self.OnApply)
        #btnaply.SetToolTipString('Set rotatable bonds to the list. Takes time for large molecule!')
        lib.SetTipString(btnaply,'Set rotatable bonds to the list. Takes time for large molecule!')
        btncls=wx.Button(self.panel,-1,"Close",pos=(xloc+150,yloc),
                         size=(60,hbtn))
        btncls.Bind(wx.EVT_BUTTON,self.OnClose)
...
class NonBondIntEnergy(wx.Object):
    ...
    def Initialize(self):
        ...
        # temp mol object
        self.opt=self.model.mol.CopyMolecule(self.model)
        ...
        for grpnam in self.grplst:
            ...
            if grpnam in self.grpvardic:
                if self.grpvardic[grpnam][0]: # center-of-mass
                    ...
                    const.CONSOLEMESSAGE('grpnam='+str(list(self.grpatmlstdic.keys())))
                    self.activeatmlst=\
    ...
    def SetvdWParams(self,ljprmdic,vdwgeom=True):
        ...
        for elm, prm in list(ljprmdic.items()):
            f=twosq6
...
class SplitAndJoinPDBFile_Frm(wx.Frame):
    def __init__(self,parent,id,winpos=[],winlabel='SplitJoin'): #,model,ctrlflag,molnam):
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[Split/JoinPDF]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
        #
    ...
    def CreateSplitPanel(self,yloc):
        ...
        lib.SetTipString(self.rbtter,'split at TER')
        ...
        lib.SetTipString(self.rbtcha,'split into chains')
        yloc += 20
        ...
        lib.SetTipString(self.rbtwat,'split into waters and the rest')
        ...
        lib.SetTipString(self.rbtpcw,mess)
        ...
        lib.SetTipString(savtxt,'Directoy to save PDB files')
...
class RMSFitting_Frm(wx.Frame):
    def __init__(self,parent,winpos=[],winsize=[]): #,model,ctrlflag,molnam,winpos):
        self.title='RMS Fitting'
        if len(winpos) <= 0:
            parpos=parent.GetPosition(); parsize=parent.GetSize()
            winpos=[parpos[0]+parsize[0],parpos[1]+40]
        winpos  = lib.SetWinPos(winpos)
        if len(winsize) <= 0: winsize=lib.WinSize((350,270)) #270) # 335)
        self.winsize=winsize
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[RMSFit]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
        #
        ans=lib.IsMoleculeObj(self.model.mol)
        if not ans:
    ...
    def CreatePanel(self):
        ...
        self.ckbresnam=wx.CheckBox(self.panel,-1,"resnam",pos=(40,yloc),
                                   size=(80,18))
        self.ckbresnmb=wx.CheckBox(self.panel,-1,"resnmb",pos=(130,yloc),
                                   size=(80,18))
        self.ckbchainnam=wx.CheckBox(self.panel,-1,"chainnam",pos=(230,yloc),
                                     size=(100,18))
        self.SetNameOptions()
        yloc += 20; tclh=45
        ...
        self.rbtcg=wx.RadioButton(self.panel,-1,"CG",pos=(110,yloc),
                                  size=(45,18),style=wx.RB_GROUP)
        self.rbtpow=wx.RadioButton(self.panel,-1,"Powell",pos=(160,yloc),
                                   size=(60,18))
        self.rbtpow.SetValue(True)
        self.btnfitundo=wx.Button(self.panel,wx.ID_ANY,"Undo",pos=(245,yloc),
                                  size=(50,hbt))
        self.btnfitundo.Bind(wx.EVT_BUTTON,self.OnRMSFitUndo)
        self.btnfitdo=wx.Button(self.panel,wx.ID_ANY,"Fit",pos=(310,yloc),
                                size=(35,hbt))
        ...
        wx.StaticText(self.panel,-1,"3)Show atoms displaced larger than:",
                      pos=(10,yloc),size=(240,18))
        #self.rbtrms.Bind(wx.EVT_RADIOBUTTON,self.OnRBTNonMatching)
        self.tclval=wx.TextCtrl(self.panel,-1,str(self.rmsdval),pos=(250,yloc),
                                size=(40,22)) # size x 28 <-30
        btnshwapl=wx.Button(self.panel,wx.ID_ANY,"Plot",pos=(300,yloc),
                            size=(45,hbt))
    ...
    def ChangeBondThickness(self):
        for atom in self.model.mol.atm:
            if atom.elm == "XX": continue
            if atom.grpnam == "mgr2": atom.thick=self.refbndthic
    ...
    def MakeCase2AtomNameDic(self,sellst,fullname):
        ...
        for i in sellst:
            ...
            if atom.grpnam == "mgr2": self.refdic[name]=i
        ...
        for i in range(len(self.model.mol.atm)):
            if fullname: name=self.model.mol.MakeFullAtomName(i,True,True,True)
                                                       ...
                                                       self.resnam,self.resnmb)
            if name in self.trgdic and name in self.refdic:
                self.matdic[name]=True

        self.difdic0={}
        for i in self.trgdic:
            if i not in self.refdic: self.difdic0[i]=self.trgdic[i]

        self.difdic1={}
        for i in self.refdic:
            if i not in self.trgdic: self.difdic1[i]=self.refdic[i]
    ...
    def MakeCase1AtomNameDic(self,selected):
        ...
        for i in sellst:
            ...
            if atom.grpnam == "mgr2": self.refdic[i-self.ntrg]=i

        self.matdic={}
        for i in range(len(self.model.mol.atm)):
            atom=self.model.mol.atm[i]
            ii=i
            if atom.grpnam == "mgr2": ii=i-self.ntrg
            if ii in self.trgdic and ii in self.refdic:
                self.matdic[ii]=True
    ...
    def ListMatchingAtoms(self):
        ...
        if self.case == 0: # selected order
            ...
            for i in sellst:
                ...
                if atom.grpnam == "base":
                    if i+self.ntrg in self.refdic:
                        self.matchlst0.append(i)
                    else: self.nonmatchlst0.append(i)
                if atom.grpnam == "mgr2":
                    if i-self.ntrg in self.trgdic:
    ...
    def OnDelReference(self,event):
        ...
        dellst=[]
        for i in range(len(self.model.mol.atm)):
            if self.model.mol.atm[i].grpnam == "mgr2": dellst.append(i)
        if len(dellst) > 0:
            self.model.mol.DelAtoms(dellst)
            self.model.DrawMol(True)
...
class TransferAtoms_Frm(RMSFitting_Frm):
    def __init__(self,parent,id,winpos=[]): #,model,ctrlflag,molnam,winpos):
        ...
        #self.rmsfit=False
        self.select=False
        self.savmol=self.model.mol.CopyMolecule(self.model)
        #
        self.rmswin=None
        self.CreateButtons()
    ...
    def CreateButtons(self):
        ...
        wx.StaticText(self.panel,-1,"4)Select atoms:",pos=(10,yloc),
                      size=(100,18))
        self.rbtnon=wx.RadioButton(self.panel,-1,"nonmatched",pos=(100,yloc),
                                   size=(115,18),style=wx.RB_GROUP)
        self.rbtmat=wx.RadioButton(self.panel,-1,"matched",pos=(205,yloc),
                                   size=(80,18))
        self.rbtnon.SetValue(True)
        btnselapl=wx.Button(self.panel,-1,"Apply",pos=(300,yloc),
                            size=(50,hbt))
    ...
    def Transfer(self):
        ...
        savmol=self.model.mol.CopyMolecule(self.model)
    ...
    def TransferAtoms(self,r2t):
        ...
        nref=0
        for i in sellst:
            if self.model.mol.atm[i].grpnam == "mgr2": nref += 1
        if nref <= 0:
    ...
    def TransferAtomsToTarget(self,newmol,sellst):
        ...
        for i in sellst:
            ...
            name=self.model.mol.MakeFullAtomName(i,self.rmswin.chainnam,
                                         self.rmswin.resnam,self.rmswin.resnmb)
            if name in self.rmswin.trgdic: continue
            res=self.model.mol.atm[i].resnmb
            atm=self.model.mol.atm[i].atmnmb
            self.totdic0[name]=[i,res,atm]
        #
        ...
        for i in trgitem:
            ...
            if name in self.rmswin.trgdic:
                ...
                natm0 += 1
            elif name in self.rmswin.refdic:
    ...
    def TransferAtomsToReference(self,newmol,sellst):
        ...
        self.totdic0={}; self.seqdic={}
        for i in range(len(self.model.mol.atm)):
            if self.model.mol.atm[i].grpnam != "mgr2": continue
            res=self.model.mol.atm[i].resnmb
            atm=self.model.mol.atm[i].atmnmb
            name=self.model.mol.MakeFullAtomName(i,self.rmswin.chainnam,
                                     self.rmswin.resnam,self.rmswin.resnmb)
        ...
        for i in sellst:
            ...
            name=self.model.mol.MakeFullAtomName(i,self.rmswin.chainnam,
                                     self.rmswin.resnam,self.rmswin.resnmb)
            if name in self.rmswin.refdic: continue
            res=self.model.mol.atm[i].resnmb
            atm=self.model.mol.atm[i].atmnmb
            self.totdic0[name]=[i,res,atm]
        ...
        for i in refitem:
            ...
            if name in self.rmswin.refdic:
                ...
                newmol.atm.append(atom)
                natm1 += 1
            elif name in self.rmswin.trgdic:
                jj=self.totdic0[name][0] #self.refdic[i]
                atomj=self.model.mol.atm[jj]
                if atomj.select:
                    self.rmswin.seqnew2old.append(-1)
...
def TruncateProtein(cap=0,model=None):
    ...
    def KeepResidues(fum,dellst):
        ...
        if len(dellst) > 0:
            keepres=[]; conresdat=[]
            parmol=fum.mol.CopyMolecule(model)
            #if cap == 0:
...
class TruncateProtein_Frm(wx.MiniFrame):
    ...
    def __init__(self,parent,id,winpos=[],savfile=None):
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[TrancateProtein]')
        #
        scrdir='H://Test'
        #
        self.Initialize()
    ...
    def CreatePanel(self):
        ...
        lib.SetTipString(st,'Leave empty to appliy default name')
...
def BuildMolFrom2D(molobj,parent):
    """ Build 3D model form 2D (e.g., mol file) """
    def FindNext(done,molobj):
        next=-1
        for i in range(len(molobj.atm)):
            for j in molobj.atm[i].conect:
                if i in done and not j in done:
                    next=i; break
        return next

    if len(molobj.atm) <= 0:
        return
    oldmol=molobj.CopyMolecule(parent)
    done=[]; dcc=numpy.array(3*[0.0])
    natm=len(molobj.atm)
    i=0; done.append(0);
    while i >= 0:
        #if not i in done:
        #    i=FindNext(done,natm)
        #    continue
        atomi=molobj.atm[i]
        elmi=atomi.elm
        coni=atomi.conect
        cci=atomi.cc[:]; oldcc=oldmol.atm[i].cc[:]
        for j in coni:
            if j in done: continue
            atomj=molobj.atm[j]
            elmj=atomj.elm
            ccj=oldmol.atm[j].cc[:]
            bl=const.ElmCov[elmi]+const.ElmCov[elmj]
            cc=lib.ChangeLength(oldcc,ccj,bl)
            for k in range(3): cc[k] += (cci[k]-oldcc[k])

            atomj.cc=cc[:]
            done.append(j)
        i=FindNext(done,molobj)

class CrystalBuilder_Frm(wx.Frame):
    def __init__(self,parent):
        self.prgnam = 'fu-crystal.py'
        self.title = 'Crystal builder'
        try:
        ...
        winpos  = lib.SetWinPos(winpos)
        winsize = lib.WinSize([300,330],False) #295)) #((275,355))
        wx.Frame.__init__(self,parent,-1,self.title,pos=winpos,size=winsize,
               style=wx.SYSTEM_MENU|wx.CAPTION|wx.CLOSE_BOX)
        #
        self.model  = parent
        self.mdlwin = self.model.mdlwin
        self.winlabel = 'CrystalBuilder'
        ...
        self.notebook = wx.Notebook(self,-1,pos=[0,0],size=winsize)

        self.curpage = 0
        self.molobj  = self.model.mol
    ...
    def GetCurrentMol(self):
        #molidx,molobj = self.model.molctrl.GetCurrentMol()
        molname,molobj = self.model.GetMol()
        return molobj

    def CreatePagePanels(self):
        self.dispwin  = CrystalDisplay_Frm(self.notebook,self) #wx.Panel(self.notebook,-1)
        self.paramwin = CrystalParams_Frm(self.notebook,self) #wx.Panel(self.notebook,-1)
        self.notebook.InsertPage(0,self.dispwin,'   Display   ')
        self.notebook.InsertPage(1,self.paramwin,' Cell params ')
        #
        self.dispwin.PageOpened()

    def OnPageChanged(self,event):
        self.curpage = self.notebook.GetSelection()
        if self.curpage == 1: self.paramwin.PageOpened()# Cell Params page
        else:                 self.dispwin.PageOpened()# Cell Display page
    ...
    def OnNotify(self,event):
        ...
        if item == 'SwitchMol' or item == 'OpenFiles':
            try: self.paramwin.Notify()
            except: pass
            try: self.dispwin.Notify()
            except: pass
            mess = 'CrystalBuilder: Current molecule is reset'
            self.Message(mess)

    def Message(self,mess):
        self.model.ConsoleMessage(mess)

class CrystalParams_Frm(wx.Panel):
    """ Subpanel of CrystalBuilder_Frm"""
    def __init__(self,parent,crystal):
        self.prgnam = 'fu-lattice.py'
        self.title = 'Lattice params'
        self.winlabel = 'CrystalBuilder'

        winpos=[-1,-1] #((275,355))
        winsize = parent.GetClientSize()

        wx.Panel.__init__(self,parent,-1,pos=winpos,size=winsize)
         #

        self.crystal = crystal
        self.model = self.crystal.model

        self.paramtclobjs = []

        self.todeg = 180.0 / numpy.pi
        self.torad = 1.0 / self.todeg
    ...
    def CreatePanel(self):
        ...
        htx=20
        yloc = 10
        wx.StaticText(self.panel,-1,"Mol. name:",pos=(10,yloc),size=(70,htx))
        self.tclmol = wx.TextCtrl(self.panel,-1,'',pos=(85,yloc),
                                  size=(w-130,htx))
        subwin.Reset_Button(self.panel,-1,retmethod=self.OnResetMol,
                                         winpos=[w-42,yloc-2])

        yloc += 30
        title = 'Cell constants(Angstroms and degrees);'
        wx.StaticText(self.panel,-1,title,pos=(10,yloc),size=(300,htx))
        yloc += 20
        label = [['   a ','   b ','   c '],[' alpha',' beta','gamma']]
        for i in range(2):
            xpos = 40
            for j in range(3):
                wx.StaticText(self.panel,-1,label[i][j],pos=(xpos,yloc),
                             size=(60,htx))
                xpos += 80
            yloc += 20
            xpos = 30
            for i in range(3):
                tclparams=wx.TextCtrl(self.panel,-1,'',pos=(xpos,yloc),
                                      size=(65,20),style=wx.TE_PROCESS_ENTER)
                self.paramtclobjs.append(tclparams)
                xpos += 80
            yloc += 25
        title = "Symmetry equivalent positions as xyz;"
        wx.StaticText(self.panel,-1,title,pos=(10,yloc),size=(300,18))
        yloc += 20
        if lib.GetPlatform() == 'MACOSX' and wx.version()[0:3] != '2.8':
            hgt=h-yloc-40
        else: hgt=h-yloc-75
        self.tclsym = wx.TextCtrl(self.panel,-1,"",pos=(30,yloc),
                              size=(w-55,hgt),style=wx.TE_MULTILINE)
        # buttons
        yloc = h - 75 #70 #65
        if lib.GetPlatform() == 'MACOSX' and wx.version()[0:3] != '2.8':
            yloc = h - 35
        ...
        yloc += 8
        xloc = 45
        btcan = wx.Button(self.panel,-1,"Cancel",pos=(xloc,yloc),size=(55,22))
        btcan.Bind(wx.EVT_BUTTON,self.OnCancel)
        btok = wx.Button(self.panel,-1,"Apply",pos=(xloc+75,yloc),size=(50,22))
        btok.Bind(wx.EVT_BUTTON,self.OnApply)
        btcls = wx.Button(self.panel,-1,"Close",pos=(xloc+150,yloc),size=(50,22))
        btcls.Bind(wx.EVT_BUTTON,self.OnClose)


    def PageOpened(self):
        self.ResetCellParams()
        self.ResetSymEquPos()
        self.tclmol.SetValue('')

        molobj = self.crystal.GetCurrentMol()
        # set mol name
        self.tclmol.SetValue(molobj.name)

        try:
            if self.crystal.molobj.crystalobj is not None:
                cell_params,sym_equ_pos = self.GetCellParamsInModel()
                if len(cell_params) > 0:
                    self.SetCellParams(cell_params)
                else: self.ResetCellParams()
                if len(sym_equ_pos)   > 0:
                    self.SetSymEquPos(sym_equ_pos)
                else: self.ResetSymEquPos()
        except:
            self.ResetCellParams()
            self.ResetSymEquPos()

    def GetCellParamsInModel(self):
        cell_params = []
        try:
             crysobj = self.crystal.molobj.crystalobj
             cell_params = crysobj.GetCellParams()
             symequpos   = crysobj.sym_equ_pos
        except: self.ResetCellParams()
        return cell_params,symequpos

    def SetCellParams(self,cell_params):
        ff10 = '%10.3f'
        params = cell_params[:]
        if len(params) <= 0: self.ResetCellParams()
        else:
            for i in range(3,6): params[i] *= self.todeg
            value = []
            for i in range(6): value.append(ff10 % params[i])
            i = -1
            for tcl in self.paramtclobjs:
                i += 1
                tcl.SetValue(value[i]) #(str(cell_params[i])) #ff10 % cell_params[i])

    def ResetCellParams(self):
        vallst = ['10.0','10.0','10.0','90.0','90.0','90.0']
        for i in range(6):
            self.paramtclobjs[i].SetValue(vallst[i])

    def SetSymEquPos(self,sym_equ_pos):
        text = ''
        if len(sym_equ_pos) <= 0: self.tclsym.SetValue("'x,y,z'")
        else:
            for s in sym_equ_pos: text += "'" + s +"', "
            text = text[:-2]
            self.tclsym.SetValue(text)

    def ResetSymEquPos(self):
        text = "'x,y,z'"
        self.tclsym.SetValue(text)
    ...
    def OnApply(self,event):
        ...
        if molobj.crystalobj is not None:
            mess  = 'Current molecule has crystal data.\n'
            mess += ' Would you like to overwrite it?'
            ans = lib.MessageBoxYesNo(mess,'fu_crystal.Params_From')
            if not ans: return


        for i in range(6): param_string += self.paramtclobjs[i].GetValue() +', '
        param_string = param_string[:-2]
        sym_string   = self.tclsym.GetValue()
        # make crystal object
        crystalobj = molec.Crystal(self.model,ciffile=None,
                                        messout1=self.model.ConsoleMessage)
        crystalobj.SetCellParams(param_string)
        crystalobj.SetSymEquPos(sym_string)
        #
        umol_xyz = []
        conlst   = []
        typlst   = []
        for atom in molobj.atm:
            umol_xyz.append([atom.atmnam,atom.elm,
                             atom.cc[0],atom.cc[1],atom.cc[2]])
            conlst.append(atom.conect)
            typlst.append(atom.bndmulti)
        #
        crystalobj.ComputeAttribs(unitmol=False)
        fract_coords = crystalobj.XYZCoordsToFracts(umol_xyz)
        crystalobj.SetFractCoords(fract_coords)

        crystalobj.unit_molecule = crystalobj.fract_coords #crystalobj.GenUnitMolecule()
        crystalobj.natm_unit_mol  = len(crystalobj.unit_molecule)

        umol_bonds = lib.BondListFromConAndTypList(conlst,typlst)
        crystalobj.SetUnitMolBonds(umol_bonds)

        crystalobj.inpfile = 'created'
        crystalobj.chem_name_com = molobj.name
        #
        molobj.crystalobj = crystalobj
        molobj.boxpnts = crystalobj.box_vertexes
        #
        mess = 'Crystal object has created in the current molecule = '
        mess += molobj.name
        self.model.Message2(mess)
        #mess  = '   cell_params = ' + param_string
        #mess += ', sym_equ_pos = ' + sym_string
        #self.model.ConsoleMessage(mess)

    def OnInputParams(self,event):
        for i in range(6):
            self.latticeconsts[i] = self.paramtclobjs[i].GetValue()

    def OnInputSymEquPos(self,event):
        obj = GetEventObject()
        value = obj.GetValue()

    def SetZValue(self,value):
        self.tclz.SetValue(value)

    def OnInputZ(self,event):
        obj=GetEventObject()
        value=obj.GetValue()
    ...
    def OnClose(self,event):
        try: self.model.winctrl.Close(self.winlabel) #Win('TreeSelWin')
        except: pass

        self.crystal.Destroy()

    def OnResetMol(self,event):
        """ Get current molecule object from fumodel """
        #name,self.crystal.molobj = self.model.GetMol()
        self.PageOpened()
...
class CrystalDisplay_Frm(wx.Panel):
    """ Subpanel of CrystalBuilder_Frm"""
    def __init__(self,parent,crystal):
        self.prgnam = 'fu-lattice.py'
        self.title = 'Display Lattice'
        self.winlabel = 'CrystalBuilder'
        ...
        self.messmethod = self.model.ConsoleMessage
        self.viewrbobj  = [None,None,None]

        self.boxpnts = None
        self.in_unit_cell = True
    ...
    def CreatePanel(self):
        ...
        htx=20
        yloc = 10
        wx.StaticText(self.panel,-1,"Mol. name:",pos=(10,yloc),size=(80,htx))
        self.tclmol = wx.TextCtrl(self.panel,-1,'',pos=(85,yloc),
                                  size=(w-130,htx))
        tips = 'Restore molecules in MdlWin'
        subwin.Reset_Button(self.panel,-1,retmethod=self.OnResetMol,
                                         winpos=[w-42,yloc-2],tips=tips)

        yloc += 25
        wx.StaticText(self.panel,-1,"Display molecules:",
                      pos=(10,yloc),size=(150,htx))
        self.btaply = wx.Button(self.panel,-1,"Apply",pos=(210,yloc+35),
                           size=(50,22))
        self.btaply.Bind(wx.EVT_BUTTON,self.OnApply)
        #self.btaply.SetToolTipString('Push to display molecules')
        lib.SetTipString(self.btaply,'Push to display molecules')

        yloc += 25
        self.rbtdisp1 = wx.RadioButton(self.panel,-1,'in unit cell',
                                    pos=(20,yloc),style=wx.RB_GROUP)
        self.rbtdisp1.Bind(wx.EVT_RADIOBUTTON,self.OnMolInCell)
        yloc += 25
        self.rbtdisp2 = wx.RadioButton(self.panel,-1,
                                       "in klm cells and on surface",
                                       pos=(20,yloc))
        self.rbtdisp2.Bind(wx.EVT_RADIOBUTTON,self.OnMolInCell)

        yloc += 20
        xpos  = 60
        wx.StaticText(self.panel,-1,'   k              l            m',
                      pos=(xpos,yloc),size=(160,htx))
        yloc += 20
        ylocz = yloc
        xpos  = 60
        self.cellindex = 0
        self.scobj = []
        for i in range(3):
            scklm = wx.SpinCtrl(self.panel,-1,value=str(self.cellindex),
                                      pos=(xpos,yloc-2),size=(45,22),
                                      style=wx.SP_ARROW_KEYS,min=0,max=9)
            self.scobj.append(scklm)
            xpos += 60

        self.DispRadioButton()

        yloc += 35
        wx.StaticText(self.panel,-1,"Display cell box:",pos=(10,yloc),
                      size=(110,htx))

        self.rbtbox2 = wx.RadioButton(self.panel,-1,'on',
                                    pos=(115,yloc),style=wx.RB_GROUP)
        self.rbtbox1 = wx.RadioButton(self.panel,-1,'off',pos=(170,yloc))
        self.rbtbox3 = wx.RadioButton(self.panel,-1,'as is',pos=(230,yloc))
        self.rbtbox1.Bind(wx.EVT_RADIOBUTTON,self.OnDispBoxOff)
        self.rbtbox2.Bind(wx.EVT_RADIOBUTTON,self.OnDispBoxOn)
        #
        yloc += 25
        wx.StaticText(self.panel,-1,"View along axis:",pos=(10,yloc),
                      size=(105,htx))
        self.viewrbobj[0] = self.rbtbox1=wx.RadioButton(self.panel,-1,'a',
                                    pos=(115,yloc),style=wx.RB_GROUP)
        self.viewrbobj[1] = wx.RadioButton(self.panel,-1,'b',pos=(150,yloc))
        self.viewrbobj[2] = wx.RadioButton(self.panel,-1,'c',pos=(190,yloc))
        self.viewasitis   = wx.RadioButton(self.panel,-1,'as is',pos=(230,yloc))
        for i in range(3):
            self.viewrbobj[i].Bind(wx.EVT_RADIOBUTTON,self.OnViewAxis)
        yloc += 25
        wx.StaticText(self.panel,-1,"Remake bonds:",pos=(10,yloc),
                      size=(100,htx))
        btndo = wx.Button(self.panel,wx.ID_ANY,"Do",pos=(120,yloc),size=(50,22))
        btndo.Bind(wx.EVT_BUTTON,self.OnRemakeBonds)
        btnundo = wx.Button(self.panel,wx.ID_ANY,"Undo",pos=(190,yloc),
                            size=(50,22))
        btnundo.Bind(wx.EVT_BUTTON,self.OnUndoRemakeBonds)
        ...
        btdel = wx.Button(self.panel,-1,"Del dispalyed mol",pos=(xloc,yloc),
                          size=(130,22))
        btdel.Bind(wx.EVT_BUTTON,self.OnDelMol)
        #btdel.SetToolTipString('Delete generated molecules')
        lib.SetTipString(btdel,'Delete generated molecules')

        btcls = wx.Button(self.panel,-1,"Close",pos=(xloc+150,yloc),size=(50,22))
        btcls.Bind(wx.EVT_BUTTON,self.OnClose)


    def OnRemakeBonds(self,event):
        self.model.SaveBonds()
        self.model.DeleteBonds('all',drw=False)
        self.model.AddBondUseBondLength(drw=True)

    def OnUndoRemakeBonds(self,event):
        self.model.RecoverBonds()

    def OnMolInCell(self,event):
        obj   = event.GetEventObject()
        label = obj.GetLabel()
        if label == 'in unit cell':
              self.in_unit_cell = True
        else: self.in_unit_cell = False
        self.DispRadioButton()

    def DispRadioButton(self):
        if self.in_unit_cell:
            self.rbtdisp1.SetValue(True)
            for i in range(3): self.scobj[i].Disable()
        else:
            self.rbtdisp2.SetValue(True)
            for i in range(3): self.scobj[i].Enable()

    def OnViewAxis(self,event):
        obj = event.GetEventObject()
        label = obj.GetLabel()
        self.ViewAlongAxis(label)
        self.viewasitis.SetValue(True)

    def ViewAlongAxis(self,axis):
        """ View along cell axis, a,b, or c

        :param str axis: 'a','b', or 'c'
        """

        if self.boxpnts is None:
        ...
        pb = self.boxpnts[2]
        pc = self.boxpnts[3]
        if   axis == 'a': vec = numpy.subtract(pa,po)
        elif axis == 'b': vec = numpy.subtract(pb,po)
        elif axis == 'c': vec = numpy.subtract(pc,po)
        ez = numpy.array([0.0,0.0,1.0])
        ax = numpy.cross(vec[:],ez[:])
        uz = lib.TrnMat(ax,ez,vec)
        #
        self.model.ResetOrientation(drw=False)
        self.model.RotateMolByMatrix(uz)

    def OnDispBoxOn(self,event):
        self.model.DrawCellBox(True,drw=True)
        self.rbtbox3.SetValue(True)

    def OnDispBoxOff(self,event):
        self.model.DrawCellBox(False,drw=True)
        self.rbtbox3.SetValue(True)

    def OnApply(self,event):
        if self.in_unit_cell:
            self.model.PackCells(klm='unit')
        else:
            klm = [0, 0, 0]
            for i in range(3): klm[i] = int(self.scobj[i].GetValue())
            self.model.PackCells(klm=klm)
    ...
    def PageOpened(self):
        ...
        self.tclmol.SetValue(molobj.name)
        #
        try: self.boxpnts = molobj.GetBoxPnts()
        except: pass
...
class HisFormChanger_Frm(subwin.OneByOneViewer_Frm):
    def __init__(self,parent,id,winpos=[],winsize=[],
                 winlabel='HisFormChanger'):
        if len(winsize) <= 0:  winsize=lib.WinSize([260,370],True)
        ...
        oneviewer=subwin.OneByOneViewer_Frm(self.model,id,winpos,winsize,menu=False)
        ...
        # icon
        try: lib.AttachIcon(self.oneviewer,const.FUMODELICON)
        except: pass
        ...
        wx.StaticLine(oneviewer.panel,pos=(0,yloc),size=(w,4),
                      style=wx.LI_HORIZONTAL)
        yloc += 10
        wx.StaticText(oneviewer.panel,-1,"Change form:",pos=(5,yloc),size=(80,18))
        self.rbtcur=wx.RadioButton(oneviewer.panel,-1,'current',pos=(100,yloc), \
                               style=wx.RB_GROUP)
        self.rbtall=wx.RadioButton(oneviewer.panel,-1,'all',pos=(170,yloc))
        yloc += 20
        #btnundo=wx.Button(oneviewer.panel,-1,"Undo",pos=(15,yloc),size=(50,25))
        #btnundo.Bind(wx.EVT_BUTTON,self.OnUndo)
        #btnundo.SetToolTipString('Undo once')
        #lib.SetTipString(btnundo,'Undo once')
        xloc=15
        btnauto=wx.Button(oneviewer.panel,-1,"Auto",pos=(xloc,yloc),
                          size=(45,25))
        btnauto.Bind(wx.EVT_BUTTON,self.OnAuto)
        mess='Determine protonation state based on possible hydrogen bonds'
        #btnauto.SetToolTipString(mess)
        lib.SetTipString(btnauto,mess)
        btnhie=wx.Button(oneviewer.panel,-1,"HIE",pos=(xloc+50,yloc),
                         size=(45,25))
        btnhie.Bind(wx.EVT_BUTTON,self.OnHIE)
        btnhid=wx.Button(oneviewer.panel,-1,"HID",pos=(xloc+100,yloc),
                         size=(45,25))
        btnhid.Bind(wx.EVT_BUTTON,self.OnHID)
        btnhip=wx.Button(oneviewer.panel,-1,"HIP",pos=(xloc+150,yloc),
                         size=(45,25))
        btnhip.Bind(wx.EVT_BUTTON,self.OnHIP)
        #yloc+=30
        btnauto=wx.Button(oneviewer.panel,-1,"C<->N",pos=(xloc+200,yloc),
                          size=(55,25))
        btnauto.Bind(wx.EVT_BUTTON,self.OnChangeCN)
        yloc+=35
        wx.StaticLine(oneviewer.panel,pos=(0,yloc),size=(w,4),
                      style=wx.LI_HORIZONTAL)
        yloc+=10
        wx.StaticText(oneviewer.panel,-1,"Reattach hydrogen:",pos=(5,yloc),size=(120,18))
        self.rbtcur1=wx.RadioButton(oneviewer.panel,-1,'current',pos=(140,yloc),
                               style=wx.RB_GROUP)
        self.rbtall1=wx.RadioButton(oneviewer.panel,-1,'all',pos=(210,yloc))
        yloc+=25
        xloc=15
        btnlabel=['All','HD1','HD2','HE1','HE2']
        for labl in btnlabel:
            btnreat=wx.Button(oneviewer.panel,-1,labl,pos=(xloc,yloc),
                              size=(45,25))
            btnreat.Bind(wx.EVT_BUTTON,self.OnReattachHydrogen)
            xloc+=50
    ...
    def OnReattachHydrogen(self,event):
        obj=event.GetEventObject()
        labl=obj.GetLabel()
        if self.rbtcur1.GetValue():
            hisdatlst = [self.oneviewer.GetCurrentItem()]
        else:
            hisdatlst=[]
        self.ReattachHydrogen(hisdatlst=hisdatlst,hname=labl)

    def ReattachHydrogen(self,hisdatlst=[],hname='all',drw=True, messout=True):
        def AtomTripletCCList(idx,histamlst):
            # [x1,x2,x3] where a hydrogen is attached at x1
            atmtriplet = [[ 'ND1','CG', 'CE1'], ['CD2', 'CG', 'NE2'],
                          ['CE1', 'ND1', 'NE2'], ['NE2', 'CE1', 'CD2']]
            atmcclst=[]
            for nam in atmtriplet[idx]:
                for i in hisatmlst:
                    atom=self.model.mol.atm[i]
                    if atom.atmnam.strip() == nam:
                        atmcclst.append(copy.deepcopy(atom.cc))
                        break
            return atmcclst
        # save mol for undo
        self.model.SaveMol()
        #
        allhyd=['HD1','HD2','HE1','HE2']
        rch=const.CovBndLen[' H C'][1]
        rnh=const.CovBndLen[' H N'][1]
        rhxlst=[rnh,rch,rch,rnh]
        hydlst=[hname]
        if hname.lower() == 'all': hydlst=allhyd
        if len(hisdatlst) <= 0:
            hisdatlst=self.model.ListResidue4('HIS')
        else:
            hisdatlst=[self.oneviewer.GetCurrentItem()]
        # check hname
        dellst=[]
        for hyd in hydlst:
            if not hyd.strip().upper() in allhyd:
                dellst.append(hyd)
                continue
        if len(dellst) > 0:
            wx.MessageBox('No atom name=' + dellst, 'ReattachHydrogen')
            for hyd in dellst:
                hydlst.remove(hyd)
        if len(hydlst) <= 0:
            return
        changelst=[]
        mess=''
        for hisres in hisdatlst:
            hisatmlst = self.model.ListResDatAtoms(hisres)
            for i in hisatmlst:
                atom=self.model.mol.atm[i]
                for hyd in hydlst:
                    if atom.atmnam.strip() == hyd:
                        idx=allhyd.index(hyd)
                        atmcclst=AtomTripletCCList(idx,hisatmlst)
                        mess,newcc=lib.CalcCCAddAtmType1A2(atmcclst,rhxlst[idx])
                        if len(mess) > 0:
                            mess='Failed to compute new hydrogen coordinate for '+hisres+', '+hyd
                            wx.MessageBox(mess,'ReattachHydrogen')
                            continue
                        oldcc=copy.deepcopy(atom.cc)
                        atom.cc = newcc[0].tolist()
                        changelst.append([hisres,hyd,oldcc,atom.cc])

        if messout:
            mess='Hydrogen atom coordinates have changed for,\n'
            for hisres,hyd,oldcc,newcc in changelst:
                mess+='   '+hisres+', '+hyd+', oldcc='+str(oldcc)+', newcc='+str(newcc)+'\n'
            self.model.ConsoleMessage(mess)
        if drw: self.model.DrawMol(True)
    ...
    def ChangeAllHIS(self,newform='',listform=False):
        ...
        for curnmb in range(ininmb,itemnmb):
            ...
            if new not in hisformdic: hisformdic[new]=[]
    ...
    def FindAppropriateHISForm(self):
        ...
        if curhis not in self.hischangelog: self.hischangelog[curhis]=[]
    ...
    def SwapCandN(self,curhis):
        def SwapAtmnamAndElm(seq1,nam1,elm1,seq2,nam2,elm2):
            self.model.mol.atm[seq1].atmnam=nam2
            self.model.mol.atm[seq1].elm=elm2
            self.model.mol.atm[seq2].atmnam=nam1
            self.model.mol.atm[seq2].elm=elm1

        mess=''
        atmnamlst=[' ND1',' HD1',' CD2',' HD2',' CE1',' HE1',' NE2']
        atmnamdic={}
        for atmnam in atmnamlst:
            atmnamdic[atmnam] = self.FindSeqNmbInCurrentResDat(curhis, atmnam)
        ...
        # Rename ' HE1' to 'DEL'
        # Add H(' HE1') at old ' NE2' atom
        # Del H(' DEL')
        for [nam1,nam2,elm1,elm2] in swaplst:
            try: seq1=atmnamdic[nam1]
            except: continue
            try: seq2=atmnamdic[nam2]
            except: continue
            SwapAtmnamAndElm(seq1,nam1,elm1,seq2,nam2,elm2)
        oldhe1 = atmnamdic[' HE1']
        self.model.mol.atm[oldhe1].atmnam=' DEL'
        oldne2=atmnamdic[' NE2']
        self.model.AttachHydrogen('1H',atmlst=[oldne2])

        seqdel = self.FindSeqNmbInCurrentResDat(curhis, ' DEL')
        self.model.mol.DelAtoms([seqdel])
        # rename hydrogens
        self.RenameHISHydrogens([curhis])
        mess1=mess2='Swap C<->N '
        self.hischangelog[curhis]=[]
        self.hischangelog[curhis].append(mess1+'\n'+mess2+'\n')

    def RenameHISHydrogens(self,hisdatlst=[]):

        hydnamdic={' CE1':' HE1',' NE2':' HE2',' ND1':' HD1',' CD2':' HD2'}
        #print('hisdat=',hisdat)
        if len(hisdatlst) <= 0:
            hisdatlst=self.model.ListResidue4('HIS')

        for hisdat in hisdatlst:
            hisatmlst = self.model.ListResDatAtoms(hisdat)
            for i in hisatmlst:
                atom = self.model.mol.atm[i]
                if atom.elm == ' H':
                    ibatm=atom.conect[0]
                    ibatmnam=self.model.mol.atm[ibatm].atmnam
                    #print('ibatmnam=',ibatmnam)
                    if ibatmnam in hydnamdic:
                        atom.atmnam=hydnamdic[ibatmnam]
    ...
    def TypeHisChangeLog(self):
        ...
        text=''
        for his,txt in list(self.hischangelog.items()):
            text=text+'"'+his+'"\n'
            text=text+txt[0]+'\n'
        return text

    def ViewHisChangeLog(self):
        header='HIS form change log at '+lib.DateTimeText()+'\n'
        ...
        self.viewer=subwin.TextViewer_Frm(self.mdlwin,title=title,
                                     text=text,menu=True)
    ...
    def OnMenu(self,event):
        ...
        if item == 'Open':
            pass
        if item == "View His form change log": self.ViewHisChangeLog()
        elif item == "Close":
            self.Close()
        # Help
        elif item == "Document": self.HelpDocument()
    ...
    def MenuItems(self):
        ...
        # File
        submenu=wx.Menu()
        submenu.Append(-1,"View His form change log","View His change log")
