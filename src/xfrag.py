...
class Fragment(object):
    ...
    def ListNoneChargeFragment(self):
        ...
        if frgattribdic is None: return frglst
    ...
    def SetFrgDatOfResidue(self,messout=True):
        ...
        resdatlst = self.model.ListResidue3('all',target='selected')

        print(('resdatlst=',str(resdatlst)))
        ...
        print(('resfrgdic=',str(self.resfrgdic)))
        ...
        print(('resdatdic=',str(resdatdic)))
        print(('frgattribdic=',str(self.model.mol.frgattribdic)))
        ...
        for frgresnam,frgdat in list(self.resfrgdic.items()):
            ...
            if fresnam in resnamlst:

                print(('fresnam in resnamlst=',fresnam))

                reslst=resdatdic[fresnam] # [frgresnam]
                ...
                for resdat in reslst:
                    ...
                    rescount += 1
                    newfrglst=self.SetFrgDatToAtomObj(resdat3,rescount,frgresnam,messout=messout)
                    #newfrglst = self.SetFrgDatToAtomObj(resdat, rescount, fresnam, messout=messout)

                    print(('newfrglst=',newfrglst))
        ...
        print(('frgnamlst=',self.frgnamlst))
        print(('frgatmdic=',self.frgatmdic))
    ...
    def SetFrgDatToAtomObj(self,resdat,rescount,frgresnam,messout=True):
        ...
        """
        print(('resdat,rescount,frgreanam in SetFrgDatToAtomObj=',resdat,rescount,frgresnam))

        newfrglst=[]
        sellst=self.model.ListTargetAtoms()
        resatmlst=self.model.ListResDatAtoms(resdat)

        print(('resatmlst=',resatmlst))
        ...
        print(('bdalst in SetFrgDatToAtomObj=',bdalst))
...
class FragMolResidue_Frm(wx.Frame):
    ...
    def __init__(self,parent,id,title='',parobj=None,winpos=[],winsize=[]): #,filelst=[]):
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[FragMol]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
        #
        # default BAA
...
class FragMolAllInOne_Frm(wx.Frame):
    ...
    def __init__(self,parent,id,title='',parobj=None,winpos=[],winsize=[]): #,filelst=[]):
        ...
        self.menubar=self.MenuItems()
        self.SetMenuBar(self.menubar)
        lib.InsertTitleMenu(self.menubar,'[FragPanel]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
        #
...
class FragmentAttrib_Pan(wx.Panel):
    ...
    def CreateButtons(self):    
        ...
        self.cmbfrg=wx.ComboBox(self,-1,'',choices=[], \
                    pos=(xloc+110,yloc),size=(80,hcmb),style=wx.CB_READONLY)                      
        btnattrib=wx.Button(self,-1,"Apply",pos=(240,yloc+2),
                       size=(50,20))
        btnattrib.Bind(wx.EVT_BUTTON,self.OnApplyAttrib)
        # Set parmas
        self.SetWidgetItems()
