...
"""
try:
    CC_TOOLS_PATH=os.environ['CC_TOOLS_PATH']
except:
    mess ='This application needs the libraries of "cc_tools".\n'
    mess+='Please set the path of "cc_tools" to the environment variable,\n'
    mess+='"CC_TOOLS_PATH"'
    wx.MessageBox(mess,'gmv_main')
    exit()
CC_TOOLS_SRC=os.path.join(CC_TOOLS_PATH,'src')
sys.path.insert(0,CC_TOOLS_SRC)
#from cc_libs import cc_lib, cc_gui, cc_rwf
from cc_libs import cc_lib as lib
from cc_libs import cc_gui as subwin
from cc_libs import cc_rwf as rwfile
sys.path.insert(0,os.path.join(CC_TOOLS_SRC,'libs'))
import cc_mollib as molec
import cc_drwlib as draw
"""
...
class View_Frm(wx.Frame):
    ...
    def __init__(self,parent,id,winpos=[],winsize=[]): #,model):
        ...
        self.font1 = wx.Font(10, wx.MODERN, wx.NORMAL, wx.NORMAL, False, 'Consolas')
        self.textcol='black'
        # set frame font
        ...
        self.Addmenu=False
        # popup menu
        self.defaultpopupmenu = None #self.menuctrl.PopupMenu
        self.defaultpopupmenuhandler = None # self.menuctrl.OnPopupMenu
        self.popupmenuitems   = None
        self.popupmemuhandler = None
        # Create StatusBar
        #wx.MessageBox('ok24')
    ...
    def CreateViewPanel(self,model,fumode):
        ...
        self.canvassize=self.framesize #[]
        #wx.MessageBox('ok31')
        # Create MenuBar
        ...
        lib.InsertTitleMenu(self.menuctrl.menubar,'[fu]')

        # Rmove FMO menu?
        ###if not self.model.setctrl.GetParam('fmo-menu'):
        ###    self.menuctrl.RemoveMenuItem('FMO')
        ...
        """
        canpos = [0,0]
        cansize = self.canvassize
        canpan = self
        viewhdr = self
        """ """
        #wx.MessageBox('ok37')
        self.draw = draw.CanvasDraw(self,canpan,canpos,cansize,viewhdr)
        #wx.MessageBox('ok38')
        """ """
        #####self.draw = draw.MolDraw(self)
        """ uncommented  """ 
    ...
    def OpenDropFiles(self,filenames):
        ...
        for file in filelst:
            ...
            self.model.OpenFiles(file,True,drop=True)

    def SetUpTextPanel(self):
        # create a instance of 'TextPanel' class
        self.textpan=TextPanel(self,'update')
        self.textpan.SetWinBGColor([]) # default: 'mdlwin' bgcolor
        self.textpan.SetTextBGColor([]) # default: yellow
        self.textpan.SetPos([10,80])
    ...
    def ResetBGColor(self):
        self.bgcolor = self.model.setctrl.GetParam('win-color')
        try:
            self.draw.SetBGColor(self.bgcolor)
        except:
            pass
    ...
    def SetPopupMenu(self,menuitems=None,menuhandler=None):
        if menuitems is None:
            self.popupmenuitems   = self.defaultpopupmenu
            self.popupmenuhandler = self.menuctrl.OnPopupMenu
        else:
            self.popupmenuitems   = menuitems
            self.popupmemuhandler = menuhandler
    ...
    def BusyIndicator(self,cmd,mess=''):
        ...
        if cmd == "On":
            ...
            if self.busyindicator is None:
                ...
                lib.SetTipString(self.busyindicator,'Left click to reset')
    ...
    def Move(self):
        ...
        try:
            if self.mousemode.IsOpen(): 
                #self.mousemode.Resize(dx,dy)
                self.model.menuctrl.OnWindow("Open MouseModeWin",False)
                self.model.menuctrl.OnWindow("Open MouseModeWin",True)
                #self.mousemode.SetOpen(True)
        except: pass
        # molchoicewin
        ...
        try:
            ...
            if self.molchoice.IsOpen(): 
                #self.molchoice.Resize(dx,dy)
                self.model.menuctrl.OnWindow("Open MolChoiceWin",False)
                self.model.menuctrl.OnWindow("Open MolChoiceWin",True)
                #self.molchoice.SetOpen(True)
        except: pass
        # for FMOViewer
        """
        ...
        if lib.GetPlatform() != 'WINDOWS': return
    ...
    def OnMouseRightDown(self,event):
        ...
        #
        label='mdlpopupmenu'
        menulst,tiplst=self.popupmenuitems()
        if len(menulst) <= 0: return
        menulst.insert(0,'CloseMe')
        retmethod=self.popupmenuhandler
        self.popupmenu=subwin.ListBoxMenu_Frm(self,-1,[],[],retmethod,
                                              menulst,tiplst)
...
class Remark_Frm(wx.Frame):
    ...
    def OnPaint(self,event):
        if lib.GetPlatform() != 'MACOSX': return
        self.DrawColorRemark()          
        event.Skip()
    ...
    def DrawColorRemark(self):
        ...
        #
        if lib.IsWxVersion2(): self.remarkbuffer=wx.EmptyBitmap(self.width,self.hight) 
        else: self.buffer=self.remarkbuffer=wx.Bitmap(self.width,self.hight) 
        ...
        if self.textfont is not None: dc.SetFont(self.textfont)
        if self.textcol is not None: dc.SetTextForeground(self.textcol)
        #
...
class MolChoice(object):
    def __init__(self,parent):
        ...
        if lib.IsWxVersion2(): self.winsize1=lib.WinSize([150,115])
        else: self.winsize1=lib.WinSize([140,100])
        self.shape=0
        self.wbox=120
        self.molnamlst=[]
        self.selectmol=''
...
class MolChoice_Frm(wx.MiniFrame):
    def __init__(self,parent,id,winpos,winsize,mymgr,shape):
        ...
        if shape == 0:
            wbox=mymgr.GetWBox()
            wwin=wbox+130 #120 #120 #115; 
            if lib.IsWxVersion2(): hwin=32
            else: hwin=28            
            winsize=(wwin,hwin) #; winpos[0] -= (wbox-120) # default wwin=230
            winpos[0] += (parwinsize[0]-wwin)
            if not lib.IsWxVersion2(pltfrm='MACOSX'):
                winpos[0] -= 5; winpos[1] += 1
        ...
        # set font
        if lib.IsWxVersion2(): pntsize=8
        else: pntsize=10
        self.font=self.GetFont()
        self.font.SetPointSize(pntsize) #8)
        ...
        # buttons
        if not lib.IsWxVersion2(pltfrm='MACOSX'):
            self.btnsize=[10,10]; self.btnpos=[2,15]; self.btnstyle=wx.BORDER_NONE
        else: 
            self.btnsize=[10,10]; self.btnpos=[2,15]; self.btnstyle=0
        if lib.GetPlatform() == 'WINDOWS': self.modebtnsize=[10,10]
    ...
    def CreatePanel0(self):
        ...
        clsbtn=wx.Button(self,-1,"",pos=(2,2),size=self.btnsize,style=self.btnstyle)
        clsbtn.SetBackgroundColour(wx.RED)
        clsbtn.Bind(wx.EVT_BUTTON,self.OnClose)
        ###clsbtn.SetToolTipString('Close')
        lib.SetTipString(clsbtn,'Close')
        shpbtn=wx.Button(self,-1,"",pos=self.btnpos,size=self.btnsize,style=self.btnstyle)
        shpbtn.SetBackgroundColour(wx.WHITE)
        ...
        self.cboxsqbt1=wx.Button(self,wx.ID_ANY,"<",pos=(xloc+wbox+60,yloc+2),size=(22,20))
        self.cboxsqbt1.Bind(wx.EVT_BUTTON,self.OnChoicePrevMol)
        self.cboxsqbt2=wx.Button(self,wx.ID_ANY,">",pos=(xloc+wbox+85,yloc+2),size=(22,20))
    ...
    def CreatePanel1(self):
        ...
        lib.InsertTitleMenu(self.menubar,'[MolChoice]')
        self.Bind(wx.EVT_MENU,self.OnMenu)
        ...
        #??xloc=self.panelpos[0]; yloc=self.panelpos[1]
        xloc=5; yloc=5
        clsbtn=wx.Button(self.panel,-1,"",pos=(2,2),size=self.btnsize,style=self.btnstyle)
        clsbtn.SetBackgroundColour(wx.RED)
        clsbtn.Bind(wx.EVT_BUTTON,self.OnClose)
        #clsbtn.SetToolTipString('Close')
        lib.SetTipString(clsbtn,'Close')
        shpbtn=wx.Button(self.panel,-1,"",pos=self.btnpos,size=self.btnsize,style=self.btnstyle)
        shpbtn.SetBackgroundColour(wx.WHITE)
        shpbtn.Bind(wx.EVT_BUTTON,self.OnShape)
        #shpbtn.SetToolTipString('Change style')
        lib.SetTipString(shpbtn,'Change style')
        ...
        st0.SetFont(self.font)
        st0.SetForegroundColour(self.fgcolor)
        self.cboxsqbt1=wx.Button(self.panel,wx.ID_ANY,"<",pos=(xloc+65,yloc),size=(22,htext))
        self.cboxsqbt1.Bind(wx.EVT_BUTTON,self.OnChoicePrevMol)
        self.cboxsqbt2=wx.Button(self.panel,wx.ID_ANY,">",pos=(xloc+95,yloc),size=(22,htext))
        self.cboxsqbt2.Bind(wx.EVT_BUTTON,self.OnChoiceNextMol)
        #xloc=5; yloc += 22; wbox=w-10
        xloc=5; yloc += 30; wbox=w-10
    ...
    def DrawShowItems(self):
        # not completed 2019.09.12
        #self.shwitemdic=self.mdlwin.model.mol.showdic

        for item, value in list(self.shwitemdic.items()):
...
class MouseMode_Frm(wx.MiniFrame):
    def __init__(self,parent,id,winpos,mymgr,shape):
        if shape == 0:
            if lib.IsWxVersion2(): winsize=[320,32] #(275,28)
            else: winsize=[320,28]
            if len(winpos) <= 0:
                if lib.GetPlatform() == 'WINDOWS':
                    winpos=parent.canvas.GetScreenPosition()
                    ...
                    if lib.IsWxVersion2():
                        winpos=[winpos[0]+1,winpos[1]+45]
                    else: winpos=[winpos[0]+1,winpos[1]+24]
            #
            wx.MiniFrame.__init__(self,parent,id,pos=winpos,size=winsize,
        ...
        self.smallredbmp=self.model.setctrl.GetIconBmp('small-red')
        self.smallwhitebmp=self.model.setctrl.GetIconBmp('small-white')
        self.bmpbtndic={}
        self.stbgcolor='White'
        # buttons
        if not lib.IsWxVersion2(pltfrm='MACOSX'):
            self.btnsize=[10,10]; self.otherbtnsize=[55,30] #; hbmpbtn=35; wbmpbtn=50
            self.modebtnsize=[25,25]; self.btnpos=[2,15]; self.btnstyle=wx.BORDER_NONE
        else: 
            self.btnsize=[10,10]; self.otherbtnsize=[40,25]; self.modebtnsize=[25,25]
            self.btnpos=[2,15] #;hbmpbtn=25; wbmpbtn=40
            self.btnstyle=0
        if lib.GetPlatform() == 'WINDOWS': 
            self.modebtnsize=[25,25]; self.otherbtnsize=[50,25]
    ...
    def CreatePanel0(self):
        ...
        clsbtn=wx.Button(self,-1,'',pos=(2,2),size=self.btnsize,style=self.btnstyle) #wx.BORDER_NONE)
        #clsbtn=wx.BitmapButton(self,-1,bitmap=self.smallredbmp,pos=(2,2),size=btnsize)
        clsbtn.SetBackgroundColour(wx.RED) #'red')
        clsbtn.Bind(wx.EVT_BUTTON,self.OnClose)
        ###clsbtn.SetToolTipString('Close')
        lib.SetTipString(clsbtn,'Close')
        shpbtn=wx.Button(self,-1,'',pos=self.btnpos,size=self.btnsize,style=self.btnstyle) #wx.BORDER_NONE) #,size=btnsize)
        #shpbtn=wx.BitmapButton(self,-1,bitmap=self.smallwhitebmp,pos=btnpos,size=btnsize)
        shpbtn.SetBackgroundColour(wx.WHITE) #'white')
        #shpbtn.SetForegroundColour(wx.WHITE)
        shpbtn.Bind(wx.EVT_BUTTON,self.OnShape)
        ###shpbtn.SetToolTipString('Change style')
        lib.SetTipString(shpbtn,'Change style')
        xloc += 20 #14 #24 #xloc1=5
        self.btnmode=wx.BitmapButton(self,-1,bitmap=self.modebmp,pos=(xloc,yloc-2),size=self.modebtnsize) #(25,25)) #,size=(hbmpbtn,hbmpbtn))
        ...
        lib.SetTipString(self.btnmode,'Switch view(white) and build(green) mode')
        ...
        for i in range(len(labellst)):
            btn=wx.BitmapButton(self,-1,bitmap=bmplst[i],pos=(xloc,yloc-2),size=self.otherbtnsize) #(55,30)) #size=(40,25)) #size=(wbmpbtn,hbmpbtn))
            btn.Bind(wx.EVT_BUTTON,self.OnBitmapButton) #ControlPanMdl)
            btn.SetLabel(labellst[i])
            ###btn.SetToolTipString(tiplst[i])
            lib.SetTipString(btn,tiplst[i])
            val=' '+vallst[i][:1].upper()+vallst[i][1:2]
            btnst=wx.StaticText(self,-1,val,pos=(xloc+50,yloc+2),size=(20,18)) #, #(20,22),
    ...
    def OnBitmapButton(self,event):
        ...
        self.lbmenu=subwin.ListBoxMenu_Frm(self,-1,winpos,[],retmethod,menulst,
                                           tiplst) #,winwidth=200)
    ...
    def ChangeModeButton(self,mdlmod):
        ...
        self.btnmode=wx.BitmapButton(self,-1,bitmap=modebmp,pos=pos,size=self.modebtnsize) #(hbmpbtn,hbmpbtn))
        self.btnmode.Bind(wx.EVT_BUTTON,self.OnBuildMode) #ControlPanMdl)
        #self.btnmode.SetToolTipString('Switch view(white) and build(green) mode')
        lib.SetTipString(self.btnmode,'Switch view(white) and build(green) mode')
        self.btnmode.SetLabel('BuildModeButton')
    ...
    def OnBuildMode(self,event):
        bndrotwin = self.model.winctrl.GetWin("RotateBond")
        if bndrotwin is not None:
            mess = 'Please switch the edit/view mode with the "Bond Rotation" button"\n'
            mess += ' of the "Rotate bond editor" panel. '
            wx.MessageBox(mess, 'MouseMode_Frm.OnBuildMode')
            return
    ...
    def CreatePanel1(self):
        ...
        xloc += 20
        self.btnmode=wx.BitmapButton(self,-1,bitmap=self.modebmp,pos=(xloc,yloc-2),size=self.modebtnsize)
        self.btnmode.Bind(wx.EVT_BUTTON,self.OnBuildMode) #ControlPanMdl)
        #self.btnmode.SetToolTipString('Switch view and build mode')
        lib.SetTipString(self.btnmode,'Switch view and build mode')
        self.btnmode.SetLabel('BuildModeButton')
        ...
        for i in range(len(labellst)):
            #xloc += 10
            btn=wx.BitmapButton(self,-1,bitmap=bmplst[i],pos=(xloc,yloc-2),size=self.otherbtnsize)
            btn.Bind(wx.EVT_BUTTON,self.OnBitmapButton) #ControlPanMdl)
            btn.SetLabel(labellst[i])
            #btn.SetToolTipString(tiplst[i])
            lib.SetTipString(btn,tiplst[i])
            val=' '+vallst[i][:1].upper()+vallst[i][1:2]
            btnst=wx.StaticText(self,-1,val,pos=(xloc+50,yloc+2),size=(20,18)) #, #22),
                                    #style=wx.BORDER|wx.ST_NO_AUTORESIZE)
            btnst.SetBackgroundColour(self.stbgcolor)
            self.bmpbtndic[labellst[i]]=[btn,btnst]
            yloc += 30
    ...
    def OnMusMode(self,item,menulabel):
        ...
        if item == 'axis-rot' and not self.model.mousectrl.IsRotAxisSet():
            ...
            wx.MessageBox(mess,'OnMusMode')
            return
        if item == 'set axis' and not self.model.mol :
            #wx.MessageBox('No molecule in fu-MdlWin','OnMusMode')
            lib.MessageBoxOK('No molecule in fu-MdlWin','OnMusMode')
            return
    ...
    def OnSelMode(self,item,menulabel):
        ...
        if not self.model.mol:
            wx.MessageBox('No molecule in fu-MdlWin')
            return
        label='SelNmbButton'
        obj=self.bmpbtndic[label][0]
        objst=self.bmpbtndic[label][1]
...
class TextPanel(object):
    ...
    :param int mode: mode:'standard' or 'update'. in the latter 'width' is
                     fixed (used in fumodel:'TextWrite'
    """
    ...
    def WriteText(self,mess):
        ...
        #winbgcolor
        self.win=TextPanel_Frm(self.mdlwin,-1,self,winpos,winsize,mess,winbgcolor)
