...
class BarGraph(wx.Panel):
    ...
    def __init__(self,parent,id,graph,pos,size,bgcolor):
        ...
        self.mousepos=[0,0]
        # set buffer for plot
        if lib.IsWxVersion2(): self.buffer=wx.EmptyBitmap(self.wplt,self.hplt)
        else: self.buffer=wx.Bitmap(self.wplt,self.hplt)
    ...
    def Plot(self,on):
        ...
        for i in range(self.begindata,ndat): #self.nfrg):
            ...
            #
            col0=[200,200,200] #'lightgray' #const.RGBColor255['gray01'] #col # 'gray01' #'lightgray'
    ...
    def OnMouseLeftDown(self,event):
        ...
        #x#self.SetCursor(wx.StockCursor(wx.CURSOR_HAND)) #wx.CURSOR_HAND))
    ...
    def OnMouseMove(self,event):
        ...
        #x#self.SetCursor(wx.StockCursor(wx.CURSOR_HAND))
        self.Replot(diff[0])
        #
        self.mousepos=pos
    ...
    def OnMouseLeftUp(self,event):
        ...
        self.mouseleftdown=False 
        #x#self.SetCursor(wx.NullCursor)
    ...
    def ShiftGraphByKeyDown(self,xmove):
        xmaxplt=self.wplt-(self.xinipos+self.wremark)-10
        pltnmb=int(xmaxplt/self.xunit)
        maxnmb=self.maxplotdata-pltnmb
        if maxnmb < 0: maxnmb=0

        self.begindata += xmove
        if self.begindata < 0: self.begindata=0
        elif self.begindata > maxnmb: self.begindata=maxnmb
...
class EnergyGraph(wx.Panel):
    # plot bargraph
    def __init__(self,parent,winpos=[-1,-1],winsize=[100,100],bgcolor='white',
                 retobj=None,enableselect=True):
        wx.Panel.__init__(self,parent,-1,pos=winpos,size=winsize) #,style=wx.MINIMIZE_BOX ) #,
        #       style=wx.SYSTEM_MENU|wx.CAPTION|wx.CLOSE_BOX) #|wx.FRAME_FLOAT_ON_PARENT)
        self.parent=parent # pltpan
        self.retobj=retobj
        # background color
        ...
        self.wytitle=5
        self.wylabel=30
        self.xinipos=self.wytitle+self.wylabel+20 #10 #5
        self.xendpos=self.wplt-15 #self.wremark-15

        self.xunit=22 #12 # barwidth*2+2
        self.barwidth=15 #5
    ...
    def SetDataIndex(self,dataidx):
        self.dataidx = dataidx

    def SetTitle(self,title):
        self.title=title

    def SetDataColor(self,datacolorlst):
    ...
    def SetFontSizeUser(self,fsize,plt=True):
        """ size: 8,10,12,..."""


        self.font = wx.Font(fsize, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL,
               wx.FONTWEIGHT_NORMAL, False, 'Courier 8 Pitch')
        self.fontsize = [fsize,1.4 * fsize] #[6,10]

        self.xinipos=self.wytitle+self.wylabel+20 + 4*(self.fontsize[0]-10) #10 #5

        if plt: self.Plot(True)

    def SetFontUser(self,font,fontsize,plt=True):
        # font: wxfont, fontsize:[w,h] in pixels
        self.font=font
        self.fontsize=fontsize
        if plt: self.Plot(True)
    ...
    def Plot(self,on):
        ...
        orgy=self.FindYPos(0.0)
        ndat=len(self.data)
        x0=self.xinipos+ 20 + 5*(self.fontsize[0]-10)#+10 #8 #self.wylabel
        barwidth=self.barwidth #30 #15
        self.xrangeofdata=[]
        ii=-1
        for i in range(ndat): #self.beginvalue,ndat): #self.nfrg):
            ii += 1
            x=x0+ii*(barwidth+15) # 8)
            ...
            for j in range(len(self.data[i])):
                ...
                if self.datacolorlst is not None: col0 = self.datacolorlst[i][j]
                if i in self.selectdatadic:
    ...
    def DrawYAxis(self,dc):
        ...
        text='0'; ns=len(text)
        dc.DrawText(text,self.xinipos-fw*ns-2,orgy-5)
        # axis label
        # positive axis
        yinc=5
        ...
        for i in range(1,np):
            ...
            dc.DrawText(text,self.xinipos-fw*ns-2,y-5)
        # negative axis
    ...
    def SetPlotSize(self,wplt,hplt):
        ...
        if hplt > hpltmax: hplt=hpltmax
        self.wplt=wplt; self.hplt=hplt
        self.xinipos=self.wytitle+self.wylabel+20  + 4*(self.fontsize[0]-10) #10 #5# 10 #5
        #!!!self.xendpos=self.wplt-self.wremark-20
        self.yinipos=self.hplt-self.hxtitle-self.hxlabel-10
        self.yendpos=self.htitle+10
        self.yunit=(self.yinipos-self.yendpos)/self.yrange
    ...
    def SetTitleMessageText(self,messagetextlst):
        self.messagetextlst = messagetextlst

    def SetTitleMessage(self,idat,iorb):
        label=['a:','b:']; fmt='%8.3f'
        if idat < 0 or iorb < 0: return #self.title=''
        value=self.data[idat][iorb]
        idx = self.dataidx[idat][iorb]
        #except: idx = iorb
        if self.messagetextlst is None: head = 'data-mo'+str(idat+1) + '-'
        else: head = self.messagetextlst[idat]+'-mo'
        self.title=head+str(idx+1)+', e='+(fmt % value)
