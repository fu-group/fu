...
class Fragment(object):
    ...
    def ListNoneChargeFragment(self,atomattr=True):
        frglst=[]; done={}
        if atomattr:
            for atom in self.model.mol.atm:
                frgnam=atom.frgnam
            ...
            try:
                for frgnam,attriblst in list(frgattribdic.items()):
                    if attriblst[0] is None: frglst.append(frgnam)
            except:
                mess='Error in getting fragment attribs in FU\n'
                mess+='Probably fragmentation is notdone.'
                wx.MessageBox(mess,'frag.ListNoneChargeFragment')
    ...
    def DictFragmentAtoms(self):
        ...
        for atom in self.model.mol.atm:
            frgnam=atom.frgnam
            if frgnam not in frgatmdic: frgatmdic[frgnam]=[]
    ...
    def FragmentSizeDistribution(self,binwidth=5,messout=True):
        ...
        rank=list(range(binwidth*(minsize-1),binwidth*(maxsize+1),binwidth))
    ...
    def CheckUnassinedResidues(self):
        nofrgnamres=self.ListUnnamedFragmentResidue()
        if len(nofrgnamres) > 0:
            mess='There are unassigned residues to a fragment.\n'
            mess+='Would you like to assign them to a fragment?\n'
            mess+=str(nofrgnamres)
            dlg = wx.MessageDialog(None, mess, 'Fragment',
                               style=wx.YES_NO | wx.ICON_QUESTION | wx.STAY_ON_TOP)
            if dlg.ShowModal() == wx.ID_YES:
                self.NameUnnamedFragments()
                nofrgnamres=[]
        return nofrgnamres

    def ListUnnamedFragmentResidue(self):
        reslst=[]
        for atom in self.model.mol.atm:
            if atom.frgnam == '':
                res=atom.resnam + ':'+ str(atom.resnmb) + ':' +atom.chainnam
                if not res in reslst: reslst.append(atom.resnam + ':'+ str(atom.resnmb) + ':' +atom.chainnam)
        return reslst

    def NameUnnamedFragments(self):
        frgnamlst=self.ListFragmentName('all')
        nfrg=len(frgnamlst)
        name=None
        for atom in self.model.mol.atm:
            if atom.frgnam == '':
                if name is None:
                    name=atom.resnam+str(atom.resnmb)+str(nfrg+1)
                atom.frgnam=name
        if name is not None:
            self.model.mol.frgattribdic[name]=[None,1,0,1]

    def CountElectrons(self,frgnamlst=[]):
        if len(frgnamlst) <= 0: frgnamlst = self.ListFragmentName('all')
        nelelst=[]
        baalst=[]
        for atom in self.model.mol.atm:
            if atom.frgbaa >= 0: baalst.append(atom.frgbaa)
        for frgnam in frgnamlst:
            nele=0
            for i in self.frgatmdic[frgnam]:
                nele+=const.ElmNmb[self.model.mol.atm[i].elm]
                if self.model.mol.atm[i].frgbaa >= 0: nele -= 1
                if i in baalst: nele += 1
            nelelst.append(nele)
        return nelelst

    def ListOddNumberOfElectronFragments(self,frgnamlst=[]):
        if len(frgnamlst) <= 0: frgnamlst = self.ListFragmentName('all')
        oddelelst=[]
        ...
        nelelst=self.CountElectrons(frgnamlst)
        for i in range(len(frgnamlst)):
            frgnam=frgnamlst[i]
            if frgattribdic[frgnam][0] is not None:
                nele=(nelelst[i]-frgattribdic[frgnam][0])
                if nele % 2 != 0: oddelelst.append(i)
        return oddelelst

    def ViewText(self):
        """ View fragment data """
        nofrgnamres=self.CheckUnassinedResidues()
        ...
        if len(frgnamlst) <= 0:
            ...
            return ''
        if len(self.ListUnnamedFragmentResidue()):
            unasmess = 'There are unassigned residues to a fragment.\n'
            unasmess += str(nofrgnamres)
        else: unasmess=''
        frgatrribdic={}
        try: frgattribdic=self.model.mol.frgattribdic
        ...
        nelelst=self.CountElectrons()
        frgnatm=list(frgnatm.values())
        maxfrg=max(frgnatm); minfrg=min(frgnatm)
        nochgfrg=self.ListNoneChargeFragment(atomattr=False)
        tcharge=0
        negchg=0
        poschg=0
        for i in range(len(frgnamlst)):
            frgnam=frgnamlst[i]
            if frgattribdic[frgnam][0] is not None:
                if frgattribdic[frgnam][0] > 0: poschg+=frgattribdic[frgnam][0]
                if frgattribdic[frgnam][0] < 0: negchg += frgattribdic[frgnam][0]
                tcharge+=frgattribdic[frgnam][0]
                #nele=(nelelst[i]-frgattribdic[frgnam][0])
                #if nele % 2 != 0: oddelelst.append(i)
        oddelelst=self.ListOddNumberOfElectronFragments()
        #
        ...
        totalchg=0
        text+='Total charge='+str(tcharge)+' (poschg='+str(poschg)
        text+=', negchg='+str(negchg)+', except unassigned charge fragments)\n'
        if len(unasmess) > 0:
            text+=unasmess
        if len(nochgfrg) > 0:
            ...
            text=text+'\n'
        if len(oddelelst) > 0:
            text = text + 'Odd number of electrons fragments=\n'
            text+='['
            for i in oddelelst:
                text=text+frgnamlst[i]+','
            text=text[:-1]+']\n'

        #text=text+'seq#,fragment name,number of atoms,charge,layer,active,'
        #text=text+'spin\n'
        text=text+'seq#,fragment name,number of atoms,number of electrons,charge,layer,'
        text=text+'spin\n'
        if frgattribdic is not None:
            for i in range(len(frgnamlst)):
                ...
                text+=('%6d' % nelelst[i])  # nele
                if frgattribdic[frgnam][0] == None: # chg
                   text=text+'  None' #str(frgattribdic[frgnam][0])
                else:    
                   text=text+('%6d' % frgattribdic[frgnam][0])

                text=text+('%6d' % frgattribdic[frgnam][1]) # layer
                #text=text+('%4d' % frgattribdic[frgnam][2])  # active
                text=text+('%6d' % frgattribdic[frgnam][3])+'\n' # spin
        return text
    ...
    def ListFragmentLayer1(self):    
        ...
        for atom in self.model.mol.atm:
            frgnam=atom.frgnam
            if frgnam not in frgdic: frglst.append([frgnam,atom.layer])
            frgdic[frgnam]=True
        return frglst
    ...
    def SetLayerColor(self):
        ...
        for atom in self.model.mol.atm:
            if atom.layer in const.LayerCol:
    ...
    def CountAtomsInFragment(self):
        ...
        for atom in self.model.mol.atm:
            frgnam=atom.frgnam
            if atom.elm == 'XX': continue
            if frgnam in frgdic: frgdic[frgnam] += 1
    ...
    def FragmentAtCsp3(self,minsize=10,option=[],drw=True):
        ...
        trgatmlst=list(range(len(self.model.mol.atm)))
    ...
    def ListSelectedFragment(self):
        ...
        if len(selatm) <= 0:
            selatm=list(range(len(self.model.mol.atm)))
        #
        frglst=[]; donedic={}
        for ia in selatm:
            frgnam=self.model.mol.atm[ia].frgnam
            if frgnam in donedic: continue
    ...
    def GetFragmentAttribute(self,attrib):
        ...
        if self.model.mol.frgattribdic is None:
            ...
            frgnamlst=self.ListFragmentName()
            for frgnam in frgnamlst:
                if frgnam in self.model.mol.frgattribdic: 
    ...
    def IsAttribute(self,attrib):
        ...
        if self.model.mol.frgattribdic is None: pass
            ...
            if attrib in self.model.mol.frgattribdic: ans=True
        return ans
    ...
    def AssignFragmenCharge(self,fchg,frgnamlst):
        for frgnam in frgnamlst:
            self.model.mol.frgattribdic[frgnam][0]=fchg
    ...
    def ListFragmentCharge(self,aareschg=True):
        ...
        for atom in self.model.mol.atm:
            ...
            if atom.frgchg is None: frgchg[ifrg]=None
                ...
                try: frgchg[ifrg] += atom.frgchg # polypeptide charge
                except:
                    frgchg[ifrg]=None
                    #print('error frgchg ifrg,seqnmb,frgchg=',ifrg,atom.seqnmb,atom.frgchg)
    ...
    def XXDeleteFragAttribute(self,attrib):
        if attrib in self.model.mol.frgattribdic:
    ...
    def OpenBDAFiles(self,mol=False,messout=True):
        ...
        if mol:
            ans=lib.AreSameBaseNames(filelst[0],self.model.mol.inpfile) #name)
            if not ans:
                mess='The base name of BDA file should be the same as '
                mess+='molecule name.\n'
                mess+='Please try to use "Open BDA file panel".'
    ...
    def MakeResBDADicFromFile(self,bdafilelst,messout=True):
        ...
        print(('checked bdafilelst=',bdafilelst))
        for file in bdafilelst:
            ...
            if lib.AreSameBaseNames(file,self.model.mol.inpfile): #name):
    ...
    def SaveBDAOnFile(self,filetype='entire'):
        ...
        atomattr=False
        if filetype != 'entire': atomattr=False

        nofrgnamres=self.CheckUnassinedResidues()
        if len(nofrgnamres) > 0:
            mess='Please assign unassigned residues to a fragment.\n'
            mess+='the residues='+str(nofrgnamres)
            return

        nochgfrg=self.ListNoneChargeFragment(atomattr)
        if len(nochgfrg) > 0:
            ...
            if ans: 
                #self.SetAllFragmentCharges(0)
                self.AssignFragmenCharge(0,nochgfrg)
            else: return
    ...
    def MolNameToFragResName(self):
        ...
        if namu in self.resfrgdic:
            resnam=namu; ok=True
        elif naml in self.resfrgdic:
            resnam=naml; ok=True
    ...
    def SetFrgDatOfResidue(self,messout=True):
        ...
        resdatlst=self.model.ListResidue3('all') #'('non-aa')
        if len(resdatlst) <= 0:
            mess='No residues in mol object.'
            lib.MessageBoxOK(mess,'Fragment(SetFrgDatOfResidue)')
            return
        # specific residue
        for frgresnam,frgdat in list(self.resfrgdic.items()):
        ...
        for resdat in resdatlst:
            ...
            if res not in resdatdic: resdatdic[res]=[]
            resdatdic[res].append(resdat)
        resnamlst=list(resdatdic.keys())
        #
        newfrgnamlst=[]
        if self.model.mol.frgattribdic is None: self.model.mol.frgattribdic={}
        for frgresnam,frgdat in list(self.resfrgdic.items()):
            # delete frgresnam attribute

            try:
                dellst=[]
                for name,lst in list(self.model.mol.frgattribdic.items()):
        ...
        if len(self.frgnamlst) <= 0:
            mess='No fragments are created.\n'
            mess+=' Please check if the first resudue name "RES:1:chain" is the same in the structure data '
            mess+='and the BDA files\n'
            mess+='and change the chain name by the "Modeling"-"Change"-"Chain name" menu '
            mess+='if needed and try again.'
            wx.MessageBox(mess,'SetFrgDatOfResidue')
    ...
    def SetFrgDatToAtomObj(self,resdat,rescount,frgresnam,messout=True):
        ...
        print(('frgresnam,frgatmdic',frgresnam,frgnamlst))
        ...
        for lst in bdalst:
            ...
            bda = lst[0]+resatmlst[0]
            baa = lst[1]+resatmlst[0]
    ...
    def RenumberFragmentNumber(self):
        ...
        frgnmblst=list(range(len(frgnamlst)))
        ind=-1
        for atom in self.model.mol.atm:
            if atom.frgnam == '': continue
            ind=-1
    ...
    def OpenAttribPanel(self):
        frgnam=self.ListFragmentName()
        if len(frgnam) <= 0:
            mess='No fragment is defined.'
            lib.MessageBoxOK(mess,'Fragment.OpenAttribPanel')
            return
        
        self.attrwin=FragMolAttrib_Frm(self.model.mdlwin,-1,self)
        try: self.attrwin.Show()
        except: pass
...
class FragMolByBDA_Frm(wx.Frame):
    ...
    def __init__(self,parent,id,parobj=None,winpos=[],winsize=[]): #,filelst=[]):
        ...
        if len(winsize) <= 0: self.winsize=lib.WinSize([320,280],True)
    ...
    def CreatePanel(self):
        ...
        xloc=200
        btnbrws=wx.Button(self.panel,-1,"Browse",pos=(xloc,yloc-2),size=(60,hbtn))
        ...
        yloc += 10
        # checkbox panel
        width=w-20; height=h-yloc-80 #40
        self.scrpan=subwin.CheckBoxPanel(self.panel,-1,pos=[10,yloc],
                                  size=[width,height])

        yloc += height+5;  xloc=w/2-100
        btnclr=wx.Button(self.panel,-1,"Clear",pos=(xloc,yloc),size=(50,hbtn))
        ...
        self.btnchk=wx.ToggleButton(self.panel,-1,"Check",pos=(xloc+160,yloc),
                                    size=(60,hbtn))
        ...
        btnaply=wx.Button(self.panel,-1,"Apply",pos=(xloc+170,yloc),
                              size=(50,hbtn))
...
class FragMolAttrib_Frm(wx.Frame):
    ...
    def __init__(self,parent,id,parobj=None,winpos=[]):
        ...
        winsize=lib.WinSize((320,120),False) #((230,135)) #110)
        if len(winpos) <= 0: winpos=lib.WinPos(parent)
        winpos  = lib.SetWinPos(winpos)
    ...
    def OnUndo(self,event):
        ...
        if attrib == 'IACTAT':
            ...
            if attrib not in self.savattribdic:
                self.model.Message2('Unable to undo')
                return
    ...
    def ViewAttributes(self):
        ...
        if len(frgdat) <= 0:
            ...
            lib.MessageBoxOK(mess,'FragAttribSetting(ViewAttributes)')
            return
...
class FragMolAllInOne_Frm(wx.Frame):
    ...
    def __init__(self,parent,id,title='',parobj=None,winpos=[],winsize=[]): #,filelst=[]):
        ...
        if len(winpos) <= 0: winpos=lib.WinPos(parent)
        winpos  = lib.SetWinPos(winpos)
        if len(winsize) <= 0: self.winsize=lib.WinSize([320,410])
    ...
    def CreatePanel(self):
        ...
        wx.StaticText(self.panel,-1,"Fragment min. size(number of atoms):",pos=(xloc,yloc),
                      size=(250,18))
        self.tclmin=wx.TextCtrl(self.panel,-1,pos=(xloc+260,yloc-2),
        ...
        ckbsp2=wx.CheckBox(self.panel,-1,"Csp2",pos=[xloc+60,yloc-2],
                                size=[55,20])
        self.baabtn.append(ckbsp2)
        ckbn=wx.CheckBox(self.panel,-1,"N",pos=[xloc+115,yloc-2],
                                size=[40,20])
        self.baabtn.append(ckbn)
        ckbo=wx.CheckBox(self.panel,-1,"O",pos=[xloc+155,yloc-2],
                                size=[40,20])
...
class FragmentAttrib_Pan(wx.Panel):
    ...
    def CreateButtons(self):    
        ...
        self.cmbattrib=wx.ComboBox(self,-1,'',choices=[], \
                       pos=(xloc,yloc-2),size=(80,hcmb),style=wx.CB_READONLY)
        self.cmbattrib.Bind(wx.EVT_COMBOBOX,self.OnAttrib)
        wx.StaticText(self,-1,"value=",pos=(xloc+100,yloc),
                      size=(45,18))
        self.cmbvalue=wx.ComboBox(self,-1,'',choices=[], \
                               pos=(xloc+145,yloc-2),size=(60,hcmb))
        yloc += 30; xloc=30
        wx.StaticText(self,-1,"Target fragment:",pos=(xloc,yloc+2),
                      size=(110,18))
        self.cmbfrg=wx.ComboBox(self,-1,'',choices=[], \
                    pos=(xloc+120,yloc),size=(100,hcmb),style=wx.CB_READONLY)
        btnattrib=wx.Button(self,-1,"Apply",pos=(260,yloc+2),
                       size=(50,22))
        btnattrib.Bind(wx.EVT_BUTTON,self.OnApplyAttrib)
        # Set parmas
        self.SetWidgetItems()
