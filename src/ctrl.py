...
class MenuCtrl():
    ...
    def CreateMenuBar(self,fumode,fmomenu,addon1menu,addon2menu):
        ...
        mtol=self.CreateMenuFromList('Tools',self.ToolsMenu()) # 'Report' menus
    ...
    def IsCheckable(self,label):
        ...
        if label in self.menulabeldic:
            menuitem=self.menulabeldic[label][0]
            if not menuitem: return False
            else: return menuitem.IsCheckable()
        else: return False
    ...
    def IsDefined(self,label):
        ...
        """
        if label in self.menulabeldic: return True
        else: return False
    ...
    def UncheckAll(self):
        ...
        if lib.GetPlatform() == 'MACOSX':
            if not lib.IsWxVersion2(): return
        
        for label,menuitemlabel in list(self.menulabeldic.items()):
            if len(label) <= 0: continue
            if menuitemlabel[0] is None: continue
    ...
    def UncheckProjectMenuItems(self):
        ...
        """
        #wx.MessageBox('ctrl. UncheckProjectMenuItems is called. returned')    
        for label,menuitemlabel in list(self.menulabeldic.items()):
    ...
    def EnableMenu(self,on):
        ...
        for i in range(nmb):
            ...
            if not on and not top in exclude:
    ...
    def MenuHandlerDic(self):
        ...
        menuhandlerdic={"File": self.OnFile,"Edit": self.OnEdit,
                        ...
                        "Modeling": self.OnModeling,"Tools": self.OnTools,
    ...
    def OnFile(self,label,bChecked):
        ...
        # wild card
        wcard=''
        wcard += 'geom file(*.pdb,*.ent,*.mol2,*.sdf,*.mol,*.xyz,*.cif,*.zmt)'
        wcard += '|*.pdb;*.ent;*.mol2;*.sdf;*.mol;*.xyz;*.xyzs;*.cif;*.zmt|'
        wcard += 'pdb file(*.pdb;*.ent)|*.pdb;*.ent|'
        wcard += 'mol2 file(*.mol2)|*.mol2|'
        wcard +=  'sdf file(*.mol;*.sdf)|*.mol;*.sdf|'
        wcard +=  'xyz file(*.xyz,*.xyzs,*.xyzfu)|*.xyz;*.xyzs;*.xyzfu|'
        wcard += 'cif file(*.cif)|*.cif|'
        wcard += 'smiles file(*.smi;*.can)|*.smi;*.can|'
        wcard += 'zmatrix file(*.zmt;*.zmtfu)|*.zmt;*.zmtfu|tinker(*.tin)|*.tin|'
        wcard += 'fu files(*.fuf,*.spl,*.mrg)|*.fuf;*.spl;*.mrg|'
        wcard += 'gamess fmo input file(*.inp)|*.inp|'
        #wcard += 'frame data(*.frm)|*.frm|' #fragment(*.frg)|*.frg|'
        wcard += 'gzip file(*.gz)|*.gz|'
        wcard +=  'all(*.*)|*.*'
        # check menu
        ...
        if label == "New":
            ...
            if len(filenamelst) > 0:
                ...
                for filename in filenamelst:
                    ...
                    self.model.OpenFiles(filename,True)
                    self.model.ChangePath(filename)
        elif label == "Merge":
            wcardm  = 'pdb,mol2,sdf(mol),xyz file(*.pdb(ent),*.mol2,*.sdf(mol),*.xyz)|'
            wcardm  += '*.pdb;*.ent;*.mol2;*.sdf;*.mol;*.xyz|'
            wcardm += 'pdb file(*.pdb;*.ent)|*.pdb;*.ent|'
            wcardm += 'sdf file(*.mol;*.sdf)|*.mol;*.sdf|'
            wcardm += 'mol2 file(*.mol2)|*.mol2|'
            wcardm += 'xyz file(*.xyz)|*.xyz|'
            wcardm += 'all(*.*)|*.*'
            filename=lib.GetFileName(self.mdlwin,wcardm,'r',True,defaultname=name)
            if len(filename) > 0: self.model.MergeToCurrent(filename)
        # data from database
        elif label == "PCCDB(access)":
            self.model.PCCDBAccess()
        elif label == "PCCDB(get sdf)":
            self.model.PCCDBGetSDF()
        elif label == "PubChem(access)":
            self.model.PubChemAccess()
        elif label == "PubChem(get smile)":
            #self.model.PubChemGetAtoms()
            self.model.PubChemGetSMILES()
        elif label == "RCSB PDB(access)":
            self.model.PDBAccess()
        elif label == "RCSB PDB(get pdb)":
            self.model.PDBGetPDB()
        # close molecule object
        ...
        #
            ...
            filename=lib.GetFileName(self.mdlwin,wcard,'w',True,
                                     defaultname=name)
            if len(filename) < 0: return
            self.model.SaveFiles(filename,False,False)
        elif label == "Save All":
            filename=""
            if len(filename) < 0: return
            self.model.MenuFiles(label,filename)
        # Save All As
        elif label == "One sdf/mol2/xyz file":
            wildcard='' #all(*.*)|*.*'
            filename=lib.GetFileName(self.mdlwin,wildcard,'w',False)
            if len(filename) < 0: return
            self.model.SaveAllOnOneFile(filename)
        elif label == "PDB files in a directory":
            wildcard='' #all(*.*)|*.*'
            savedir = lib.GetDirectoryName()
            if len(savedir) < 0: return
            filetype = "pdb files"
            self.model.SaveAllInDirectory(filetype,savedir)
    ...
    def OnEdit(self,label,bChecked):
        ...
        self.CheckMenu(label,bChecked)
        #
        if label == "Copy mol object":
            if not lib.IsMoleculeObj(self.model.mol): return
            self.model.CopyMolToClipboard(False)
        ...
        elif label == "in xyz form":  self.model.CopyMolXYZToClipboard()
        elif label == "in XYZ form": self.model.PasteMolXYZFromClipboard()
        elif label == "in sdf form":  self.model.CopyMolSDFToClipboard()
        elif label == "in SDF form": self.model.PasteMolSDFFromClipboard()
        elif label == "in mol2 form":  self.model.CopyMolMOL2ToClipboard()
        elif label == "in MOL2 form": self.model.PasteMolMOL2FromClipboard()
        elif label == "in pdb form":  self.model.CopyMolPDBToClipboard()
        elif label == "in PDB form": self.model.PasteMolPDBFromClipboard()
        elif label == "in SMILES": self.model.PasteSMILESFromClipboard()
        elif label == "XYZ/SDF in JSON object":
            self.model.PasteMolJSONFromClipboard()
    ...
    def OnShow(self,label,bChecked,drw=True):
        """
        ...
        if label != "Preference":
            ...
            if not ans: return
                ...
                self.model.mol.showdic[label]=bChecked
        # echo and log
        ...
        elif label == "Reset orientation": self.model.ResetOrientation()
        elif label == "Redraw":
            self.model.Redraw()
        elif label == "Hide selected": self.model.HideSelected(bChecked,drw=drw)
        elif label == "Hide environment": self.model.HideEnvironment(bChecked,drw=drw)
        #elif label == "Hide all peptide atoms":
        #    if not self.ctrlflag.ready: return #event.Skip()
        #    self.model.HidePeptideAtoms(bChecked)
        #    return
        elif label == "Hide hydrogen": self.model.HideHydrogen(bChecked,drw=drw)
        elif label == "Hide water": self.model.HideWater(bChecked,drw=drw)
        elif label == "Hide AA side chains": self.model.HideAASideChains(bChecked,drw=drw)
        elif label == "Hide all atoms": self.model.HideAllAtoms(bChecked,drw=drw)
        elif label == "Selected only": self.model.ShowSelectedOnly(bChecked,drw=drw)
        elif label == "Side chain only":
            self.model.ShowAASideChainOnly(bChecked,drw=drw)
        elif label == "Backbone only": self.model.ShowAABackboneOnly(bChecked,drw=drw)
        ...
        elif label == "Kite": self.model.DrawChainKite(bChecked)
        elif label == "Secondary structure": self.model.DrawChainRibbon(bChecked)
        ...
        elif label == "Group name": self.model.DrawLabelGroup(bChecked)
        ...
        elif label == "Apply courrent model to all":
            self.model.ApplyCurrentModelToAll()
        elif label == "Stereo":
            if bChecked: self.model.SetStereo(True)
            else: self.model.SetStereo(False)
        elif label == "xyz-axis": self.model.DrawXYZAxis(bChecked)
        elif label == "cell-box": self.model.DrawCellBox(bChecked)
        # Color submenu in Show Menu
            ...
            self.mdlwin.winctrl.Open(label)
        else:
    ...
    def OnWindow(self,label,bChecked):
        ...
        elif label == "Open MolIconTable":
            if bChecked: self.model.winctrl.Open(label)
            else: self.model.winctrl.Hide(label)
        elif label == "Open ImageSheet":
            if bChecked: self.model.winctrl.Open(label)
            else: self.model.winctrl.Hide(label)
        ...
        elif label == "Open MolIconWin":
            if not self.model.HaveOpenBabel(): return
    ...
    def OnModeling(self,label,bChecked):
        ...
        :param bool bChecked: True for checked, False for unchecked.
        """
        if not label in ["Open Model Builder","Open Peptide Builder"]:
            ans = lib.IsMoleculeObj(self.model.mol)
            if not ans: return
        # echo and log
        checkstr='True'
        ...
        elif label == "to non-polypeptide(use valence)":
            self.model.AddHydrogenUseBondMultiplicity()
        ...
        elif label == "Bonds between": self.model.DeleteBonds('between',True)
        ...
        elif label == "Molecule name ": self.model.ChangeMoleculeName()
        ...
        elif label == "Set std bond lengths to all":
            build.BuildMolFrom2D(self.model.mol,self.model)
            self.model.FitToScreen(True,True)
            mess='All bond lengths were reset to standard values.'
        ...
        elif label == "Open Crystal Builder":
            self.model.winctrl.Open('CrystalBuilder')
            #fu_crystal.CrystalBuilder_Frm(self.model)
            #build.CrystalBuilder_Frm(self.model)
        # not used anymore (16Aug2017)
        #elif label == "Change bonds in unit":
        #    self.model.ChangeBondsInUnitMol()
        # Edit conformation
        ...
        # "Box waters"
          ...
          # Update coordinates
        elif label == "Open MMMinimizer":
            self.model.winctrl.Open('MMMinimizer')
    ...
    def OnTools(self,label,bChecked):
        """
        ...
        if nc < 0:
            ...
            if not ans: return
        # echo and log
        self.EchoAndLog('Tools',label,bChecked)
        # check menu
        self.CheckMenu(label,bChecked)
        #
        ...
        elif label == "Non-planer peptide bonds": # "Peptide torsion angles":
            self.model.geom.PrintNonPlanerPeptideBonds()
        elif label == "Peptide phi and psi angles":
            self.model.geom.PrintPeptidePhiPsiAngles()
        ...
        #        except: pass
        # renumber
        elif label == "Reorder atoms":
            self.model.ReorderAtomsEachMols()
        # openbabel params
        #elif label == "OpenBabel params":
        #    self.model.ChangeOpenBabelParams()
        elif label == "ChemInfo tools":
            self.model.winctrl.Open('ChemInfoTools')

        elif label == "Generate isomers":
            self.model.GenerateIsomers()
        elif label == "Generate tautomers":
            self.model.GenerateTautomers()
        ...
        # others
        elif label == "Open DrawCubeWin":
            #if bChecked: self.model.winctrl.Open(label)
            #else: self.model.winctrl.Close(label)
            self.model.winctrl.Open(label)
        elif label == "Open DrawMOsWin":
            #if bChecked: self.model.winctrl.Open(label)
            #else: self.model.winctrl.Close(label)
            self.model.winctrl.Open(label)
        elif label == "Open PlotEnergyWin":
            self.model.winctrl.Open(label)
        elif label == "Open EasyPlotWin":
            ...
            if bChecked:
                win=subwin.ExecuteScript_Frm(self.mdlwin,winpos=[0,20])
                self.model.winctrl.SetOpenWin(label,win)
                self.model.winctrl.Open(label)
            else: self.model.winctrl.Close(label)
            ...
            mess=self.classnam+'(OnTools): '+'"'+label
            mess=mess+'" menu item is not defined'
            lib.MessageBoxOK(mess,"")
    ...
    def OnHelp(self,label,bChecked):
        ...
        elif label == "Bugs": self.model.HelpBugs()
    ...
    def OnHidden(self,label,bChecked):
        ...
        elif label == "Make hydrogen bond":
            self.model.ChangeBondMultiplicity(5)
        # Project
        ...
        elif label == 'RecreateMdlWin': # recreate MdlWin when fatal error occured
            self.model.RecreateMdlWin()
        elif label == 'WhoAmI':
        ...
        elif label == 'MDLWIN image to Image Sheet':
            #('label',label))
            self.model.ExportMDLWINImageToImageSheet()
        else:
    ...
    def FileMenu(self):
        ...
        menuitems= (
                  ...
                  ("","",False),
                  ("Public database", (
                      ("PCCDB(access)","Access to Pcublic Chem Databas",False),
                      ("PCCDB(get sdf)","Download molecule data in PCCDB",False),
                      ("","",False),
                      ("PubChem(access)","Access to PcubChem database",False),
                      ("PubChem(get smile)","Access to PcubChem database",False),
                      ("","",False),
                      ("RCSB PDB(access)","Access to PDB database",False),
                      ("RCSB PDB(get pdb)","Download PDB data",False))),
                  ("","",False),
                  ...
                  #("Save All As","Save all on one file(sdf or xyz)",False),
                  ("Save All as", (
                      ("One sdf/mol2/xyz file","one file",False),
                      ("PDB files in a directory","separate files",False)
                      )),
    ...
    def EditMenu(self):
        ...
        if self.fumode == 1: # fuplot mode
            ...
            menuitems= (
                      ...
                      ("","",False),
                      ("Copy mol string", (
                          ("in xyz form","Copy molecule xyz data to clipboard",False),
                          ("in sdf form","Copy molecule sdf data to clipboard",False),
                          ("in mol2 form","Copy molecule mol2 data to clipboard",False),
                          ("in pdb form","Copy molecule pdb data to clipboard",False))),
                      ("Paste mol string", (
                          ("in XYZ form","Paste molecule xyz data in clipboard",False),
                          ("in SDF form","Paste molecule sdf data in clipboard",False),
                          ("in MOL2 form","Paste molecule mol2 data in clipboard",False),
                          ("in PDB form","Paste molecule pdb data in clipboard",False),
                          ("in SMILES","Paste molecule pdb data in clipboard",False),
                          ("","",False),
                          ("XYZ/SDF in JSON object","Pasete json object string from clipboard",False))),
                      ("","",False),
    ...
    def ShowMenu(self):
        ...
        menuitems= (
                  ...
                  ("Reset orientation","Reset orientation",False),
                  ("Redraw","Redraw",False),
                  ("","",False),
                  ("Show", (
                  ...
                  ("Hide", (
                      ...
                      ("Hide AA side chains","Hide AA side chains",True),
                      ("","",False),
                  ...
                  ("Peptide chain", (
                      ...
                      #("", "", False),
                      #("Secondary structure", "Show peptide chain by ribbon", True),
                      #("Kite", "Show peptide main chain in kite train", True))),
                   ("Label", (
                      ...
                      ("Group name","show group name",True),
                      ("","",False),
                  ...
                  ("Molecular model", (
                      ...
                      ("CPK","CPK model",False),
                      ("","",False),
                      ("Apply courrent model to all","apply current model to all",False),
                      ("","",False),
        ...
        menuitems=(
                  ...
                  ("Add hydrogens", (
                      ...
                      ("to non-polypeptide(use bond length)",
                           "Add hydrogen to non-polypeptides and waters",False),
                      ("to non-polypeptide(use valence)",
                           "Add hydrogen to non-polypeptides and waters",False),
                      ("","",False),
                  ...
                  ("","",False),
                  ("Replace ligand", "Replace ligand",False),
                  #("Replace resudue", (
                  ...
                  ("Change", (
                      ...
                      ("","",False),
                      ("Bond length","",False),
                      ("Set std bond lengths to all",
                                        "Set standard bond lengthes",False),
    ...
    def ToolsMenu(self):
        ...
        menuitems= (
                  ...
                  ("Geometry", (
                       ...
                       ("", "", False),
                       ("Non-planer peptide bonds","Unfinished",False),
                       ("Peptide phi and psi angles", "Unfinished", False),
                       ("","",False),
                  ...
                  ("Reorder atoms","Reorder atoms in islolated molecules",False), # (
                  #     ("in isolated mols","atoms in each molecules",False),
                  #     ("","",False))), # to be added
                  ("","",False),                  #["Open ChildWin,"Chaild view window",True],
                  ("ChemInfo tools", ( #"OpenBabel/RDkit tools
                      ("Generate isomers","Generate isomers",False),
                      ("Generate tautomers","Generate tautomers",False))),
                  ("","",False),                  #["Open ChildWin,"Chaild view window",True],
                  #("Open DrawCubeWin","Open Cube Plot window",False),
                  #("Open DrawMOsWin","Open MO Plot window",False),
                  #("Open PlotEnergyWin","Open energy Plot window",False),
                  #("OpenBabel params","Set params for OpenBabel minimiz",False), # (
                  #("","",False),                  #["Open ChildWin,"Chaild view window",True],
                  #("","",False),                  #["Open ChildWin,"Chaild view window",True],
    ...
    def WindowMenu(self):
        ...
        menuitems= [
                  ...
                  ["","",False],
                  ["Open MolIconTable","Open molecule icons panel",True],
                  ["Open ImageSheet","Open image sheet",True],
                  ["","",False],
    ...
    def HiddenMenu(self):
        ...
        menuitems= (
                  ("Hidden", (
                      ...
                      ('', '', False),
                      ('RecreateMdlWin', 'recreate MdlWin,False'),
                      ('','',False),
    ...
    def PrependProjectMenu(self,prjnamlst,helplst):
        ...
        for i in range(len(prjnamlst)):
            ...
            if lib.IsWxVersion2():
                ...
                menuitem=submenu.AppendCheckItem(-1,label,helpString=help) 
            self.menulabeldic[label]=[menuitem,self.toplabel]
        filemenu.PrependMenu(-1,'Project',submenu)

    def PrependFileHistoryMenu(self,hisnamlst,helplst,itempos=0):
        """ Prepend project menu
        ...
        """
        self.toplabel='File'
        filemenu=self.menubar.GetMenu(itempos)
        submenu=wx.Menu()
        for i in range(len(hisnamlst)):
            label=hisnamlst[i]; help=helplst[i]
 
            if lib.IsWxVersion2():
                menuitem=submenu.Append(-1,label,help=help)
            else: menuitem=submenu.Append(-1,label,helpString=help)                 
            self.menulabeldic[label]=[menuitem,self.toplabel]
        submenu.AppendSeparator()
        label='Clear history'
        if lib.IsWxVersion2():
            menuitem=submenu.Append(-1,label,help='Clear history')
        else: menuitem=submenu.Append(-1,label,helpString='Clear history')
        self.menulabeldic[label]=[menuitem,self.toplabel]
        try: # delete the menu if exists
            item=filemenu.FindItem('File history')
            filemenu.DestroyItem(item)
        except: pass
        if lib.IsWxVersion2(): filemenu.PrependMenu(-1,'File history',submenu)
        else: filemenu.Prepend(-1,'File history',submenu)
            
    def xxRemoveMenuItem(self,toplabel):
    ...
    def CreateMenuFromList(self,toplabel,submenud):
        ...
        for item in submenud:
            if len(item) == 2:
                ...
                newid=wx.NewId()
                if lib.IsWxVersion2(): menu.AppendMenu(newid,label,submenu)
                else: menu.Append(newid,label,submenu)
    ...
    def CreateProjectMenuItems(self):
        wx.MessageBox('ctrl. CreateProjectMenuItem is called')
    ...
    def CreateFileHistoryMenuItems(self,itempos=0):
        ...
        self.PrependFileHistoryMenu(hisnamlst,helplst,itempos=itempos)
    ...
    def MakeMenuLabelDic(self):
        ...
        toplabel=['File','Edit','Show','Select','Modeling','Tools',
                  'FMO','Window','Hidden']
        menudmethod=[self.FileMenu,self.EditMenu,self.ShowMenu,self.SelectMenu,
                self.ModelingMenu,self.ToolsMenu,self.FMOMenu,self.WindowMenu,
                self.HiddenMenu]
        #
        self.menulabellst=[]
    ...
    def ListMenuItems(self,toplabel):
        ...
        itemlst=[]; itemdic={}
        if toplabel =='all':
            for label,lst in list(self.menulabeldic.items()):
                if not lst[1] in itemdic: itemdic[lst[1]]=[]
...
class MouseCtrl():
    ...
    def KeyDown(self,keycode):
        ...
        #elif keychar == 'x': # export MDLWIN image to Image Sheet
        #    print 'x key pressed',keychar
        #    self.model.menuctrl.OnHidden('MDLWIN image to Image Sheet',False)

        else:
            if keychar in shortcutkeydic:
                ...
                if topmenu in keydic: keydic[topmenu](label,check)
        return
        """ -------------------- """
        ...
        elif self.mdlmod == 3 and (keycode >= 48 and keycode <= 53): # 0-4: del bond, 1: add single, 2:double, 3:triple, 4:aromatic bonds
            print(('keycode,keycode-48',keycode,keycode-48))
...
class SettingCtrl():
    def __init__(self,parent,fuversion):
        ...
        # get 'FUSOURTHROOT'
        self.fusrcdir=self.initdic['progpath'] #'FUSOURCEPATH'
        ...
        self.fudirs=['Scratch','FUdata','Projects','Customize','Programs',
                     ...
                     'FUlog','FUlogfiles','Tests'] #,'Docs','Images','Draws']
        ...
        #self.prjfiles={}
        loggingmethodfile='FUlog//logging.method'
        ...
        self.envatmcolor=self.GetParam('env-atom-color')

    def ResetParamsForAtomObject(self):
        self.molmodel=self.GetParam('mol-model')
        self.bondthick=self.GetParam('bond-thick')
        self.bondthickbs=self.GetParam('bond-thick-bs')
        self.atomradius=self.GetParam('atom-sphere-radius')
    ...
    def InitialSetting(self):
        """ Set initial parameters in 'fumodel.ini' file"""
        self.progpath=self.initdic['progpath']
    ...
    def DefineIconFiles(self):
        ...
        def IconFile(name):
            icondir=os.path.join(self.progpath,'Icons')
    ...
    def MakeFrameDataDic(self):
        ...
        framedatdic={}
        for fil in frmfiles:
            resnam,natm,condat=rwfile.ReadFrameFile(fil)
            if resnam == "": continue
            if resnam in framedatdic:
                mess='MakeFrameDataDic: duplicate frame data. name="'+resnam+'"'
                self.SettingMessage(mess)
            framedatdic[resnam]=[natm,condat]
        self.framedatdic=framedatdic
        return framedatdic

    def SetFrameData(self,name,frmdat,replace=True):
        if not replace and name in self.framedatdic:
    ...
    def GetFrameData(self,name):
        if name in self.framedatdic: return self.framedatdic[name]
        else:
    ...
    def ChangeProject(self,prjnam):
        ...
        if not prjnam in self.prjdatdic:
    ...
    def ExecuteSettingScript(self,setfile):
        ...
        if retmess.find('Traceback') >= 0:
            ...
            text+='or execute and debug the script,\n'
            text+='>>> fum.ExecuteScript1("'+setfile+'")\n'
            text+='in the pyshell console.'
    ...
    def GetDefaultParam(self,prmnam):
        ...
        if not prmnam in default: return None
    ...
    def SetParam(self,name,value):
        ...
        if name == 'bond-thick-bs': self.bondthicbs = value
        ...
        if name == 'env-atom-color': self.envatmcolor=value
        if name == 'win-color':
            try:
                self.model.mdlwin.darw.ResetBGColor(value)
            except: pass
        #else:
    ...
    def DelShortcutKey(self,keycharacter):
        ...
        """
        if keycharacter in self.params['shortcut-key']:
    ...
    def SetDefaultParams(self):
        ...
        for prmnam,lst in list(table.items()): params[prmnam]=lst[0]
        return params

    def ListParams(self,filename=''):
        text='# Parameters in ctrl.SettingCtrl:\n'
        prms=list(self.params.keys())
        prms.sort()
    ...
    def ListSettingParams(self):
        def CountMax(tdic):
            maxchar=0
            for nam in tdic.keys():
                if len(str(nam)) > maxchar: maxchar=len(str(nam))
            return maxchar
        ...
        for prm in prms:
            if prm == 'mol-model':
                text+='# model-model: 0:line, 1:stick, 2:ball-and-stick, 3:CPK\n'
            if prm == 'element-color':
                text+='# element-color: element names are right-adjusted two uppercase letters\n'
                text+='# items with names other than elements are used internally\n'
            if prm == 'vdw-radius':
                text+='# vdw-radius: unknown values are set to arbitrary values\n'
            maxprm = CountMax(self.params)
            if lib.StringType(self.params[prm]) == 'dict':
                text += prm + '(dict):\n'
                maxnum=CountMax(self.params[prm])
                for sub in self.params[prm].keys():
                    text += '   '+ str(sub)+maxnum*' '+' = '+str(self.params[prm][sub])+'\n'
            #if lib.StringType(self.params[prm]) == 'list':
            #    text += prm + '(list):\n'
            #    for lst in self.params[prm]:
            #        text += '   ' + str(self.params[prm]) +'\n'
            else:
                text += prm + maxprm * ' '+ ' = ' + str(self.params[prm]) + '\n'
        return text

    def MakeParamTypeDic(self):
        paramtypedic={}
        table=self.TableOfParameters()
        for prmnam,lst in list(table.items()): paramtypedic[prmnam]=lst[1]
        return paramtypedic

    def MakeParamDefaultDic(self):
        paramdefaultdic={}
        table=self.TableOfParameters()
        for prmnam,lst in list(table.items()): paramdefaultdic[prmnam]=lst[0]
        return paramdefaultdic

    def MakeParamTipsDic(self):
        paramtipsdic={}
        table=self.TableOfParameters()
        for prmnam,lst in list(table.items()): paramtipsdic[prmnam]=lst[2]
        return paramtipsdic
    ...
    def TableOfParameters(self):
        ...
        :rtype: dic
        """
        # [default, type, tip_string, group#]
        # group # definition
        # 0:internal,1:general, 2:model, 3:font.image.message, 4:draw,
        # 5:shortcut key, 6: openbabel, rdkit, 7: misc
        params = {
        # general
        'platform':[self.platform,'str','plattform',-1],
        #'win-pos':[-1,-1], # window position
        'win-size':[[640,400],'size','main window (mdlwin) size',1],
        #'win-size-w':[640,'int','only for internal use',1],
        #'win-size-h':[400,'int','only for internal use',1],
        'shell-win-pos':[[],'size','pyshell window position',1],
        'shell-win-size':[[640,200],'size','pyshell window size',1],
        'win-color':[[0.0,0.0,0.0,1.0],'color',
                     'background color of main window (mdlwin)',1],
        'win-color-geom-mode':[[0.0,0.0,0.0,1.0],'color',
                               'background color of mdlwin in change geometry mode',1],
        'helpwin-bgcolor':[[0.99,1.0,0.81],'color',
                           'background color of help window',1],
        'helpwin-fgcolor':[[0.0,0.0,0.0],'color',
                           'foregrounf color of help window',1],
        'open-mouse-mode-win':[True,'bool',
                               'open mouse mode window at starting',1],
        'mousemode-win-shape':[0,'int',
                    'shape of mouse mode window. 0:horizontal, 1:vertical',1],
        'molchoice-win-shape':[0,'int',
                     'shape of mol choice window. 0:horizontal, 1:vertical',1],
        'open-mol-choice-win':[True,'bool',
                               'open mol choice window at starting',1],
        'pyshellwin-move-free':[False,'bool',
                                'pyshell win move together with mdlwin',1],
        'projects':[{},'dic','used to store project data. internal use only.',0],
        #'initial-project':'',
        #'initial-curdir':'',
        #'initila-setfile':'',
        #'curdir':'',
        ...
        'font':[0,'font',
                'use the "lib.GetFont(family,size)" to obtain font object',3],
        'text-font-size':[8,'int','font size'],
        'text-font':[0,'font',
                'use the "lib.GetFont(family,size)" to obtain font object',3],
        'text-font-size':[8,'int','font size',3],
        'text-font-color':[[0.0,0.0,0.0],'color','text color(black)',3], #black
        'text-message-color':[[1.0,1.0,0.0,1.0],
                              'color','text message color(yellow)',3],
        'text-message-fontstyle':[5,'int','text message fontstyle,see fu_draw',3],
        'image-format':['png','str','image data format to save canvas image',3],
        'image-bgcolor':[[1.0,1.0,1.0,1.0],
                         'color','background color in capture canvas image',3],
        'echo':[False,'bool','echo executed menu',0],
        'messageboxok':[True,'bool','enable/disable MessageBoxOK',3],
        'messageboxyesno':[True,'bool','enable/disable MessageBoxYesNo',3],
        'messageboxokcancel':[True,'bool','enable/disable MessageBoxOKCancel',3],
        'redirect':[True,'bool','redirect stdout and stderr to shell console',3],
        'warning':[True,'bool','worrning message',3],
        'dump-draw-items':[False,'bool',
                           'dump draw items when closing molecule',0], # dump draw items on close molecule
        'max-undo':[3,'int',
                    'max. number of undo in change geometry operations',0],
        'max-undo-zmt':[3,'int','max. number of undo in Z-matrix editor',0],
        'max-file-history':[10,'int','max. number of file history',0],
        'del-image-files':[True,'bool',
                           'delete files in "Images" folder without asking',0],
        'del-scratch-files':[True,'bool',
                'delete scratch files in "Scratch" directory at starting',0],
        'pycrust-shell':[False,
                         'bool','shell choice. True: PyCrust, False: PyShell',0],
        'fmo-menu':[True,'bool','activate "FMO" menu',0],
        'add-on1-menu':[[],'menu',
              'add-on menu item data given by "setting" script or "Customize"',0],
        'add-on2-menu':[[],'menu',
            'second add-on menu item data given by "setting" script"',0],
        'control-win-menu':[self.DefaultControlWinMenu(),
                            'method','menu items in "ControlWin"',-1],
        'tools':[False,'bool','Tools commands enable(True)/disabel(False)',0],
        'hide-mdlwin':[False,'bool','Hide mdlwin(main) window',0],
        # modeling
        'auto-add-hydrogens':[False,'bool',
            'enable add hydrogens to polypeptides at reading structure data',2],
        'auto-del-ters':[False,'bool',
                         'delete "TER"s in PDB data at reading file',2],
        'use-fmoutil':[True,
                       'bool','use fmoutil to add hydrogens to polypeptide',2],
        'tinker-atom-name':[False,'bool','change atom name for "TINKER"',2],
        'tinker-atom-order':[False,'bool','change atom order for "TINKER"',2],
        # atom params
        'mol-model':[0,'model,0:line,1:stick,2:ball-and-stick,3:CPK',
                     'molecular model. 0:line,1:stcik,2:ball-and-stick, 3:CPK',2],
        'element-color':[SettingCtrl.DefaultElementColor(),
                         'dic-color','element color dictionary',-2],
        'atom-sphere-radius':[0.3,'float','shere radius of atom',2],
        'vdw-radius':[SettingCtrl.DefaultVdwRadius(),
                      'dic-float','dictionary of van-der-waals radius',-2],
        'bond-thick':[1,'int',
                      'bond thickness in line, stick model',2],
        'bond-thick-bs':[0.5,'float',
                      'bond thickness in ball-and-stic model',2],
        # draw params
        'fog':[False,'bool','apply "Fog" effect',4], # fog
        'select-color':[[0.0,1.0,0.0,1.0],'color','color for selected atoms',4],
        'env-atom-color':[[0.68,0.02,0.64,1.0],
                          'color','color for environment atoms',4],
        'sel-box-color':[[0.0,1.0,0.0,0.3],
                         'color','color for box selected region',4],
        'sel-sphere-color':[[0.0,1.0,0.0,0.3],
                            'color','color for sphere selected region',4],
        'draw-message-font':[0,'int',
           'use the "GetBitmapFont(style)" staticmethod to obtain font object',4],
        'draw-message-color':[[0.0,1.0,0.0,1.0],
                              'color','color of messeges on glcanvas',4],
        'mode-message-color':[[1.0,1.0,1.0,1.0],
                              'color','text color for "mode message"',4], # mode message color
        'aa-chain-color':[SettingCtrl.DefaultAAChainColor(),
                          'dic-color','polypeptide chain color',-4],
        'aa-residue-color':[SettingCtrl.DefaultAAResidueColor(),
                            'dic-color','amino-acid residue color',-4],

        'draw-distance-line-thick':[1.0,'float','line thickness of drawdistance',4],
        'rot-axis-arrow-thick':[1,'int','thickness of rotation axis',4],
        'rot-axis-arrow-head':[0.2,'float','ratio of arrow head',4],
        'rot-axis-arrow-color':[[1.0,1.0,0.0,1.0],
                                'color','color for rotation-axis',4],
        'draw-torsion-angle':[False,'bool','draw torsion angle when axis rot',4],
        'beep-short-contact':[0.8,'bool','beep when short contact occured',0],
        'label-color':[[0.0,1.0,0.0],'color',
                       'label (elm,atmnam,resnam,..) color on glcanvas',4],
        'extra-bond-thick':[1,'int','thickness for extra bond',4],
        'hydrogen-bond-thick':[1,'int','thickness for hydrohgen-bond',4],
        'hydrogen-bond-color':[[0.0,1.0,1.0,1.0],'color','hydrogen-bond color',4], # cyan
        'vdw-bond-color':[[0.0,1.0,1.0,1.0],'color','color for vdw-contact',4], #cyan
        'stick-radius':[0.2,'float',
                        'radius for stick in "stik" and "ball-and-stic model',4],
        'ball-radius':[0.4,'float','ball radius for "ball-and-stick" model',4],
        'cpk-radius-scale':[1.0,'float','scale for vdw radius',4],

        'aa-tube-color':[[1.0,1.0,1.0,1.0],'color','color of polupeptide tube',4],
        'aa-tube-radius':[0.1,'float','radius of polypeptide tube',4],
        'calpha-line-thick':[1,'int','line thickness of CAlpha chain',4],
        'alpha-helix-color':[[1.0,0.0,0.0,1.0],'color','alpha-helix color',4],
        'beta-strand-color':[[0.0, 1.0, 1.0, 1.0], 'color', 'beta-strand color',4],
        'coil-color':[[0.8, 0.8, 0.8, 1.0], 'color',  'random coil color',4],
        'vib-vec-arrow-thick':[1,'int','thickness of arrow',4],
        'vib-vec-arrow-head':[0.25,'float','arrow head raitio',4],
        'vib-vec-arrow-color':[[1.0,1.0,0.0,1.0],'color','arrow color',4],
        'xyz-axis-thick':[1,'int','thickeness of xyz-axis line',4],
        'x-axis-color':[[1.0,0.0,0.6],'color','color of x-axis line',4],
        'y-axis-color':[[1.0,1.0,0.0],'color','color of y-axis line',4],
        'z-axis-color':[[0.0,1.0,1.0],'color','color of z-axis line',4],

        'cell-box-lines': [True,'bool','model of cell box, lines or solid',4],
        'cell-box-thick':[1,'int','thickeness of cell edge lines',4],
        'o-label-color':[[1.0,1.0,1.0],'color','color of origin label "o"',4],
        'a-axis-color':[[1.0,0.0,0.6],'color','color of a-axis line',4],
        'b-axis-color':[[1.0,1.0,0.0],'color','color of b-axis line',4],
        'c-axis-color':[[0.0,1.0,1.0],'color','color of c-axis line',4],
        'd-axis-color':[[0.8,0.8,0.8],'color','color of other box edges',4],

        'cube-line-thick':[0.5,'float',
                      'thickness of polygon lines, draw cube',4],
        'cube-mo-grid-size':[0.2,'float','grid size in Angs for mo cube data',4],
        'cube-mo-grid-margin':[2.0,'float','grid margin in Angs for mo cube data',4],
        'cube-mo-range':[[10,10],'lst','mo range in occ and vac orb numbers',4],

        'stereo-eye':[1,'int','stereo eye choice. 1:cross, 2:parallel',4],
        # shortcut key definition
        'custom-shortcut-key':[SettingCtrl.CustomShortcutKey(),
                            'method','dictionary data for shortcut key assign',-5],
        'reserved-shortcut-key':[SettingCtrl.ReservedShortcutKey(),
                            'method','dictionary data for shortcut key assign',-5],
        'shortcut-key':[SettingCtrl.ShortcutKey(),
                        'dic-int','dictionary data for shortcut key assign',-5],
        'zoom-speed':[10,'int','spped for zoom by key',5], # picxel
        'translate-speed':[2,'int','spped for translate by key',5], # pixcel
        # pdb-ftp-server to get monomer (frame) data
        'pdb-monomer-ftp-server':[['ftp.wwpdb.org','guest','',
             'pub/pdb/data/monomers'],'lst','pdb-ftp server to get frame data',0],
        'pdb-model-ftp-server':[['ftp.pdbj.org','anonymous','','model'],
                                'lst','pdb-ftp server to get model data',0],
        'pdb-site':['http://www.rcsb.org/pdb/home/home.do','str','URL of PDB',0],
        'check-pdb-atmnam':[True,'bool','Check atmnam of PDB is new?',2],
        # openbabel
        'ob-opt-method':['cg','str','minimization method in openbabel(OB)',6],
        'ob-forcefield':['uff','str','forcefiled in openbabel',6],
        'ob-opt-max-steps':[200,'int','max. steps in openbabel minimization',6],
        'ob-opt-crit':[0.01,'float','energy conv. crit. in OBminimization',6],
        # cheminfo tools
        'n-gen-confs':[10,'int','numnber of conformers to generate',6],
        # misc
        'output-filter':['','str','default filter file for reading output',0],
        'save-log':[False,'bool','save log file flag',0],
        'logfile-dir':[None,'str','logfile directory',0],
        'logging-method-file':[self.loggingmethodfile,'str',
                               'logging method definition file',0],
        'suppress-message':[False,'bool','suppress message',0],
        'message-level':[1,'int','Message level, 0(minimum)-5(maximum)',0],
        # fragmentation
        'split-option':[[0,1],'lst',
               'allow to split at option bonds in auto fragmentation',0],
        # MolIconTable
        'mol-icontable-mode':[1,'int','datawin in MolIconTable',0],
        'messbox_MMMinimizer':[True,'bool','enabel/disable "Update" messagebox',0],
        # variables
        'defaultfilename':['','str','default file name in ;ib.GetFileNmae',0],
        # constants
        ...
        'linux-html-viewer':['/usr/bin/firefox-esr','str','default html viewer on Linux',0],
        }
        return params
    ...
    def CustomShortcutKey():
        ...
        customkeydic={  'f':['Show','Fit to screen',False,'in all modes'], # in standard mode
                        ...
                        '5':['Hidden','Make hydrogen bond',False,
                             'in non "standard" mode'], # in mdlmod=3
                        ...
                        'x':['Hidden','MDLWIN image to Image Sheet',False,
                             'MDLWIN image to Image Sheet'],
                        }
        return customkeydic
    @staticmethod
    ...
    def GetElementColor(self,elm):
        return self.params['element-color'][elm]

    def SetElementColor(self,elm,color):
        self.params['element-color'][elm]=color
        self.elmcolordic = self.GetParam('element-color')
    ...
    def LoadParamSetFile(self,filename):
        """ Read the parameter set file in the Customize directory
        and update "self.params"(2020/12/08)

        :param str filename: the name of parameter set file,
                             xxx.general,xxx.model, xxx.others
        """
        ctmdir=self.GetDir('Customize')
        filename=os.path.join(ctmdir,filename)
        if not os.path.exists(filename):
            mess = '(LoadParamSetFile): file '
            mess = mess + '"' + filename + '" does not exist.'
            self.SettingMessage(self.classnam + mess)

        paramtypedic = self.MakeParamTypeDic()
        paramdic=SettingCtrl.ReadParamSetFile(filename,paramtypedic)

        self.params.update(paramdic)

        try:
            self.model.mdlwin.ResetBGColor(self.params['win-color'])
        except:
            pass

    def LoadGeneralParamSet(self,prmsetnam,messout=False):
        # prmsetnam='general' , 'model' or 'others'
        #paramtypedic=self.MakeParamTypeDic() #self.paramtypedic #SettingCtrl.ParamTypeDic()
        filename=self.MakeFullPathNameOfParamFile(prmsetnam,'.general')
        if not os.path.exists(filename):
        ...
        paramdic = rwfile.ReadParameterFile(filename)
        self.params.update(paramdic)

        self.paramsetgeneral=prmsetnam

        try:
            self.model.mdlwin.darw.ResetBGColor(self.params['win-color'])
        except:
            pass

        if messout:
            mess='(LoadGeneralParamSet): file '+'"'+filename+'" was loaded.'
            self.SettingMessage(self.classnam+mess)

    def LoadOthersParamSet(self,prmsetnam,messout=False):
        # setnam='others'
        #paramtypedic=self.MakeParamTypeDic() #self.paramtypedic #SettingCtrl.ParamTypeDic()
        filename=self.MakeFullPathNameOfParamFile(prmsetnam,'.others')
        if not os.path.exists(filename):
            mess='(LoadOthersParamSet): file '
            mess=mess+'"'+filename+'" does not exist.'
            self.SettingMessage(self.classnam+mess)
            return
        ...
        self.paramsetothers=prmsetnam

        if messout:
            mess='(LoadOthersParamSet): file '+'"'+filename+'" was loaded.'
            self.SettingMessage(self.classnam+mess)

    def LoadModelParamSet(self,prmsetnam,messout=False):
        #self.SettingMessage('---prmsetnam in LoadModelParamSet='+prmsetnam)
        # prmsetnam='model'
        ###paramtypedic=self.MakeParamTypeDic()
        filename=self.MakeFullPathNameOfParamFile(prmsetnam,'.model')
        #self.SettingMessage('---filename='+filename)
        if not os.path.exists(filename):
        ...
        paramdic=rwfile.ReadParameterFile(filename)
        if 'mol-model' in paramdic:
            try:
                paramdic['mol-model']=modeldic[paramdic['mol-model']]
            except: pass

        if 'element-color' in paramdic:
            addlst=[]
            dellst=[]
            for elm in paramdic['element-color']:
                if len(elm.strip()) == 1:
                    newelm=' '+elm
                    dellst.append(elm)
                    addlst.append([newelm,paramdic['element-color'][elm]])
            if len(dellst) > 0:
                for elm in dellst:
                    del paramdic['element-color'][elm]
            if len(addlst) > 0:
                for [elm,value] in addlst:
                    paramdic['element-color'][elm]=value

        self.params.update(paramdic)

        self.ResetParamsForAtomObject()

        self.paramsetmodel=prmsetnam

        if messout:
            mess='(LoadModelParamSet): file '+'"'+filename+'" was loaded.'
            self.SettingMessage(self.classnam+mess)

    def xxStripQuortOfName(self,paramdic):
        """  """
        changedic={}
        if 'element-color' in paramdic:
            for elm0 in paramdic['element-color'].keys():
            ...
            for elm0 in changedic.keys():
                elm=changedic[elm0]
                paramdic['element-color'][elm]=paramdic['element-color'][elm0]
                del paramdic['element-color'][elm0]
        return paramdic
    ...
    def GetParam(self,name):
        ...
        if name in self.params: value=self.params[name]
        else: value=None
        if name == 'font':
            if 'font-size' in self.params: size=self.params['font-size']
    ...
    def GetParamSetNameList(self):
        return [self.paramsetgeneral,self.paramsetmodel,self.paramsetothers]

    def GetTestFile(self):
        testfile=''
        if not 'test' in self.fufiles: return testfile
    ...
    def GetShortcutKeyChar(self,cmd):
        ...
        for keynam,lst in list(shortcutkeydic.items()):
    ...
    def GetMyParam(self,prmnam):
        ...
        """
        if prmnam in self.myparams: return self.myparams[prmnam]
        else: return ''
    ...
    def DelMyParam(self,prmnam):
        ...
        if prmnam == 'all': self.myparams={}
            ...
            if prmnam in self.myparams: del self.myparams[prmnam]
            else: pass
            ...
            for s in f.readlines():
                ...
                if len(items) >= 2:
                    ...
                    if items[0].strip() == 'winsize':
                        ...
                        sizex = int(sizexy[0])
                        if sizex < 0: sizex = 400
                        winsize.append(sizex)
                        sizey = int(sizexy[1])
                        if sizey < 0: sizey = 400
                        winsize.append(sizey)
    ...
    def xxReadParamSetFile(filename,paramtypedic):
        ...
        for s in f.readlines():
            ...
            if s.find('=') > 0:
                ...
                s=s[1:]; prmnam=s[:-1]
                prmnam, s=lib.SplitVariableAndValue(s)
                print('prmnam,value=',prmna,value)
                colordic={}
                continue
            else:
    ...
    def ReadParamSetFile(filename): #,paramtypedic):
        ...
        for s in f.readlines():
            ...
            if neq > 0: # variable=value
                ...
                print('s,neq,subnam,param,value=',neq,s,subnam,param,value)
            elif nrs >= 0: # ends of subgroup data
                subnam=None
                print('nrs,s,subnam=',nrs,s,subnam)
            elif nls >= 0: # begins of subgroup data
                subnam=s[:nls].strip()
                print('nls,s,subnam=',nls,s,subnam)
            elif nrg >= 0: # ends of group data
                subnam=None
                group=None
                print('nrg,s=',nrs,s)
            elif nlg > 0: # begins group data
                group=s[:nlg].strip()
                print('nlg,s=',nrs,s)
        f.close()
    ...
    def ReadAddonMenuFile(filename):
        print(('ReadAddonMenuFile is called file=', filename))
        addon=''; menulabel=''; menuitemlst=[]
        found=False
        print(('filename=',filename))
        with open(filename,"r") as f:
            textdat=f.read()
        print(('textdat=',textdat))
        f=open(filename,"r")
        for s in f.readlines():
            s=s.strip()
            if len(s) <= 0: continue
    ...
    def IsDfined(self,name):
        if name in self.fufiles: return True
        else: return False
    ...
    def GetFUPath(self):
        """ not used (use the GetFUDir method insted) """
        return self.fupath
    ...
    def ChangeScriptsDir(self,scriptdir):
        name='Scripts'
        if len(scriptdir) <= 0:
            mess='Directory name is null.'
            self.model.ConsoleMessage(mess)
            self.SettingMessage(self.classnam + '(ChangeScriptDir): ' + mess)
            return
    ...
    def GetFilesOfName(self,name):
        if name in self.fufiles: return self.fufiles[name]
        else: return []
    ...
    def GetNames(self):
        lst=[]
        for item,value in list(self.fufiles.items()): lst.append(item)
        return lst

    def DeleteFilesInDir(self,dirnam):
        nfil=len(self.fufiles[dirnam])
        if not dirnam in self.fufiles or nfil <= 0: return
    ...
    def GetParamSetFiles(self,paramsetnam):
        ...
        if not paramsetnam in filextdic:
...
class WinCtrl():
    ...
    def IsOpened(self,name):
        ...
        if not name in self.openwindic: return False
    ...
    def GetOpened(self):
        ...
        """
        windic={}
        for nam, win in list(self.openwindic.items()):
            if not win: continue
            windic[nam]=win
        return windic
    ...
    def GetWin(self,label):
        if label in self.openwindic: return self.openwindic[label]
        elif label == 'Show ToolsWin':
    ...
    def SetOpenWin(self,winlabel,winobj):
        """
        ...
        if not winlabel in self.openwindic:
            self.openwindic[winlabel] = winobj
    ...
    def Open(self,winlabel,openmethod=None):
        ...
        if self.IsOpened(winlabel):
            ...
            if win != None:
                #wx.MessageBox(winlabel + ' is already opened','winctrl.Open')
                try: win.Show()
                except: pass
                try: win.SetFocus()
                except: pass
        ...
        if openmethod:
        ...
        elif winlabel == 'Open MolIconTable':
            win=self.OpenMolIconTable(winlabel)
        elif winlabel == "Open ImageSheet":
            win=self.OpenImageSheet(winlabel)
        ...
        elif winlabel == 'Open DrawCubeWin':
            win=self.OpenDrawCubeWin(winlabel)
        elif winlabel == 'Open DrawMOsWin':

            winlabel,count = self.ModifyWinLabel(winlabel)
            if count > 1:
                #mess = winlabel + ' exceeded max. number=5'
                #wx.MessageBox(mess,'WinCtrl.Open')
                return

            win=self.OpenDrawMOsWin(winlabel,count=count)
        elif winlabel == 'Open PlotEnergyWin':
            win=self.OpenPlotEnergyWin(winlabel)
            ...
            win=custom.Customize_Frm(self.mdlwin,self.model,winpos=winpos,winlabel=winlabel)
        ...
        #    win=subwin.ModelBuilder_Frm(self.mdlwin,-1,winpos,winlabel)
        elif winlabel == "Preference":
            win=subwin.Preference_Frm(self.mdlwin)
            ...
            win=subwin.SlideShowCtrl_Frm(self.model)
        ...
        #    win=self.OpenMutateAAResWin(winlabel)
            ...
            win=build.HisFormChanger_Frm(self.model,-1,winlabel=winlabel)
        ...
        elif winlabel == 'CrystalBuilder':
            win = build.CrystalBuilder_Frm(self.model)
        elif winlabel == 'MMMinimizer':
            win = mol_minimize.MMMinimizer_Frm(self.model)
        #elif winlabel == 'LatticeParamPanel':
         #   win=build.LatticeParam_Frm(self.model)
        # Utility
        elif winlabel == 'SplitJoin':
            win=build.SplitAndJoinPDBFile_Frm(self.model,-1)
        elif winlabel == 'ChemInfoTools':
            win = cheminfo.ChemInfoTools_Frm(self.model,-1,winlabel=winlabel)
        #
        elif winlabel == 'Tests':
            pass
    ...
    def OpenMolIconTable(self,winlabel):
        """ Open Molecule icons window.

        :param str winlabel: window label-'MolIconWin'
        """
        mode = self.setctrl.GetParam('mol-icontable-mode')
        if mode == 0: datawin = True
        else: datawin = False
        win = subwin.MolIconTable_Frm(self.mdlwin,datawin=datawin)
        win.Show()
        return win

    def OpenImageSheet(self,winlabel):
        """ Open Image sheet window. """
        win = subwin.ImageSheet_Frm(self.mdlwin)
        win.Show()
        return win
    ...
    def ModifyWinLabel(self,winlabel):
        countlst = [0]
        count = 0
        newwinlabel = winlabel
        if self.IsOpened(winlabel):
            for label,obj in list(self.openwindic.items()):
                winname,winnum = lib.SplitNumberInString(label)
                if winnum is not None: countlst.append(winnum)
            count = max(countlst) + 1
            newwinlabel = winlabel + '-' + str(count)
        return newwinlabel,count

    def OpenPlotEnergyWin(self,winlabel):
        win = cube.PlotEnergy_Frm(self.mdlwin,self.model,
                 norbdat=1,
                 orbtitle=None,erange=None) #,model,ctrlflag,molnam,winpos):
        win.Show()
        return win

    def OpenDrawMOsWin(self,winlabel,count=None):
        mode = 2 # no menu mode
        pos0 = 0
        ...
        win = cube.DrawCubeData_Frm(self.mdlwin,-1,winpos,[],self.model,
                                    self.mdlwin,mode,winlabel=winlabel,
                                    wincount=count) # mode=1
        win.Show()
        return win

    def OpenDrawCubeWin(self,winlabel):
        mode=0 # no menu mode
        mdlwinpos=self.mdlwin.GetPosition()
        mdlwinsize=self.mdlwin.GetClientSize()
        ...
        if mode == 1: winsize[1] -= 25 # no menu in the case of mode=1
        win = cube.DrawCubeData_Frm(self.mdlwin,-1,winpos,[],self.model,
                                    self.mdlwin,mode,winlabel=winlabel) # mode=1
        win.Show()
        return win
    ...
    def OpenEasyPlotWin(self,winlabel):
        ...
        winpos=[0,20]; winsiz=[750,300]
        title='Plot Output'
        win=subwin.ExecProg_Frm(self.mdlwin,-1,self.model,winpos,winsiz,title,
                                winlabel)
        # remove several menu items
        win.menubar.Remove(3) # remove 'status' menu items
        #win.menubar.Remove(4) # help
        ...
        for txt in itemtxt:
            ...
            if lib.IsWxVersion2(): menu.DeleteItem(item)
            else: menu.Delete(item)
        items=menu.GetMenuItems()
        nsep=4; isep=0
        for item in items:
            if item.IsSeparator():
                #py3#menu.RemoveItem(item); isep += 1
                menu.Remove(item);
                isep += 1
                if isep == nsep: break
        #
    ...
    def OpenTextEditor(self,winlabel):
        ...
        winpos=[0,20] #wx.GetMousePosition()
        winsize=[480,300]
        parent=self.mdlwin
...
class MolCtrl():
    ...
    def GetMolByName(self,molnam):
        molobj = None
        if molnam in self.molnamlst:
            molidx = self.GetMolIndex(molnam)
            if molidx >= 0: molobj = self.GetMol(molidx)
        return molobj
...
class HelpCtrl(object):
    ...
    def GetTutorialContents(self,helpname):
        ...
        if helpname in self.cntfiledic:
            cntfile=self.cntfiledic[helpname]
            contentslst,textdic,htmldic,scriptdic,demodic= \
        ...
        for i in range(len(contentslst)):
            ...
            if name[:1] == '@':
                tname=name[1:]
                if not tname in self.cntfiledic: continue
    ...
    def SetFullPathNameInFileDic(self,helpdir,filedic):
        ...
        """
        for name,file in list(filedic.items()):
