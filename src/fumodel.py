...
sys.path.insert(0,os.path.abspath('./fubin'))
sys.path.insert(0,'./')
...
try:
    import matplotlib
    const.MATPLOTLIB=True
except:
    const.MATPLOTLIB=False
...
try:
    import pubchempy
    const.PUBCHEMPY = True
except: pass

@lib.CLASSDECORATOR(lib.FUNCCALL)
class Model_Frm(wx.MiniFrame):
    """ main program of FU """
    def __init__(self,parent,progpath,fumode,*args,**kwargs):
        ...
        self.initdic=self.SetInitialParms(progpath)
        # check FUDATASET
        fudir=self.initdic['FUDATASET']
        """
        if not os.path.isdir(fudir):
            text = 'Input directory of "FUDATASET-0.5.0'
            fudwin = lib.GetDirFromUser(self,retmethod=self.FUDATASETFromUser,
                               ismodal=True,title='FUDATASET directory',
                               caption=text,default=fudir)
            fudir = self.initdic['FUDATASET']
        """
        if not os.path.isdir(fudir):
            mess='Not found the runtime data set "FUDATASET-0.5.0"'
            mess=mess+'. Unable to continue.\n'
            mess=mess+'Set the FUDATASET directory in the "fu-0.5.1//fumodel.ini" file.'
            wx.MessageBox(mess,'Moldel_Frm',wx.OK)
            self.Quit()
        #
        ...
        self.messchecker=lib.MessageChecker(self)
        #
    ...
    def FUDATASETFromUser(self,dirname):
        self.initdic['FUDATASET'] = dirname
        print(('fudatasetdir',dirname))

    def SetInitialParms(self,progpath):
        """ read paramters in fumodel.ini file """
        initdic={}
        inifile=progpath+'//fumodel.ini'
        if not os.path.exists(inifile):
            if platformos.system().upper() == 'WINDOWS':
                initdic['FUDATASET']='~//FUDATASET'
            else:
                initdic['FUDATASET']=lib.ExpandUserFileName('~//FUDATASET-0.5.0')
        else: initdic=rwfile.ReadIniFile(inifile)
        initdic['progpath']=progpath
    ...
    def OpenMdlWin(self,parent,winpos=[],winsize=[]):
        ...
        if len(winsize) <= 0:
            ...
            if len(winsize) <= 0: winsize = [0,0]
            if winsize[0] < 400: winsize[0]=500
            if winsize[1] < 100: winsize[1]=500
        if len(winpos) <= 0:
            winpos=self.setctrl.winpos
        ...
        #project#self.mdlwin.SetProjectNameOnTitle(self.curprj)
        ...
        #project#wx.MessageBox('fumodel. self.fcurprj='+self.curprj)
        self.menuctrl.CheckMenu("Enable MessageBox",True)
        const.EXECMENU=self.menuctrl.ExecMenu
        self.mdlwin.defaultpopupmenu = self.menuctrl.PopupMenu
        self.mdlwin.defaultpopupmenuhandler = self.menuctrl.OnPopupMenu
        self.mdlwin.SetPopupMenu()
        # Text buffer (On memory file)
        self.textbuff=lib.TextBuffer(self.mdlwin,self)
        # check fortran modules
        self.CheckFortranModules()
        # check cheminfo modules
        self.CheckChemInfoModules()
        # help controller
    ...
    def CheckFortranModules(self):
        # fortlib
        self.ConsoleMessage('')
        self.ConsoleMessage('# Imported Fortran modules:')
        if not importfortlib:
            mess='Warning: Fortran module "fortlib" is not imported.'
        else: mess='"fortlib" is imported.'
        self.ConsoleMessage(mess)
        # fmopdb
        if not importfmopdb:
            mess='Warning: Fortran module "fmopdb" is not imported.'
        else: mess='"fmopdb" is imported.'
        self.ConsoleMessage(mess)
        # fucubelib
        if not importfucubelib:
            mess='Warning: Fortran module "fucubelib" is not imported.'
        else: mess='"fucubelib" is imported.'
        self.ConsoleMessage(mess)
        # comp_mo
        if not importcompmo:
            mess='Warning: Fortran module "comp_mo" is not imported.'
        else: mess='"comp_mo" is imported.'
        self.ConsoleMessage(mess)

    def CheckChemInfoModules(self):
        # OpenBabel
        self.ConsoleMessage('')
        self.ConsoleMessage('# Imported ChemInfo modules:')
        if not const.OPENBABEL:
            mess = 'Warning: "OpenBabel" is not installed.'
        else:
            mess = '"OpenBabel" release version '+openbabel.OBReleaseVersion()+' is imported.'
        self.ConsoleMessage(mess)
        # pybel
        if not const.PYBEL:
            mess = 'Warning: "pybel" is not installed.'
        else:
            mess = '"pybel" is imported.'
        self.ConsoleMessage(mess)
        # RDKit
        if not const.RDKIT:
            mess = 'Warning: "RDKit" is not installed.'
        else:
            mess = '"RDKit" version '+rdkit.__version__+' is imported.'
        self.ConsoleMessage(mess)
    ...
    def Quit(self):
        ...
        """
        try: const.CONSOLEMESSAGE('Quit is called')
        except: pass
    ...
    def ExitModel(self):
        ...
        """
        try:
            if not self.winctrl.IsOpened('Open PyShell'):
                mess = 'Please open PyShell before quit.'
                wx.MessageBox(mess,'Model.ExitModel')
                return
        except: pass

        try: self.WriteIniFile()
        except: pass
        #
        try:
    ...
    def WriteIniFile(self):
        ...
        if self.curdir == '': self.curdir=self.setctrl.GetFUDir()
        if lib.GetPlatform() != "WINDOWS":
    ...
    def WriteProjectIniFile(self):
        ...
        #project#print 'inifile in WriteProjectIniFile',inifile
        f=open(inifile,"w")
    ...
    def MessAtomLabel(self,intval,boolbal):
        pass
    ...
    def xxWarningMess(self,classnam,methodnam,mess):
        mess='Warning('+self.classnam+'('+methodnam+'):'+mess
        return mess

    def WarningMess(self,classnam,methodnam,mess,once=True):
        if not classnam in self.self.warningdic:
            self.self.warningdic[classnam] = {}
        if not methodnam in self.self.warningdic[classnam]:
            self.warningdic[classnam][methodnam] = 0
        self.warningdic[classnam][methodnam] += 1
        if once and self.warningdic[classnam][methodnam] > 1:
            mess=''
        else:
            mess = 'Warning(' + self.classnam + '(' + methodnam + '):' + mess
        return mess
    ...
    def TextMessage(self,mess,color=[],drw=True,fontstyle=None):
        ...
        if len(mess) > 0:
            ...
            drwlst=[]
            drwlst.append([mess,fontstyle,'left',20,'top',50,color])
    ...
    def TextMessage1(self,messlst,color=[],drw=True,fontstyle=None):
        """ Write message on canvas.
        ...
        if len(messlst) > 0:
            ...
            drwlst=[]
            top = 25
            for i in range(len(messlst)):
                top += 18
                drwlst.append([messlst[i],fontstyle,'left',20,'top',top,color])
            self.draw.SetDrawData(drwlabel,'MESSAGE',drwlst)
    ...
    def ExecuteAddOnScript(self,script,bChecked):
        ...
        if not os.path.exists(scriptfile):
            ...
            #py3#method='execfile('+"'"+scriptfile+"'"+')'
            self.scriptcheck=bChecked
            # run shell
            #py3#pyshell.shell.run(method)
            print(('scriptfile=',scriptfile))
            method='exec(open('+"'"+scriptfile+"'"+').read())'
    ...
    def ExecuteToolsScript(self,script):
        ...
        # run shell
        pyshell.shell.run(method)
        #pyshell.shell.runfile(scriptfile)
    ...
    def DrawPlotData(self,funcnam):
        ...
        if on:
            if item == 1: # fragment
                for i in range(len(pltcol)):
                    for j in range(len(self.mol.atm)):
    ...
    def SetUpDraw(self,fit,messout=True):
        self.mousectrl.Reset()
        self.menuctrl.UncheckAll()
        
        """
        try: self.menuctrl.UncheckAll()
        except: 
            wx.MessageBox('fumodel.SetUpDraw self.menuctrl.Uncheck='+str(self.menuctrl))
            pass
        """
        on=self.setctrl.GetParam('messageboxok')
        try: self.menuctrl.CheckMenu("Enable MessageBoxOK",on)
        except: 
            wx.MessageBox('fumodel.SetUpDraw self.menuctrl.Check='+str(self.menuctrl))
            pass
        if self.setctrl.GetParam('save-log'):
            self.menuctrl.CheckMenu('Save log',True)
        try: self.menuctrl.CheckMenu(self.curprj,True)
        except: pass
        ...
        self.Message('',1,'')
        # draw molecule
        if messout: self.MsgNumberOfAtoms(1)
        if fit:
    ...
    def DumpDrawItems(self,drwfile=''):
        ...
        drwdic['distance']=self.ctrlflag.Get('Distance')
        print(('Strore distance=', str(drwdic['distance'])))
        #
        f=open(drwfile,'wb')
        pickle.dump(drwdic,f)
        f.close()
        #
    ...
    def RestoreDrawItems(self,drwfile=''):
        ...
        if not os.path.exists(drwfile):
            mess='Draw pickle file, '+drwfile+' is not found.'
            self.ConsoleMessage(mess); return
        f=open(drwfile,'rb')
        drwitemdic=pickle.load(f)
        f.close()
        #
        ...
        try:
            self.ctrlflag.Set('Distance',drwitemdic['distance'])
            print(('Restrore distance=',str(drwitemdic['distance'])))
        except: pass
    ...
    def ChangeBackgroundColor(self,rgb,logging=False):
        ...
        if self.mdlwin.textpan.IsOpen(): self.mdlwin.textpan.RecoverText()
    ...
    def ChangeBondMultiplicity(self,bndknd):
        ...
        4:aromatic bonds
        """
        def GetAtm1And2(name):
            nsel,lst=self.ListSelectedAtom()
            if nsel == 2:
                atm1=lst[0]; atm2=lst[1]
            else:
                mess=name+'Select two atoms then hit number key.'
                self.Message(mess,0,'')
                atm1=-1
                atm2=-1
            return atm1,atm2
        ...
        name=str(bndknd)+' bond: '
        atm1,atm2 = GetAtm1And2(name)
        if atm1 < 0: return
        ...
        self.TextMessage('[Make Bond]: '+mess,[]) # [1,1,0]) #'yellow')
        self.mousectrl.pntatmhis.Clear()
        # is the corrent mol a pack object?
        if self.IsCrystalObject(): self.ChangeBondsInUnitMol()
    ...
    def SetBondKind(self,atm1,atm2,bndknd):
        ...
        if kind > 4:
            ...
            if (bnd1 < 0 or bnd2 < 0) or bndknd not in bonddic:
    ...
    def FindBondSeqNmb(self,atm1,atm2):
        nseq=-1
        for i in range(len(self.mol.atm[atm1].conect)):
    ...
    def SelectNCofCutAA(self,term='NC',drw=True):
        self.SetSelectAll(False)
        for atom in self.mol.atm:
            nb=len(atom.conect)
            atm=atom.atmnam
    ...
    def SelectMissingAtoms(self,selflg=True):
        ...
        for atom in self.mol.atm:
            ...
            if res in resdic: atom.select=True
        self.DrawMol(True)
        # !!! new one
        ...
        for atom in self.mol.atm:
            ...
            if res in resdic: atom.select=resdic[res]
    ...
    def SetSelectAtoms(self,atmlst,selflg):
        ...
        for atom in self.mol.atm:
            if atom.atmnam in atmdic: atom.select=selflg
    ...
    def FindConnectedGroup(self,onlynatm=False):
        ...
        if onlynatm:
            ...
            isol    = [x for x in netx.isolates(G)]
    ...
    def FindTerminalRes(self,term,targetlst):
        ...
        for i in targetlst:
            atom=self.mol.atm[i]
            resnam=atom.resnam; resnmb=atom.resnmb; chain=atom.chainnam
            if resnam not in const.AmiRes3: continue
            resdat=lib.PackResDat(resnam,resnmb,chain)
            if resdat in resdic: resdic[resdat].append(i)
            else: resdic[resdat]=[]; resdic[resdat].append(i)
        if len(resdic) > 0:
            for resdat,lst in list(resdic.items()):
                for i in lst:
                    atom=self.mol.atm[i]
                    if term == 'NH3':
    ...
    def MakeBondList(self):
        ...
        natm = len(self.mol.atm)
        for i in range(natm):
            conlst = self.mol.atm[i].conect
            for j in range(len(conlst)):
                ia = conlst[j]
                ii  = min(i,ia)
                jj  = max(i,ia)
                print(('i,bndmulti',i,self.mol.atm[i].bndmulti))
                typ = self.mol.atm[i].bndmulti[j]
    ...
    def CountNumberOfResidues(self):
        ...
        for atom in self.mol.atm:
            resdat=lib.ResDat(atom)
            if resdat in resdic: continue
            resdic[resdat]=True
        return len(resdic)

    def CountResidues(self,trgatmlst=[],messout=True):
        if len(trgatmlst) <= 0:
            trgatmlst=list(range(len(self.mol.atm)))
        resdic = {}
        done = {}
        nres = 0
        #for atom in self.mol.atm:
        for i in trgatmlst:
            atom=self.mol.atm[i]
            resdat = atom.resnam + str(atom.resnmb)
            if resdat in done: continue
            done[resdat] = True
            resnam = atom.resnam
            if resnam not in resdic:resdic[resnam] = 0
            resdic[resnam] += 1
            nres += 1
        if messout:
            mess  = 'nres = ' + str(nres) +';  ' + str(resdic)
            self.ConsoleMessage(mess)
        return nres,resdic

    def CountGroup(self,groupname=None):
        ngrp = 0
        donelst = []
        ...
        for atom in self.mol.atm:
            ...
            if groupname is not None:
                if grpi != groupname: continue
            if grpi in donelst: continue
            donelst.append(grpi)
            nmb=atom.resnmb
            ngrp += 1
        return ngrp
    ...
    def CountResidue1(self,res,allatm):
        ...
        nres=0
        if allatm: target=list(range(len(self.mol.atm)))
    ...
    def MakeBondedAtomGroupList(self,lst):
        ...
        grplst=[]
        if len(lst) <= 0:
            for i in range(len(self.mol.atm)): lst.append(i)
    ...
    def SetDrawDistance(self,on):
        ...
        if on:
            ...
            self.TextMessage('[Show:Distance]',[]) #[1.0,1.0,0.0])
    ...
    def MoleculeInfo(self,itemlst=[],outfile='',viewout=True,prthisform=True,messout=False):
        # count
        if len(itemlst) <= 0:
        ...
        for item in itemlst:
            ...
            if item == "Number of electrons":
                ...
                for i in range(len(self.mol.atm)):
                    ...
                    if atom.elm == 'XX': nter += 1
                        ...
                        try:
                            nele+=const.ElmNmb[atom.elm]
                        except:
                            wx.MessageBox('Unkown element='+atom.elm,'MoleculeInfo')
                mess='Total number of electrons of neutral molecule='+str(nele)+"\n"
                text=text+mess
            ...
            if item == 'Residues':
                ...
                for resdat in reslst:
                    ...
                    if res not in resdic: resdic[res]=1
                    else: resdic[res] += 1
                    if chain not in chaindic: chaindic[chain]={}
                    if res not in chaindic[chain]: chaindic[chain][res]=1
                ...
                text=text+'\n'
                # HIS kind(HID,HIE,HIP)
                if prthisform and 'HIS' in resdic:
                    hidlst,hielst,hiplst,misclst=self.ListHisForm()
                    text+='His form: nhid='+str(len(hidlst))+', nhie='+str(len(hielst))+', nhip='+str(len(hiplst))+'\n'
                    # hisformlst [[hiddat,hisfrom],...]
                    if (len(hidlst)) > 0:
                        text+='   HID='+str(hidlst)+'\n'
                    if (len(hielst)) > 0:
                        text += '   HIE=' + str(hielst) + '\n'
                    if (len(hiplst)) > 0:
                        text += '   HIP=' + str(hiplst) + '\n'
                    if len(misclst) > 0:
                        text += '  misc=' + str(misclst) + '\n'
                # chain
                mess=''
                chainlst=list(chaindic.keys())
                text=text+'\n'
                ...
                if len(chaindic) > 1:
                    ...
                    for i in range(len(chainlst)):
                        ...
                        resnamlst=list(chaindic[chainnam].keys())
    ...
    def ListHisForm(self):
        hidlst=[]
        hielst=[]
        hiplst=[]
        misclst=[]
        hisdatlst=self.ListResidue4('HIS')
        for hisdat in hisdatlst:
            hisatmlst=self.ListResDatAtoms(hisdat)
            hd1=False
            he2=False
            for i in hisatmlst:
                atom=self.mol.atm[i]
                if atom.atmnam.strip() == 'HD1':
                    hd1=True
                if atom.atmnam.strip() == 'HE2':
                    he2=True
            if hd1 and he2:
                hiplst.append(hisdat)
            elif hd1:
                hidlst.append(hisdat)
            elif he2:
                hielst.append(hisdat)
            else:
                misclst.append(hisdat)
        return hidlst,hielst,hiplst,misclst
    ...
    def SummaryOfAddHydrogens(self):
        ...
        if len(watlst) > 0:
            ...
            for atom in self.mol.atm: #range(len(watlst)):
                ...
                resdat=lib.ResDat(atom)
                if resdat not in atmnmbdic: atmnmbdic[resdat]=0
                atmnmbdic[resdat] += 1
            for resdat,nmb in list(atmnmbdic.items()):
    ...
    def DrawHBOrVdwBond(self,on,atmlst=[],addto=False,drw=True):
        ...
        if on:
            ...
            extdat=self.MakeDrawExtraBond(atmlst,within=False)
            if len(extdat) <= 0:
                allatms=list(range(len(self.mol.atm)))
                extdatall=self.MakeDrawExtraBond(allatms)
                if len(extdatall) <= 0:
    ...
    def MakeDrawExtraBond(self,atmlst=[],within=True):
        def CheckDone(donedic,i,j):
            done=False
            ii=min(i,j); jj=max(i,j)
            idx=str(ii)+':'+str(jj)
            if idx in donedic: done=True
            return done
        ...
        extlst=[]; donedic={}
        thick = self.setctrl.GetParam('hydrogen-bond-thick')
        col = self.setctrl.GetParam('hydrogen-bond-color')
        for i in target:
            if not self.mol.atm[i].show: continue
            cc0=self.mol.atm[i].cc
    ...
    def ListHydrogenBond(self):
        ...
        def IsHB(i,j,hdic):
            ...
            for k in range(nh):
                ia,ja,r = hdic[k]
                lst = [ia,ja]
                if i in lst and j in lst: ans = True
            return ans
        ...
        for i in range(natm):
            atomi = self.mol.atm[i]
            ci = atomi.cc
            for j in atomi.extraconect:
                if IsHB(i,j,hdic): continue
                k += 1
                cj = self.mol.atm[j].cc
                r2 = (ci[0]-cj[0])**2 + (ci[1]-cj[1])**2 + (ci[2]-cj[2])**2
                r = numpy.sqrt(r2)
                hdic[k] = [i,j,r]
        nbnd = len(hdic)
        self.Message2("Number of hydrogen bonds found = " + str(nbnd))
        fi4='%4d'; ff63='%6.3f'; fi3='%03d'
        mess =  '\n       #    atom#(name:res)   atom#(name:res)'
        mess += '   r\n' #       ang\n'
        for i in range(len(hdic)):
            iii,jjj,r = hdic[i]
            resi = self.mol.atm[iii].resnam
            resi += fi3 % self.mol.atm[iii].resnmb
            resj = self.mol.atm[jjj].resnam
            resj += fi3 % self.mol.atm[jjj].resnmb
            labi = self.mol.atm[iii].atmnam + ':' + resi
            labj = self.mol.atm[jjj].atmnam + ':' + resj
            mess += '    ' + (fi4 % (i+1)) + '  ' + (fi4 % (iii+1))
            mess += '(' + labi +') '
            mess += (fi4 % (jjj+1)) + '(' + labj +')'
            mess +=  (ff63 % r) + '    ' + '\n' #(ff6 % ang) + '\n'
        self.ConsoleMessage(mess)
    ...
    def MsgAtomDistance(self,atm0,atm1):
        ...
        if self.ctrlflag.Get('Mode(on/off)'):
            if self.ctrlflag.IsDefined(drwlabel):
                ...
                if ia in distancedic:
                    for j in range(len(distancedic[ia])):
                        if distancedic[ia][j] == []:
            ...
            self.draw.SetDrawData(drwlabel,'LABEL',drwlst)
            self.draw.SetDrawData(drwlabel,'LINE',drwline)
            self.TextMessage('[Show:Distance] '+mess2,[]) # [1.0,1.0,0.0])
            self.DrawMol(True)
    ...
    def MakeDrawDistanceData(self):
        ...
        stipple=[2,0xcccc]
        thick=0.5
        thick=self.setctrl.GetParam('draw-distance-line-thick')
        distancedic=self.ctrlflag.Get(drwlabel)
    ...
    def ListGroup(self,atmlst=[]):
        """ Retrun group name and the first atom sequence number

        :return: grouplst - [[group name,sequence number of the first atom],..]
        """
        lst=atmlst
        if len(lst) <= 0: lst=self.ListTargetAtoms()
        grouplst=[]; done={} # [name0,number0],[name1,numbe1],...]
        prv=""; nmb=0
        for i in lst:
            atom=self.mol.atm[i]
            if atom.elm == 'XX': continue
            ...
            grouplst.append([group,i]); done[group]=True
        #
        return grouplst
    ...
    def ListResidue3(self,reskind,target='all'):
        """

        :param str reskind: 'all','aa', 'non-aa', or 'water'
        """
        if target == 'selected': lst=self.ListTargetAtoms()
        else: lst=list(range(len(self.mol.atm)))
        reslst=[]; tmpdic={}
        watlst=['HOH','WAT','H2O']
        #for atom in self.mol.atm:
        for i in lst:
            ...
            resnam=atom.resnam; resnmb=atom.resnmb; chain=atom.chainnam
            #if resnam == '   ': continue
            if reskind == 'aa' and resnam not in const.AmiRes3: continue
            elif reskind == 'non-aa' and resnam in const.AmiRes3:
                continue
            elif reskind == 'water' and not resnam in const.WaterRes: continue
            resdat=lib.ResDat(atom)
            if resdat in tmpdic: continue
            tmpdic[resdat]=True
            reslst.append(resdat)
        return reslst

    def ListResidueAtom(self,resdat):
        """ Retuen list of atoms in resdat

        :param resdat: packe dresdiue name(="resnam:resnmb:chain")
        :return: atmlst(str) - [seq#1, seq#2,...]
        """
        atmlst=[]
        for i in range(len(self.mol.atm)):
            res=lib.ResDat(self.mol.atm[i])
            if res == resdat:
                atmlst.append(i)
        return atmlst
    ...
    def ListAAResidueAtoms(self,lst=[]):
        ...
        for i in atmlst:
            ...
            if res in const.AmiRes3: aaresatm.append(i)
        return aaresatm
    ...
    def ListNonAAResidueAtoms(self,wat=False,atmlst=[]):
        # wat: True for NOn-AA res, False: not
        if len(atmlst) <= 0: atmlst=list(range(len(self.mol.atm)))
        lstatm=[]
        for i in atmlst:
            atom=self.mol.atm[i]; res=atom.resnam
            if res in const.AmiRes3: continue
    ...
    def ReportMissingAtomsInAAResidue(self):
        ...
        if nres <= 0:
            ...
            for res,atmlst in list(missingatmlstdic.items()):
                mess=mess+res+'\n'
                mess=mess+str(atmlst)+'\n'
        self.ConsoleMessage(mess)
    ...
    def ListMissingAtomsOfAAResidue(self):
        ...
        for i in range(len(reslst)): #chain loop
            ...
            for res,nmb in lst: # res loop
                ...
                resatm=list(const.AmiResBnd[res].keys())
    ...
    def ListCationicMetals(self):
        ...
        for i in range(len(self.mol.atm)):
            ...
            if elm == '': continue
            if elm == 'XX': continue
            if elm in const.CationChg: ionlst.append(i)
        return ionlst
    ...
    def ListAnionicMetals(self):
        ...
        for i in range(len(self.mol.atm)):
            ...
            if elm == '': continue
            if elm == 'XX': continue
            if elm in const.AnionChg: ionlst.append(i)
        return ionlst

    def ListElement(self,uniq=True):
        elmdic={}; elmlst=[]
        nknd=0
        for atom in self.mol.atm:
            elm=atom.elm
    ...
    def ListAtomName(self):
        ...
        for atom in self.mol.atm:
            ...
            if atm == '': continue
            if elm == 'XX': continue
            if atm not in atmdic:
                nknd += 1; atmdic[atm]=nknd
        if len(atmdic) > 0:
            atmlst=list(atmdic.keys()); atmlst.sort()
    ...
    def ListResidueName(self):
        ...
        if len(resdic) > 0:
            reslst=list(resdic.keys()); reslst.sort()
        return reslst
    ...
    def ListUniqueResidueAndNumbers(self):
        ...
        for atom in self.mol.atm:
            ...
            if resdat not in tmpdic: tmpdic[resdat]=1
        for resdat,dum in list(tmpdic.items()):
            res,nmb,chain=lib.UnpackResDat(resdat)
            if res in resdic: resdic[res] += 1
            else: resdic[res]=1
        return resdic
    ...
    def ListChainName(self):
        ...
        for atom in self.mol.atm:
            cha=atom.chainnam
            if cha == '': continue
            if cha not in chadic:
                nknd += 1; chadic[cha]=nknd
        if len(chadic) > 0:
            chalst=list(chadic.keys()); chalst.sort()
        return chalst
    ...
    def ListPeptideChainAtoms(self,lst):
        ...
        for i in lst:
            ...
            if resnam in const.AmiRes3: reslst.append(i)
        if len(reslst) <= 0:
    ...
    def ListGroupName(self):
        ...
        for atom in self.mol.atm:
            grp=atom.grpnam
            if grp == '': continue
            if grp not in grpdic:
                nknd += 1; grpdic[grp]=nknd
        if len(grpdic) > 0:
            grplst=list(grpdic.keys()); grplst.sort()
        return grplst
    ...
    def ListResidueAtoms(self,trgatmlst=[]):
        if len(trgatmlst) <= 0:
            trgatmlst=list(range(len(self.mol.atm)))
        # frgatmlst:[[0,1,2,...].[8,9..],...]
        fi3='%03d'
    ...
    def ListResDatAtoms(self,resdat,nameonly=False):
        # frgatmlst:[[0,1,2,...].[8,9..],...]
        atmlst=[]
        for atom in self.mol.atm:
            if atom.elm == 'XX': continue
            res=lib.ResDat(atom)
            if nameonly:
                if res[:3] == resdat: atmlst.append(atom.seqnmb)
            else:
    ...
    def SelectElmNam(self,elmnam,selflg,messout=True):
        if elmnam == '': return
        nsel=0
        for atom in self.mol.atm:
            i=atom.seqnmb
            ...
            if elm == elmnam:
                nsel += 1; self.SetSelectedAtom(i,selflg)
        mess='Select element(s): '+elmnam+'\n'
        mess+='  Nnumber of selected items='+str(nsel)
        if messout: self.ConsoleMessage(mess)
    ...
    def SelectAtmNam(self,atmnam,selflg,messout=True):
        if atmnam == '': return
        nsel=0
        for atom in self.mol.atm:
            i=atom.seqnmb
    ...
    def SetSelectResNam1(self,resnam,selflg,all):
        # all: True for all, False for selected
        if resnam == '': return
        if all: target=list(range(len(self.mol.atm)))
        else: target=self.ListTargetAtoms()
        #
        nsel=0
        for i in range(len(self.mol.atm)):
    ...
    def SetSelectResDat(self,resdatdic,selflg):
        ...
        for atom in self.mol.atm:
            res=atom.resnam
            resdat=lib.ResDat(atom)
            if res in resdatdic: resres=res
            elif resdat in resdatdic: resres=resdat
            else: continue
    ...
    def GetFragmentAttribute(self,attrib):
        attriblst=[]
        if attrib in self.mol.fragattribdic:
    ...
    def SaveAtomModel(self,on):
        if on:
            savatmmdl = []
            for atom in self.mol.atm: savatmmdl.append(atom.model)
            self.ctrlflag.Set('savatmmdl',savatmmdl)
        else:
            savatmmdl=self.ctrlflag.Get('savatmmdl')
            for i in range(len(self.mol.atm)):
                self.mol.atm[i].model = savatmmdl[i]
    ...
    def SetSection(self,on):
        ...
        #color = self.setctrl.GetParam('text-message-color')
        if on: self.TextMessage('[Select:Section]',[])
    ...
    def DrawSection(self):
        ...
        if sectionz < rasposminz:
            ...
            sectionz=rasposmaxz
            self.ctrlflag.Set('sectionz',sectionz)
            self.Message('Section reached maximum.',0,'')
            return
        mess='Section position: '+(ff6 % secpos)
        self.Message(mess,0,'')
        # draw message
        drwtext=[]
        ...
        #color = self.setctrl.GetParam('text-message-color')
        self.TextMessage('[Select:Section] '+text,[]) #color)
    ...
    def RemoveAllLabels(self,drw=True):
        ...
        for label,value in list(labeldic.items()): labeldic[label]=False
        #
    ...
    def ResetOrientation(self,drw=True):
        # Reset rotation
    ...
    def HideSelected(self,on,drw=True):
        value=on
        if value:
            na=0
            for atom in self.mol.atm:
    ...
    def HideAASideChains(self,on,trgatmlst=[],drw=True):
        if len(trgatmlst) <= 0:
            trgatmlst=self.ListTargetAtoms()

        backbones=['N','H','CA','C','O']

        if not on:
            for atom in self.mol.atm:
                if atom.elm == 'XX': continue
                res = atom.resnam
                if res in const.AmiRes3:
                    if not atom.atmnam.strip() in backbones:
                        atom.show = True
            self.menuctrl.CheckMenu('Hide AA side chains', False)
        else:
            nside=0
            aareslst=[]
            for i in trgatmlst:
                ...
                if atom.elm == 'XX':
                    continue
                if atom.resnam in const.AmiRes3:
                    if atom.atmnam.strip() in backbones:
                        atom.show =True
                    else:
                        atom.show=False
                        nside+=1
            if nside <= 0:
                self.Message('No AA residue side chains',0,'black')
                return
            self.menuctrl.CheckMenu('Hide AA side chains', True)
            self.Message('Hide AA side chains',1,"black")
    ...
    def ShowAllAtom(self,drw=True):
        ...
        self.FitToScreen(False,drw=False)
    ...
    def DrawCubeData(self,on,property=None,cubefile=None,value=None,
               interpol=None,style=None,rgbpos=None,rgbneg=None,opacity=None):
        """ Draw cube data
    ...
    def DrawCellBox(self,on,drw=True,bulkbox=False,edges=False):
        """ Draw cell box """

        linebox = self.setctrl.GetParam('cell-box-lines')
        if not linebox:
            self.DrawRectangle(on,drw=True,edges=False)
            return

        if self.mol.boxpnts is None:
            mess  = 'The current molecule does not have box data.'
            #lib.MessageBoxOK(mess,'model.DrawCellBox')
            self.Message2(mess)
            return

        drwlabel='cell-box'
        if on:
            drwlst=self.MakeDrawCellBoxData()

            self.draw.SetDrawData(drwlabel,'CELLBOX',drwlst)
            self.SetDrawItemsAndCheckMenu(drwlabel,True)
    ...
    def DrawRectangle(self,on,edges=False,drw=True):
        ...
        drwlabel='rectangle'
        if on:
            drwlst=self.MakeDrawRectangleData(edges=edges)

            self.draw.SetDrawData(drwlabel,'RECTANGLE',drwlst)
            self.SetDrawItemsAndCheckMenu(drwlabel,True)
    ...
    def MakeDrawRectangleData(self,pnts=None,color=None,edges=False):
        """
        order of pnts,
        pnt nmb     0     1     2     3     4     5     6     7
        labels = ['000','100','010','001','110','011','101','111']
        """
        drwlst = []
        if color is None: color = [0.0,1.0,0.0,0.2] # green
        if pnts is None: pnts = self.mol.boxpnts

        ocolor = self.setctrl.GetParam('o-label-color')
        acolor = self.setctrl.GetParam('a-axis-color')
        bcolor = self.setctrl.GetParam('b-axis-color')
        ccolor = self.setctrl.GetParam('c-axis-color')

        surface = []
        #surface.append([pnts[6],pnts[7],pnts[4],pnts[1],color]) # front
        surface.append([pnts[1],pnts[4],pnts[7],pnts[6],color]) # front
        #surface.append([pnts[4],pnts[7],pnts[5],pnts[2],color]) # right side
        surface.append([pnts[2],pnts[5],pnts[7],pnts[4],color]) # right side
        #surface.append([pnts[1],pnts[0],pnts[3],pnts[6],color]) # left size
        surface.append([pnts[6],pnts[3],pnts[0],pnts[1],color]) # left size
        surface.append([pnts[0],pnts[3],pnts[5],pnts[2],color]) # back

        #surface.append([pnts[3],pnts[5],pnts[7],pnts[6],color]) # top
        surface.append([pnts[6],pnts[7],pnts[5],pnts[3],color]) # top
        #surface.append([pnts[0],pnts[1],pnts[4],pnts[2],color]) # bottom
        surface.append([pnts[2],pnts[4],pnts[1],pnts[0],color]) # bottom

        #
        #drwlst.append(surface)
        drwlst.append(surface)
        #drwlst.append(color)

        drwlst.append(['o','a','b','c'])
        drwlst.append([pnts[0],pnts[1],pnts[2],pnts[3]])
        drwlst.append([ocolor,acolor,bcolor,ccolor])
        if edges:
            edgelst = [[pnts[0],pnts[1]],[pnts[0],pnts[2]],[pnts[0],pnts[3]],
                       [pnts[1],pnts[4]],[pnts[1],pnts[6]],[pnts[2],pnts[4]],
                       [pnts[2],pnts[5]],[pnts[3],pnts[6]],[pnts[3],pnts[5]],
                       [pnts[7],pnts[4]],[pnts[7],pnts[5]],[pnts[7],pnts[6]]]
            thick  = self.setctrl.GetParam('cell-box-thick')
            ocolor = self.setctrl.GetParam('o-label-color')
            acolor = self.setctrl.GetParam('a-axis-color')
            bcolor = self.setctrl.GetParam('b-axis-color')
            ccolor = self.setctrl.GetParam('c-axis-color')
            dcolor = self.setctrl.GetParam('d-axis-color') # other edges
            collst = [acolor,bcolor,ccolor,dcolor,dcolor,dcolor,
                      dcolor,dcolor,dcolor,dcolor,dcolor,dcolor]
            drwlst.append(edgelst)
            drwlst.append(collst)

        return drwlst


    def MakeDrawCellBoxData(self):
        """
        order of pnts,
        labels = ['000','100','010','001','110','011','101','111']
        """
        drwlst = []
        pnts   = self.mol.boxpnts
        edgelst = [[pnts[0],pnts[1]],[pnts[0],pnts[2]],[pnts[0],pnts[3]],
                   [pnts[1],pnts[4]],[pnts[1],pnts[6]],[pnts[2],pnts[4]],
                   [pnts[2],pnts[5]],[pnts[3],pnts[6]],[pnts[3],pnts[5]],
                   [pnts[7],pnts[4]],[pnts[7],pnts[5]],[pnts[7],pnts[6]]]
    ...
    def MakeDrawXYZAxisData(self):
        ...
        #thick=1;
        thick  = self.setctrl.GetParam('xyz-axis-thick')
        xcolor = self.setctrl.GetParam('x-axis-color')
        ycolor = self.setctrl.GetParam('y-axis-color')
        zcolor = self.setctrl.GetParam('z-axis-color')
        #
    ...
    def MakeAtomCCForDraw(self,lst):
        if self.mol is None: return []
        if len(lst) <= 0: lst=list(range(len(self.mol.atm)))
        atmcc=[]
        for i in lst:
    ...
    def MakeDrawBondData(self,lst):
        ...
        cc0b,cc1b(lst): coordinates of connected atoms to i and j, respectively.
            the data is used to  for drawing multiple bond.
                ...
                if visiblei and visiblej:
                    ...
                    if self.ctrlflag.Get(drwlabel): # for multiple bond
                        ...
                        for k in range(len(atom.conect)):
                            kk = atom.conect[k]
                            if kk != jx and atom.bndmulti[k] == 4:
                                ixx = kk
                                cc0b=self.mol.atm[kk].cc; break
                    seqnmb1=jx
                    visible1=visiblej
                    cc1=atomjx.cc
                    col=atomjx.color
                    ...
                    if self.ctrlflag.Get(drwlabel): # for multiple bond
                        ...
                        if len(atomjx.conect) > 0:
                            for k in range(len(atomjx.conect)):
                                kk = atomjx.conect[k]
                                if kk != i and atomjx.bndmulti[k] == 4:
                                    jxx = kk
                                    cc1b=self.mol.atm[kk].cc; break
    ...
    def MakeDrawTubeData(self,trgatmlst=[],tuberadius=None,colordic={}):
        """

        :param trgatmlst:
        :param tuberadius:
        :param tubecolordic: {'alpha-helox':[1.0,0.0,0.0,1.0], 'beta-strand':[i,j,k],
                              'random-coil:[]}
        :return:
        """
        ...
        reslst=[]
        colorlst=[]
        # ahelixlst=[[resdati-1,resdati,resdati+1],...]
        ahelixlst, bstrandlst, otherlst = self.geom.SecondaryStructureOfProtein(trgatmlst=[], amergin=20.0,
                                                                                bmergin=20.0)
        """
        #colordic=self.SecondaryStructureColor()
        if colordic.has_key('helix'):
            helixcolor=colordic['helix']
        else: helixcolor=self.setctrl.GetParam('alpha-helix-color')
        if colordic.has_key('strand'):
            strandcolor=colordic['strand']
        else: strandcolor=self.setctrl.GetParam('beta-strand-color')
        if colordic.has_key('coil'):
            coilcolor=colordic['coil']
        else:
            coilcolor=self.setctrl.GetParam('coil')
        """
        helixcolor,strandcolor,coilcolor=self.SecondStructureColor1(colordic=colordic)


        helixlst=[]
        strandlst=[]
        for lst in ahelixlst:
            if not lst in helixlst:
                helixlst.append(lst[1])
        for lst in bstrandlst:
            if not lst in strandlst:
                strandlst.append(lst[1])

        if len(trgatmlst) <= 0:
            trgatmlst=self.ListTargetAtoms()
        for i in trgatmlst:
            atom=self.mol.atm[i]
            res=atom.resnam.strip()
            if res not in const.AmiRes3: continue
            atm=atom.atmnam.strip()
        ...
        #
        #if tubecolor is not None:
        #    color=tubecolor
        #    if tubecolor is None:
        #        color=self.setctrl.GetParam('aa-tube-color') #self.mol.tube_color

        rad=tuberadius
        if tuberadius is None:
        ...
        for i in reslst:
            ...
            if chain != prvchain:
                ...
                #if tubecolor is None:
                if resdat in helixlst:
                    color = helixcolor
                elif resdat in strandlst:
                    color = strandcolor
                else:
                    color = coilcolor
                wt=1.0 #0.01 #1.0
                cc=atom.cc
                ###if atm == ' CA ': wt=0.1 #1.0 #0.1
                res.append([cc,numpy.array(color),wt,rad])
        if len(res) > 0: chaindata.append(res)
        #
        return chaindata

    def MakeDrawCAlphaData(self,trgatmlst=[],linethick=None,linecolor=None):
        # lst(lst): target atom list. if [], all atoms are targetted.
        if linethick is None:
        ...
        for i in trgatmlst:
            ...
            if atm != 'CA': continue
            if chainnam not in chaindic: chaindic[chainnam]=[]
        ...
        ic=-1
        for chain,calst in list(chaindic.items()):
            ic += 1; chainlst=[]
            if ic >= len(colordic): ic=0
            color=colordic[ic]
            if linecolor is not None: color=linecolor
    ...
    def MakeDrawChainRibbonData(self,trgatmlst=[],linethick=2,linecolor=None,oneline=True):
        def MakeChainList(lstlst,structlstlst):
            pntlst = []
            colorlst=[]
            thicklst=[]
            j=-1
            for lst in lstlst:
                ...
                for i in range(len(lst) - 1):
                    ...
                    cci = atomi.cc[:]
                    ccj = atomj.cc[:]
                    pntlst.append([cci,ccj])
                    try: colorlst.append(colordic[structlstlst[j][i]][0])
                    except: colorlst.append(colordic['coil'][0])
                    thicklst.append(2)
            return pntlst,colorlst,thicklst

        def IsResDat(resdat, lstlst):
            ans = False
            for lst in lstlst:
                if resdat in lst:
                #if resdat == lst[1]:
        ...
        if linethick is None: linethick=2
        ahelixlst, bstrandlst, otherlst = self.geom.SecondaryStructureOfProtein(trgatmlst=[], amergin=20.0,
                                                                                bmergin=20.0)
        colordic=self.SecondaryStructureColor()
        ...
        for i in trgatmlst:
            ...
            chainnam = atom.chainnam
            if chainnam not in chaindic: chaindic[chainnam] = []
            if not resdat in chaindic[chainnam]:
                chaindic[chainnam].append(resdat)
            nres += 1
        if nres <= 0: return []
        #
        #colordic = self.setctrl.GetParam('aa-chain-color')  # {1:color1,2:color2,...}
        helixcolor=colordic['helix'][0]
        strandcolor=colordic['strand'][0]
        coilcolor=colordic['coil'][0]
        ic = 0
        line0lstlst = []
        line1lstlst = []
        line2lstlst = []
        line3lstlst = []
        structlstlst = []
        #colorlst = [colordic[0], colordic[1], colordic[3], colordic[5]]

        findnamlst=['N','C','H','O']
        resdic = {}
        strandresdic = {}
        for chain, resdatlst in list(chaindic.items()):
            ic = 0
            chainlst = []
            line0lst=[] # N
            line1lst=[] # C
            ...
            for resdat in resdatlst:
                if resdat not in resdic:
                    ic += 1
                if ic < 2: continue
                resdic[resdat] = ic
                [ian,iac,iah,iao]=self.FindAtomNameAtomInResidue(findnamlst,resdat=resdat)
                # alpha-helix residue
                if IsResDat(resdat, ahelixlst):
                    ibs = 0
                    if ian >= 0:
                        line0lst.append(ian)
                        structlst.append('helix')
                    if iac >= 0: line1lst.append(iac)
                    if iah >= 0: line2lst.append(iah)
                    if iao >= 0: line3lst.append(iao)
                # beta-strand residue
                elif IsResDat(resdat, bstrandlst):
                    struct = 'strand'
                    if resdat not in strandresdic:
                        ibs += 1
                        strandresdic[resdat] = ibs
                    if ibs == 1:
                        if iac >= 0:
                            line0lst.append(iac)
                            structlst.append(struct)
                            line1lst.append(iac)
                            line2lst.append(iac)
                            line3lst.append(iac)
                            continue
                    if (ibs % 2) == 0: # even number
                        if ian >= 0:
                            line0lst.append(ian)
                            structlst.append(struct)
                        if iac >= 0:
                ...
                # randam coil residue
                    ...
                    if iac >= 0:
                        line0lst.append(iac)
                        structlst.append('coil')
                        line1lst.append(iac)
                        line2lst.append(iac)
                        line3lst.append(iac)
            #
            structlstlst.append(structlst)
            line0lstlst.append(line0lst)
            line1lstlst.append(line1lst)
            line2lstlst.append(line2lst)
            line3lstlst.append(line3lst)

        if oneline:
            lines = [line0lstlst]
        else:
            lines = [line0lstlst, line1lstlst, line2lstlst, line3lstlst]

        for lstlst in lines:
            pntlst,colorlst,thicklst = MakeChainList(lstlst,structlstlst)
            # linedata: [pnts, color, thick, stipple]
            for i in range(len(pntlst)):
                ribbondata.append([pntlst[i],colorlst[i],thicklst[i],[]])

        return ribbondata

    def PeptidePlanes(self,trgatmlst=[]):
        """ Return atoms conposing peptide plane


        :param trgatmlst:
        :return: pepatmlst(lst) - [[atmseq1,atmseq2,atmseq3,atmseq4,chain#],,,]
                                   atmseqx is sequence number of atoms,
                                   x=1 CA(i) x=2 H(i), x=3 CA(i+1), and X=4 O(i)
        """
        pepatmlst=[]
        chaindic = {}
        ...
        olstlst = []
        reslstlst=[]
        # rectangle composed of CA-H-CA(i+1)-O
        ic=0
        for chain, atmlst in list(chaindic.items()):
            ic = 0
            #if ic == 1: continue
            calst = []
            hlst = []
            ...
            chainlst = []
            resdic={}
            if len(atmlst) <= 1: continue
            for i in range(len(atmlst)):
                atm = self.mol.atm[i].atmnam.strip()
                resdat=lib.ResDat(self.mol.atm[i])
                if resdat in resdic:
                    pass
                else:
                    ic+=1
                    ...
                    [ica,io,ih]=self.FindAtomNameAtomInResidue(findnamlst,resdat=resdat,alth=True)
                    if ica >= 0:
                        calst.append(ica)
                        reslst.append(resdat)
                    if io >= 0: olst.append(io)
                    if ih >= 0: hlst.append(ih)
            #
            calstlst.append(calst)
            hlstlst.append(hlst)
            olstlst.append(olst)
            reslstlst.append(reslst)

        for i in range(len(calstlst)):
            for j in range(len(calstlst[i])-1):
                # rectangle composed of CA(j)-H(j)-CA(j+1)-O(j)
                try:
                    p1 = calstlst[i][j]
                    p2= hlstlst[i][j]
                    p3= calstlst[i][j+1]
                    p4 = olstlst[i][j]
                    resdat=reslstlst[i][j]
                except: continue
                pepatmlst.append([p1, p2, p3, p4,i,resdat])
        return pepatmlst

    def SecondaryStructureColor(self,colordic={},noback=False):
        # strand
        if 'strandfrontcolor' in colordic:
            strandfrontcolor =colordic['strandfrontcolor']
        else:
            strandfrontcolor = [0.0 / 256.0, 255.0 / 256.0, 255.0 / 256.0, 1.0]
        if 'strandbackcolor' in colordic:
            strandfrontcolor =colordic['strandbackcolor']
        else:
            strandbackcolor = [0.0 / 256.0, 100.0 / 256.0, 100.0 / 256.0, 1.0]
        # helix
        if 'helixfrontcolor' in colordic:
            helixfrontcolor =colordic['helixfrontcolor']
        else:
            helixfrontcolor = [1.0,0.0,0.0, 1.0]
        if 'helixbackcolor' in colordic:
            helixbackcolor =colordic['helixbackcolor']
        else:
            helixbackcolor = [120.0 / 256.0, 0.0, 0.0, 1.0]
        # coil
        if 'coilfrontcolor' in colordic:
            coilfrontcolor =colordic['coilfrontcolor']
        else:
            coilfrontcolor = [0.8, 0.8, 0.8, 1.0]
        if 'coilbackcolor' in colordic:
            coilbackcolor =colordic['coilbackcolor']
        else:
            coilbackcolor = [100.0 / 256.0, 100.0 / 256.0, 100.0 / 256.0, 1.0]

        if not noback:
            structcolordic={'helix' : [helixfrontcolor,helixbackcolor],
                            'strand': [strandfrontcolor,strandbackcolor],
                            'coil'  : [coilfrontcolor, coilbackcolor] }
        else:
            structcolordic={'helix' : [helixfrontcolor,helixfrontcolor],
                            'strand': [strandfrontcolor,strandfrontcolor],
                            'coil'  : [coilfrontcolor, coilfrontcolor] }

        return structcolordic

    def SecondStructureColor1(self,colordic={}):
        """

        :param struct: 'helix','strand',or 'coil'
        :param colordic: {'helix':[frontcolor,backcolor], 'strand':[,],'coil':[,]}
        :return:
        """
        if 'helix' in colordic:
             helixcolor=colordic['helix']
        else:
            helixcolor=self.setctrl.GetParam('alpha-helix-color')

        if 'strand' in colordic:
            strandcolor=colordic['strand'][0]
        else: strandcolor=self.setctrl.GetParam('beta-strand-color')
        ...
        if 'coil' in colordic:
            ...
            coilcolor=self.setctrl.GetParam('coil-color')

        return helixcolor,strandcolor,coilcolor

    def MakeDrawChainKiteData(self, trgatmlst=[],frontcolordic={},backcolordic={}):
        """
        ...
        def SetColor(struct):
            if struct in frontcolordic:
                color0 = frontcolordic[struct]
            else:
                color0=None

            if struct in backcolordic:
                color1 = backcolordic[struct]
            else:
                color1=color0
            return color0,color1

        kitedata=[]
        pepatmlst=self.PeptidePlanes(trgatmlst=trgatmlst)
        # secondary structure color
        ahelixlst, bstrandlst, otherlst = self.geom.SecondaryStructureOfProtein(trgatmlst=[], amergin=20.0,bmergin=20.0)
        #colordic=self.SecondaryStructureColor(noback=noback)
        helixcolor0, strandcolor0, coilcolor0=self.SecondStructureColor1(colordic=frontcolordic)
        helixcolor1, strandcolor1, coilcolor1 =self.SecondStructureColor1(colordic=backcolordic)

        for p1,p2,p3,p4,i,resdat in pepatmlst:
            struct=WhichStrcuture(resdat)
            if struct == 'helix':
                color0=helixcolor0
                color1 = helixcolor1
            elif struct == 'strand':
            ...
            colorlst=[color0,color1] # front color,backcolor

            # rectangle composed of CA(j)-H(j)-CA(j+1)-O(j)
            atomp1 = self.mol.atm[p1]
            atomp2 = self.mol.atm[p2]
            atomp3 = self.mol.atm[p3]
            atomp4 = self.mol.atm[p4]
            cc1 = atomp1.cc[:]
            cc2 = atomp2.cc[:]
            cc3 = atomp3.cc[:]
            cc4 = atomp4.cc[:]
            kitedata.append([cc1, cc2, cc3, cc4, colorlst])
    ...
    def MakeDrawFormalChargeData(self):
        ...
        for i in lst:
            if not self.mol.atm[i].show: continue
            if self.mol.atm[i].frgchg is not None:
    ...
    def MakeDrawLabelResData(self,drwlabel):
        ...
        for i in lst:
            ...
            snmb = str(nmb).strip()
            rnam = sres + snmb
            aa   = res in const.AmiRes3
            if rnam in donedic: continue
            if aa:
    ...
    def MakeDrawLabelElmData(self,drwlabel,atmlst=[],nohydrogen=True,nocarbon=True):
        """ drwlabel(str): dummy """
        lst=atmlst
        if len(lst) <= 0: lst=self.ListTargetAtoms()
        nelm=0; drwlst=[]
        ...
        for i in lst:
            ...
            if nohydrogen and elm == 'H': continue
            if nocarbon and elm == 'C': continue
            lab=elm
            drwlst.append([lab,None,pos,color])
            nelm += 1
        return nelm,drwlst
    ...
    def DrawLabelGroup(self,on,drw=True):
        drwlabel='Group name'
        if on:
            nchain,drwlst=self.MakeDrawLabelGroupData(drwlabel)
            if nchain > 0:
                # set draw data
                self.SetDrawItemsAndCheckMenu(drwlabel,True)
                self.draw.SetDrawData(drwlabel,'LABEL',drwlst)
    ...
    def MakeDrawLabelGroupData(self,drwlabel,atmlst=[]):
        """ drwlabel(str): dummy """
        lst=atmlst
        if len(lst) <= 0: lst=self.ListTargetAtoms()
        ngroup=0; drwlst=[]
        color=self.setctrl.GetParam('label-color') #self.mol.labelcolor #[0.0,1.0,0.0]
        grouplst=self.ListGroup()
        for group,i in grouplst:
            cc=self.mol.atm[i].cc[:]
            cc[0] += 0.1; cc[1] += 0.1; cc[2] += 0.1
            pos=numpy.array(cc) #(atom.cc)
            lab=self.mol.atm[i].grpnam
            drwlst.append([lab,None,pos,color]); ngroup += 1
        return ngroup,drwlst
    ...
    def SetElementColor(self):
        ...
        if nsel <= 0:
            for atom in self.mol.atm:
                ...
                elm=atom.elm
                if elm not in const.ElmCol: col=const.ElmCol['??']
    ...
    def SetResidueColor(self):
        ...
        if prvres in rescolordic: col=rescolordic[prvres]
        else: col=rescolordic['???']
        for atom in self.mol.atm:
            if atom.elm == 'XX': continue
            if atom.envflag: continue
            res=atom.resnam
            if res in rescolordic: col=rescolordic[res]
            else: col=rescolordic['???']
            atom.color=col[:]
        return 1
    ...
    def ChangeOpacity(self,opacity,trgatmlst=[],reset=False):
    ...
    def ShowSelectedOnly(self,on,drw=True):
        if on:
    ...
    def ShowAASideChainOnly(self,on,trgatmlst=[],drw=True):
        natm=0
        if len(trgatmlst) <= 0:
            trgatmlst=self.ListTargetAtoms()
        if on:
    ...
    def ShowAABackboneOnly(self,on,trgatmlst=[],drw=True):
        natm=0
        if len(trgatmlst) <= 0:
            trgatmlst=self.ListTargetAtoms()
        if on:
        ...
        if natm > 0 and drw: self.DrawMol(True)

    def DrawChainKite(self,on,colordic={},noback=True):
        """ test"""
        drwlabel='Kite'
        self.ctrlflag.Set(drwlabel,False)
        self.menuctrl.CheckMenu("Kite",False)
        if on:
            drwlst=self.MakeDrawChainKiteData([],frontcolordic={},backcolordic={})
            if len(drwlst) > 0:

                self.draw.SetDrawData(drwlabel,'PLANE',drwlst)
                # testself.draw.SetDrawData('LINE2D',label,True,drwlst)
                self.menuctrl.CheckMenu("Kite", True)
                self.ctrlflag.Set(drwlabel,True)
            else:
                self.menuctrl.CheckMenu("Kite", False)
                self.ctrlflag.Set(drwlabel,False)
                self.ConsoleMessage('No peptides to draw chain')
                return
        else:
            self.draw.DelDrawObj(drwlabel)
            #
    ...
    def DrawChainTube(self,on,trgatmlst=[],colordic={},tuberadius=None):
        """
        :param tubecolordic: {'alpha-helox':[1.0,0.0,0.0,1.0], 'beta-strand':[i,j,k],
                              'random-coil:[]}
        """
        ...
        if len(trgatmlst) <= 0:
            trgatmlst=self.ListTargetAtoms()
        if on:
            drwlst=self.MakeDrawTubeData(trgatmlst,colordic=colordic,tuberadius=tuberadius)
            if len(drwlst) > 0:
    ...
    def ChangeChainTube(self,tuberadius=None,tubecolordic={}):
        """ change radius/color of chain tube

        :param tuberadius: tube radius, e.g. 0.4
        :param tubecolor: tube color, [1.0, 0.0, 0.0, 1.0]
        :return:
        """
        drwlabel='Tube'
        self.draw.DelDrawObj(drwlabel)  # ,'TUBE',[])
        ...
        #self.menuctrl.CheckMenu("CAlpha",False)
        #self.menuctrl.CheckMenu("Kite train",False)
        self.menuctrl.CheckMenu("Tube",True)
        drwlst=self.MakeDrawTubeData([],tuberadius,tubecolordic)
        if len(drwlst) > 0:
            self.draw.SetDrawData(drwlabel,'TUBE',drwlst)
            self.ctrlflag.Set(drwlabel,True)
            self.DrawMol(True)

    def DrawChainCAlpha(self,on,trgatmlst=[],linecolor=None,linethick=None):
        """
        ...
        if len(trgatmlst) <= 0:
            trgatmlst=self.ListTargetAtoms()
        if on:
            drwlst=self.MakeDrawCAlphaData(trgatmlst,linethick,linecolor)
            if len(drwlst) > 0:
                self.draw.SetDrawData(drwlabel,'LINE',drwlst)
                self.ctrlflag.Set(drwlabel,True)
            else:
                self.ConsoleMessage('No peptides to draw C-alpha')
                self.menuctrl.CheckMenu("CAlpha", False)
                return
        else:
            self.draw.DelDrawObj(drwlabel) #,'TUBE',[])
    ...
    def DrawChainRibbon(self,on,trgatmlst=[],linecolor=None,linethick=2,oneline=False):
        """ under construction 2019.08.13

        :param bool on: True for darw, False for do not
        :param lst color: [r(float),b(float),b(float),opacity(float)]
        """
        drwlabel='CahinRibbon'
        self.ctrlflag.Set(drwlabel,False)
        #self.menuctrl.CheckMenu("Tube",False)
        #self.menuctrl.CheckMenu("Kite train",False)
        self.menuctrl.CheckMenu("ChainRibbon",on)
        if len(trgatmlst) <= 0:
            trgatmlst=self.ListTargetAtoms()
        if on:
            drwlst=self.MakeDrawChainRibbonData(trgatmlst,linethick=linethick,linecolor=linecolor,oneline=oneline)
            if len(drwlst) > 0:
                self.draw.SetDrawData(drwlabel,'LINE',drwlst)
                self.ctrlflag.Set(drwlabel,True)
            else: return
    ...
    def ChangeCAlphaLine(self,linecolor=None,linethick=None):
        """
        ...
        #self.menuctrl.CheckMenu("Tube",False)
        #self.menuctrl.CheckMenu("Kite train",False)
        self.menuctrl.CheckMenu("CAlpha",True)
        drwlst=self.MakeDrawCAlphaData([],linethick,linecolor)
        if len(drwlst) > 0:
            self.draw.SetDrawData(drwlabel,'LINE',drwlst)
            self.ctrlflag.Set(drwlabel,True)
            self.DrawMol(True)
    ...
    def ListSp3CCBonds(self,lst):
        ...
        for i in lst:
            ...
            if elmi == ' C':
                ...
                for j in range(len(atomi.conect)):
                    ib=atomi.conect[j]
                    if ib >= ia: continue
    ...
    def Redraw(self):
        ...
        self.mdlwin.draw.SetBGColor(bgcolor)

        self.DrawMol(True)

    def RecreateMdlWin(self):
        """ Recreate MdlWin("view.View_Frm") when a fatal error occured in the 'fu_draw.CanvasDraw.Draw' method

        Known causes of the error:
            1. (Windows 32-bit python) Turn on "stereo view" in python 3.6-wxPython4.1 (python2.7-wxPython2.8 is ok)
        """
        def RemoveCauseOfFatalErrors(case):
            # Reset the causes of a fatal error in fu_draw.CanvasDraw.Draw()
            if case == 1:
                try:
                    # This error occurs in python3.6-wxPython4.1 (not in python2.7-wxPython2.8)
                    self.mol.showdic['Stereo'] = False
                    self.ConsoleMessage('the "Stereo" option has been reset to False.')
                except: pass

        molctrl=copy.copy(self.molctrl)
        mousectrl=self.mdlwin.mousectrl
        #
        molwin=self.mdlwin.molwin
        # destroy
        self.mdlwin.Destroy()
        # create mdlwin
        self.OpenMdlWin(self)
        # recover controls
        self.mdlwin.mousectrl=mousectrl
        self.molctrl=molctrl
        self.mdlwin.molchoice.mdlwin=self.mdlwin
        self.mdlwin.molchoice.model = self
        self.mdlwin.molchoice.molctrl = self.molctrl
        # open molchoice win
        if molwin:
            self.mdlwin.OpenMolChoiceWin()
        # remove cause of error before DrawMol
        RemoveCauseOfFatalErrors(1)
        # draw molecule
        try:
            self.FitToScreen(True,True)
        except:
        ...
        self.ConsoleMessage('MdlWin has been recreated.')

    # ----- Methods related to Build
    def BuildMolFromFile(self,form,filename,addbond=False):
        # form(str): 'new','pdb','xyz','tin'
        # fiename(str): file name
        # multiple molecules
        if form == 'xyz': #'xyzs': # or form == 'sdfs':
            self.BuildFromXYZFile(filename,addbond=addbond)
            return
        elif form == 'sdf' or form == 'mol': #'xyzs': # or form == 'sdfs':
            #####self.BuildMolFromSDFFile(filename)
            self.BuildFromSDFFile(filename)
            return
        elif form == 'mol2':
            self.BuildFromMOL2File(filename)
            return

        """ pdb: very sllow! use old version """
        ###elif form == 'pdb':
        ###    self.BuildFromPDBFile(filename,addbond=addbond)
        ###    return
        ...
        if form == "new":
            ...
            if frgdic.has_key('LAYER'):
                #####26Jly2017#mol.SetFragmentLayer(frgdic['LAYER'])
    ...
    def BuildMol(self,form,filename,addbond=False):
        """
        """
        # molecule object
        mol    = molec.Molecule(self)
        ...
        if form == "new":
            ...
            if 'ICHARG' in frgdic:
                mol.SetFragmentAttributeList('ICHARG',frgdic['ICHARG'])
            if 'RESNAM' in frgdic and 'RESATM' in frgdic:
                mol.SetResidueName(frgdic['RESATM'],frgdic['RESNAM'])
            if 'ATMNAM' in frgdic: mol.SetAtomName(frgdic['ATMNAM'])
            if 'LAYER' in frgdic:
                #####26Jly2017#mol.SetFragmentLayer(frgdic['LAYER'])
                mol.SetFragmentAttributeList('LAYER',frgdic['LAYER'])
            if 'IACTAT' in frgdic:
    ...
    def SwitchMol(self,curmol,fit,drw,messout=True,notify=True):
        ...
        :param bool drw: True for draw, False for do not
        ...
        if curmol < 0:
            ...
            self.mol.SetShowAtomDict()
            #print('Set showdic=', self.mol.showdic)
            ...
            #
            #if drw:
            if prvmolnam != self.mol.name:
                mess='Current molecule is switched' # from "'+prvmolnam
                mess+=' to '+self.mol.name
                self.ConsoleMessage(mess)
                #self.Message(mess,0,'')
            #
            #print('#+++Before SetUpDraw in SwitchMol')
            ###self.SetUpDraw(fit,messout=messout)
            self.SetUpDraw(False, messout=messout)
            #print('#+++After SetUpDraw in SwitchMol')
            drwobjs=self.mol.GetDrawObjs()
            viewitems=self.mol.GetViewItems()
            showatmdic={}

            showatmdic=self.mol.GetShowAtomDict()

            # print(('Get showatmdic=',showatmdic))

            #print('pos #7 in SwitchMol H.show=',self.mol.atm[641].show)
            if len(drwobjs) > 0:
                self.RecoverDrawObjs(drwobjs) # this calls DrawMol!!!
            if len(viewitems) > 0:
                self.draw.SetViewItems(viewitems)
            if len(showatmdic) > 0:
                for label, value in list(showatmdic.items()):
                    if value:
                        self.menuctrl.OnShow(label,value,drw=False)
                self.DrawMol(True)

            ###self.DrawMol(True)
            #print('pos #10 in SwitchMol H.show=',self.mol.atm[641].show)
            name = self.mol.name
            self.mdlwin.molchoice.SetMolSelection(name)
            #print('pos #11 in SwitchMol H.show=',self.mol.atm[641].show)
            if notify: self.NotifyToSubWin('SwitchMol')
            else: self.NotifyToSubWin('Nameless')
            """
            ...
            """
            #print('pos #12 in SwitchMol H.show=',self.mol.atm[641].show)

    def NotifyToSubWin(self,myname,destwindic={}):


        if len(destwindic) > 0: destdic=destwindic
        else: destdic = self.winctrl.GetOpenedWin()
        if len(destdic) <= 0:
            destdic=self.winctrl.GetOpenedWin()
        if len(destdic) <= 0: return
        for winlabel,winobj in list(destdic.items()):
    ...
    def RecoverDrawObjs(self,drwobjdic):
        for drwlabel,lst in list(drwobjdic.items()):
            obj=lst[0]; data=lst[1]
            self.draw.SetDrawData(drwlabel,obj,data)
            self.SetDrawItemsAndCheckMenu(drwlabel,True)

    def CapNCtermsWithHydrogens(self,nterm='NH2',drw=True):
        """


        :param str nterm: 'NH2' or 'NH3+'
        :param bool drw:
        :return:
        """
        if nterm.upper() == 'NH2':
            ...
            lst=self.ListTargetAtoms()
            self.AttachHydrogen(nhtype,lst)
        elif nterm.upper() == 'NH3':
            nhtype='2H'
            # add H at N-terms
            self.SelectNCofCutAA(drw=False,term='N')
            nlst = self.ListTargetAtoms()
            self.AttachHydrogen(nhtype,nlst)
            # add H at C-terms
            self.SelectNCofCutAA(drw=False,term='C')
            clst = self.ListTargetAtoms()
            self.AttachHydrogen('1H', clst)
    ...
    def DeleteBondBetweenNC(self):
        self.SetSelectAll(False)
        for atom in self.mol.atm:
            if atom.resnam not in const.AmiRes3: continue
            nb=len(atom.conect)
            atm=atom.atmnam
            # find NH2-CHO
            nhd=2
    ...
    def MakeHydrogenBond(self,drw=True,prt=False):
        """ Make hydrogen bonds. See the parameters HBndElm,HBndLen,
            and HBndAng in the const module.

        param bool drw: True for redraw, False for do not
        param bool prt: True for print, False for do not
        """
        target = self.ListTargetAtoms()
        nbnd,hbnddic=self.mol.MakeHydrogenBond(target)
        self.Message2("Number of hydrogen bonds found = " + str(nbnd))
        if prt and nbnd > 0:
            fi4='%4d'; ff62='%6.2f'; ff63='%6.3f'; fi3='%03d'
            mess =  '\n       #    atom#(name:res)   atom#(name:res)'
            mess += '   r       ang\n'
            for i in range(len(hbnddic)):
                iii,jjj,r,ang = hbnddic[i]
                resi = self.mol.atm[iii].resnam
                resi += fi3 % self.mol.atm[iii].resnmb
                resj = self.mol.atm[jjj].resnam
                resj += fi3 % self.mol.atm[jjj].resnmb
    ...
    def FindSaltBridge(self):
        ...
        for i,j in bndatmlst:
            ...
            if resnami in cationdic and resnamj in aniondic:
                #print 'cationic1',resnami,resnmbi,resnamj,resnmbj
                if atmi in atmdic[resnami] and \
                                              atmj in atmdic[resnamj]:
                    if resdati+resdatj not in donedic:
                        sbatmlst.append([i,j]); donedic[resdati+resdatj]=True
            elif resnamj in cationdic and resnami in aniondic:
                #print 'cationic2',resnamj,resnmbi,resnami,resnmbj
                if atmi in atmdic[resnami] and \
                                               atmj in atmdic[resnamj]:
                    if resdatj+resdati not in donedic:
                        sbatmlst.append([j,i]); donedic[resdatj+resdati]=True
        return sbatmlst

    def ChangeChainName(self,drw=True):
        """ Change chain name(input from wx.GetTextFromUser) """
    ...
    def ChangeMoleculeName(self):
        """ Change molecule name(input from wx.GetTextFromUser) """

        newnam=wx.GetTextFromUser('Enter mol name(ex. "molecule_1")',
                                    'Change mol name')
        newnam=newnam.strip()
        if newnam[0] == '"' or newnam[0] == "'": newnam = newnam[1:]
        if newnam[-1] == '"' or newnam[-1] == "'": newnam =newnam[:-1]
        if len(newnam) <= 0:
            print('Input molecule name.')
            return
        oldnam = self.mol.name
        self.mol.name = newnam
        mess='Molecule name was changed from ' + oldnam + ' to ' + newnam
        self.ConsoleMessage(mess)

        self.molctrl.ChangeName(oldnam,newnam)

    def NameResidureAuto(self,drw=True):
        grplst = self.FindConnectedGroup()
        ngrp = len(grplst)

        for i in range(ngrp):
            for j in grplst[i]:
                self.mol.atm[j].resnam = 'MOL'
                self.mol.atm[j].resnmb = i + 1
        self.DrawLabelRes(drw,1)
        navatm_in_grp = 0
        try: navatm_in_grp = len(self.mol.atm) / ngrp
        except: navatm_in_grp = 0
        return ngrp,navatm_in_grp

    def ChangeResidueName(self,drw=True):
        """ Change molecule name(input from wx.GetTextFromUser) """

        mess = 'Enter residue name,number,chain name.\n'
        mess += '(ex. "ARG:1:A") or "autoname"'
        res  = wx.GetTextFromUser(mess, 'Change residue name')
        #res=lib.GetStringBetweenQuotation(res)
        res  = res.strip()
        res  = lib.RemoveQuots(res)
        if res.lower() == 'autoname' or res.lower() == 'auto':
            self.NameResidureAuto(drw)
            return
        #item=res[0].split(",")
        #resnam=item[0].strip()
        #resnmb=-1
        ...
        for i in lst:
            ...
            #if chain != '*': self.mol.atm[i].chainnam=chain
        mess='Residure name has changed to '+res+' of selected atoms'
        self.Message2(mess)
    ...
    def ListResDat(self,trgatmlst=[]):
        if len(trgatmlst) <= 0:
            trgatmlst=self.ListTargetAtoms()
        resdatlst=[]
        for i in range(len(trgatmlst)):
            atom=self.mol.atm[i]
            resdat=lib.ResDat(atom)
            if not resdat in resdatlst:
                resdatlst.append(resdat)
        return resdatlst
    ...
    def ReorderAtomsEachMols(self):
        """ Find connected groups and renumer atoms in each connected group
            Usage example: 'Crystal builder' in 'ViewerLite5.0'(a trade mark of
            Accelrys Inc.) gives atoms in order of symmetry operations.
            This function collects atoms in a molecule(connected atom group)
            and reorder atoms.
        ...
        for i in range(natm):
            ii = newold[i]
            atom = self.mol.CopyIthAtom(ii)
            for k in range(len(atom.conect)):
                atom.seqnmb    = i
                atom.conect[k] = oldnew[atom.conect[k]]
            newatm.append(atom)
        self.mol.atm = newatm
        mess = 'Atoms were reordered in isolated molecules'
        self.Message2(mess)
    ...
    def RenumberResidue(self):
        ...
        # newlst: [['ALA',5],...]
        rendic={}
        reslst=self.ListResidue(True)[0]
        for i in range(len(reslst)):
            rendic[reslst[i][0]+':'+str(reslst[i][1])]=[reslst[i][0],
                                                        i+1]
        for atom in self.mol.atm:
            res=atom.resnam+':'+str(atom.resnmb)
            if res in rendic:
    ...
    def ChangeAllHis(self,newhis,all):
        ...
        for i in lst:
            ...
            if res == "HIS" or res == "HID" or res == "HIE" or res == "HIP":
                ...
                if resnam+":"+str(resnmb) in hisatmdic: continue
    ...
    def RenameAtoms(self,atmlst,newlst):
        ...
        for atom in self.mol.atm:
            if atom.atmnam in atmdic: atom.atmnam=atmdic[atom.atmnam]
    ...
    def RenameChains(self,namlst,newlst):
        ...
        for atom in self.mol.atm:
            if atom.chainnam in namdic:
                atom.chainnam=namdic[atom.chainnam]

    def ChangeAtomName(self):
        """ Change atom name(input from wx.GetTextFromUser) """
    ...
    def ChangeBondLength(self):
        """ Change bond length(input from wx.GetTextFromUser) """
    ...
    def ChangeElementAndLength(self):
        """ Change element and bond length(input from wx.GetTextFromUser) """

        mess='Enter element name[,length](ex. " O", 1.2)'
        text=wx.GetTextFromUser(mess, 'Change element and the bond length')
        if len(text) <= 0: return
        items=lib.SplitStringAtSeparator(text,',')
        items[0] = lib.RemoveQuots(items[0])
    ...
    def ChangeElement(self,elmnam,lst,drw=False,messout=False):
        ...
        for i in lst:
            ...
            if messout:
                mess='Element '+elm1+'('+str(i+1) + ') is changed to '+elmnam
                self.ConsoleMessage(mess)
        if drw:
            self.DrawMol(True)
    ...
    def ChangeLength(self,atm1,atm2,r,messout=False,drw=False):
        ...
        if messout:
            mess='Changed length between '+str(atm1+1)+' and '+str(atm2+1)
            mess=mess+'to r='+'%6.3f' % r
            self.ConsoleMessage(mess)
        if drw:
            self.DrawMol(True)
            if self.draw.labelelm: self.DrawLabelElm(True,0)
    ...
    def DeleteHydrogenBond(self,drw=True):
    ...
    def MakeSuperMolecule(self):
        ...
        for atom in self.mol.atm:
            grp=atom.grpnam
            if grp in curgrpdic: continue
            i += 1; curgrpdic[grp]=molnamlst[i]
        for atom in self.mol.atm:
            grp=atom.grpnam
            if grp in curgrpdic: atom.grpnam=curgrpdic[grp]
        #
        name=self.mol.name
    ...
    def PasteMolXYZFromClipboard(self):
        ...
        """
        xyzstring = lib.PasteTextFromClipboard()
        self.ConsoleMessage('Paste XYZString')
        imol = self.BuildFromXYZString(xyzstring,addbond=True)
        if imol > 0:
            mess  = 'name: ' + self.mol.name
            mess += ', number of atoms = ' + str(len(self.mol.atm))
            self.Message2(mess)

    def PasteSMILESFromClipboard(self):
        if not self.HaveOpenBabel(): return
        
        smistring = lib.PasteTextFromClipboard()
        self.ConsoleMessage('Paste SMILE String')
        imol = self.BuildFromSMILESString(smistring)
        if imol > 0:
            mess  = 'name: ' + self.mol.name
            mess += ', number of atoms = ' + str(len(self.mol.atm))
            self.Message2(mess)

    def PasteAtomSDFFromClipboard(self):
        """ Pasete atom coordinates(xyz) from clipboad
    ...
    def PasteMolJSONFromClipboard(self):
        """ paste molecule coordinates(sdf or xyz) in JSON dictionary
            jsondic: {'xyz:xyzstring} or {'sdf': sdfstring}

        """
        self.ConsoleMessage('Paste JSONString')
        string = lib.PasteTextFromClipboard()
        jsondic = json.loads(string)
        #
        if 'sdf' in jsondic:
            form = 'sdf'
            sdfstring = jsondic['sdf']
            imol = self.BuildFromSDFString(sdfstring)
            self.AddBondUseBondLength()
        elif 'xyz' in jsondic:
            form = 'xyz'
            xyzstring = jsondic['xyz']
            imol = self.BuildFromXYZString(xyzstring)
        else: form = None
        #
        if form is None:
            mess  = 'No "xyz" or "sdf" key in the json object'
        elif imol <= 0:
            mess = 'No molecular data'
        else:
            mess  = 'name: ' + self.mol.name
            mess += ', number of atoms = ' + str(len(self.mol.atm))
        self.Message2(mess)

    def PasteMolSDFFromClipboard(self):
        """ paste molecule coordinates(sdf) in clipboard and merge current mol

        coordinates(str): elm,(an(int)),x,y,z,\n...
        """
        self.ready = True

        self.ConsoleMessage('Paste SDFString')
        sdfstring = lib.PasteTextFromClipboard()
        self.BuildFromSDFString(sdfstring)
        mess  = 'name: ' + self.mol.name
        mess += ', number of atoms = ' + str(len(self.mol.atm))
        self.Message2(mess)
        #
        self.DrawMultipleBond(True)
        self.menuctrl.CheckMenu("Multiple bond",True)

    def PasteMolMOL2FromClipboard(self):
        """ paste molecule coordinates(sdf) in clipboard and merge current mol

        coordinates(str): elm,(an(int)),x,y,z,\n...
        """
        self.ConsoleMessage('Paste MOL2String')
        mol2string = lib.PasteTextFromClipboard()
        self.BuildFromMOL2String(mol2string)
        mess  = 'name: ' + self.mol.name
        mess += ', number of atoms = ' + str(len(self.mol.atm))
        self.Message2(mess)
    ...
    def CopyMolXYZToClipboard(self):
        ...
        ff12='%12.8f'
        nl  = '\n'
        text = str(len(self.mol.atm)) + nl
        text += self.mol.name + nl
        for atom in self.mol.atm:
            text=text+'  '+atom.elm+'  '
            text=text+(ff12 % atom.cc[0])+' '
            text=text+(ff12 % atom.cc[1])+' '
            text=text+(ff12 % atom.cc[2])+nl
        #
        try:
            lib.CopyTextToClipboard(text)
            natm = len(self.mol.atm)
            mess = "XYZString is copied to clipboard. ntom=" +str(natm)
        except:
            mess = 'Faled to copy sdf coordinate data to clipboard.'
        self.Message2(mess)

    def CopyMolSDFToClipboard(self):
        # copy selected atoms to clipboard
        # cut(bool): True for cut and copy, Falce for copy

        name = self.mol.name
        atomlst,conlst,typlst = lib.MolToAtomCC(self.mol)
        natm = len(self.mol.atm)
        if natm > 999:
            self.Message2('Too many atoms for SDF format(max.=999): natm = '
                           +str(natm))
            return
        text = lib.AtomCCToSDFText(atomlst,conlst,typlst,name=name)
        #
        try:
            lib.CopyTextToClipboard(text)
            mess = "SDFString is copied to clipboard. natm=" + str(natm)
        except:
            mess = 'Faled to copy sdf coordinate data to clipboard.'
        self.Message2(mess)

    def CopyMolMOL2ToClipboard(self):
        # copy selected atoms to clipboard
        # cut(bool): True for cut and copy, Falce for copy

        name = self.mol.name
        atomcc,atomattr,conlst,typlst = lib.AtomCCFromMol(self.mol)
        natm = len(self.mol.atm)
        text = lib.MOL2TextFromMol(self.mol)
        #
        try:
            lib.CopyTextToClipboard(text)
            mess = "MOL2String is copied to clipboard. natm=" + str(natm)
        except:
            mess = 'Faled to copy mol2 coordinate data to clipboard.'
        self.Message2(mess)


    def CopyMolPDBToClipboard(self):
        """ Copy PDB formatted structure data to clipboard """
        natm = len(self.mol.atm)
        text = lib.PDBTextFromMol(self.mol)
        #
        try:
            lib.CopyTextToClipboard(text)
            mess = "PDBString is copied to clipboard. natm=" + str(natm)
        except:
            mess = 'Faled to copy PDB coordinate data to clipboard.'
        self.Message2(mess)

    def PasteMolPDBFromClipboard(self):
        """ paste molecule coordinates(pdb) in clipboard """
        self.ConsoleMessage('Paste PDBString')
        pdbstring = lib.PasteTextFromClipboard()
        self.BuildFromPDBString(pdbstring)
        mess = 'name: ' + self.mol.name
        mess += ', number of atoms = ' + str(len(self.mol.atm))
        self.Message2(mess)
    ...
    def MergeToCurrent(self,filename,drw=True):
        ...
        if ext == ".pdb" or ext == ".ent":
           ...
           # self.mol.mergedmolname=molec.Molecule.MakeMolNameWithExt(filename)
            ...
            stringlst=rwfile.ReadMol2File(filename)
            for string in stringlst:
                tmpmol.SetAtomsFromMOL2Text(string)
        self.mol.mergedfile=filename
        self.mol.mergedmolname=molec.Molecule.MakeMolNameWithExt(filename)
    ...
    def MergeMolecule(self,mrgatm,check=True,drw=True,gname=None):
        ...
        fromargs=False
        if mrgatm == None:
            if 'MergeMolecule' not in self.mdlargs: return
            fromargs=True
        ...
        if len(self.mol.atm) == 0:  # new molecule
            ...
            for atom in mrgatm:
                ...
                if gname is None: gname = 'mgr'
                atom.grpnam = gname + str(ngrp+1)
                atom.select=True
                # renumber conect data
                if len(atom.bndmulti) <= 0: addmutli=True
                else: addmulti=False
                for j in range(len(atom.conect)):
                    #try:
                    atom.conect[j] += norg
                    if addmulti: atom.bndmulti.append(1)
                    #except: pass
    ...
    def ConnectGroups(self,atm1,atm2,select=False,drw=True):
        ...
        for ig,atmlst in list(grpdic.items()):
            if atm1 in atmlst:
    ...
    def FindAtomNameAtomInResidue(self,findatmnamlst,resdat=None,atmlst=[],alth=True,messout=False):
        """

        :param resdat: packed residue name, "resnam:resnmb:chain"
        :param atmnamlst: [atmnam1,atmnam2,...], e.g., atmnam1='CA'
        :param alth: use 'CD' the altenative of H in proline
        :return: atmseqlst(lst) - [atmseq# of atmnamlst[0],...]
        """
        atmseqlst=[]
        namlst=[x.strip() for x in findatmnamlst]
        if resdat is not None:
            res=resdat[:3]
        else:
            res=self.mol.atm[atmlst[0]].resnam
        if res == 'PRO' and alth:
            idx=namlst.index('H')
            if idx >= 0: namlst[idx]='CD'
        #
        if resdat is not None: resatmlst=self.ListResidueAtom(resdat)
        else: resatmlst=atmlst

        atmseqlst=len(namlst)*[-1]
        if len(resatmlst) <= 0:
            mess='No atoms in resdat "'+str(resdat)+'" or atmlst\n'
            self.ConsoleMessage(mess)
            if messout: wx.MessageBox(mess,'FindAtomNameAtomInResidue')
            return atmseqlst
        #
        namdic={}
        for i in resatmlst:
            atom=self.mol.atm[i]
            atmnam=atom.atmnam.strip()
            if atmnam in namlst:
                namdic[atmnam]=i
        for atmnam,i in list(namdic.items()):
            idx=namlst.index(atmnam)
            atmseqlst[idx]=i
        return atmseqlst
    ...
    def ConnectGroupsType2(self,atm1,atm2,grpnam,grpatmlst,select=True):
        ...
        def Message():
            mess='Sorry, I can not treat this form.'
    ...
    def FindRotatableBonds(self,exclude=True):
        ...
        for i in sellst:
            ...
            for j in range(len(atom.conect)):
                ...
                if str(i)+':'+str(jj) in donedic: continue
                if str(jj)+':'+str(i) in donedic: continue
                ...
                for grpnmb,lst in list(grpdic.items()):
                    if i in lst:
    ...
    def CheckShortContact2(self,mol1,mol2):
        fromargs=False
        if mol1 == None:
            if 'CheckShortContact2' not in self.mdlargs: return
    ...
    def CheckShortContact(self,rmax,bell=False):
        ...
        try: # fortran module
            ...
            mess='CheckShortContact: Non-Fortran routine is executed!'
            if self.messchecker.CheckMessage(self.classnam,"CheckShortContact",mess,maxtimes=1):
                print(mess)
    ...
    def CheckValence(self):
        ...
        for i in range(natm):
            elm = self.mol.atm[i].elm
            if elm not in const.ElmValence: continue
            nval = const.ElmValence[elm]
            nbnd = 0
            for j in range(len(self.mol.atm[i].conect)):
                nbnd += self.mol.atm[i].bndmulti[j]
            if nbnd > nval: errlst.append(i+1)
        return errlst
    ...
    def RotateMolByMatrix(self,rotmat):
        self.draw.RotateByMatrix(rotmat)
        self.SetDrawItemsToSeriesMol()
        self.draw.Paint()

    def RotateMolByAngle(self,axis,angle,radian=True):
        """ Rotate molecule by angle around axis

        :param lst axis: axis vector, [x,y,z]
        :param float angel: angle
        """
        torad = 0.017453292
        ang   = angle
        if not radian: ang = torad * angle
        rotmat = lib.RotMatAxis(axis,-ang)
        #
        self.draw.RotateByMatrix(rotmat)
        self.SetDrawItemsToSeriesMol()
        self.draw.Paint()
    ...
    def RotateSelected(self,axis,dif,trgatmlst=[]):
    ...
    def TranslateSelected(self,dif,drw=True):
        ...
        if self.mol.cubeobj is not None:
            try:
                tobohr = 1.8870854
                atomcc = []
                natm = len(self.mol.atm)
                for i in range(natm):
                    x = self.mol.atm[i].cc[0] * tobohr
                    y = self.mol.atm[i].cc[1] * tobohr
                    z = self.mol.atm[i].cc[2] * tobohr
                    atomcc.append([x,y,z])
                self.mol.cubeobj.SetAtomCC(atomcc)
                nshl = self.mol.cubeobj.basdic['shlxyz']
                for i in range(natm):
                    for j in range(nshl):
                        if self.mol.cubeobj.basdic['shlcntr'] == i:
                            self.mol.cubeobj.basdic['shlxyz'] = atomcc[i]
            except: pass
    ...
    def ChangeMolName(self,newname,filename=True):
        ###if self.molmolnam[0:4] != 'temp': return
        if filename: name=molec.Molecule.MakeMolNameWithExt(newname)
        else: name = newname
        self.RenameMolecule(self.mol.name,name)
    ...
    def ReadFiles(self,filename,draw,logging=True,drop=False):
        """ The old version(before ver0.5.0) of "OpenFiles" """
        mess  = 'Warning: Use "Openfiles" insted of old "ReadFiles"'
        mess +=  ' in your script.'
        self.ConsoleMessage(mess)
        self.OpenFiles(filename=filename,dodraw=draw,logging=logging,drop=drop)
        self.NotifyToSubWin('ReadFiles')
    #@lib.LOGGINGFUNCTIONCALL
    def OpenFiles(self,filename,dodraw=False,logging=True,drop=False):
        """ Open files

        :param str filename: file name
        :param bool draw: True for draw mol after reading, False do not.
        """
        if len(filename) <= 0:
            self.Message2('Model(OpenFiles): Empty file name')
            return

        # get form from file extension
        root,ext = os.path.splitext(filename)
       ...
       # form from file extension
        ext  = ext.lower()
        form = ext[1:]
        # print mess
        mess = 'Open file = ' + filename
        self.Message2(mess)
        #
        donedrw = False
        notify  = True
        if form == 'xyz' or form == 'zmt': addbond = True
        elif form == 'zmt' or form == 'tin': addbond = True
        elif form == 'inp': addbond = True
        elif form == 'cif': addbond = True
        else: addbond = False
        #
        formgrp1 = ['sdf','mol','xyz','mol2'] #,'pdb','zmt','tin']
        formgrp2 = ['new',
                    'xyzgms',
                    'zmt','zmtfu'
                    'tin','inp',]
        ...
        if form in formgrp1:
            ...
            self.BuildMolFromFile(form,filename,addbond=addbond)

            if form == 'sdf' or form == 'mol' or form == 'mol2':
                self.DrawMultipleBond(True)
                self.menuctrl.CheckMenu("Multiple bond",True)
            ...
            #self.BuildMolFromFile('pdb',filename)
            self.OpenFilesPDB(filename,False) #True)
            self.NotifyToSubWin('OpenFiles')
            self.mdlwin.BusyIndicator('Off')
            self.ctrlflag.Set('ready',True)
            return
        elif form == 'cif':
            self.OpenCIFFile(filename,addbond=addbond)
            return
        elif form == 'smi' or form == 'can':
            if not self.HaveOpenBabel(): return
            self.OpenSMILESFile(filename)
            return
        # non-structure files
        elif form == 'frm' or form == 'frg':
            self.OpenNonStructureFiles(form,filename)

        elif form == 'json':
            self.OpenJSONFiles(filename)

        elif form == 'fuf' or form == 'spl' or form == '.mrg':
            self.OpenFUFiles(form,filename)
            ...
            mol = molec.Molecule(self,self.Message,self.ConsoleMessage)
            mol.SetXYZAtoms(xyzmol,bond,resfrg)
            if len(mol.name) <= 0: mol.name = base
            mol.inpfile = filename
            #
            self.SetUpBuildMol(mol)
        ...
        elif form in formgrp2: 
            self.BuildMol(form,filename,addbond=addbond)
            donedrw = True
        elif form == 'inp':
            self.AddBondUseBondLength()
            ...
            mess  = 'The file extension is not supported as a structure file.'
            mess += ' file= ' + filename + '.'
            self.Message2(mess)

        # option for each form
        if dodraw and not donedrw:
            self.SetDrawObjsAndDraw()
            donedrw=True

        if notify: self.NotifyToSubWin('OpenFiles')


        self.mdlwin.BusyIndicator('Off')
        self.ctrlflag.Set('ready',True)

    def BuildFromXYZFile(self,filename,addbond=True):
        """ Build multiple mol objects

        :param str form: 'xyz' or 'sdf'
        :param lst moldatlst:
        """
        imol      = 0
        seriesmol = []
        ...
        for s in f.readlines():
            ndat  += 1
            if ndat != 2 and len(s.strip()) <= 0: continue
            nline += 1
            if ndat == 1:
                natm = int(s.strip())
            if ndat >= natm + 2:
                ...
                imol += 1
                mol = molec.Molecule(self,self.Message,self.ConsoleMessage)
                mol.SetAtomsFromText('xyz',string)
                if len(mol.name) <= 0: mol.name = base + '-' + str(imol)
                mol.inpfile = filename
                if addbond:
                    mol.AddBondUseBL([])
                    mess = 'Connect data were created base on bond length.'
                    self.ConsoleMessage(mess)
                #
                self.SetUpBuildMol(mol)
                #
        ...
        #
        self.curmol,self.mol = self.molctrl.GetCurrentMol()
        mess = 'Number of molecule data in "' + filename + '" = '+str(imol)
        self.Message2(mess)
    ...
    def BuildFromXYZString(self,xyzstring,addbond=True):
        """ Build multiple mol objects
        ...
        tmpstr = xyzstring.replace('\r\n','\n',1000000)
        textlst = tmpstr.split('\n')
        stringlst = []
        text = ''
        nline = 0
        ndat  = 0
        ...
        for i in range(len(textlst)):
            ndat = len(textlst[i]) - 2
            nline += 1
            if nline == 1:
                if textlst[i].strip().isdigit():
                ...
                if (textlst[i].strip()) > 0: text += textlst[i] + '\n'
        #
        for string in stringlst:
            if len(string.split('\n')) <= 2: return
            imol += 1
        ...
        #
        self.curmol,self.mol = self.molctrl.GetCurrentMol()
        mess = 'Number of molecule data in string.xyz = ' + str(imol)
        self.Message2(mess)
    ...
    def BuildMolFromXYZsFile(self,filename):
        ...
        self.BuildMolFromXYZs(xyzmollst,filename)

    def BuildMolFromXYZs(self,xyzmollst,filename):
    ...
    def CheckDuplicateMolName(self,mol):
        oldnam = mol.name
        modname = self.CheckSameMolName(oldnam)
        return modname

    def CheckSameMolName(self,molnam):
        modnam  = molnam
        molnam0 = molnam
        nummol = self.molctrl.GetNumberOfMols()
        molnamlst = []
        for idx in range(nummol):
            molnamlst.append(self.molctrl.GetMolName(idx))
        inum = -1
        while True:
            if molnam in molnamlst:
                inum += 1
                if inum == 0: molnam += '.x'
                else:         molnam += 'x' #.' + str(inum)
            else: break
        if inum > -1:
            mess  = 'Molecule name was changed from ' + molnam0 + ' to '
            mess += molnam
            self.Message2(mess)
            modnam = molnam
        return modnam

    def SetUpBuildMol(self,mol):
        """ Add a new molecule to molctrl and set up draw objs """
        modnam = self.CheckDuplicateMolName(mol)
        if modnam is not None: mol.name = modnam

        self.molctrl.Add(mol,mess=False)
        #
        self.curmol,self.mol = self.molctrl.GetCurrentMol()
        self.SetUpDraw(True,messout=True)
        """self.DrawMol(True)"""
        drwobjs = self.draw.GetDrawObjs()
        self.mol.SetDrawObjs(drwobjs)
        viewitems = self.draw.GetViewItems()
    ...
    def BuildFromSDFFile(self,filename):
        """ Build molecule objects from file

        :param str filename: sdfile name
        """
        # read data
        f = open(filename,'r')
        sdstring = f.read()
        ...
        imol = self.BuildFromSDFString(sdstring)

        mess = 'Number of molecule data in "' + filename + '" = '+str(imol)
        self.Message2(mess)


    def BuildFromMOL2File(self,filename):
        """ Build molecule objects from file

        :param str filename: mol2file name
        """
        def CreateNewMolObject(string):
            mol = molec.Molecule(self,self.Message,self.ConsoleMessage)
            mol.SetAtomsFromText('mol2',string)
            if len(mol.name) <= 0: mol.name = base + '-' + str(imol)
            mol.inpfile = filename
            self.SetUpBuildMol(mol)
            return mol
    ...
    def BuildFromSDFString(self,sdfstring,name=None):
        """ Build molecule objects from sdf string

        :param str sdfstring: sdf string
        """
        filename  = 'string.sdf'
        base      = 'string'
        imol      = 0
        ...
        string = ''
        #
        tmpstr = sdfstring.replace('\r\n','\n',1000000)
        stringlst = tmpstr.split('$$$$\n')
        nmol = len(stringlst)
        for string in stringlst:
            if len(string.split('\n')) <= 3: return
            imol += 1
            mol = molec.Molecule(self,self.Message,self.ConsoleMessage)
            string += '$$$$\n'
            mol.SetAtomsFromText('sdf',string)
            if name is not None:
                mol.name = name
                if nmol >= 2: mol.name += '-' + str(imol)
            if len(mol.name) <= 0: mol.name = base + '-' + str(imol)
            mol.inpfile = filename
            self.SetUpBuildMol(mol)
            ngrp,navatm_in_grp = self.NameResidureAuto(drw=False)
            seriesmol.append(mol)
        #
        self.curmol,self.mol = self.molctrl.GetCurrentMol()
        mess = 'Number of molecule data in string.sdf = ' + str(imol)
        self.Message2(mess)
    ...
    def BuildFromMOL2String(self,mol2string):
        """ Build molecule objects from sdf string

        :param str mol2string: mol2 string
        """
        filename  = 'string.mol2'
        base      = 'string'
        imol      = 0
        seriesmol = []
        #
        ...
        string = ''
        #
        stringlst = lib.SplitMOL2String(mol2string)
        for string in stringlst:
            imol += 1
            mol = molec.Molecule(self,self.Message,self.ConsoleMessage)
            mol.SetAtomsFromText('mol2',string)
        ...
        #
        self.curmol,self.mol = self.molctrl.GetCurrentMol()
        mess = 'Number of molecule data in string.mol2 = ' + str(imol)
        self.Message2(mess)
    ...
    def BuildFromPDBFile(self,filename,addbond=False):
        """ Build a molecule object from file

        :param str filename: pdbfile name
        """
        head,tail = os.path.split(filename)
        base,ext  = os.path.splitext(tail)
        #
        ...
        mol = molec.Molecule(self,self.Message,self.ConsoleMessage)
        mol.SetAtomsFromText('pdb',pdbstring)
        if len(mol.name) <= 0: mol.name = base
        mol.inpfile = filename
    ...
    def BuildFromPDBString(self,pdbstring,addbond=False):
        """ Build a molecule object from pdb string

        :param str pdbstring: string in pdb format
        """
        filename  = 'pdbstring.pdb'
        base      = 'pdbstring'
        #
        string = pdbstring.replace('\r\n','\n',1000000)

        mol = molec.Molecule(self,self.Message,self.ConsoleMessage)
        mol.SetAtomsFromText('pdb',string)
        if len(mol.name) <= 0: mol.name = base
        mol.inpfile = filename
        #
        if addbond: mol.AddBondUseBL([])
    ...
    def OpenCIFFile(self,filename,addbond=True):
        elmlst = [' H','LI',' C',' N',' O',' F',
                  'SI',' P',' S','CL']
        def HeavyElement(umol_xyz_coords):
            ans = False
            natm = len(umol_xyz_coords)
            for i in range(natm):
                elm = lib.ElementNameFromString(umol_xyz_coords[i][1])
                if not elm in elmlst:
                    ans = True
                    break
            return ans

        # make an instance of fu_crystal.Crystal class
        ###crystalobj = build.Crystal(self,filename,
        crystalobj = molec.Crystal(self,filename,
                                        messout1=self.ConsoleMessage)
        cifdata = crystalobj.GetCIFData()
        umol_xyz_coords = crystalobj.UnitMolecule()
        if HeavyElement(umol_xyz_coords): crystalobj.Warning()
        #
        inpfile = cifdata["input_ciffile"]
        name = cifdata["chemical_name_com"]
        if name is None or name.strip() == '?' or name.strip() == '':
            head,tail = os.path.split(inpfile)
            base,ext  = os.path.splitext(tail)
            name      = base
        #
        mol = molec.Molecule(self,self.Message,self.ConsoleMessage)
        #
        #mol.cifdata = cifdata
        mol.crystalobj = crystalobj
        mol.boxpnts    = crystalobj.box_vertexes
        mol.inpfile = inpfile
        mol.name    = name
        mol.SetAtomsCIF(umol_xyz_coords)

        if addbond: mol.AddBondUseBL([])
        self.SetUpBuildMol(mol)

        if addbond: self.ChangeBondsInUnitMol()

        natm = len(mol.atm)
        mess = 'Number of atoms = ' + str(natm)
        self.Message2(mess)

        self.mdlwin.BusyIndicator('Off')
        self.ctrlflag.Set('ready',True)

    def HaveOpenBabel(self):
        ans = True
        if not const.OPENBABEL or not const.PYBEL:
            mess  = 'Pybel and OpenBabel are not installed.\n'
            mess += ' Please install them.'
            wx.MessageBox(mess,'Model.OpenSMILESFile')
            ans = False
        return ans

    def DefaultOpenBabelParams(self,forcefield,method,max_steps,crit):
        method0     = self.setctrl.GetParam('ob-opt-method')
        forcefield0 = self.setctrl.GetParam('ob-forcefield')
        max_steps0  =self.setctrl.GetParam('ob-opt-max-steps')
        crit0       = self.setctrl.GetParam('ob-opt-crit')
        if forcefield is None: forcefield = forcefield0
        if method is None:     method     = method0
        if max_steps is None:  max_steps  = max_steps0
        if crit is None:       crit       = crit0

        return forcefield,method,max_steps,crit

    def Make3DFromSMILES(self,smistring,forcefield=None,method=None,
                       max_steps=None,crit=None,outform='sdf',name=None):
        """ Make 3D models from SMILES using openbabel tools

        :param str smiles: SMILES string
        :param str forcefiled: name of force field, 'uff','mmff94','gaff','
                               or 'ghemical' (supported ff in openbabel)
        :param str method: minimization method, 'sd'(steepest descent,
                           or 'cg'(conjugate gradient)
        :param int max_steps: max. steps in ff minimization
        :param float crit: energy convergence threshold in minimization
        :param str outform: output format of 3D coordinates, 'sdf','mol2',...
        :return: sdfstring(str) - string in 'outform' format
        """
        if not self.HaveOpenBabel(): return
        #
        forcefield,method,max_steps,crit = \
               self.DefaultOpenBabelParams(forcefield,method,max_steps,crit)
        #print('forcefield',forcefield)
        #print('method',method)
        #print('max_steps',max_steps)
        #print('crit',crit)
        fflst = ['uff','mmff94','gaff','ghemical']
        if not forcefield.lower() in fflst:
            mess  = 'Not supported forcefiled = ' + forcefield + '\n'
            mess += ' Please select one in ' + str(fflst)
            wx.MessageBox(mess,'Model.Make3DFromSMILES')
            return
        #
        obc   = openbabel.OBConversion()
        obb   = openbabel.OBBuilder()
        obf   = openbabel.OBForceField.FindForceField(forcefield)
        obmol = openbabel.OBMol()

        obc.SetInFormat('smi')
        obc.ReadString(obmol,smistring)
        obmol.AddHydrogens()
        obb.Build(obmol)
        obf.Setup(obmol)
        if method == 'sd': obf.SteepestDescent(max_steps,crit)
        else: obf.ConjugateGradients(max_steps,crit)
        obf.UpdateCoordinates(obmol)
        if name is not None: obmol.SetTitle(name)
        #
        obc.SetOutFormat('sdf')
        sdfstring = obc.WriteString(obmol)
        return sdfstring

    def OpenSMILESFile(self,filename,forcefield=None,method=None,
                       max_steps=None,crit=None):
        """ Read SMILES file and make 3D models

        :param str filename: filename
                             the data are asummed,
                             smile string, [molecule name] \n
                             ...                           \n
        :param str forcefiled: name of force field, 'uff','mmff94','gaff','
                               or 'ghemical' (supported ff in openbabel)
        :param str method: minimization method, 'sd'(steepest descent,
        ...
        nmol = self.BuildFromSMILESString(smistring,forcefield=forcefield,
                                   method=method,max_steps=max_steps,
                                   crit=crit)

    def BuildFromSMILESString(self,smistring,forcefield=None,method=None,
                       max_steps=None,crit=None):
        #
        ...
        textlst = smistring.split('\n')
        if len(textlst) <= 0:
            mess  = 'Empty file = ' + filename
            wx.MessageBox(mess,'Model.OpenSMILESFile')
            return
        #
        filename = 'string.smi'
        base = lib.BaseNameInFileName(filename)
        textlst = smistring.split('\n')
        if len(textlst) <= 0:
            mess  = 'Empty file = ' + filename
            wx.MessageBox(mess,'Model.OpenSMILESFile')
        ...
        for s in textlst:
            ...
            if len(items[0]) <= 0: continue
            if len(items) >= 2: name = items[1]
            else: name = base + str(nmol)
            string = self.Make3DFromSMILES(items[0],forcefield=forcefield,
                                              method=method,max_steps=max_steps,
                                              crit=crit,outform='sdf',name=name)
            nmol += 1
            sdfstring += string
        #
        mess = 'SDF data were created from SMILES.\n'
        mess += '    Molecule object(s) are created from SDF data.\n'
        mess += '    Number of created molecules = ' + str(nmol)
        self.ConsoleMessage(mess)
        #
        nmol = self.BuildFromSDFString(sdfstring)
    ...
    def IsCrystalObject(self):
        if self.mol is None: return False
        ans = True
        try: crystalobj = self.mol.crystalobj
        except: crystalobj = None
        if crystalobj is None:  ans = False
        return ans

    def ChangeBondsInUnitMol(self):
        if not self.IsCrystalObject(): return
        crystalobj = self.mol.crystalobj
        ...
        bnddat = lib.BondListFromConAndTypList(conlst,typlst)
        self.mol.crystalobj.unit_mol_bonds = bnddat

    def PackCells(self,klm=None,addbond=False):
        """ Pack KLM cells with molecules """
        if not self.IsCrystalObject():
            mess  = 'The current molecule does not have cif data.\n'
            mess += ' Unable to pack cells.'
            lib.MessageBoxOK(mess,'model.PackCells')
            return
        crystalobj = self.mol.crystalobj
        #
        if klm is None:
            # input klm
            mess = 'Enter k,l,m(should be positive) (ex. "1,1,1")'
            klm  = lib.GetIntegersFromUser(mess)
            if len(klm) <= 0: return
            if min(klm) < 0:
                mess = 'Negative values are not accepted for k,l,m'
                self.Message2(mess)
                return
        #
        self.mdlwin.BusyIndicator('On','Packing ...')
        mols_xyz_coords = crystalobj.MoleculesInKLMCells(klm)
        # create mol
        mol = molec.Molecule(self,self.Message,self.ConsoleMessage)
        mol.SetAtomsCIF(mols_xyz_coords)
        #
        mol.inpfile   = crystalobj.inpfile
        mol.boxpnts   = crystalobj.box_vertexes
        chem_name_com = crystalobj.chem_name_com
        base = 'mol'
        try: base = lib.BaseNameInFileName(mol.inpfile)
        except: pass
        if chem_name_com is not None: name = chem_name_com
        else: name = base
        if name == '?': name = base
        mol.name = base + '-' + str(klm[0])+str(klm[1])+str(klm[2])
        # add bonds and residue number
        natm       = len(mol.atm)
        umol_natm  = crystalobj.natm_unit_mol
        nmol       = natm / umol_natm
        umol_bonds = crystalobj.unit_mol_bonds
        conlst,typlst = lib.ConListFromBondList(umol_natm,umol_bonds)
        ires = 0
        rnam = 'MOL'
        for im in range(nmol):
            ist = im * umol_natm
            con = copy.deepcopy(conlst)
            for i in range(len(conlst)):
                for j in range(len(conlst[i])): con[i][j] += ist
            ires += 1
            for i in range(umol_natm):
                ia = ist + i
                mol.atm[ia].conect   = con[i]
                mol.atm[ia].bndmulti = typlst[i]
                mol.atm[ia].resnmb = ires
                mol.atm[ia].resnam = rnam
        self.SetUpBuildMol(mol)
        if addbond:
            self.DeleteBonds('all',drw=False)
            self.AddBondUseBondLength(drw=True)
        # print
        mess = 'Packing cells ' + str(klm)
        self.Message2(mess)
        mess = '   Number of generated molecules = ' +str(nmol)
        self.Message2(mess)
        mess = '   Total number of atoms = ' + str(natm)
        self.Message2(mess)

        self.mdlwin.BusyIndicator('Off')
        self.ctrlflag.Set('ready',True)

    def OpenFUFiles(self,form,filename):
        filelst = self.GetFUFileList(filename)
        if len(filelst) <= 0:
            self.Message('no files in fuf file. skipped.',0,'')
        else:
            if form == 'mrg':
                self.OpenFilesPDB(filelst[0],True)
                ...
                for file in filelst:
                    ...
                    fufform = fufext[:1]
                    if fufform == 'pdb' or fufform == 'ent':
                        self.OpenFilesPDB(file,True)
                    elif fufform == 'frm':
    ...
    def GetFUFileList(self,fuffile):
        filelst=[]
        f=open(fuffile,'r')
        for s in f.readlines():
            s=s.replace('\r','')
    ...
    def OpenJSONFiles(self,filename):
        if not os.path.exists(filename):
            self.Message2('Not found file = ' + filename)
            return
        #
        f = open(filename,'r')
        jsondic = json.load(f)
        ...
        if 'sdf' in jsondic:
            ...
            mess = 'Not found key "sdf" or "xyz" in Json data'
        if imol <= 0:
            mess = 'No molecular data'
        if len(mess) > 0: self.Message2(mess)


    def OpenFilesPDB(self,filename,drw):
        donedrw=False
        if not os.path.isfile(filename):
            mess='file not found. file='+filename
            #self.Message(mess,0,'')
            #self.ConsoleMessage(mess)
            wx.MessageBox(mess,'OpenFilesPDB')
            return
        #mess='Open file='+filename
        ...
        if len(missatoms) > 0 or len(missres) > 0:
            wx.MessageBox("PDB info: there are missing atoms/residues.","")
        # check atmnam
        if self.setctrl.GetParam('check-pdb-atmnam'):
            ...
            if not ans:
                ...
                wx.MessageBox(mess,'Model(OpenFilesPDB)')
        # delete TERS
        ...
        if self.setctrl.GetParam('auto-add-hydrogens'): #self.autoaddhydrogen:
            ...
            err=0
            if nres > 0:
                cpy=self.mol.CopyMolecule(parent=self)
                # python code -> fortran
        ...
        if self.setctrl.GetParam('tinker-atom-name'): #self.tinkeratmnam:
            self.RenameAtmNamIntoTinker()
            #print 'tinker name'
        if drw and not donedrw: # and not donedrw:
    ...
    def TextCoordsInXYZ(self,molobj):
        fi3 = '%3d'
        ff12 = '%12.6f'
        blk2 = 2 * ' '
        nl = '\n'
        ...
        for atom in molobj.atm:
            text += blk2 + atom.elm
            for i in range(3): text += (ff12 % atom.cc[i])
            text += nl
        return text

    def TextCoordsInSDF(self,molobj):
        fi3 = '%3d'
        ff12 = '%12.6f'
        blk2 = 2 * ' '
        nl = '\n'
        ...
        for atom in molobj.atm:
            ...
            for i in range(3): text += (ff12 % atom.cc[i])
            text += nl
        #self.ConsoleMessage('name='+name)
        #self.ConsoleMessage(text)
        return text


    def SaveAllOnOneFile(self,filename):
        def AtomCCAndConlst(tmpmol):
            atomcc = []
            conlst = []
            typlst = []
            for atom in tmpmol.atm:
        ...
        if len(filename) <= 0: return
        self.Message2('filename in SaveAllOnOneFiles ='+filename)
        if len(filename) <= 0: return
        base,ext = os.path.splitext(filename)
        formlst = ['.sdf','.xyz','.sdfs','.xyzs','.mol2']
        if not ext in formlst:
            mess = "The format " + ext + " is not supported.\n"
            mess += " supported formats are .sdf and .xyz"
            self.ConsoleMessage(mess)
            return
        #
        outtext = ''
        nmol = self.molctrl.GetNumberOfMols()
        for im in range(nmol):
            tmpmol = self.molctrl.GetMol(im)
            name = tmpmol.name
            natm = len(tmpmol.atm)
            atomcc,conlst,typlst = AtomCCAndConlst(tmpmol)
            if ext == '.xyz' or ext == '.xyzs':
                outtext += lib.AtomCCToXYZText(atomcc,title=name)
            elif ext == '.sdf' or ext == '.sdfs':
                outtext += lib.AtomCCToSDFText(atomcc,conlst,typlst,
                                  name=name,resnam='',comment='',
                                  itemlst=None)
            elif ext == '.mol2':
                 outtext += lib.MOL2TextFromMol(tmpmol,name=name,mol2dic={})
        #
        f=open(filename,"w")
        f.write(outtext)
        f.close()
        #
        mess  = "Multiple structure data are written on file =" + filename + '\n'
        mess += '   Numer of molecules = ' + str(nmol)
        self.ConsoleMessage(mess)
        #

    def SaveAllInDirectory(self,filetype,savedir):
        """ Save all molecules

        :param str filetype: "pdb files","mol files",...
        :param str savedir: directory name to make files
        """
        if len(savedir) <= 0: return
        ...
        for i in range(nmol):
            mol=self.molctrl.GetMol(i)
            base,ext = os.path.splitext(mol.name)
            base = lib.SuppressBlanks(base)
            filename = base + fileextdic[filetype]#mol.inpfile
            #head,tail=os.path.split(filename)
            #base,ext=os.path.splitext(tail)
            #filename=base+fileextdic[filetype] #'.pdb'
            filename=os.path.join(savedir,filename)
            #mol.WritePDBMol(filename,"","",True)
            self.SaveFiles(filename,False,True,mol) # save=True, all=True, nolobj=mol
            #mess='Save file='+filename
            #self.ConsoleMessage(mess)

    def SaveFiles(self,filename,save,all,molobj=None,check=True):
        # save=True for "Save" and =False for "Save As"
        # all: True for all atoms, False for selected atoms only
        extlst=['.gz','.ent','.pdb','.xyz','.xyzfu','.xyzs','.tin',
                '.mol','.sdf','.mol2','.zmt','.zmtfu','.drwpic','.cif']
        ...
        if not ext in extlst:
            ...
            lib.MessageBoxOK(mess,'Model(SaveFiles)')
            return
        # gz file ?
        if ext == '.gz':
            gzfile=filename
        ...
        tmp=molobj.CopyMolecule(parent=self)
        if ext == '.ent' or ext == '.pdb':
            ...
            self.DeleteAllTers()
            molobj.RenumberConDat()
            if ext == '.xyzfu': molobj.WriteXYZMol(filename)
            elif ext == '.xyz': molobj.WriteXYZMol(filename,name=self.mol.name)
            ...
            bndlst = self.MakeBondList()
            if len(molobj.atm) < 1000 and len(bndlst) < 1000:
                molobj.WriteSDFMol(filename,resnam='',title='',comment='')
            else:
                if len(molobj.atm) >= 1000:
                    mess  = 'Too many atoms, ' + str(len(molobj.atm))
                    mess += ', max=1000.\n'
                elif len(bndlst) >= 1000:
                    mess  = 'Too many bonds, ' + str(len(bndlst))
                    mess += ', max=1000.\n'
                mess += ' Use mol2, pdb, or xyz format'
                lib.MessageBoxOK(mess,'WriteFiles')
                return
        elif ext == '.mol2':
            molobj.WriteMOL2Mol(filename,resnam='',title='',comment='')
            ...
            if not self.IsCrystalObject():
                mess = 'The current molecule is not a pack object.'
                mess += ' Unableto save as cif file'
                self.Message2(mess)
                return
            molobj.WriteCIFMol(filename)
        #
        molobj.atm=tmp.atm
    ...
    def RemoveMol(self,allmol=False,dellst=None,curmol=None):
        #Remove molecule
        #if len(self.molnam) <=0: return
        if dellst is not None:
            for i in dellst: self.molctrl.Del(i)
            nmol = self.molctrl.GetNumberOfMols()
            self.mol = self.molctrl.GetMol(nmol-1)
    ...
    def SaveBonds(self):
        self.savebonds = []
        for atom in self.mol.atm:
            self.savebonds.append([atom.conect,atom.bndmulti])

    def RecoverBonds(self,drw=True):
        if len(self.savebonds) <= 0:
            mess = 'No bond data to recover'
            wx.MessageBox(mess,'Model.RecoverBonds')
            return
        natm = len(self.mol.atm)
        if len(self.savebonds) != natm:
            mess = 'The number of atoms are different in Mol and saved bonds'
            mess += ' Unable to recover bonds'
            wx.MessageBox(mess,'Model.RecoverBonds')
            return
        for i in range(natm):
            self.mol.atm[i].conect   = self.savebonds[i][0]
            self.mol.atm[i].bndmulti = self.savebonds[i][1]
        if drw: self.DrawMol(True)
    ...
    def SaveAtomCC(self,all=False):
        ...
        """
        if all: savlst=list(range(len(self.mol.atm)))
    ...
    def SaveMol(self,all=True):
        ...
        molobj.atm=self.mol.CopyAtom()
        molobj.name=self.mol.name
    ...
    def RecoverMol(self,drw=True):
        ...
        self.mol=molobj.CopyMolecule(parent=self)
    ...
    def JoinSplitFiles(self,pdbfile,splfile,drw=True,messout=True):
        ...
        self.OpenFiles(filelst[0],False)
        for i in range(1,len(filelst)):
            file=filelst[i]
            molobj=molec.Molecule(self)
    ...
    def SplitIsolatedMols(self):
        ...
        for atom in self.mol.atm:
            ...
            elif atom.resnam in const.AmiRes3:
                prodic['pro'].append(atom.seqnmb)
            else: prodic['chem'].append(atom.seqnmb)


    def ExtractPeptides(self):
        pepatmlstlst=[] # [[atm1,atm2,...],[atomi,...atmi+1,...],...]
        # split into isolated groups
        # grpdic={0:[0,1,2,...], 1:[i,i+1,...],...n:[...]}
        grpdic = self.FindConnectedGroup(onlynatm=False)
        # find peptide mols
        grpnamlst=list(grpdic.keys())
        grpnamlst.sort()
        for grpnam in grpnamlst:
            grpatmlst=grpdic[grpnam]
            if self.mol.atm[grpatmlst[0]].resnam in const.AmiRes3:
                pepatmlstlst.append(grpatmlst)
        return pepatmlstlst
        ...
        for i in range(natm):
            ...
            if elm == 'XX' or (lasta and len(moldat)) > 0:
                ...
                for j in range(len(moldat)):
                    ...
                    kk=-1; deltmp=[]
                    for k in range(len(moldat[j].conect)):
                        kk += 1
    ...
    def AddHydrogenUseFrameData(self,wat=False,drw=True,atmlst=[],bondmulti=True):
        lst=atmlst
        if len(lst) <= 0: lst=self.ListTargetAtoms()
        if len(lst) <= 0:
            mess = 'No non-polypeptide redisues. Nothing is done.'
            self.ConsoleMessage(mess)
            return
        ...
        if not wat:
            for watres in const.WaterRes:
                if watres in framedatdic: del framedatdic[watres]
        self.mol.AddBondUseFrame(lst,framedatdic)
        self.mol.AddHydrogenUseFrame(lst,framedatdic)
        if bondmulti: self.AddBondMultiplicity(clearmulti=True, drw=False)
        #
        self.ResetPosAtm()
        if drw: self.DrawMol(True)
    ...
    def DownloadFrameData(self,lst,messout=True):
        ...
        ftpserver=['ftp.wwpdb.org','guest','','pub/pdb/data/monomers']
        """
        if messout:
        ...
        for i in lst:
            ...
            resnam=atom.resnam; resnmb=atom.resnmb; chain=atom.chainnam
            #resdat=lib.ResDat(atom)
            if resnam in const.AmiRes3: continue
            if resnam in const.WaterRes: continue
            if resnam not in framedatdic: noframedic[resnam]=True
            else:
        ...
        if messout and len(existlst) > 0:
            ...
            self.ConsoleMessage(mess)
        if len(noframedic) > 0:
            searchlst=list(noframedic.keys())
    ...
    def AddBondMultiplicity(self,clearmulti=True,drw=True):
        ...
        #
        self.mdlwin.BusyIndicator('On','Bond multiplicity...')
        ...
        # COO
        for atom in self.mol.atm:
            if atom.elm == ' C' and len(atom.conect) == 3:
                olst = []
                for j in range(len(atom.conect)):
                    if self.mol.atm[atom.conect[j]].elm == ' O' and\
                            len(self.mol.atm[atom.conect[j]].conect) == 1:
                        olst.append(atom.conect[j])
                if len(olst) != 2: continue
                ia = atom.seqnmb
                for ib in olst:
                    idx = atom.conect.index(ib)
                    atom.bndmulti[idx] = 4
                    batom = self.mol.atm[ib]
                    idx = batom.conect.index(ia)
                    batom.bndmulti[idx] = 4
        # terminal O not in COO
        for atom in self.mol.atm:
            if atom.elm == ' O' and len(atom.conect) == 1 and\
                    atom.bndmulti[0] == 1:
                atom.bndmulti[0] = 2
                batom = self.mol.atm[atom.conect[0]]
                idx = batom.conect.index(atom.seqnmb)
                batom.bndmulti[idx] = 2
        # ARG NE-CZ-NH1-NH2
        atmnamlst = [' NE ',' NH1',' NH2']
        for atom in self.mol.atm:
            if atom.resnam == 'ARG':
                resnmb = atom.resnmb
                if atom.atmnam in atmnamlst:
                    ia = atom.seqnmb
                    for j in range(len(atom.conect)):
                        ib = atom.conect[j]
                        if self.mol.atm[ib].resnam == 'ARG' and\
                              self.mol.atm[ib].resnmb == resnmb:
                            if self.mol.atm[ib].atmnam == ' CZ ':
                                atom.bndmulti[j] = 4
                                idx = self.mol.atm[ib].conect.index(ia)
                                self.mol.atm[ib].bndmulti[idx] = 4
        # TRP NE1-CD1-CG-CD2-CE2-CZ2-CE3-CH2-CZ3
        atmnamlst = [' NE1',' CD1',' CG ',' CD2',' CE2',' CE3',' CZ2',' CZ3',
                     ' CH2']
        for atom in self.mol.atm:
            if atom.resnam == 'TRP':
                resnmb = atom.resnmb
                if atom.atmnam in atmnamlst:
                    ...
                    for j in range(len(atom.conect)):
                        ib = atom.conect[j]
                        if self.mol.atm[ib].resnam == 'TRP' and\
                              self.mol.atm[ib].resnmb == resnmb and\
                                  self.mol.atm[ib].elm != ' H':
                            atom.bndmulti[j] = 4
        #
        if len(ringlst) > 0:
        ...
        mess = 'NOTE: The bond multiplicities were computed.\n'
        mess += 'Please check the result carefully and fix it if neceessary.'
        #wx.MessageBox(mess,'fumodel.AddBondMultiplicity')
        self.ConsoleMessage(mess)
    ...
    def AddMissingNterm(self,drw=False):
        ...
        def Renumber(ith):
            for i in range(ith,len(self.mol.atm)):
    ...
    def AddHydrogenToAAResidue(self,drw=True,atmlst=[],checknterm=True):
        ...
        if self.setctrl.GetParam('use-fmoutil'): #self.usefmoutil:
            try:
                ...
                mess='fmoutil failed in AddHydrogenToAAResidue). '
                mess=mess+'Python code is used.'
                self.ConsoleMessage(mess)
                nhtotal,nhchainlst=self.mol.AddHydrogenToProtein(lst,addbond=False)
                #nhtotal, nhchainlst = self.mol.OldAddHydrogenToProtein(lst)
        else:
    ...
    def AddHydrogens(self,resknd=['aa','non-aa','wat'],frame=True,
        ...
        atmlst=list(range(len(self.mol.atm)))
        if len(atmlst) <= 0: return
        #
        try: self.mdlwin.BusyIndicator('On','Adding hydrogens...')
        except: pass
    ...
    def AddHydrogenUseBondLength(self,drw=True,atmlst=[]):
        #??self.ctrlflag.SetCtrlFlag('busy',True)
        self.Message('Running Add Hydrogen...',0,'')
        targetatm=atmlst
    ...
    def AddHydrogensAt(self,at,htype,bndlst,rhx,hname=[]):
        ...
        nh=0
        natm = len(self.mol.atm)
        atoma = self.mol.atm[at]
        AN = const.ElmNmb[atoma.elm]
        #
        atmlst.append(at)
        if htype == '1A1':   atmlst = atmlst + bndlst
            ...
            ib = bndlst[1]
            atomb = li_atoms[ib]
            if len(li_connect[ib]) <= 0:
                mess = 'fumodel.AddHydrogens: unable to attach hydrogen at '
                mess +=  + str(at)
                self.Message2(mess)
            else:
                ic = li_connect[ib][0]
            ...
            ib = bndlst[1]
            atomb = self.mol.atm[ib]
            if len(li_connect[ib]) < 1:
                mess = 'fumodel.AddHydrogens: unable to attach hydrogen '
                mess += 'at ' + str(at)
                self.Message2(mess)
            else:
                ic = li_connect[ib][0]
                if ic == at: ic = li_connect[ib][1]
                atmlst = atmlst + bndlst
                atmlst.append(ic)
        elif htype == '3A1': atmlst = atmlst + bndlst
        elif htype == '1A4': atmlst = atmlst + bndlst
        elif htype == '4A0': pass
        elif htype == '2A0': pass
        else:
            mess = 'fumodel.AddHydrogens: failed to Add hydrogen at '
            mess += str(at) + '.'
            self.Message2(mess)
        if htype == '1A1': nh,coord = GetCCAddAtmType1A1(li_atoms,atmlst,rhx)
        if htype == '1A2': nh,coord = GetCCAddAtmType1A2(li_atoms,atmlst,rhx)
        if htype == '1A3': nh,coord = GetCCAddAtmType1A3(li_atoms,atmlst,rhx,-1,True) # trans
        if htype == '2A1': nh,coord = GetCCAddAtmType2A1(li_atoms,atmlst,rhx)
        if htype == '2A2': nh,coord = GetCCAddAtmType2A2(li_atoms,atmlst,rhx)
        if htype == '3A1': nh,coord = GetCCAddAtmType3A1(li_atoms,atmlst,rhx)
        if htype == '2A0': nh,coord = GetCCAddAtmType2A0(li_atoms,atmlst,rhx,angle=120.0)
        if htype == '1A0': nh,coord = GetCCAddAtmType1A0(li_atoms,rhx)
        if htype == '1A4': nh,coord = GetCCAddAtmType1A4(li_atoms,atmlst,rhx)
        if htype == '4A0': nh,coord = GetCCAddAtmType4A0(li_atoms,rhx)

        #check interatomic distance
        nc = len(coord)
        shrt2 = 1.73 * 1.73
        if AN == 7: shrt2 = 1.63 * 1.63
        ...
        if nh > 0:
            ...
            for j in range(nh):
                iatm += 1
                li_atoms.append([1]+list(coord[j]))
                li_connect[at].append(iatm)
                li_connect.append([at])
        return li_atoms, li_connect

    def AddHydrogenUseBondMultiplicity(self,drw=True,atmlst=[]):
        #??self.ctrlflag.SetCtrlFlag('busy',True)
        self.Message('Running Add hydrogens ...',0,'')
        targetatm=atmlst
        if len(targetatm) <= 0: targetatm=self.ListTargetAtoms()
        if len(targetatm) <= 0: return
        #
        ...
        addh = self.mol.AddHydrogenUseBM(targetatm)

        mess = 'Number of added hydrogens = ' + str(addh)
        self.Message2(mess)
    ...
    def AttachHydrogen(self,group,atmlst=[],atmnamlst=None):
        ...
        #mgrplst=['-CH3','-ACE','-NME']
        if group in hgrplst:
            if group == '1H': self.mol.AddGroup1Hydrogen(lst,atmnamlst)
            elif group == '2H': self.mol.AddGroup2Hydrogen(lst,atmnamlst)
            elif group == '3H': self.mol.AddGroup3Hydrogen(lst,atmnamlst)
    ...
    def ReplaceXHWithXGroup(self,group,atmlst=[]):
        ...
        for i in lst:
            ...
            atm2=iatm+nmol0
            tmpatm=mol.CopyAtom()
            self.MergeMolecule(tmpatm,check=False,drw=False)
    ...
    def RepalceResdiueCoordinates(self,pdbfile='',drw=True):
        ...
        for resdat in newreslst:
            for atom in self.mol.atm:
                res=lib.ResDat(atom)
                if res == resdat:
                    if resdat not in oldatmlstdic:
                        oldatmlstdic[resdat]=[]
                    oldatmlstdic[resdat].append(atom.seqnmb)
        changelst=[]
        for resdat in newreslst:
            if resdat not in oldatmlstdic: continue
    ...
    def RepalceResdiues(self,pdbfile='',drw=True):
        ...
        dellst=[]
        for resdat,newresmol in list(newresmoldic.items()):
            if resdat in oldresmoldic:
    ...
    def DeleteSelected(self,drw=True,messout=True):
        ...
        if messout: self.ConsoleMessage('Deleted number of atoms='+str(len(lst)))
    ...
    def DeleteTerAtHead(self,mol,drw=True):
        fromargs=False
        if mol == None:
            if 'DeleteTerAtHead' not in self.mdlargs: return
    ...
    def DeleteBonds(self,kind,drw=True):
        ...
        if kind == 'all':  # tentative
            ...
            elmlst = lib.GetElmPairFromUser('Model.DeleteBonds')
            if len(elmlst) <= 0: return
            natm = len(self.mol.atm)
            nbnd = 0
            for i in range(natm):
                elmi = self.mol.atm[i].elm
                for j in self.mol.atm[i].conect:
                    elmj = self.mol.atm[j].elm
                    for pair in elmlst:
                        if elmi in pair and elmj in pair:
                            self.mol.DelBond(i,j)
                            nbnd += 1
            mess = '\nBonds between ' + str(elmlst) +' were deleted.\n'
            mess += '    the number of deleted bonds = ' + str(nbnd)
            self.ConsoleMessage(mess)
    ...
    def IsPDBAtmNamNew(self):
        ans=True
        oldnamlst=list(const.AtmNamOldToNew.keys())
        for atom in self.mol.atm:
            if atom.resnam in const.AmiRes3: continue
    ...
    def ChangeAtmNamToNew(self):
        for atom in self.mol.atm:
            res=atom.resnam; atmnam=atom.atmnam
            if res not in const.AmiRes3: continue
            if atmnam in const.AtmNamOldToNew:
    ...
    def MakeIonChargeData(self):
        chgdat=[]
        for atom in self.mol.atm:
            if not atom.elm in const.IonAtmChg: continue
            elm=atom.elm
    ...
    def OpenNonStructureFiles(self,ext,filename):
        if ext == '.frm':
    ...
    def ReorderHydrogensIntoTinker(self):
        ...
        # make bond data
        lst=[]
        for i in range(len(self.mol)): lst.append(i)
        self.mol.DelAllKindBonds(lst)
        self.mol.AddBondUseBL(lst)
    ...
    def ComputeBondLength(self,rmin=-1,rmax=-1):
        ...
        try:
            ...
            if rmax < 0: rmax=3.0
            cc=[]; iopt=0
            for i in range(natm): cc.append(self.mol.atm[i].cc)
            cc=numpy.array(cc)
    ...
    def ExportMDLWINImageToImageSheet(self):
        print('ExportMDLWINImageToImageSheet is called')
        active = True
        try:
            win = self.winctrl.GetWin("Open ImageSheet")
            if win is None: active = False
            else:  win.OnGetImage(1)
        except: active = False
        if not active:
            mess = '"ImageSheet" is not opened'
            wx.MessageBox(mess,'Model.ExportMDLWINImageToImageSheet')

    def GenerateIsomers(self,nconfs=None,sortbyeff=True):
        """

        :param bool sortbyeff: True for sorting confomers in forcefield
                               energies, False for do not
        """
        if len(self.mol.atm) <= 0: return
        ...
        #nconfs = 10
        if nconfs is None:
            nconfs = self.setctrl.GetParam('n-gen-confs')
            print(('nconfs',nconfs))
        #
        self.mdlwin.BusyIndicator('On','Gen isomers ...')
        #
        err_valence = self.CheckValence()
        if len(err_valence) > 0:
            mess = 'Error: Number of bonds exceeded the valence,\n'
            mess += '    error atom numbers = ' + str(err_valence)
            self.ConsoleMessage(mess)
            wx.MessageBox(mess,'Model.GenerateIsomers')
            return
        if nconfs is None: nconfs = self.setctrl.GetParam('n-gen-confs')
        sdflst = self.GenerateIsomersByRD(nconfs=nconfs,argdic=argdic,
                                          sortbyeff=sortbyeff)
        if len(sdflst) <= 0: return
        nconf = len(sdflst)
        for i in range(nconf):
            sdf = sdflst[i]
            if len(sdf.split()) <= 0: continue
            if sdf.find('$$$$') < 0: sdf += '$$$$\n'
            self.BuildFromSDFString(sdf)
        #
        mess   = 'Number of generated conformers = ' + str(nconf)
        if sortbyeff:
            mess += ', The confomers are sorted by forcefield energy'
        self.ConsoleMessage(mess)

        self.mdlwin.BusyIndicator('Off')

    def GenerateIsomersByRD(self,nconfs=None,argdic=None,sortbyeff=True):
        """ max_isomers = GetTextFromUsers """

        if not cheminfo.ReadyRDKit():
            mess = 'Please install RDKit.'
            wx.MessageBox(mess,'Model.GenerateIsomers')
            return
        if argdic is not None:
            if 'forcefield' in argdic: ff = argdic['forcefield'].lower()
            else: ff = 'uff'
            if ff == 'uff' or ff == 'mmff94': pass
            else:
                mess = 'The forcefield ' + ff + ' is not defined.'
                mess += ' Skipped minimization'
                wx.MessageBox(mess,'Model.GenerateConformers')
                return
        #
        confsdflst = []
        enelst = []
        rdmol  = cheminfo.RDMolFromFUMol(self.mol,sanitize=True)
        name   = self.mol.name
        sdfstr = Chem.MolToMolBlock(rdmol)
        natm = rdmol.GetNumAtoms()
        if len(sdfstr.strip()) <= 0: return confsdflst
        #
        confs  = AllChem.EmbedMultipleConfs(rdmol,numConfs=nconfs)
        # rough minimization
        #confmollst = []
        for confid in confs:
            if ff == 'mmff94':
                ff = AllChem.MMFFGetMoleculeForceField(rdmol,confId=confid)
            else: # 'uff'
                ff = AllChem.UFFGetMoleculeForceField(rdmol,confId=confid)
            #
            ff.Minimize()
            enelst.append(ff.CalcEnergy())

        nconf = len(confs)
        if sortbyeff: sortkey = numpy.argsort(numpy.array(enelst))
        else: sortkey = list(range(nconf))
        for i in range(nconf):
            confid = sortkey[i]
            confmol = Chem.MolFromMolBlock(sdfstr,sanitize=False,
                                 removeHs=False)
            for j in range(natm):
                pos = rdmol.GetConformer(confid).GetAtomPosition(j)
                confmol.GetConformer(0).SetAtomPosition(j,pos)
            cfname = str(name + '-'+'conf-' + str(i+1))
            confmol.SetProp("_Name",cfname)
            confmol.SetProp("FF_energy",str(enelst[confid]))
            confsdflst.append(Chem.MolToMolBlock(confmol))

        return confsdflst

    def GetAMBITPath(self,message=True):
        ambit = None
        pltfrm = lib.GetPlatform()
        if   pltfrm == 'WINDOWS': ext = '.win'
        elif pltfrm == 'MACOSX':  ext = '.mac'
        ambitpath = self.setctrl.GetDir('Programs') + '//ambit//ambitpath'
        ambitpath = ambitpath + ext
        if not os.path.exists(ambitpath):
            # no pathfile
            mess = ambitpath + ' is not found.\n'
            wx.MessageBox(mess,'Model.GenerateTautomers')
            return None
        #
        f = open(ambitpath,'r')
        data = f.read()
        f.close()
        ambit = lib.SplitStringAtSpaces(data)[1].strip()
        # no excutable file
        if not os.path.exists(ambit):
            mess = '"ambit" executable file does not exist,\n'
            mess += '    ambit = ' + ambit
            wx.MessageBox(mess,'Model.GenerateTautomers')
            return None
        #
        if message:
            mess = 'ambit excecutable file = ' + ambit
            self.ConsoleMessage(mess)
        return ambit

    def GenerateTautomers(self,htr15=True,htr17=False,ffargs=None):
        if len(self.mol.atm) <= 0: return
        ambit = self.GetAMBITPath(message=False)
        scrdir = self.setctrl.GetDir('Scratch')
        outfile = scrdir + '//ambit_out.sdf'
        inpfile = scrdir + '//ambit_inp.sdf'
        logfile = scrdir + '//ambit_inp.log'
        self.mol.WriteSDFMol(inpfile)
        basename = self.mol.name
        #
        mess  = '\nGenerateTautomers using,\n'
        mess += '"ambit" - http://onlinelibrary.wiley.com/doi/10.1002/minf.201200133/'
        mess += 'abstract\n'
        mess += '    ambit excutabel file  = ' + ambit + '\n'
        mess += '    temporary input file  = ' + inpfile +'\n'
        mess += '    temporary output file = ' + outfile
        self.ConsoleMessage(mess)
        # forcefiled options
        if ffargs is None:
            ff = 'uff'
            ...
            if 'forcefield' in ffargs: ff = ffargs['forcefield']
            if 'max_steps' in ffargs:  max_steps = ffargs['&max_steps']
        #
        self.mdlwin.BusyIndicator('On','Gen tautomers...')
        mess = 'GenTautomers started at ' + lib.DateTimeText()
        self.ConsoleMessage(mess)
        start_time = time.clock()
        #
        self.GenerateTautomersByAMBIT(inpfile=inpfile,outfile=outfile,
                                               logfile=logfile)
        ChemTool = 'RD' # 'RD'
        nmol = 0
        if ChemTool == 'RD':
            try:
                mols,enelst = cheminfo.RDAddHsAndMinimize(outfile,FF=ff,
                                                      max_steps=max_steps)
            except:
                mess='Error occured at "cheminfo.RDAddHsAndMinimize".\n'
                mess+='Probably the "ambit" program terminated abnormally'
                wx.MessageBox(mess,"GenerateTautomers")
                return
            sdfstr = ''
            nmol = len(mols)
            for i in range(nmol):
                sdf = Chem.MolToMolBlock(mols[i])
                if sdf.find('$$$$') < 0: sdf += '$$$$\n'
                sdfstr += sdf
                name = basename + '-taut-' + str(i)
                self.BuildFromSDFString(sdf,name=name)
            #
        mess = 'Number of generated toutomers = ' + str(nmol)
        self.ConsoleMessage(mess)
        #
        self.mdlwin.BusyIndicator('Off')
        mess = 'Job ended at ' + lib.DateTimeText() +'\n'
        mess = 'Elapsed time(sec) ='+'%8.2f' % (time.clock()-start_time) + '\n'
        self.ConsoleMessage(mess)

    def GenerateTautomersByAMBIT(self,inpfile=None,outfile=None,logfile=None):
        ambit = self.GetAMBITPath(message=False)
        if ambit is None: return

        sdflst = []
        #cmd1 = [gen_tautomers,'-f',inpfile,'-o',outfile,'-3','off','-5','off','-7','off']
        cmd1 = [ambit,'-f',inpfile,'-o',outfile]
        proc = subprocess.Popen(cmd1,stdout=subprocess.PIPE,stderr=None,
                                shell=True)
        output = proc.communicate()
        if logfile is not None: logfile = output[0]
    ...
    def PCCDBAccess(self):
        hp = const.WebAddress['PCCDB']
        lib.ViewHtmlFile(hp)

    def PCCDBGetSDF(self,waittime=1):
        """ Get SDF data In PCCDB and build molecules """
        mess = "Input cid's, i.e., '3,5-10,12,...'(do not input quotations)'"
        cidlst = lib.GetIntegersFromUser(mess)
        if len(cidlst) <= 0: return
        #
        self.mdlwin.BusyIndicator('On','PCCDBGetData ...')
        #
        sdflst,getsucceed,getfailed = self.PCCDBGetSDFString(cidlst,
                                                         waittime=waittime)
        bldfailed  = []
        bldsucceed = []
        if len(sdflst) > 0:
            for i in range(len(sdflst)):
                sdf = sdflst[i]
                if len(sdf.split()) <= 0: continue
                if sdf.find('$$$$') < 0: sdf += '$$$$\n'
                try:
                    name = sdf[:50][:]
                    name = name.replace('PCCDB-ID:','').strip()
                    name = 'ID-' + lib.SplitStringAtSpaces(name)[0].strip()
                    self.BuildFromSDFString(sdf,name=name)
                    bldsucceed.append(getsucceed[i])
                except: bldfailed.append(getsucceed[i])

        self.DrawMultipleBond(True)

        self.mdlwin.BusyIndicator('Off')
        mess  = "PCCDB: get sdf of cids,\n"
        mess += "    get succeeded cids  = " + str(getsucceed) + '\n'
        mess += "    get failed cids     = " + str(getfailed)
        if len(bldfailed) > 0:
            mess += "\n    build failed cids   = " + str(bldfailed)
        self.ConsoleMessage(mess)

    def PCCDBGetSDFString(self,cidlst,waittime=1):
        """ Get SDF data In PCCDB

        :param lst,str,int cidlst: cid's in list, string, or an integer
        :return: sdflst(lst)  - list of sdf strings
                 secceed(lst) - list of get-succeeded cid's
                 failed(lst)  - list of get-failed cid's
        """
        hp = const.WebAddress['PCCDB']
        url_base = hp + '/search_pubchemqc/get_sdf/ver0.2/'
        #
        if lib.ObjectType(cidlst) == 'list': pass
        if lib.ObjectType(cidlst) == 'int': cidlst = [cidlst]
        elif lib.ObjectType(cidlst) == 'str':
            cidlst = lib.StringToInteger(cidlst)
        #
        sdflst  = []
        failed  = []
        succeed = []
        for i in cidlst:
            ...
            try:
                ...
                text = ''
                for line in response: text += line
                datdic = json.loads(text)
                sdfstring = datdic['sdf']
                sdflst.append(sdfstring)
                succeed.append(i)
            except: failed.append(i)
            if waittime > 0: time.sleep(waittime)
        return sdflst,succeed,failed

    def PubChemAccess(self):
        hp = const.WebAddress['PubChem']
        lib.ViewHtmlFile(hp)

    def ReadyPubChemPy(self):
        if not const.PUBCHEMPY:
            mess = 'Please install "pubchempy",\n >pip install pubchempy\n'
            mess += ' and try again.'
            wx.MessageBox(mess,'Model.GetPropsInPubChem')
        return const.PUBCHEM

    def PubChemGetAtoms(self,waittime=1,build3d=False,ff='uff'):
        #
        if not self.ReadyPubChemPy: return
        #

        mess = "Input pdbid's, i.e., '1crn,...'(do not input quotations)'"
        cidlst = lib.GetStringsFromUser(mess)
        if len(cidlst) <= 0: return
        #
        self.mdlwin.BusyIndicator('On','PDBGetData ...')
        #
        props = ['atoms','bonds']
        propdiclst = []
        for cid in cidlst:
            comp = pubchempy.Compound.from_cid(cid)
            propdic = comp.to_dict(properties=props) # ['atoms', 'bonds'])
            propdiclst.append([cid,propdic])

        #propdiclst = self.PubChemGetProps(cidlst,props)
        atomcclst,bnddatlst,getsucceed,getfailed = \
              self.PubChemAtomsAndBonds(propdiclst)
       #
        bldfailed  = []
        bldsucceed = []
        if len(atomcclst) > 0:
            for i in range(len(atomcclst)):
                atomcc = atomcclst[i]
                natm = len(atomcc)
                name = 'cid-' + str(cidlst[i])
                conlst,typlst = lib.ConListFromBondList(natm,bnddatlst[i])
                sdf = lib.AtomCCToSDFText(atomcc,conlst,typlst,name=name)
                if len(sdf) <= 0: continue
                try:
                    if build3d:
                        id = 0
                        rdmol = Chem.MolFromMolBlock(sdf,sanitize=True)
                        if ff == 'mmff94': AllChem.MMFFOptimizeMolecule(rdmol)
                        else: AllChem.UFFOptimizeMolecule(rdmol)
                        sdf = Chem.MolToMolBlock(rdmol)
                    if sdf.find('$$$$') < 0: sdf += '$$$$\n'
                    self.BuildFromSDFString(sdf)
                    bldsucceed.append(getsucceed[i])
                except: bldfailed.append(getsucceed[i])
    ...
    def PubChemGetSMILES(self,build3d=False,ff='uff'):
        #
        if not self.ReadyPubChemPy: return
        #
        mess = "Input pdbid's, i.e., '1crn,...'(do not input quotations)'"
        ...
        for cid in cidlst:
            comp = pubchempy.Compound.from_cid(cid)
            smilst.append(comp.canonical_smiles) # isomeric_smiles
        #
        bldfailed  = []
        bldsucceed = []
        for i in range(len(smilst)):
            smi = str(smilst[i])
            ...
            try:
                ...
                self.BuildFromSDFString(sdf)
                self.DrawMultipleBond(True)
                bldsucceed.append(cidlst[i])
            except: bldfailed.append(cidlst[i])
        #
        self.mdlwin.BusyIndicator('Off')
        mess  = "PubChem: get SMILES,\n"
        mess += "    get succeeded cids  = " + str(bldsucceed) + '\n'
        mess += "    get failed cids     = " + str(bldfailed)
        self.ConsoleMessage(mess)

    def PubChemAtomsAndBonds(self,propdiclst):
        """

        install pubchempy: >pip install pubchempy
        ...
        bndtyp = {'single':1,'double':2,'triple':3,'aromatic':4}
        #
        succeed   = []
        failed    = []
        atomcclst = []
        bnddatlst = []
        ndat = len(propdiclst)
        for cid,propdic in propdiclst:
            atomcc = []
            bnddat = []
            try:
                natm = len(propdic['atoms'])
                nbnd = len(propdic['bonds'])
                if natm > 0: succeed.append(cid)
                for i in range(natm):
                    atom = propdic['atoms'][i]
                    elm = lib.ElementNameFromString(atom['element'])
                    x   = atom['x']
                    y   = atom['y']
                    z   = 0.0
                    atomcc.append([elm,x,y,z])
                for i in range(nbnd):
                    bond  = propdic['bonds'][i]
                    iatm  = bond['aid1'] - 1
                    jatm  = bond['aid2'] - 1
                    try:    multi = bndtype[bond['order']]
                    except: multi = 1
                    bnddat.append([iatm,jatm,multi])
                atomcclst.append(atomcc)
                bnddatlst.append(bnddat)
            except: failed.append(cid)
        return atomcclst,bnddatlst,succeed,failed

    def PDBAccess(self):
        hp = const.WebAddress['PDB']
        lib.ViewHtmlFile(hp)

    def PDBGetPDB(self,waittime=1):
        """ Get PDB data In RCSB PDB """
        mess = "Input pdbid's, i.e., '1crn,...'(do not input quotations)'"
        idlst = lib.GetStringsFromUser(mess)
        if len(idlst) <= 0: return
        #
        self.mdlwin.BusyIndicator('On','PDBGetData ...')
        #
        savedir = self.setctrl.GetDir('Scratch')
        savedlst,getfailed = self.PDBGetPDBFile(idlst,savedir)
        #           waittime=waittime)
        getsucceed = savedlst
        bldfailed  = []
        bldsucceed = []
        if len(savedlst) > 0:
            for i in range(len(savedlst)):
                try:
                    pdbfile = os.path.join(savedir,savedlst[i])
                    unzipfile = lib.ZipUnzipFile(pdbfile,zip=False,check=False)
                    self.BuildFromPDBFile(unzipfile)
                    bldsucceed.append(getsucceed[i])
                except:
                    bldfailed.append(getsucceed[i])
        #
        self.mdlwin.BusyIndicator('Off')
        mess  = "PDB: get pdbfile of pdb ids,\n"
        mess += "    get succeeded pdbids  = " + str(getsucceed) + '\n'
        mess += "    get failed pdbids     = " + str(getfailed)
        if len(bldfailed) > 0:
            mess += "\n    build failed pdbis   = " + str(bldfailed)
        self.ConsoleMessage(mess)

    def PDBGetPDBFile(self,pdbids,savedir,waittime=1):
        """ Get PDB data In RCSB PDB

        :param lst,str ids: pdbid's list or a string separated by a comma
        :return: pdbfilelst(lst)  - list of pdb files
                 secceed(lst)     - list of get-succeeded pdbid's
                 failed(lst)      - list of get-failed pdbid's
        server = "ftp://ftp.rcsb.org/pub/pdb/data/structures/divided/pdb/xx/filename"
        """
        #
        if lib.ObjectType(pdbids) == 'list': idlst = pdbids[:]
        elif lib.ObjectType(pdbids) == 'str':
            idlst = lib.SplitStringAtSeparator(pdbids,',')
        #
        if not os.path.isdir(savedir):
            mess = 'Not found directory = ' + savedir +'\n'
            mess += 'Would you liketo create it?'
            ans = lib.MessageBoxYesNo(mess,title='Model.GetPDBInPB')
            if not ans: return
            else: os.mkdir(savedir)
            mess = 'Created a new save directory = ' + savedir
            self.ConsoleMessage(mess)

        mess,savedlst,failedlst = lib.GetStructureFromPDBFTPServer('',
                                      idlst,savedir)
        #
        return savedlst,failedlst
    ...
    def SetLoggingMethod(self):
        ...
        if len(logmethfile) <= 0:
            curdir=os.getcwd()
            logdir=self.setctrl.GetDir('FUlogs')
            os.chdir(logdir)
        ...
        if len(const.LOGGINGMETHODDIC) <= 0:
            ...
            methnmb=len(list(const.LOGGINGMETHODDIC.keys()))
    ...
    def SaveLog(self,on,logfile=''):
        logmethfile = self.setctrl.GetParam('logging-method-file')
        mess = 'No logging methods definition file="' + logmethfile + '".\n'
        mess += 'Therefore the logging is not available.\n'
        mess += 'Check the definition of loggingmethodfile in ctrl.SetCtrl.__init__\n'
        mess += 'and the file in FUDATASET.'
        try:
            if len(const.LOGGINGMETHODDIC) <= 0:
                self.ConsoleMessage(mess)
        ...
        if on:
            ...
            if logfile == '':
                logdir=self.setctrl.GetDir('FUlogfiles')
                base='fu-'; ext='.logging'
            ...
            if len(lib.FILENAMEDIC) > 0:
                ...
                text1=[]
                for s in text:
                    for name,filename in list(lib.FILENAMEDIC.items()):
                        ns=s.find(filename)
    ...
    def ViewSettingParams(self):
        ...
        text  += '# All parameters are redefined in the "fumodelset.py" '
        text  += ' script\n'
        text  += '# which is executed at the activation of the program.\n'
        text  += '# The "Help"-"Customize" menu allows you to set these '
        text  += 'parameters via a GUI tool.\n\n'
        text  += self.setctrl.ListSettingParams()
        title  = "Setting paramters";
        winsize=[640,400]
        parpos=self.mdlwin.GetPosition()
        winpos=[parpos[0]+self.mdlwin.GetClientSize()[0],parpos[1]+20]
        #viewer = subwin.TextViewer_Frm(self.mdlwin,title=title,text=text,
        #                             winsize=winsize,menu=True)
        viewer = subwin.TextEditor_Frm(self.mdlwin,-1,title=title,text=text,
                                     winpos=winpos,winsize=winsize,
                                     filemenu=False,mode='View',bgcolor=[253,255,206])

        #viewer.RemoveOpenMenu()

    def HelpBugs(self):
        text  = ''
        text += '1. It may happen that the drawing disappears in the screen.\n'
        text += '    Even in that case, internal data may alive.\n'
        text += '    Type the following command in Pyshell,\n'
        text += '    >>> len (fum.mol.atm)\n'
        text += '    to see the number of atoms in FU.\n'
        text += '    If there is data, save it with "File" - "Save as"\n'
        text += '    and close FU.\n'
        text += '    You can continue work by opening the FU again and\n'
        text += '    reading the saved file.\n'
        subwin.HelpMessage(text,datatype='text',viewer='Text',
                           parent=self.mdlwin,winsize=(460,200))
